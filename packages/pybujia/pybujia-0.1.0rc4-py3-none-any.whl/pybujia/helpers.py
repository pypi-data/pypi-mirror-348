...
def spark_method_test(
    method: Callable,
    fixtures: PyBujia,
    input_args: dict[str, Union[str, Literal]],
    ...
    Args:
        ...
        input_args (dict[str, Union[str, Literal]]): Mapping of method argument names to either:
    ...
    prepared_arguments = {
        arg_name: fixtures.get_dataframe(arg_value) if not isinstance(arg_value, Literal) else arg_value.unwrap()
    ...
    for res_val, res_exp_val in zip(result, expected_result):
        val = res_val.unwrap() if isinstance(res_val, Literal) else res_val
        val_exp = res_exp_val.unwrap() if isinstance(res_exp_val, Literal) else fixtures.get_dataframe(res_exp_val)
        ...
        if isinstance(val, (float, Decimal)) and isinstance(val_exp, (float, Decimal)):
