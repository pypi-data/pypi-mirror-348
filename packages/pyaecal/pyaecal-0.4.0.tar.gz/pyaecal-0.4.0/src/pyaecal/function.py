...
class Function(Extract):
    ...
    def process(self):
        ...
        while not self.evals.empty():
            ...
            # Handle tuple output (from Trigger class)
            if isinstance(eval_result, tuple):
                pre_df, post_df = eval_result
                if not pre_df.is_empty() or not post_df.is_empty():
                    res[filename] = {"pre_triggers": pre_df, "post_triggers": post_df}
                else:
                    logging.debug(f"No evaluation data for {filename}")
            # Handle single DataFrame (from other Function subclasses)
            else:
                if not eval_result.is_empty():
