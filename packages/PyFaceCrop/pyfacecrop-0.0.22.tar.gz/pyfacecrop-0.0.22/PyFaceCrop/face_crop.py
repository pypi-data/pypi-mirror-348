...
def get_haarcascade_path(filename):
    """Get the absolute path of a Haarcascade file."""
    try:
        with pkg_resources.path(PyFaceCrop, f'haarcascades/{filename}') as p:
            cascade_path = str(p.as_posix())  # Convert path to a string
            print(f"Resolved path for {filename}: {cascade_path}")  # Debugging output
            return cascade_path
