...
"""
import kytest
from kytest.core.web import WebElem
...
class LoginPage(kytest.Page):
    ...
    login_or_reg = WebElem(text="登录/注册", exact=True)
    pwd_login = WebElem(text='密码登录')
    phone_input = WebElem(placeholder='请输入手机号码')
    pwd_input = WebElem(placeholder='请输入密码')
    accept = \
        WebElem(locator='form div') \
            .filter(has_text="我已阅读并同意《企知道平台服务协议》、 《企知道隐私权政策》、 《企知道商城/商贸空间使用须知》") \
            .locator("span") \
            .nth(3)
    login_now = WebElem(role='button', name='立即登录')
    first_company = WebElem().get_by_text('北京大胖涮锅城有限公司')
