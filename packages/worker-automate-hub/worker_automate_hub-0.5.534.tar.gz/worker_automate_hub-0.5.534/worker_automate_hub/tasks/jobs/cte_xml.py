...
async def importar_xml_conhecimento():
    ...
    try:
        await worker_sleep(5)
        app = Application(backend="win32").connect(title="Importar XML Conhecimento", found_index=0)
        main_window = app["Importar XML Conhecimento"]

        # Clicar em aquisicao de servico de transporte D/E
        aquisicao_servico = main_window.child_window(class_name="TDBIComboBox", found_index=1)
...
async def janela_conhecimento_frete(cte):
    ...
    app = Application(backend="win32").connect(title="Conhecimento de Frete", found_index=0)
    ...
    select_other = main_window.child_window(class_name="TDBIComboBoxValues", found_index=1)
    select_other.select("9 - Outros")
    
    await worker_sleep(2)
    
    # Preencher Tipo de despesa
    campo_tipo_despesa = main_window.child_window(class_name="TDBIEditCode", found_index=2).click()
    await worker_sleep(2)   
    type_text_into_field(text=codigo_despesa, field=campo_tipo_despesa, empty_before=True, chars_to_empty="3")
    ...
    base_icms = main_window.child_window(class_name="TDBIEditNumber", found_index=20).click()
    base_icms.type_keys("{DEL}")
    base_icms.type_keys(valor_frete)
    
    #Preencher Valor frete
    base_icms = main_window.child_window(class_name="TDBIEditNumber", found_index=14).click()
    for _ in range(10):
        base_icms.type_keys("{DEL}")
    base_icms.type_keys(valor_frete)
    ...
    data_vencimento = main_window.child_window(class_name="TDBIEditDate", found_index=0).click()
    await worker_sleep(2)   
    type_text_into_field(vencimento, data_vencimento, True, "2")
    
    # clicar no botao de inserir o valor 
    btn_inserir_valor = main_window.child_window(class_name="TBitBtn", found_index=0).click()

    # Clicar no botão + para salvar
    btn_salvar = main_window.child_window(class_name="TDBIBitBtn", found_index=5).click()
