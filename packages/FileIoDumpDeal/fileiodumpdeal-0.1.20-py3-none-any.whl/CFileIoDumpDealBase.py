...
from weberFuncs import SubSecondsYmdHnsX, GetCurrentTime
import os
...
class CFileIoDumpDealBase(object):
    ...
    def _LoopProcessTaskResult(self):
        iSleepSeconds = 60
        iLoopCnt = 0
        while True:
            try:
                ...
                PrintTimeMsg(f"_LoopProcessTaskResult.e={repr(e)}=WARNING!")
            PrintAndSleep(iSleepSeconds, f'_LoopProcessTaskResult.iLoopCnt={iLoopCnt}=iProcessCnt={iProcessCnt}=',
                          iLoopCnt % 10 == 0)
            iLoopCnt += 1
    ...
    def _LoopDumpFile4Query(self):
        ...
        while True:
            ...
            iDumpCnt = 0
            try:
                iDumpCnt, sNextMatchTime = self.DumpFile4Query(sQueryDir)
                if sNextMatchTime:
                    PrintTimeMsg(f"_LoopDumpFile4Query.sNextMatchTime={sNextMatchTime}=")
                    try:
                        sTm1 = sNextMatchTime + '.0'
                        sTm2 = GetCurrentTime() + '.0'
                        iSleepSecs = int(SubSecondsYmdHnsX(sTm1, sTm2))
                    except Exception as e:
                        PrintTimeMsg(f"_LoopDumpFile4Query.SubSecondsYmdHnsX({sTm1}, {sTm2})={repr(e)}=!")
                if iSleepSecs < 60:
                    iSleepSecs = 60
                if iSleepSecs > 60*60:
                    iSleepSecs = 60*60
    ...
    def LoopFileIoDumpDeal(self, sTaskType):
        ...
        if sTaskType == 'dump':
            ...
            return self._LoopProcessTaskResult()
