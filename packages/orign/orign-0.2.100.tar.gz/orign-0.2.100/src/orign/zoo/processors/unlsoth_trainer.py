    ...
    try:
        ...
        if adapter_resource:
            ...
            try:
                ...
                # If sync is successful and directory is not empty, try to find a specific checkpoint
                if os.path.exists(local_sft_runs_dir) and os.listdir(
                    local_sft_runs_dir
                ):
                    latest_sft_checkpoint = find_latest_checkpoint(local_sft_runs_dir)
                    if latest_sft_checkpoint:
                        sft_checkpoint_to_resume_from = latest_sft_checkpoint
                        print(
                            f"Found latest SFT checkpoint to resume from: {sft_checkpoint_to_resume_from}"
                        )
                    else:
                        print(
                            f"Warning: No valid checkpoint subdirectory found in {local_sft_runs_dir} despite it not being empty."
                        )
                        is_continue = (
                            False  # Cannot resume if no actual checkpoint subdir
                        )
                else:
                    print(
