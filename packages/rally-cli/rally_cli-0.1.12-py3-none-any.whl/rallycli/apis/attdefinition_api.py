...
class AttdefinitionAPI(BaseAPI):
    ...
    def create_attribute(self, attribute: Attdefinition) -> Attdefinition:
        created: dict = self._create_from_model(attribute, rally_type=type_names.ATTDEFINITION)
    ...
    def update_attribute(
        ...
        return Attdefinition(
            **self._update_from_model(attribute.copy_for_update(key_set), type_names.ATTDEFINITION)
        )
    ...
    def add_new_attribute_allowed_values(
        ...
        for value in allowedvaluelist:
            value.AttributeDefinition = attribute.ref
            self._create_from_model(value.copy_for_create(), type_names.ALLOWEDVALUE)
    ...
    def update_allowed_value(
        ...
        updated: dict = self._update_from_model(
            allowed_value.copy_for_update(body_keys=keyset), type_names.ALLOWEDVALUE
        )
