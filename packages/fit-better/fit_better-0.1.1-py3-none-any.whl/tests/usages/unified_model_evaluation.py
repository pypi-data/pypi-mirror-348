...
def main():
    ...
    # Data input options
    parser.add_argument("--input-dir", default="data", help="Directory containing input data files")
    parser.add_argument("--x-train", default="X_train.npy", help="Filename for X_train data")
    parser.add_argument("--y-train", default="y_train.npy", help="Filename for y_train data")
    parser.add_argument("--x-test", default="X_test.npy", help="Filename for X_test data")
    parser.add_argument("--y-test", default="y_test.npy", help="Filename for y_test data")
    parser.add_argument("--delimiter", default=None, help="Delimiter for CSV/TXT files")
    parser.add_argument("--header", default="infer", help="How to handle headers in CSV/TXT files")
    ...
    try:
        ...
        if args.evaluation_mode == "single":
            ...
            logger.info(f"Evaluating {regressor_type} with {partition_mode} partitioning (n={args.n_partitions})")
            ...
            if args.save_predictions:
                ...
                train_df.to_csv(os.path.join(predictions_dir, "best_model_train_predictions.csv"), index=False)
                test_df.to_csv(os.path.join(predictions_dir, "best_model_test_predictions.csv"), index=False)
                logger.info(f"Saved predictions to {predictions_dir}")
