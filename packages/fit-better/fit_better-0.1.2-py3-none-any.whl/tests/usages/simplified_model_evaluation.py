...
def main():
    ...
    try:
        ...
        logger.info(f"Starting simplified model evaluation in {args.evaluation_mode} mode")
        ...
        if not all(os.path.exists(p) for p in input_paths):
            ...
            if args.delimiter is None:
                if args.x_train.endswith(".txt"):
                    ...
                    args.delimiter = ","
                logger.info(f"Using default delimiter: '{args.delimiter}' based on file type")
        ...
        if args.evaluation_mode == "single":
            ...
            logger.info(f"Training a single model: {regressor_type.name} with {partition_mode.name} partitioning")
            ...
            # Log results
            logger.info(f"Evaluation completed: {result.model_type.name} with {result.partition_mode.name}")
            logger.info(f"Test metrics: R² = {result.metrics['r2']:.4f}, MAE = {result.metrics['mae']:.4f}")
            ...
            regressor_types = [RegressorType.LINEAR, RegressorType.RANDOM_FOREST, RegressorType.GRADIENT_BOOSTING]
            ...
            best_idx = 0
            best_value = -float('inf') if metric_to_optimize == Metric.R2 else float('inf')
            
            for i, res in enumerate(results):
                metric_value = res['r2'] if metric_to_optimize == Metric.R2 else res['mae']
            ...
            for res in results:
                logger.info(f"{res['regressor']} + {res['partition']}: R² = {res['r2']:.4f}, MAE = {res['mae']:.4f}")
                
            logger.info(f"Best configuration: {results[best_idx]['regressor']} with {results[best_idx]['partition']}")
            ...
            logger.info(f"Finding best model among {len(partition_modes)} partition modes and {len(regressor_types)} regressor types")
            ...
            logger.info(f"Found best model: {result.model_type.name} with {result.partition_mode.name}")
            logger.info(f"Test metrics: R² = {result.metrics['r2']:.4f}, MAE = {result.metrics['mae']:.4f}")
        ...
        if args.save_predictions:
            ...
            train_df.to_csv(os.path.join(predictions_dir, "train_predictions.csv"), index=False)
            test_df.to_csv(os.path.join(predictions_dir, "test_predictions.csv"), index=False)
        ...
        print(f"Partitioning: {result.partition_mode.name} with {result.n_partitions or 'N/A'} partitions")
        print(f"Test metrics: R² = {result.metrics['r2']:.4f}, MAE = {result.metrics['mae']:.4f}")
