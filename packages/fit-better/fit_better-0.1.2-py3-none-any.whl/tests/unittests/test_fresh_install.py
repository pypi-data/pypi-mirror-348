...
class FreshInstallTest(unittest.TestCase):
    ...
    def tearDown(self):
        ...
        try:
            ...
            results_archive_dir = os.path.join(self.project_root, "tests", "data_gen", 
            ...
            status = self.test_results["summary"]["status"] if "summary" in self.test_results else "unknown"
            ...
            if os.path.exists(self.results_dir) and os.listdir(self.results_dir):
                ...
                try:
                    os.symlink(os.path.basename(archive_path), latest_link, target_is_directory=True)
                    ...
                    try:
                        ...
                        print(f"Warning: Failed to create latest results copy: {str(copy_error)}")
    ...
    def test_fresh_install_and_tests(self):
        ...
        try:
            ...
            pip_conf_dir = os.path.join(self.venv_dir, "pip.conf" if sys.platform != "win32" else "pip.ini")
            ...
            step_duration = (datetime.datetime.now() - step_start).total_seconds()
            self._log_step_result("Create virtual environment", False, step_duration, f"Error: {str(e)}")
            raise
        ...
        deps_result = self._run_command(f"{pip_exe} install --upgrade pip wheel setuptools", verbose=True)
        ...
        if not success:
            step_duration = (datetime.datetime.now() - step_start).total_seconds()
            self._log_step_result("Build wheel package", False, step_duration, 
        ...
        if len(wheel_files) == 0:
            step_duration = (datetime.datetime.now() - step_start).total_seconds()
            self._log_step_result("Build wheel package", False, step_duration, "No wheel file was generated")
        ...
        if install_output is None:
            step_duration = (datetime.datetime.now() - step_start).total_seconds()
            self._log_step_result("Install wheel package", False, step_duration, "Failed to install the wheel package")
            raise AssertionError("Failed to install the wheel package")
        
        # Install test dependencies
        print("Installing test dependencies...")
        self._run_command(f"{pip_exe} install pytest numpy scikit-learn matplotlib joblib")
        ...
        step_duration = (datetime.datetime.now() - step_start).total_seconds()
        self._log_step_result("Install wheel package", True, step_duration, f"Installed {os.path.basename(wheel_file)}")
...
''')
        ...
        import_test = self._run_command(f"{python_exe} {import_test_script}", verbose=True)
        if import_test is None:
            step_duration = (datetime.datetime.now() - step_start).total_seconds()
            self._log_step_result("Verify package import", False, step_duration, "Failed to import the fit_better package")
        ...
        self._run_command(f"cp -r {os.path.join(self.project_root, 'tests')}/* {test_dir}/")
        ...
        if os.path.exists(test_data_script):
            data_gen_output = self._run_command(f"{python_exe} {test_data_script}", cwd=test_dir)
        ...
        if unit_test_output:
            ...
            with open(os.path.join(self.results_dir, "unit_tests_output.txt"), "w") as f:
        ...
        example_script = os.path.join(test_dir, "usages", "simplified_model_evaluation.py")
        if os.path.exists(example_script):
            example_output = self._run_command(f"{python_exe} {example_script}", cwd=test_dir)
        ...
        step_duration = (datetime.datetime.now() - step_start).total_seconds()
        self._log_step_result("Run tests with installed package", tests_passed, step_duration, 
                             "; ".join(test_messages))
...
''')
        ...
        step_duration = (datetime.datetime.now() - step_start).total_seconds()
        self._log_step_result("Package structure check", structure_success, step_duration, 
                             "Structure check completed" if structure_success else "Structure check failed")
        ...
        self.test_results["summary"]["status"] = "passed" if successful_steps == total_steps else "failed"
        self.test_results["summary"]["duration"] = total_duration
        self.test_results["summary"]["details"] = f"Completed {successful_steps}/{total_steps} steps successfully"
