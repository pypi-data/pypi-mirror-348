...
class PartitionTransformer(BaseEstimator, TransformerMixin):
    ...
    def _create_kmeans_boundaries(self, X: np.ndarray):
        ...
        kmeans = KMeans(n_clusters=self.n_parts, random_state=42)
        kmeans.fit(X_reshaped)

        # Get cluster centers and sort them
        centers = sorted(kmeans.cluster_centers_.ravel())

        # Create boundaries as midpoints between consecutive centers
        self.boundaries_ = [
...
class AdaptivePartitionRegressor(BaseEstimator):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray):
        ...
        self.partitioner_ = PartitionTransformer(
            n_parts=self.n_parts, partition_mode=self.partition_mode
        )
...
def train_adaptive_model(X, y, n_parts=3, partition_mode="kmeans", n_jobs=1, cv=5):
    """
    ...
    model = AdaptivePartitionRegressor(
        n_parts=n_parts, partition_mode=partition_mode, cv=cv, n_jobs=n_jobs
    )
    model.fit(X, y)
    return model


def create_ensemble_model(X, y, n_parts=3, partition_mode="kmeans", n_jobs=1):
    """
    ...
    for mode in partition_modes:
        model = AdaptivePartitionRegressor(
            n_parts=n_parts, partition_mode=mode, n_jobs=n_jobs
        )
...
def create_stacking_model(X, y, n_parts=3, partition_mode="kmeans", n_jobs=1, cv=5):
    """
    ...
    partitioner = PartitionTransformer(n_parts=n_parts, partition_mode=partition_mode)
...
class PolyPartitionPipeline:
    ...
    def __init__(self, poly_degree=2, n_parts=3, partition_mode="kmeans", n_jobs=1):
        """
