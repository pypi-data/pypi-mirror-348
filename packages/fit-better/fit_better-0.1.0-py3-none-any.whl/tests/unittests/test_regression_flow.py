...
class TestRegressionFlow:
    ...
    def test_initialization(self):
        ...
        # Check initial state
        assert flow.best_model_internal is None, "Initial best_model_internal should be None"
        assert (
        ...
        # Check scaler
        assert hasattr(flow, "scaler_internal"), "RegressionFlow should have a scaler attribute"
    ...
    def test_find_best_strategy_linear(self, sample_linear_data):
        ...
        assert hasattr(
            result, "model_type"
        ), "Result should have model_type attribute"
    ...
    def test_find_best_strategy_nonlinear(self, sample_nonlinear_data):
        ...
        if isinstance(flow.best_model_internal, dict) and "model" in flow.best_model_internal:
    ...
    def test_find_best_strategy_multivariate(self, sample_multivariate_data):
        ...
        assert result.metrics["r2"] > 0.4, "Multivariate data should have reasonable R² (> 0.4)"
        assert result.metrics["rmse"] < 1.0, "Multivariate data should have reasonable RMSE"
    ...
    def test_without_partitioning(self, sample_linear_data):
        ...
        assert result.metrics["r2"] > 0, "Linear regression without partitioning should have positive R²"
