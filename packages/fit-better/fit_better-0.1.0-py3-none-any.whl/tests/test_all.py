#!/usr/bin/env python3
"""
Run all regression tests for the fit-better package.
This script executes various tests to ensure the regression functionality
works correctly. Tests include partitioning, model training/evaluation,
...
TESTS = {
    ...
    "best_partition_and_regressor_example": str(USAGES_DIR / "best_partition_and_regressor_example.py"),
    "simplified_model_evaluation": str(USAGES_DIR / "simplified_model_evaluation.py"),
    "partition_and_regressor_example": str(USAGES_DIR / "partition_and_regressor_example.py"),
    ...
    for function_type in ["sine", "polynomial"]:
        ...
        try:
            ...
            try:
                ...
                process.kill()
                logger.error(f"Generating {function_type} data timed out after 300 seconds")
            ...
            if process.returncode == 0:
                ...
                print(f">>> FAILED: {function_type} data generation failed with code {process.returncode} <<<")
                all_succeeded = False
...
def clean_data_directories():
    ...
    # Clean logs, data, and visualization directories
    for path in [LOGS_DIR, DATA_DIR, VISUALIZATION_DIR]:
        if path.exists():
