...
class Wrap:
		...
		if path == "":
			...
			if not class_name == "":
				...
				if not config['module'] == "":
					...
					modul = modul(loader=kimin, base_path=kimin.base_path)
	...
	def _ImportModul(kimin, alias:str, *args, **params) -> Any:
		s = time.process_time()
		...
		kimin._loader.add(alias, *args, **params)
		if kimin.debug:
	...
	def Load(kimin, alias:str, *args, **params) -> Any:
		if alias in kimin.modul:
			return kimin.modul[alias]
		return kimin._ImportModul(alias, *args, **params)
