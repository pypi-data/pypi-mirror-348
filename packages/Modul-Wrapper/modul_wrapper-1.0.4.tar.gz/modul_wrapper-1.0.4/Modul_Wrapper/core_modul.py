...
class Wrap:
	...
	def Path_Function(kimin, config, *args, **kwargs):
		path = config.get('import_path', '')
		module_name = config.get('module', '')
		object_name = config.get('object', '')

		if path == "":
			# Import langsung dari nama module atau object
			if object_name != "":
				modul = importlib.import_module(object_name)
				return modul
			elif module_name != "":
				modul = importlib.import_module(module_name)
				return modul
			else:
				raise ImportError("Baik 'module' maupun 'object' kosong tanpa 'import_path'")
		else:
			modul = importlib.import_module(path)

			if module_name and object_name:
				# load class, panggil, ambil method
				cls = getattr(modul, module_name)
				instance = cls(loader=kimin, base_path=kimin.base_path)
				return getattr(instance, object_name)

			elif module_name and not object_name:
				# hanya ambil class
				return getattr(modul, module_name)

			elif not module_name and object_name:
				# hanya ambil fungsi global
				return getattr(modul, object_name)

			else:
				# cuma import modulnya aja
				return modul
