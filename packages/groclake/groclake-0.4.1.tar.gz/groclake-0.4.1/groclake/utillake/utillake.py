...
class GrocAgent:
    ...
    def __init__(self, app, agent_name, initial_intent=None, intent_description=None, intent_handler=None, adaptor_config=None):
        ...
        self._agent_uuid = adaptor_config.get('agent_uuid') if adaptor_config else None
        self._resource_agent_uuid = adaptor_config.get('resource_agent_uuid') if adaptor_config else None
        ...
        if openai_tool_config:
            ...
            os.environ["OPENAI_API_KEY"] = self.openai_tool_config["api_key"]  # Replace with secured key handling
        
        # Use the first lake resource (or adapt as needed)
        if lake_resource:
            self.lake_resource = lake_resource
            self.lake_config = lake_resource['resource_config'].get('lake_config', {})
            self.index_name = lake_resource['resource_id']
            self.lake_id = lake_resource['resource_id']

        self.update_adaptor_config(adaptor_config)
        ...
        try:
            query = """
                ...
                WHERE status = 'active'
                  AND agent_uuid = %s
            """
            results = self._mysql_connection.read(query, (self._agent_uuid,), multiple=True)
            intent_configs = []
            for row in results:
...
class AttpAdaptor:
    ...
    def query_handler(self):
        try:
            ...
            # Send the HTTP POST request
            if self._log_level == 'LOG_REQUEST_RESPONSE':
            ...
            # Send the HTTP POST request
            if self._log_level == 'LOG_REQUEST_RESPONSE' and response.get("metadata", {}).get("intent_handler_status", "") == "success":
