...
def connection(redis_container):
    ...
    with patch("redis_tools.connection.RedisToolsSettings", return_value=DummySettings(host, port)):
...
def test_delete_key(connection):
    ...
    assert connection.get_key_data("key_str_0")["value"] is None
