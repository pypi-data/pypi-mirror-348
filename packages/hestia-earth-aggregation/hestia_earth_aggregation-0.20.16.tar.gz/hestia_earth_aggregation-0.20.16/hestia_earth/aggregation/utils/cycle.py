...
def format_terms_results(data: dict, product: dict):
...
def format_country_results(data: dict, product: dict, site: dict):
    inputs = data.get('inputs', [])
    practices = data.get('practices', [])
    products = data.get('products', [])
    emissions = data.get('emissions', [])
    ...
    if len(cycles) > 0:
        ...
        return remove_empty_fields({
            ...
            **_format_results(cycle, product, site, completeness, inputs, practices, products, emissions),
