...
system_start.insert_Boot_up(cmd='nohup kcwebplus /index/index/mysocket/servers --cli > app/runtime/log/mysocket.log 2>&1 &',name='通知服务自启',types='kcwebplus',icon='https://img.kwebapp.cn/icon/kcweb.png')
...
class mysocket:
    def mysend():
        ...
        return successjson()
    def servers():
        #kcwebplus /index/index/mysocket/servers
        if config.app['cli']: #cli模式下运行
            kcwebplusdomain.banddomain(proxyitem={'notes':'首页通知服务代理','types':'websocket','rule':'/index/index/ws','url':'http://127.0.0.1:39030'}) #绑定代理域名
            socket=sockets()
    ...
    def restart():
        ...
        if 'Linux' in get_sysinfo()['platform']:
            ...
            cmd="nohup kcwebplus /index/index/mysocket/servers --cli > app/runtime/log/mysocket.log 2>&1 &"
            os.system(cmd)
        else:
