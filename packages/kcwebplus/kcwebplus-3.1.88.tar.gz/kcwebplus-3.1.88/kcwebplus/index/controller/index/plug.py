...
class plug:
    ...
    def uploadplug():
        ...
        if kcwebuserinfo:
            ...
            if data1[0]:
                data1=server.uploadplug(data['name'],kcwebuserinfo['username'],kcwebuserinfo['password'])
                if data1[0]:
                    return successjson()
                else:
            ...
            return errorjson(msg="请先配置kcweb账号")
    #临时增加
    ...
    def installplug():
        ...
        if os.path.exists(dirname):
            ...
            kcwebzip.packzip(dirname,"backup/"+dirname+"/backup.zip")
        ...
        if data[0]:
            ...
            if os.path.isfile("backup/"+dirname+"/backup.zip"):
                kcwebzip.unzip_file("backup/"+dirname+"/backup.zip",dirname)
            if not t:
