...
def check_role():
    ...
    if not status:
        if 'GET' == request.HEADER.Method() and not request.args.get('account_token'):
            return response.tpl("/common/html/error",title="无权访问",content="抱歉...，当前页面被黑名单拦截",imgsrc=config.domain['kcwebimg']+"/icon/suo.png",status="401 error")
        else:
    ...
    if ts != '/index/index/index/index' and ts != '/index/index/index/menu' and G.userinfo['role']['id'] !=1:
        ...
        if not status:
            if 'GET' == request.HEADER.Method() and not request.args.get('account_token'):
                return response.tpl("/common/html/error",title="无权访问",content="抱歉...，您当前没有此页面访问权限，请联系管理员",imgsrc=config.domain['kcwebimg']+"/icon/suo.png",status="401 error")
            else:
