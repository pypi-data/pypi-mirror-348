...
class Shelf(collections.abc.MutableMapping):
    ...
    def __init__(self, dict: sqlite._Database, protocol=None, writeback=False, keyencoding="utf-8", serializer=None):
        self.dict = dict
        if protocol is None:
            protocol = 5
        ...
        if serializer is None:
            self.serializer = serialer.PickleSerializer(protocol=protocol)
        else:
...
class DbfilenameShelf(Shelf):
    ...
    def __init__(self, filename, flag="c", protocol=None, writeback=False, serializer=None):
        sqlite3_kargs = dict(autocommit=True, check_same_thread=False)
        Shelf.__init__(
            self, sqlite.open(filename, flag, sqlite3_kargs=sqlite3_kargs), protocol, writeback, serializer=serializer
        )
...
def open(filename, flag="c", protocol=None, writeback=False, serializer=None):
    ...
    return DbfilenameShelf(filename, flag, protocol, writeback, serializer=serializer)
