...
from appdynamics.lib import default_log_formatter, change_permission_for_file
...
AGENT_BASE_DIR_PERMISSION = 0o744
...
PROXY_SUPPORT_PACKAGE_CHECK = (check_module_presence('appdynamics-proxysupport-linux-x64') or
                               check_module_presence('appdynamics-proxysupport-alpine-linux-x64') or
                               check_module_presence('appdynamics-proxysupport-linux-aarch64'))
    ...
    def mkdir(path, mode=0o777):
        try:
            ...
            if exc.errno != errno.EEXIST:
                raise
    mkdir(config.AGENT_BASE_DIR, AGENT_BASE_DIR_PERMISSION)
    mkdir(config.PROXY_RUN_DIR, AGENT_BASE_DIR_PERMISSION)
    mkdir(config.PROXY_CONTROL_PATH, AGENT_BASE_DIR_PERMISSION)
    mkdir(config.LOGS_DIR, AGENT_BASE_DIR_PERMISSION)
...
def start_watchdog(run_proxy_script=None, script_args=None, debug=False):
    ...
    if watchdog_pid == 0:
        ...
        try:
            ...
            fd = os.open(watchdog_pid_file, os.O_CREAT | os.O_EXCL | os.O_RDWR, mode=AGENT_BASE_DIR_PERMISSION)
        except OSError as e:
        ...
        os.close(fd)
        # changing permission for watchdog.pid if required
        change_permission_for_file(watchdog_pid_file, AGENT_BASE_DIR_PERMISSION)
...
def start_proxy(run_proxy_script=None, script_args=None, debug=False, proxy_logger=None):
    ...
    # changing permission for proxy.pid if required
    change_permission_for_file(proxy_pid_file, AGENT_BASE_DIR_PERMISSION)
...
def stop_process(name, pid_file):
    try:
        ...
        # changing permission for pid files if required
        change_permission_for_file(pid_file, AGENT_BASE_DIR_PERMISSION)
        return pid
    except IOError:
