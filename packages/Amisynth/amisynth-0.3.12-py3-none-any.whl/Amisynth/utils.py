...
class ContextAmisynth:
    ...
    def set_context(self, ctx):
        """Establece o cambia el contexto actual y asigna atributos basados en el tipo de contexto."""
        ...
        if isinstance(ctx, (tuple, list)):
            ...
            for c in ctx:
                ...
                self._assign_context(c)
            self.ctx = ctx[-1]  # Aquí asignamos el último contexto como el principal
            ContextAmisynth._last_ctx = ctx[-1]
            
        else:
            
            self.ctx = ctx
            ContextAmisynth._last_ctx = ctx
            self._assign_context(ctx)
            

        ContextAmisynth._instances[type(self.ctx).__name__] = self
        


    def _assign_context(self, ctx):
        if isinstance(ctx, datetime.datetime):
            self.timestamp = ctx
