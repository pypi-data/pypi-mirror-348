"""
storekissライブラリで並列処理を使用して大量のドキュメントを効率的に更新するテスト（バグ修正版）

このスクリプトは、storekissライブラリを使用して、並列処理により大量のドキュメントを
効率的に更新する方法を示します。前バージョンで発生した1件のドキュメントが更新されない
バグを修正しています。
"""
...
def update_document(doc, retry_count=3):
    """
    ドキュメントを更新する関数（並列処理で使用）
    
    Args:
        doc: 更新するドキュメント
        retry_count: 更新失敗時の再試行回数
        
    Returns:
        tuple: (ドキュメントID, 成功したかどうか, エラーメッセージ)
    """
    doc_id = doc.id
    attempts = 0
    ...
    while attempts < retry_count:
        try:
            # 更新を実行
            doc.reference.update({"updated": True, "timestamp": time.time(), "attempt": attempts + 1})
            
            # 更新が成功したことを確認
            updated_doc = doc.reference._document.get()
            updated_data = updated_doc.to_dict()
            
            if updated_data.get("updated") is True:
                logger.debug(f"ドキュメント {doc_id} の更新が成功しました (attempt={attempts + 1})")
                return (doc_id, True, None)
            else:
                logger.warning(f"ドキュメント {doc_id} の更新が反映されていません (attempt={attempts + 1})")
                attempts += 1
                time.sleep(0.01 * attempts)  # 再試行前に少し待機
        except Exception as e:
            error_msg = str(e)
            logger.error(f"ドキュメント {doc_id} の更新中にエラーが発生しました: {error_msg} (attempt={attempts + 1})")
            attempts += 1
            time.sleep(0.01 * attempts)  # 再試行前に少し待機
    
    return (doc_id, False, f"最大再試行回数({retry_count})を超えました")
...
def test_parallel_stream_update():
    ...
    # データベースファイルのパス
    db_path = "parallel_stream_fixed_test.db"
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 各ドキュメントの更新タスクを作成
        future_to_doc = {executor.submit(update_document, doc): doc.id for doc in all_docs}
        ...
        for i, future in enumerate(concurrent.futures.as_completed(future_to_doc)):
            ...
            try:
                ...
                if success:
                    ...
                    failed_docs.append((result_id, error_msg))
                
                # 進捗を表示
                if (i + 1) % 100 == 0 or i == len(all_docs) - 1:
                    logger.info(f"{i + 1}/{len(all_docs)}件処理しました (成功: {success_count}件, 失敗: {len(failed_docs)}件)")
            except Exception as e:
                logger.error(f"ドキュメント {doc_id} の処理中に例外が発生しました: {str(e)}")
    ...
    # 失敗したドキュメントがある場合は再試行
    if failed_docs:
        logger.warning(f"{len(failed_docs)}件のドキュメントの更新に失敗しました。再試行します...")
        retry_success = 0
        ...
        for doc_id, error_msg in failed_docs:
            try:
                ...
                doc = doc_ref.get()
                result_id, success, retry_error = update_document(doc, retry_count=5)
                
                if success:
                    retry_success += 1
                    logger.info(f"ドキュメント {doc_id} の再試行が成功しました")
                else:
                    logger.error(f"ドキュメント {doc_id} の再試行が失敗しました: {retry_error}")
            except Exception as e:
                logger.error(f"ドキュメント {doc_id} の再試行中に例外が発生しました: {str(e)}")
        
        logger.info(f"再試行結果: {retry_success}/{len(failed_docs)}件成功")
    ...
    if updated_count == doc_count:
        ...
        logger.error(f"更新されていないドキュメント: {not_updated_ids}")
        
        # 更新されていないドキュメントを個別に確認
        if not_updated_ids:
            logger.info("\n更新されていないドキュメントを詳細確認します...")
            for doc_id in not_updated_ids:
                try:
                    doc = collection.document(doc_id).get()
                    doc_data = doc.to_dict()
                    logger.info(f"ドキュメント {doc_id} の内容: {doc_data}")
                    
                    # 最終的な修正として強制的に更新
                    collection.document(doc_id).update({"updated": True, "timestamp": time.time(), "final_fix": True})
                    logger.info(f"ドキュメント {doc_id} を強制的に更新しました")
                except Exception as e:
                    logger.error(f"ドキュメント {doc_id} の確認中にエラーが発生しました: {str(e)}")
