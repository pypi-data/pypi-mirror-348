"""
storekissライブラリにFirestoreと互換性のあるdoc.reference.update()をサポートする例

このスクリプトでは、storekissライブラリのDocumentSnapshotクラスに
Firestoreのようなreference属性を追加し、doc.reference.update()のような
使い方をサポートする方法を示します。
"""

import os
import uuid
from storekiss import litestore
from storekiss.litestore import DocumentReference
import logkiss as logging

logger = logging.getLogger()
logger.setLevel(logging.DEBUG)

# DocumentSnapshotクラスの定義を取得
from storekiss.crud import DocumentSnapshot as OriginalDocumentSnapshot

# オリジナルのDocumentSnapshotクラスを保存
Original_DocumentSnapshot = OriginalDocumentSnapshot

# Firestoreと互換性のあるDocumentSnapshotクラスを実装
class FirestoreCompatibleDocumentSnapshot(OriginalDocumentSnapshot):
    """
    Firestoreと互換性のあるドキュメントスナップショット
    
    reference属性を追加して、doc.reference.update()のような使い方をサポートします。
    """
    
    @property
    def reference(self):
        """
        ドキュメント参照を返します。
        
        Returns:
            DocumentReference: ドキュメント参照
        """
        if not hasattr(self, '_reference') or self._reference is None:
            if self._id is not None and self._collection is not None and self._store is not None:
                # Documentオブジェクトを作成
                from storekiss.crud import Document
                doc = Document(self._store, self._collection, self._id)
                # DocumentReferenceオブジェクトを作成
                self._reference = DocumentReference(doc)
        return self._reference

def test_firestore_compatible_reference():
    """
    Firestoreと互換性のあるdoc.reference.update()のテスト
    """
    # データベースファイルのパス
    db_path = "firestore_compatible_test.db"
    
    # 既存のファイルがあれば削除
    if os.path.exists(db_path):
        os.remove(db_path)
    
    # クライアントを作成
    db = litestore.Client(db_path)
    
    # コレクション名
    collection_name = 'test_firestore_compatible'
    collection_ref = db.collection(collection_name)
    
    # テストデータを作成
    logger.info("テストデータを作成します")
    doc_ids = []
    for i in range(5):
        doc_id = str(uuid.uuid4())
        doc_ids.append(doc_id)
        collection_ref.document(doc_id).set({
            'name': f'Document {i}',
            'value': i * 10
        })
        logger.info("ドキュメント %s を作成しました", doc_id)
    
    # 元のDocumentSnapshotクラスを保存
    from storekiss.crud import DocumentSnapshot
    original_document_snapshot = DocumentSnapshot
    
    # 新しいDocumentSnapshotクラスを設定
    from storekiss.crud import DocumentSnapshot
    storekiss.crud.DocumentSnapshot = FirestoreCompatibleDocumentSnapshot
    
    try:
        # stream()メソッドを使用してすべてのドキュメントを取得
        logger.info("\nstream()メソッドを使用してドキュメントを取得")
        docs = collection_ref.stream()
        
        # Firestoreと同じ構文で更新: doc.reference.update()
        logger.info("\nFirestoreと同じ構文で更新: doc.reference.update()")
        for doc in docs:
            # doc.reference.update()を使用して更新
            doc.reference.update({'check': True})
            logger.info("ドキュメント %s を更新しました (check=True)", doc.id)
        
        # 更新後のドキュメントを確認
        logger.info("\n更新後のドキュメントを確認")
        updated_docs = collection_ref.get()
        success_count = 0
        for doc in updated_docs:
            doc_data = doc.to_dict()
            logger.info("ドキュメントID: %s, データ: %s", doc.id, doc_data)
            
            # checkフィールドが正しく設定されているか確認
            if doc_data.get('check') is True:
                logger.info("✓ checkフィールドが正しく設定されています")
                success_count += 1
            else:
                logger.error("✗ checkフィールドが設定されていません")
        
        # 結果を表示
        if success_count == len(doc_ids):
            logger.info("\n✅ すべてのドキュメントが正しく更新されました (%d/%d)", success_count, len(doc_ids))
        else:
            logger.error("\n❌ 一部のドキュメントが更新されていません (%d/%d)", success_count, len(doc_ids))
    
    finally:
        # 元のDocumentSnapshotクラスに戻す
        storekiss.crud.DocumentSnapshot = original_document_snapshot

if __name__ == "__main__":
    test_firestore_compatible_reference()
