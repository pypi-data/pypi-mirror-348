...
class CompositeColumn(Record):
    ...
    def TryGetColumnUnits(self, __unit: None=None) -> Array[OntologyAnnotation] | None:
        this: CompositeColumn = self
        def _arrow633(__unit: None=None) -> IEnumerable_1[OntologyAnnotation]:
            def _arrow632(cell: CompositeCell) -> IEnumerable_1[OntologyAnnotation]:
                return singleton(cell.AsUnitized[1]) if cell.is_unitized else empty()

            return collect(_arrow632, this.Cells)

        arr: Array[OntologyAnnotation] = to_array(delay(_arrow633))
...
CompositeColumn_reflection = _expr635
