...
def encode(row: FsRow) -> IEncodable:
    def _arrow282(__unit: None=None, row: Any=row) -> IEncodable:
    ...
    values: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("number", _arrow282()), ("cells", seq(map(mapping, row.Cells)))])
...
def _arrow290(builder: IGetters) -> tuple[int | None, IEnumerable_1[FsCell]]:
    n: int | None
...
decode: Decoder_1[tuple[int | None, IEnumerable_1[FsCell]]] = object(_arrow290)
