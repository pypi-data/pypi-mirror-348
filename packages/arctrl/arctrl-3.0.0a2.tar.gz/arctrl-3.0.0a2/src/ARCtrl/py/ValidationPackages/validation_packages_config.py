...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackagesConfig = self
        def _arrow3322(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow3321(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3320(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3319(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow3318(__unit: None=None) -> IEnumerable_1[str]:
                            def _arrow3317(__unit: None=None) -> IEnumerable_1[str]:
                                return singleton("}")

                            return append(singleton("]"), delay(_arrow3317))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow3318))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow3319))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow3320))

            return append(singleton("{"), delay(_arrow3321))

        return join("\n", to_list(delay(_arrow3322)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow3325(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow3324(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow3324, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow3325))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr3329
