        ...
        def _arrow3315(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow3314(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3313(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3312(__unit: None=None) -> IEnumerable_1[str]:
                        return singleton("}")

                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow3312))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow3313))

            return append(singleton("{"), delay(_arrow3314))

        return join("\n", to_list(delay(_arrow3315)))
...
ValidationPackage_reflection = _expr3316
