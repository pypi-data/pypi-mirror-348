...
def encoder(dc: DataContext) -> IEncodable:
    ...
    def _arrow2779(oa: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa)

    def _arrow2780(oa_1: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_1)

    def _arrow2782(oa_2: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_2)

    def _arrow2787(value: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2791(value_2: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2795(value_4: str, dc: Any=dc) -> IEncodable:
    ...
    values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([("data", encoder_1(dc)), try_include("explication", _arrow2779, DataContext__get_Explication(dc)), try_include("unit", _arrow2780, DataContext__get_Unit(dc)), try_include("objectType", _arrow2782, DataContext__get_ObjectType(dc)), try_include("description", _arrow2787, DataContext__get_Description(dc)), try_include("generatedBy", _arrow2791, DataContext__get_GeneratedBy(dc)), try_include("label", _arrow2795, DataContext__get_Label(dc))]))
...
def _arrow2816(get: IGetters) -> DataContext:
    data: Data
    object_arg: IRequiredGetter = get.Required
    data = object_arg.Field("data", decoder_1)
    def _arrow2805(__unit: None=None) -> OntologyAnnotation | None:
        object_arg_1: IOptionalGetter = get.Optional
        return object_arg_1.Field("explication", OntologyAnnotation_decoder)

    def _arrow2809(__unit: None=None) -> OntologyAnnotation | None:
        object_arg_2: IOptionalGetter = get.Optional
        return object_arg_2.Field("unit", OntologyAnnotation_decoder)

    def _arrow2810(__unit: None=None) -> OntologyAnnotation | None:
    ...
    return DataContext__ctor_Z780A8A2A(data.ID, data.Name, data.DataType, data.Format, data.SelectorFormat, _arrow2805(), _arrow2809(), _arrow2810(), _arrow2811(), _arrow2814(), _arrow2815(), data.Comments)


decoder: Decoder_1[DataContext] = object(_arrow2816)
