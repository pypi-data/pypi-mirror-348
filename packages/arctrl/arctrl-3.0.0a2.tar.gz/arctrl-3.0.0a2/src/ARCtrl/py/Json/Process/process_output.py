...
def _arrow2658(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2659(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2660(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ROCrate_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2658, ROCrate_decoder_1), map(_arrow2659, ROCrate_decoder_2), map(_arrow2660, ROCrate_decoder_3)]))
...
def _arrow2662(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2663(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2664(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ISAJson_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2662, ISAJson_decoder_1), map(_arrow2663, ISAJson_decoder_2), map(_arrow2664, ISAJson_decoder_3)]))
