...
def encoder(obj: LDGraph) -> IEncodable:
    ...
    def _arrow1960(__unit: None=None, obj: Any=obj) -> IEnumerable_1[tuple[str, IEncodable | None]]:
        def _arrow1955(value: str) -> IEncodable:
        ...
        def _arrow1959(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable | None]]:
            def _arrow1958(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable | None]]:
                def _arrow1956(kv: Any) -> IEnumerable_1[tuple[str, IEncodable | None]]:
                    l: str = kv[0].lower()
                ...
                def _arrow1957(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable | None]]:
                    return singleton(("@graph", seq(map_1(encoder_2, obj.Nodes))))

                return append(collect(_arrow1956, obj.GetProperties(True)), delay(_arrow1957))

            return append(singleton(try_include("@context", encoder_1, obj.TryGetContext())), delay(_arrow1958))

        return append(singleton(try_include("@id", _arrow1955, obj.Id)), delay(_arrow1959))

    values_1: FSharpList[tuple[str, IEncodable]] = choose(chooser, to_list(delay(_arrow1960)))
...
class ObjectExpr1963(Decoder_1[LDGraph]):
...
decoder: Decoder_1[LDGraph] = ObjectExpr1963()
