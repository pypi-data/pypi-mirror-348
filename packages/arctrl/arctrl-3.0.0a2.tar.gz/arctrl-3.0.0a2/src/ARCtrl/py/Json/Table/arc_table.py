...
def encoder(table: ArcTable) -> IEncodable:
    def _arrow2777(__unit: None=None, table: Any=table) -> IEnumerable_1[tuple[str, IEncodable]]:
        ...
        def _arrow2776(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
            ...
            def _arrow2775(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
                def key_encoder(tupled_arg: tuple[int, int]) -> IEncodable:
                    def _arrow2769(value: int, tupled_arg: Any=tupled_arg) -> IEncodable:
                    ...
                    def _arrow2771(value_2: int, tupled_arg: Any=tupled_arg) -> IEncodable:
                    ...
                    return tuple2(_arrow2769, _arrow2771, tupled_arg[0], tupled_arg[1])

                def _arrow2773(__unit: None=None) -> IEnumerable_1[tuple[tuple[int, int], CompositeCell]]:
                    def _arrow2772(match_value: Any) -> IEnumerable_1[tuple[tuple[int, int], CompositeCell]]:
                    ...
                    return collect(_arrow2772, table.Values)

                class ObjectExpr2774:
                    @property
                    def Compare(self) -> Callable[[tuple[int, int], tuple[int, int]], int]:
                        return compare_arrays

                return singleton(("values", map_1(key_encoder, encoder_2, of_seq(to_list(delay(_arrow2773)), ObjectExpr2774())))) if (len(table.Values) != 0) else empty()

            return append(singleton(("header", list_1(to_list(delay(_arrow2767))))) if (len(table.Headers) != 0) else empty(), delay(_arrow2775))

        return append(singleton(("name", _arrow2766())), delay(_arrow2776))

    values: IEnumerable_1[tuple[str, IEncodable]] = to_list(delay(_arrow2777))
...
def _arrow2789(get: IGetters) -> ArcTable:
    def _arrow2781(__unit: None=None) -> FSharpList[CompositeHeader] | None:
        arg_1: Decoder_1[FSharpList[CompositeHeader]] = list_1_1(decoder_1)
        object_arg: IOptionalGetter = get.Optional
        return object_arg.Field("header", arg_1)

    decoded_header: Array[CompositeHeader] = list(default_arg(_arrow2781(), empty_1()))
    def _arrow2783(__unit: None=None) -> Any | None:
        arg_3: Decoder_1[Any] = map_0027(tuple2_1(int_1, int_1), decoder_2)
        object_arg_1: IOptionalGetter = get.Optional
        return object_arg_1.Field("values", arg_3)

    class ObjectExpr2784:
        @property
        def Compare(self) -> Callable[[tuple[int, int], tuple[int, int]], int]:
            return compare_arrays
    ...
    decoded_values: Any = Dictionary(default_arg(_arrow2783(), empty_2(ObjectExpr2784())), ObjectExpr2785())
    def _arrow2788(__unit: None=None) -> str:
        object_arg_2: IRequiredGetter = get.Required
        return object_arg_2.Field("name", string)

    return ArcTable.create(_arrow2788(), decoded_header, decoded_values)


decoder: Decoder_1[ArcTable] = object(_arrow2789)
...
def encoder_compressed_column(column_index: int, row_count: int, cell_table: Any, table: ArcTable) -> IEncodable:
    if True if table.Headers[column_index].IsIOType else (row_count < 100):
        def _arrow2793(__unit: None=None, column_index: Any=column_index, row_count: Any=row_count, cell_table: Any=cell_table, table: Any=table) -> IEnumerable_1[IEncodable]:
            def _arrow2792(r: int) -> IEncodable:
                return encode_cell(cell_table, get_item_from_dict(table.Values, (column_index, r)))

            return map(_arrow2792, range_big_int(0, 1, row_count - 1))

        values: Array[IEncodable] = to_array(delay(_arrow2793))
        ...
        from_: int = 0
        def _arrow2827(__unit: None=None, column_index: Any=column_index, row_count: Any=row_count, cell_table: Any=cell_table, table: Any=table) -> IEnumerable_1[IEncodable]:
            def _arrow2808(i: int) -> IEnumerable_1[IEncodable]:
                next_1: CompositeCell = get_item_from_dict(table.Values, (column_index, i))
                def _arrow2806(__unit: None=None) -> IEncodable:
                    def _arrow2799(__unit: None=None) -> IEncodable:
                    ...
                    values_1: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("f", _arrow2799()), ("t", _arrow2801()), ("v", encode_cell(cell_table, current))])
                ...
                def _arrow2807(__unit: None=None) -> IEnumerable_1[IEncodable]:
                    nonlocal current, from_
                    current = next_1
                    from_ = i or 0
                    return empty()

                return append(singleton(_arrow2806()), delay(_arrow2807)) if (not equals(next_1, current)) else empty()

            def _arrow2825(__unit: None=None) -> IEnumerable_1[IEncodable]:
                def _arrow2824(__unit: None=None) -> IEncodable:
                    def _arrow2813(__unit: None=None) -> IEncodable:
                    ...
                    def _arrow2820(__unit: None=None) -> IEncodable:
                    ...
                    values_2: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("f", _arrow2813()), ("t", _arrow2820()), ("v", encode_cell(cell_table, current))])
                ...
                return singleton(_arrow2824())

            return append(collect(_arrow2808, range_big_int(1, 1, row_count - 1)), delay(_arrow2825))

        values_3: Array[IEncodable] = to_array(delay(_arrow2827))
...
def decoder_compressed_column(cell_table: Array[CompositeCell], table: ArcTable, column_index: int) -> Decoder_1[None]:
    class ObjectExpr2834(Decoder_1[None]):
        def Decode(self, helper: IDecoderHelpers_1[Any], column: Any, cell_table: Any=cell_table, table: Any=table, column_index: Any=column_index) -> FSharpResult_2[None, tuple[str, ErrorReason_1[__A_]]]:
            ...
            if match_value.tag == 1:
                def _arrow2833(get: IGetters) -> None:
                    from_: int
                ...
                range_decoder: Decoder_1[None] = object(_arrow2833)
...
def arrayi(decoderi: Callable[[int], Decoder_1[_VALUE]]) -> Decoder_1[Array[Any]]:
    class ObjectExpr2844(Decoder_1[Array[_VALUE_]]):
                ...
                def folder(acc: FSharpResult_2[Array[_VALUE_], tuple[str, ErrorReason_1[__A_]]], value_1: __A_) -> FSharpResult_2[Array[_VALUE_], tuple[str, ErrorReason_1[__A_]]]:
                    ...
                    if acc.tag == 0:
                        ...
                        if match_value.tag == 0:
                            ...
                            def _arrow2843(__unit: None=None, acc: Any=acc, value_1: Any=value_1) -> tuple[str, ErrorReason_1[__A_]]:
                            ...
                            return FSharpResult_2(1, _arrow2843())
...
def encoder_compressed(string_table: Any, oa_table: Any, cell_table: Any, table: ArcTable) -> IEncodable:
    def _arrow2856(__unit: None=None, string_table: Any=string_table, oa_table: Any=oa_table, cell_table: Any=cell_table, table: Any=table) -> IEnumerable_1[tuple[str, IEncodable]]:
        def _arrow2855(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
            def _arrow2848(__unit: None=None) -> IEnumerable_1[IEncodable]:
                return map(encoder_1, table.Headers)

            def _arrow2854(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
                if len(table.Values) != 0:
                    row_count: int = table.RowCount or 0
                    def _arrow2850(__unit: None=None) -> IEnumerable_1[IEncodable]:
                        def _arrow2849(c: int) -> IEncodable:
                            return encoder_compressed_column(c, row_count, cell_table, table)

                        return map(_arrow2849, range_big_int(0, 1, table.ColumnCount - 1))

                    columns: Array[IEncodable] = to_array(delay(_arrow2850))
                    ...
                    return singleton(("c", ObjectExpr2853()))

                else: 
                    return empty()


            return append(singleton(("h", list_1(to_list(delay(_arrow2848))))) if (len(table.Headers) != 0) else empty(), delay(_arrow2854))

        return append(singleton(("n", encode_string(string_table, table.Name))), delay(_arrow2855))

    values_1: IEnumerable_1[tuple[str, IEncodable]] = to_list(delay(_arrow2856))
...
def decoder_compressed(string_table: Array[str], oa_table: Array[OntologyAnnotation], cell_table: Array[CompositeCell]) -> Decoder_1[ArcTable]:
    def _arrow2871(get: IGetters, string_table: Any=string_table, oa_table: Any=oa_table, cell_table: Any=cell_table) -> ArcTable:
        def _arrow2864(__unit: None=None) -> FSharpList[CompositeHeader] | None:
            arg_1: Decoder_1[FSharpList[CompositeHeader]] = list_1_1(decoder_1)
            object_arg: IOptionalGetter = get.Optional
            return object_arg.Field("h", arg_1)

        decoded_header: Array[CompositeHeader] = list(default_arg(_arrow2864(), empty_1()))
        ...
        table: ArcTable = ArcTable.create(_arrow2865(), decoded_header, Dictionary([], ObjectExpr2866()))
        def _arrow2868(__unit: None=None) -> Array[None] | None:
            def _arrow2867(column_index: int) -> Decoder_1[None]:
                return decoder_compressed_column(cell_table, table, column_index)

            arg_5: Decoder_1[Array[None]] = arrayi(_arrow2867)
