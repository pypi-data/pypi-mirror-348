...
def ActivePattern_ontologyAnnotationFromFsCells(tsr_col: int | None, tan_col: int | None, cells: FSharpList[FsCell]) -> OntologyAnnotation:
    ...
    def _arrow1352(i: int, tsr_col: Any=tsr_col, tan_col: Any=tan_col, cells: Any=cells) -> str:
        return item(i, cell_values)

    tsr: str | None = map_1(_arrow1352, tsr_col)
    def _arrow1353(i_1: int, tsr_col: Any=tsr_col, tan_col: Any=tan_col, cells: Any=cells) -> str:
        return item(i_1, cell_values)

    tan: str | None = map_1(_arrow1353, tan_col)
...
def ActivePattern_dataFromFsCells(format: int | None, selector_format: int | None, cells: FSharpList[FsCell]) -> Data:
    ...
    cell_values: FSharpList[str] = map(mapping, cells)
    def _arrow1354(i: int, format: Any=format, selector_format: Any=selector_format, cells: Any=cells) -> str:
        return item(i, cell_values)

    format_1: str | None = map_1(_arrow1354, format)
    def _arrow1355(i_1: int, format: Any=format, selector_format: Any=selector_format, cells: Any=cells) -> str:
        return item(i_1, cell_values)

    selector_format_1: str | None = map_1(_arrow1355, selector_format)
...
def ActivePattern__007CExplication_007C__007C(cells: FSharpList[FsCell]) -> Callable[[DataContext, FSharpList[FsCell]], DataContext] | None:
    ...
    if active_pattern_result is not None:
        ...
        def Value(dc: DataContext, cells: Any=cells) -> Callable[[FSharpList[FsCell]], DataContext]:
            def _arrow1356(cells_1: FSharpList[FsCell], dc: Any=dc) -> DataContext:
...
def ActivePattern__007CUnit_007C__007C(cells: FSharpList[FsCell]) -> Callable[[DataContext, FSharpList[FsCell]], DataContext] | None:
    ...
    if active_pattern_result is not None:
        r: Callable[[FSharpList[FsCell]], OntologyAnnotation] = active_pattern_result
        def Value(dc: DataContext, cells: Any=cells) -> Callable[[FSharpList[FsCell]], DataContext]:
            def _arrow1357(cells_1: FSharpList[FsCell], dc: Any=dc) -> DataContext:
...
def ActivePattern__007CObjectType_007C__007C(cells: FSharpList[FsCell]) -> Callable[[DataContext, FSharpList[FsCell]], DataContext] | None:
    ...
    if active_pattern_result is not None:
        r: Callable[[FSharpList[FsCell]], OntologyAnnotation] = active_pattern_result
        def Value(dc: DataContext, cells: Any=cells) -> Callable[[FSharpList[FsCell]], DataContext]:
            def _arrow1358(cells_1: FSharpList[FsCell], dc: Any=dc) -> DataContext:
...
def from_fs_cells(cells: FSharpList[FsCell]) -> Callable[[DataContext, FSharpList[FsCell]], DataContext]:
    ...
    if pattern_matching_result == 0:
        def _arrow1366(dc: DataContext, cells: Any=cells) -> Callable[[FSharpList[FsCell]], DataContext]:
            def _arrow1365(cells_1: FSharpList[FsCell]) -> DataContext:
                return r(dc)(cells_1)
        ...
        def _arrow1367(arg_1: DataContext, cells: Any=cells) -> Callable[[FSharpList[FsCell]], DataContext]:
...
def to_fs_cells(comment_keys: FSharpList[str]) -> FSharpList[FsCell]:
    def _arrow1375(__unit: None=None, comment_keys: Any=comment_keys) -> IEnumerable_1[FsCell]:
            ...
            def _arrow1373(__unit: None=None) -> IEnumerable_1[FsCell]:
                def _arrow1372(__unit: None=None) -> IEnumerable_1[FsCell]:
                    def _arrow1371(__unit: None=None) -> IEnumerable_1[FsCell]:
                        def _arrow1370(__unit: None=None) -> IEnumerable_1[FsCell]:
                            def _arrow1369(__unit: None=None) -> IEnumerable_1[FsCell]:
                                def _arrow1368(__unit: None=None) -> IEnumerable_1[FsCell]:
                                    return map_2(FsCell, comment_keys)

                                return append(to_enumerable([FsCell("Label")]), delay(_arrow1368))

                            return append(to_enumerable([FsCell("Generated By")]), delay(_arrow1369))

                        return append(to_enumerable([FsCell("Description")]), delay(_arrow1370))

                    return append(to_enumerable([FsCell("Object Type"), FsCell("Term Source REF"), FsCell("Term Accession Number")]), delay(_arrow1371))

                return append(to_enumerable([FsCell("Unit"), FsCell("Term Source REF"), FsCell("Term Accession Number")]), delay(_arrow1372))

            return append(to_enumerable([FsCell("Explication"), FsCell("Term Source REF"), FsCell("Term Accession Number")]), delay(_arrow1373))

        return append(to_enumerable([FsCell("Data"), FsCell("Data Format"), FsCell("Data Selector Format")]), delay(_arrow1374))

    return to_list(delay(_arrow1375))
