...
def from_string(pub_med_id: str | None, doi: str | None, author: str | None, title: str | None, status: str | None, status_term_source_ref: str | None, status_term_accession_number: str | None, comments: Array[Comment]) -> Publication:
    ...
    def _arrow927(s: str, pub_med_id: Any=pub_med_id, doi: Any=doi, author: Any=author, title: Any=title, status: Any=status, status_term_source_ref: Any=status_term_source_ref, status_term_accession_number: Any=status_term_accession_number, comments: Any=comments) -> str:
        return URIModule_fromString(s)

    pub_med_id_1: str | None = map(_arrow927, pub_med_id)
...
def from_sparse_table(matrix: SparseTable) -> FSharpList[Publication]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow930(__unit: None=None, matrix: Any=matrix) -> Publication:
        ...
        def _arrow932(i: int, matrix: Any=matrix) -> Publication:
        ...
        return initialize(matrix.ColumnCount, _arrow932)
...
def to_sparse_table(publications: FSharpList[Publication]) -> SparseTable:
    ...
    class ObjectExpr943:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow942(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr943())), matrix.ColumnCount)
