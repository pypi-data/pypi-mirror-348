...
def Clone_createCloneContract_5000466F(remote_url: str, merge: bool | None=None, branch: str | None=None, token: tuple[str, str] | None=None, nolfs: bool | None=None) -> Contract:
    ...
    def _arrow3472(__unit: None=None, remote_url: Any=remote_url, merge: Any=merge, branch: Any=branch, token: Any=token, nolfs: Any=nolfs) -> IEnumerable_1[str]:
            ...
            def _arrow3470(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow3469(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3468(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow3467(__unit: None=None) -> IEnumerable_1[str]:
                            return singleton(".") if merge_1 else empty()

                        return append(singleton(remote_url_1), delay(_arrow3467))

                    return append(singleton(value(branch)) if (branch is not None) else empty(), delay(_arrow3468))

                return append(singleton(Clone_get_branchFlag()) if (branch is not None) else empty(), delay(_arrow3469))

            return append(singleton(Clone_get_noLFSConfig()) if nolfs_1 else empty(), delay(_arrow3470))

        return append(singleton(Clone_get_clone()), delay(_arrow3471))

    return create_git_contract(to_array(delay(_arrow3472)))
