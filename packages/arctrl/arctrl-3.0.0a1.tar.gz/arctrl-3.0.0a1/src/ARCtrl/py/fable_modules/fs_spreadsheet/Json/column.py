...
def encode(col: FsColumn) -> IEncodable:
    def _arrow303(__unit: None=None, col: Any=col) -> IEncodable:
    ...
    values: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("number", _arrow303()), ("cells", seq(map(mapping, col.Cells)))])
    class ObjectExpr307(IEncodable):
...
def encode_no_numbers(col: IEnumerable_1[FsCell]) -> IEncodable:
    ...
    class ObjectExpr311(IEncodable):
...
def _arrow313(builder: IGetters) -> tuple[int | None, IEnumerable_1[FsCell]]:
    n: int | None
    object_arg: IOptionalGetter = builder.Optional
    n = object_arg.Field("number", int_1)
    def _arrow312(__unit: None=None) -> IEnumerable_1[FsCell] | None:
    ...
    return (n, default_arg(_arrow312(), empty()))


decode: Decoder_1[tuple[int | None, IEnumerable_1[FsCell]]] = object(_arrow313)
