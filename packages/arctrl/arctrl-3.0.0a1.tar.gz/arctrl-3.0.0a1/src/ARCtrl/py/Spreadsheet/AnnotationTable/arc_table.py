...
def compose_columns(string_cell_columns: Array[Array[str]]) -> Array[CompositeColumn]:
    def _arrow1329(columns: Array[Array[str]], string_cell_columns: Any=string_cell_columns) -> CompositeColumn:
        return from_string_cell_columns(columns)

    return map_1(_arrow1329, group_columns_by_header(string_cell_columns), None)
...
def try_from_fs_worksheet(sheet: FsWorksheet) -> ArcTable | None:
    try: 
        ...
        if match_value is None:
            ...
            def _arrow1333(__unit: None=None) -> IEnumerable_1[Array[str]]:
                def _arrow1332(c: int) -> Array[str]:
                    def _arrow1331(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow1330(r: int) -> IEnumerable_1[str]:
                        ...
                        return collect(_arrow1330, range_big_int(1, 1, FsAddress__get_RowNumber(FsRangeAddress__get_LastAddress(FsRangeBase__get_RangeAddress(t)))))

                    return to_array_1(delay(_arrow1331))

                return map_2(_arrow1332, range_big_int(1, 1, FsAddress__get_ColumnNumber(FsRangeAddress__get_LastAddress(FsRangeBase__get_RangeAddress(t)))))

            composite_columns: Array[CompositeColumn] = compose_columns(map_1(fix_deprecated_ioheader, to_array_1(delay(_arrow1333)), None))
...
def to_fs_worksheet(index: int | None, table: ArcTable) -> FsWorksheet:
    ...
    if table.ColumnCount == 0:
        ...
        def _arrow1334(column_1: CompositeColumn, index: Any=index, table: Any=table) -> FSharpList[FSharpList[str]]:
            return to_string_cell_columns(column_1)

        def _arrow1335(column: CompositeColumn, index: Any=index, table: Any=table) -> enum_type("ARCtrl.Spreadsheet.ArcTable.ColumnOrder", int, [("InputClass", 1.0), ("ProtocolClass", 2.0), ("ParamsClass", 3.0), ("OutputClass", 4.0)]):
            return classify_column_order(column)

        class ObjectExpr1336:
            @property
        ...
        columns: FSharpList[FSharpList[str]] = collect_1(_arrow1334, sort_by(_arrow1335, of_array(table.Columns), ObjectExpr1336()))
        table_row_count: int
        def folder(acc: int, c: FSharpList[str], index: Any=index, table: Any=table) -> int:
            def _arrow1337(x_1: int, y_1: int, acc: Any=acc, c: Any=c) -> int:
                return compare_primitives(x_1, y_1)

            return max(_arrow1337, acc, length(c))
