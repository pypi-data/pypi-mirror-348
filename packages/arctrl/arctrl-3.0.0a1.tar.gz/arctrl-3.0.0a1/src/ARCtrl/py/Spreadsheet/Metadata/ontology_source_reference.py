...
def from_sparse_table(matrix: SparseTable) -> FSharpList[OntologySourceReference]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow1095(__unit: None=None, matrix: Any=matrix) -> OntologySourceReference:
            return_val: OntologySourceReference = OntologySourceReference.create()
            return_val.Comments = comments
            return return_val

        return singleton(_arrow1095())

    else: 
        def _arrow1096(i: int, matrix: Any=matrix) -> OntologySourceReference:
        ...
        return initialize(matrix.ColumnCount, _arrow1096)
...
def to_sparse_table(ontology_sources: FSharpList[OntologySourceReference]) -> SparseTable:
    ...
    class ObjectExpr1098:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow1097(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr1098())), matrix.ColumnCount)
