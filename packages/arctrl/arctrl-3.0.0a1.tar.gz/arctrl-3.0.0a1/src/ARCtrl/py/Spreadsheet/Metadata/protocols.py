...
def from_sparse_table(matrix: SparseTable) -> FSharpList[Protocol]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow887(i: int, matrix: Any=matrix) -> Protocol:
        ...
        return initialize(matrix.ColumnCount, _arrow887)
...
def to_sparse_table(protocols: FSharpList[Protocol]) -> SparseTable:
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr889())), matrix.ColumnCount)
