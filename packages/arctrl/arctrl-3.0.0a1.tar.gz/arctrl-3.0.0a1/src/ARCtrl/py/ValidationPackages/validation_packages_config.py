...
class ValidationPackagesConfig:
                ...
                def _arrow3321(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow3320(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow3319(__unit: None=None) -> IEnumerable_1[str]:
                            ...
                            return append(singleton("]"), delay(_arrow3318))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow3319))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow3320))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow3321))

            return append(singleton("{"), delay(_arrow3322))

        return join("\n", to_list(delay(_arrow3323)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        def sort(arg: Array[ValidationPackage]) -> Array[ValidationPackage]:
            ...
            return sort_by(projection, list(arg), ObjectExpr3324())
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow3326(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow3325(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow3325, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow3326))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr3327
