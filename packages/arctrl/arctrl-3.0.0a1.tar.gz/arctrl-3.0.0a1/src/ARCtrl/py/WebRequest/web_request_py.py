...
Response_reflection = _expr3608

def _expr3609() -> TypeInfo:
...
def Requests_createHeadersDict_9F3777D(headers: Any | None=None) -> Any | None:
    def mapping(values: Any, headers: Any=headers) -> Any:
        def _arrow3611(__unit: None=None, values: Any=values) -> IEnumerable_1[tuple[str, Any]]:
            def _arrow3610(pair: Any) -> tuple[str, Any]:
                return (pair[0], pair[1])

            return map_1(_arrow3610, values)

        return create_obj(to_list(delay(_arrow3611)))
...
def Requests_mapResponseType_Z730CA7D4(response: InteropResponseType) -> Response:
    ...
    def _arrow3613(__unit: None=None, response: Any=response) -> IEnumerable_1[tuple[str, str]]:
        def _arrow3612(header_name: str) -> IEnumerable_1[tuple[str, str]]:
            return singleton((header_name, response.headers[header_name]))

        return collect(_arrow3612, list(response.headers.keys()))

    class ObjectExpr3614:
        @property
        def Compare(self) -> Callable[[str, str], int]:
            return compare_primitives

    return Response(status_code, text, of_list(to_list(delay(_arrow3613)), ObjectExpr3614()), encoding)
