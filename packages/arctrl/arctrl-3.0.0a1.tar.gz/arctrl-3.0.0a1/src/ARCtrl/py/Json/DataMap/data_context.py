...
def encoder(dc: DataContext) -> IEncodable:
    ...
    def _arrow2800(oa: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa)

    def _arrow2802(oa_1: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_1)

    def _arrow2803(oa_2: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_2)

    def _arrow2809(value: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2812(value_2: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2814(value_4: str, dc: Any=dc) -> IEncodable:
    ...
    values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([("data", encoder_1(dc)), try_include("explication", _arrow2800, DataContext__get_Explication(dc)), try_include("unit", _arrow2802, DataContext__get_Unit(dc)), try_include("objectType", _arrow2803, DataContext__get_ObjectType(dc)), try_include("description", _arrow2809, DataContext__get_Description(dc)), try_include("generatedBy", _arrow2812, DataContext__get_GeneratedBy(dc)), try_include("label", _arrow2814, DataContext__get_Label(dc))]))
...
def _arrow2825(get: IGetters) -> DataContext:
    data: Data
    object_arg: IRequiredGetter = get.Required
    data = object_arg.Field("data", decoder_1)
    def _arrow2818(__unit: None=None) -> OntologyAnnotation | None:
        object_arg_1: IOptionalGetter = get.Optional
        return object_arg_1.Field("explication", OntologyAnnotation_decoder)

    def _arrow2819(__unit: None=None) -> OntologyAnnotation | None:
        object_arg_2: IOptionalGetter = get.Optional
        return object_arg_2.Field("unit", OntologyAnnotation_decoder)

    def _arrow2820(__unit: None=None) -> OntologyAnnotation | None:
    ...
    def _arrow2823(__unit: None=None) -> str | None:
        object_arg_6: IOptionalGetter = get.Optional
        return object_arg_6.Field("generatedBy", string)

    return DataContext__ctor_Z780A8A2A(data.ID, data.Name, data.DataType, data.Format, data.SelectorFormat, _arrow2818(), _arrow2819(), _arrow2820(), _arrow2821(), _arrow2822(), _arrow2823(), data.Comments)


decoder: Decoder_1[DataContext] = object(_arrow2825)
