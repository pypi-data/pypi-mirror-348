...
def _arrow2632(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2633(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2635(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ROCrate_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2632, ROCrate_decoder_1), map(_arrow2633, ROCrate_decoder_2), map(_arrow2635, ROCrate_decoder_3)]))
...
def _arrow2639(Item: Sample) -> ProcessOutput:
    return ProcessOutput(0, Item)


def _arrow2640(Item_1: Data) -> ProcessOutput:
    return ProcessOutput(1, Item_1)


def _arrow2642(Item_2: Material) -> ProcessOutput:
    return ProcessOutput(2, Item_2)


ISAJson_decoder: Decoder_1[ProcessOutput] = one_of(of_array([map(_arrow2639, ISAJson_decoder_1), map(_arrow2640, ISAJson_decoder_2), map(_arrow2642, ISAJson_decoder_3)]))
