...
def _arrow2236(Item: int) -> Value:
    return Value(1, Item)


def _arrow2237(Item_1: float) -> Value:
    return Value(2, Item_1)


def _arrow2238(Item_2: OntologyAnnotation) -> Value:
    return Value(0, Item_2)


def _arrow2239(Item_3: str) -> Value:
    return Value(3, Item_3)


decoder: Decoder_1[Value] = one_of(of_array([map(_arrow2236, int_1), map(_arrow2237, float_1), map(_arrow2238, OntologyAnnotation_ISAJson_decoder), map(_arrow2239, string)]))
