...
def encoder(id_map: Any | None, value: ProtocolParameter) -> IEncodable:
    def f(value_1: ProtocolParameter, id_map: Any=id_map, value: Any=value) -> IEncodable:
        ...
        def _arrow2448(value_2: str, value_1: Any=value_1) -> IEncodable:
        ...
        def _arrow2449(oa: OntologyAnnotation, value_1: Any=value_1) -> IEncodable:
            return OntologyAnnotation_ISAJson_encoder(id_map, oa)

        values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([try_include("@id", _arrow2448, gen_id(value_1)), try_include("parameterName", _arrow2449, value_1.ParameterName)]))
        class ObjectExpr2450(IEncodable):
    ...
    if id_map is not None:
        def _arrow2451(p_1: ProtocolParameter, id_map: Any=id_map, value: Any=value) -> str:
            return gen_id(p_1)

        return encode(_arrow2451, f, value, id_map)
...
def _arrow2453(get: IGetters) -> ProtocolParameter:
    def _arrow2452(__unit: None=None) -> OntologyAnnotation | None:
        object_arg: IOptionalGetter = get.Optional
        return object_arg.Field("parameterName", OntologyAnnotation_ISAJson_decoder)

    return ProtocolParameter(None, _arrow2452())


decoder: Decoder_1[ProtocolParameter] = object(_arrow2453)
