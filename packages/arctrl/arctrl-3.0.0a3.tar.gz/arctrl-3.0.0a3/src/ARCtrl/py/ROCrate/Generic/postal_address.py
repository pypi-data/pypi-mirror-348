...
class LDPostalAddress:
    ...
    def gen_id(address_country: str | None=None, postal_code: str | None=None, street_address: str | None=None) -> str:
        def _arrow1509(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow1508(__unit: None=None) -> IEnumerable_1[str]:
                ...
                return append(singleton("postalCode") if (postal_code is not None) else empty(), delay(_arrow1507))

            return append(singleton("addressCountry") if (address_country is not None) else empty(), delay(_arrow1508))

        items: FSharpList[str] = to_list(delay(_arrow1509))
        def _arrow1510(__unit: None=None) -> str:
        ...
        return clean(create_missing_identifier() if is_empty(items) else _arrow1510())
...
LDPostalAddress_reflection = _expr1511
