...
def try_int(str_1: str) -> int | None:
    ...
    out_arg: int = 0
    def _arrow3483(__unit: None=None, str_1: Any=str_1) -> int:
        return out_arg

    def _arrow3484(v: int, str_1: Any=str_1) -> None:
        nonlocal out_arg
        out_arg = v or 0

    match_value = (try_parse(str_1, 511, False, 32, FSharpRef(_arrow3483, _arrow3484)), out_arg)
    if match_value[0]:
...
def try_get_parameter_column_index(param_value: ProcessParameterValue) -> int | None:
    def _arrow3487(param: ProtocolParameter, param_value: Any=param_value) -> int | None:
        return try_get_parameter_index(param)

    return bind(_arrow3487, param_value.Category)
