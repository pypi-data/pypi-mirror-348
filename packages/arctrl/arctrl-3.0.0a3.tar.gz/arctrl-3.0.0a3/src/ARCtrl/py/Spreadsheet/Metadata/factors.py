...
def from_sparse_table(matrix: SparseTable) -> FSharpList[Factor]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow882(i: int, matrix: Any=matrix) -> Factor:
        ...
        return initialize(matrix.ColumnCount, _arrow882)
...
def to_sparse_table(factors: FSharpList[Factor]) -> SparseTable:
    ...
    class ObjectExpr884:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow883(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr884())), matrix.ColumnCount)
