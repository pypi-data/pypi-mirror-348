...
def encoder(dc: DataContext) -> IEncodable:
    ...
    def _arrow2808(oa: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa)

    def _arrow2809(oa_1: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_1)

    def _arrow2810(oa_2: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_2)

    def _arrow2812(value: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2816(value_2: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2819(value_4: str, dc: Any=dc) -> IEncodable:
    ...
    values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([("data", encoder_1(dc)), try_include("explication", _arrow2808, DataContext__get_Explication(dc)), try_include("unit", _arrow2809, DataContext__get_Unit(dc)), try_include("objectType", _arrow2810, DataContext__get_ObjectType(dc)), try_include("description", _arrow2812, DataContext__get_Description(dc)), try_include("generatedBy", _arrow2816, DataContext__get_GeneratedBy(dc)), try_include("label", _arrow2819, DataContext__get_Label(dc))]))
...
def _arrow2837(get: IGetters) -> DataContext:
    ...
    def _arrow2836(__unit: None=None) -> str | None:
        object_arg_6: IOptionalGetter = get.Optional
        return object_arg_6.Field("generatedBy", string)

    return DataContext__ctor_Z780A8A2A(data.ID, data.Name, data.DataType, data.Format, data.SelectorFormat, _arrow2829(), _arrow2830(), _arrow2831(), _arrow2833(), _arrow2835(), _arrow2836(), data.Comments)


decoder: Decoder_1[DataContext] = object(_arrow2837)
