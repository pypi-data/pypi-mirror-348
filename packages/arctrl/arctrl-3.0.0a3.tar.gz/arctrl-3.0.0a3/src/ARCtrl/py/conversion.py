...
def DateTime_tryFromString(s: str) -> Any | None:
    try: 
        def _arrow3666(__unit: None=None) -> Any:
...
def ColumnIndex_tryInt(str_1: str) -> int | None:
    ...
    out_arg: int = 0
    def _arrow3667(__unit: None=None, str_1: Any=str_1) -> int:
        return out_arg

    def _arrow3668(v: int, str_1: Any=str_1) -> None:
        nonlocal out_arg
        out_arg = v or 0

    match_value = (try_parse(str_1, 511, False, 32, FSharpRef(_arrow3667, _arrow3668)), out_arg)
    if match_value[0]:
...
BaseTypes_reflection = _expr3669
...
ProcessConversion_reflection = _expr3670
...
def ProcessConversion_tryComponentGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if value_header.tag == 0:
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3671(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryParameterGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if value_header.tag == 3:
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3672(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryFactorGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if value_header.tag == 2:
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3673(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryCharacteristicGetter(general_i: int, value_i: int, value_header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if value_header.tag == 1:
        def Value(matrix: Any, general_i: Any=general_i, value_i: Any=value_i, value_header: Any=value_header) -> Callable[[int], LDNode]:
            def _arrow3674(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryGetProtocolTypeGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if header.tag == 4:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], LDNode]:
            def _arrow3675(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_tryGetProtocolREFGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 8:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3676(i: int, matrix: Any=matrix) -> str:
...
def ProcessConversion_tryGetProtocolDescriptionGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 5:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3677(i: int, matrix: Any=matrix) -> str:
                return get_item_from_dict(matrix, (general_i, i)).AsFreeText
...
def ProcessConversion_tryGetProtocolURIGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 6:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3678(i: int, matrix: Any=matrix) -> str:
                return get_item_from_dict(matrix, (general_i, i)).AsFreeText
...
def ProcessConversion_tryGetProtocolVersionGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], str] | None:
    if header.tag == 7:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], str]:
            def _arrow3679(i: int, matrix: Any=matrix) -> str:
                return get_item_from_dict(matrix, (general_i, i)).AsFreeText
...
def ProcessConversion_tryGetPerformerGetter(general_i: int, header: CompositeHeader) -> Callable[[Any, int], LDNode] | None:
    if header.tag == 9:
        def Value(matrix: Any, general_i: Any=general_i, header: Any=header) -> Callable[[int], LDNode]:
            def _arrow3683(i: int, matrix: Any=matrix) -> LDNode:
...
def ProcessConversion_getProcessGetter(process_name_root: str, headers: IEnumerable_1[CompositeHeader]) -> Callable[[Any, int], LDNode]:
    ...
    if match_value is None:
        def _arrow3685(matrix_1: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3684(i_1: int) -> Array[LDNode]:
        ...
        def _arrow3687(matrix_2: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3686(i_2: int) -> Array[LDNode]:
                return []

            return _arrow3686

        input_getter_1 = _arrow3685 if (length(char_getters) != 0) else _arrow3687

    else: 
        input_getter: Callable[[Any, int], LDNode] = match_value
        def _arrow3689(matrix: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3688(i: int) -> Array[LDNode]:
    ...
    if match_value_1 is None:
        def _arrow3691(matrix_4: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3690(i_4: int) -> Array[LDNode]:
        ...
        def _arrow3693(matrix_5: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3692(i_5: int) -> Array[LDNode]:
                return []

            return _arrow3692

        output_getter_1 = _arrow3691 if (length(factor_value_getters) != 0) else _arrow3693

    else: 
        output_getter: Callable[[Any, int], LDNode] = match_value_1
        def _arrow3695(matrix_3: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], Array[LDNode]]:
            def _arrow3694(i_3: int) -> Array[LDNode]:
        ...
        output_getter_1 = _arrow3695

    def _arrow3698(matrix_6: Any, process_name_root: Any=process_name_root, headers: Any=headers) -> Callable[[int], LDNode]:
        def _arrow3697(i_6: int) -> LDNode:
            def mapping_4(tuple_1: tuple[int, int]) -> int:
                return tuple_1[1]

            class ObjectExpr3696:
                @property
                def Compare(self) -> Callable[[int, int], int]:
                    return compare_primitives

            pn: str = process_name_root if ((1 + max(map_1(mapping_4, matrix_6.keys()), ObjectExpr3696())) == 1) else ProcessConversion_composeProcessName(process_name_root, i_6)
...
def ProcessConversion_groupProcesses_Z27F0B586(processes: FSharpList[LDNode], graph: LDGraph | None=None, context: LDContext | None=None) -> FSharpList[tuple[str, FSharpList[LDNode]]]:
    def projection(p: LDNode, processes: Any=processes, graph: Any=graph, context: Any=context) -> str:
        ...
        if match_value is not None:
            if ProcessConversion_decomposeProcessName_Z721C83C5(match_value)[1] is not None:
                ...
                def _arrow3699(__unit: None=None, p: Any=p) -> bool:
                ...
                if _arrow3699():
                    ...
                    def _arrow3700(__unit: None=None, p: Any=p) -> bool:
                ...
                def _arrow3701(__unit: None=None, p: Any=p) -> bool:
            ...
            def _arrow3702(__unit: None=None, p: Any=p) -> bool:
    ...
    class ObjectExpr3704:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow3703(x: str, y: str) -> bool:
                return x == y
    ...
    return List_groupBy(projection, processes, ObjectExpr3704())
...
def ProcessConversion_processToRows_Z6839B9E8(p: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> Array[FSharpList[tuple[CompositeHeader, CompositeCell]]]:
    ...
    if match_value_1 is None:
        ...
        def _arrow3713(__unit: None=None, p: Any=p, graph: Any=graph, context: Any=context) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            def _arrow3705(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            ...
            def _arrow3712(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow3706(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                ...
                def _arrow3711(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    def _arrow3707(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    ...
                    def _arrow3710(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        def _arrow3708(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        ...
                        def _arrow3709(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        ...
                        return append(_arrow3708(), delay(_arrow3709))

                    return append(_arrow3707(), delay(_arrow3710))

                return append(_arrow3706(), delay(_arrow3711))

            return append(_arrow3705(), delay(_arrow3712))

        prot_vals = to_list(delay(_arrow3713))
    ...
    def _arrow3714(Value: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value

    def _arrow3715(Value_1: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value_1

    def _arrow3716(Value_2: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value_2

    def _arrow3717(Value_3: LDNode, p: Any=p, graph: Any=graph, context: Any=context) -> LDNode | None:
        return Value_3

    pattern_input: tuple[Array[LDNode | None], Array[LDNode | None]] = ((ResizeArray_create(len(outputs), None), ResizeArray_map(_arrow3714, outputs))) if ((len(outputs) != 0) if (len(inputs) == 0) else False) else (((ResizeArray_map(_arrow3715, inputs), ResizeArray_create(len(inputs), None))) if ((len(outputs) == 0) if (len(inputs) != 0) else False) else ((ResizeArray_map(_arrow3716, inputs), ResizeArray_map(_arrow3717, outputs))))
    ...
    if (len(outputs_1) == 0) if (len(inputs_1) == 0) else False:
        ...
        def _arrow3719(__unit: None=None, p: Any=p, graph: Any=graph, context: Any=context) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
            def _arrow3718(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                return pvs

            return append(components, delay(_arrow3718))

        class ObjectExpr3720:
            @property
            def Compare(self) -> Callable[[int, int], int]:
                return compare_primitives

        vals: FSharpList[tuple[CompositeHeader, CompositeCell]] = map_2(mapping, sort_by(projection, to_list(delay(_arrow3719)), ObjectExpr3720()))
        def _arrow3723(__unit: None=None, p: Any=p, graph: Any=graph, context: Any=context) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
            def _arrow3722(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow3721(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    return comments

                return append(vals, delay(_arrow3721))

            return append(prot_vals, delay(_arrow3722))

        return ResizeArray_singleton(to_list(delay(_arrow3723)))
        ...
        def f_5(tupled_arg: tuple[LDNode | None, LDNode | None], p: Any=p, graph: Any=graph, context: Any=context) -> FSharpList[tuple[CompositeHeader, CompositeCell]]:
            ...
            def _arrow3727(__unit: None=None, tupled_arg: Any=tupled_arg) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                def _arrow3726(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                    def _arrow3725(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                        def _arrow3724(__unit: None=None) -> IEnumerable_1[tuple[tuple[CompositeHeader, CompositeCell], int | None]]:
                            return factors

                        return append(pvs, delay(_arrow3724))

                    return append(components, delay(_arrow3725))

                return append(chars, delay(_arrow3726))

            class ObjectExpr3728:
                @property
                def Compare(self) -> Callable[[int, int], int]:
                    return compare_primitives

            vals_1: FSharpList[tuple[CompositeHeader, CompositeCell]] = map_2(mapping_1, sort_by(projection_1, to_list(delay(_arrow3727)), ObjectExpr3728()))
            def _arrow3733(__unit: None=None, tupled_arg: Any=tupled_arg) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                def _arrow3732(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                    def _arrow3731(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                        def _arrow3730(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                            def _arrow3729(__unit: None=None) -> IEnumerable_1[tuple[CompositeHeader, CompositeCell]]:
                                return singleton(BaseTypes_decomposeProcessOutput_Z2F770004(value_4(o), context)) if (o is not None) else empty_1()

                            return append(comments, delay(_arrow3729))

                        return append(vals_1, delay(_arrow3730))

                    return append(prot_vals, delay(_arrow3731))

                return append(singleton(BaseTypes_decomposeProcessInput_Z2F770004(value_4(i), context)) if (i is not None) else empty_1(), delay(_arrow3732))

            return to_list(delay(_arrow3733))
...
def ARCtrl_ArcTable__ArcTable_GetProtocols(this: ArcTable) -> FSharpList[LDNode]:
    if this.RowCount == 0:
        def _arrow3734(__unit: None=None, this: Any=this) -> LDNode:
        ...
        return singleton_2(_arrow3734())

    else: 
        def _arrow3735(i: int, this: Any=this) -> LDNode:
        ...
        return List_distinct(initialize(this.RowCount, _arrow3735), ObjectExpr3736())
...
def ARCtrl_ArcTable__ArcTable_GetProcesses(this: ArcTable) -> FSharpList[LDNode]:
    if this.RowCount == 0:
        ...
        def _arrow3738(__unit: None=None, this: Any=this) -> IEnumerable_1[LDNode]:
            def _arrow3737(i: int) -> LDNode:
                return getter(this.Values)(i)

            return map_1(_arrow3737, range_big_int(0, 1, this.RowCount - 1))

        return to_list(delay(_arrow3738))
...
DatamapConversion_reflection = _expr3739
...
PersonConversion_reflection = _expr3740
...
def PersonConversion_composeAddress_Z721C83C5(address: str) -> Any:
    try: 
        def _arrow3741(__unit: None=None) -> LDNode:
...
ScholarlyArticleConversion_reflection = _expr3742
...
def ScholarlyArticleConversion_composeScholarlyArticle_D324A6D(publication: Publication) -> LDNode:
    ...
    def _arrow3744(__unit: None=None, publication: Any=publication) -> IEnumerable_1[LDNode]:
        def _arrow3743(__unit: None=None) -> IEnumerable_1[LDNode]:
        ...
        return append(singleton(LDPropertyValue.create_doi(value_4(publication.DOI))) if ((value_4(publication.DOI) != "") if (publication.DOI is not None) else False) else empty_1(), delay(_arrow3743))

    identifiers: Array[LDNode] = list(to_list(delay(_arrow3744)))
...
AssayConversion_reflection = _expr3745
...
def AssayConversion_composeAssay_1501C0F8(assay: ArcAssay) -> LDNode:
    ...
    def _arrow3746(__unit: None=None, assay: Any=assay) -> Array[LDNode] | None:
        fds_1: Array[LDNode] = fragment_descriptors
        return fds_1

    def _arrow3747(__unit: None=None, assay: Any=assay) -> Array[LDNode] | None:
        vm_1: LDNode = variable_measured
        return ResizeArray_singleton(vm_1)

    def _arrow3748(__unit: None=None, assay: Any=assay) -> Array[LDNode] | None:
        fds: Array[LDNode] = fragment_descriptors
        vm: LDNode = variable_measured
        return ResizeArray_appendSingleton(vm, fds)

    variable_measureds: Array[LDNode] | None = (None if (fragment_descriptors is None) else _arrow3746()) if (variable_measured is None) else (_arrow3747() if (fragment_descriptors is None) else _arrow3748())
...
StudyConversion_reflection = _expr3749

def StudyConversion_composeStudy_1680536E(study: ArcStudy) -> LDNode:
    def _arrow3750(s: str, study: Any=study) -> Any | None:
        return DateTime_tryFromString(s)

    date_created: Any | None = bind(_arrow3750, study.SubmissionDate)
    def _arrow3751(s_1: str, study: Any=study) -> Any | None:
        return DateTime_tryFromString(s_1)

    date_published: Any | None = bind(_arrow3751, study.PublicReleaseDate)
...
def StudyConversion_decomposeStudy_Z6839B9E8(study: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> ArcStudy:
    def _arrow3752(d: Any, study: Any=study, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d)

    date_created: str | None = map(_arrow3752, LDDataset.try_get_date_created_as_date_time(study, context))
    def _arrow3753(d_1: Any, study: Any=study, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d_1)

    date_published: str | None = map(_arrow3753, LDDataset.try_get_date_published_as_date_time(study, context))
...
InvestigationConversion_reflection = _expr3754
...
def InvestigationConversion_composeInvestigation_Z720BD3FF(investigation: ArcInvestigation) -> LDNode:
    ...
    def _arrow3755(s: str, investigation: Any=investigation) -> Any | None:
        return DateTime_tryFromString(s)

    date_created: Any | None = bind(_arrow3755, investigation.SubmissionDate)
    def _arrow3756(s_1: str, investigation: Any=investigation) -> Any | None:
        return DateTime_tryFromString(s_1)

    date_published: Any = default_arg(bind(_arrow3756, investigation.PublicReleaseDate), now())
    ...
    def _arrow3757(__unit: None=None, investigation: Any=investigation) -> Array[LDNode]:
    ...
    has_parts: Array[LDNode] | None = Option_fromSeq(_arrow3757())
...
def InvestigationConversion_decomposeInvestigation_Z6839B9E8(investigation: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> ArcInvestigation:
    ...
    def _arrow3758(d: Any, investigation: Any=investigation, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d)

    date_created: str | None = map(_arrow3758, LDDataset.try_get_date_created_as_date_time(investigation, context))
    def _arrow3759(d_1: Any, investigation: Any=investigation, graph: Any=graph, context: Any=context) -> str:
        return DateTime_toString(d_1)

    date_published: str | None = map(_arrow3759, LDDataset.try_get_date_published_as_date_time(investigation, context))
...
TypeExtensions_Conversion_reflection = _expr3760
