...
def encoder_compressed_column(column_index: int, row_count: int, cell_table: Any, table: ArcTable) -> IEncodable:
    if True if table.Headers[column_index].IsIOType else (row_count < 100):
        ...
        from_: int = 0
        def _arrow2804(__unit: None=None, column_index: Any=column_index, row_count: Any=row_count, cell_table: Any=cell_table, table: Any=table) -> IEnumerable_1[IEncodable]:
            ...
            def _arrow2803(__unit: None=None) -> IEnumerable_1[IEncodable]:
                def _arrow2802(__unit: None=None) -> IEncodable:
                    ...
                    values_2: IEnumerable_1[tuple[str, IEncodable]] = to_enumerable([("f", _arrow2798()), ("t", _arrow2800()), ("v", encode_cell(cell_table, current))])
                ...
                return singleton(_arrow2802())

            return append(collect(_arrow2796, range_big_int(1, 1, row_count - 1)), delay(_arrow2803))

        values_3: Array[IEncodable] = to_array(delay(_arrow2804))
...
def decoder_compressed_column(cell_table: Array[CompositeCell], table: ArcTable, column_index: int) -> Decoder_1[None]:
    class ObjectExpr2807(Decoder_1[None]):
        def Decode(self, helper: IDecoderHelpers_1[Any], column: Any, cell_table: Any=cell_table, table: Any=table, column_index: Any=column_index) -> FSharpResult_2[None, tuple[str, ErrorReason_1[__A_]]]:
            ...
            if match_value.tag == 1:
                def _arrow2806(get: IGetters) -> None:
                    from_: int
                ...
                range_decoder: Decoder_1[None] = object(_arrow2806)
...
def arrayi(decoderi: Callable[[int], Decoder_1[_VALUE]]) -> Decoder_1[Array[Any]]:
    class ObjectExpr2814(Decoder_1[Array[_VALUE_]]):
                ...
                def folder(acc: FSharpResult_2[Array[_VALUE_], tuple[str, ErrorReason_1[__A_]]], value_1: __A_) -> FSharpResult_2[Array[_VALUE_], tuple[str, ErrorReason_1[__A_]]]:
                    ...
                    if acc.tag == 0:
                        ...
                        if match_value.tag == 0:
                            ...
                            def _arrow2813(__unit: None=None, acc: Any=acc, value_1: Any=value_1) -> tuple[str, ErrorReason_1[__A_]]:
                            ...
                            return FSharpResult_2(1, _arrow2813())
...
def encoder_compressed(string_table: Any, oa_table: Any, cell_table: Any, table: ArcTable) -> IEncodable:
    def _arrow2825(__unit: None=None, string_table: Any=string_table, oa_table: Any=oa_table, cell_table: Any=cell_table, table: Any=table) -> IEnumerable_1[tuple[str, IEncodable]]:
        def _arrow2824(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
            def _arrow2817(__unit: None=None) -> IEnumerable_1[IEncodable]:
                return map(encoder_1, table.Headers)

            def _arrow2823(__unit: None=None) -> IEnumerable_1[tuple[str, IEncodable]]:
                if len(table.Values) != 0:
                    row_count: int = table.RowCount or 0
                    def _arrow2821(__unit: None=None) -> IEnumerable_1[IEncodable]:
                        def _arrow2820(c: int) -> IEncodable:
                            return encoder_compressed_column(c, row_count, cell_table, table)

                        return map(_arrow2820, range_big_int(0, 1, table.ColumnCount - 1))

                    columns: Array[IEncodable] = to_array(delay(_arrow2821))
                    class ObjectExpr2822(IEncodable):
                        def Encode(self, helpers: IEncoderHelpers_1[Any]) -> Any:
                            def mapping(v: IEncodable) -> __A_:
                                return v.Encode(helpers)
                    ...
                    return singleton(("c", ObjectExpr2822()))

                else: 
                    return empty()


            return append(singleton(("h", list_1(to_list(delay(_arrow2817))))) if (len(table.Headers) != 0) else empty(), delay(_arrow2823))

        return append(singleton(("n", encode_string(string_table, table.Name))), delay(_arrow2824))

    values_1: IEnumerable_1[tuple[str, IEncodable]] = to_list(delay(_arrow2825))
...
def decoder_compressed(string_table: Array[str], oa_table: Array[OntologyAnnotation], cell_table: Array[CompositeCell]) -> Decoder_1[ArcTable]:
    def _arrow2840(get: IGetters, string_table: Any=string_table, oa_table: Any=oa_table, cell_table: Any=cell_table) -> ArcTable:
        def _arrow2828(__unit: None=None) -> FSharpList[CompositeHeader] | None:
            arg_1: Decoder_1[FSharpList[CompositeHeader]] = list_1_1(decoder_1)
            object_arg: IOptionalGetter = get.Optional
            return object_arg.Field("h", arg_1)

        decoded_header: Array[CompositeHeader] = list(default_arg(_arrow2828(), empty_1()))
        def _arrow2832(__unit: None=None) -> str:
        ...
        table: ArcTable = ArcTable.create(_arrow2832(), decoded_header, Dictionary([], ObjectExpr2834()))
        def _arrow2839(__unit: None=None) -> Array[None] | None:
            def _arrow2838(column_index: int) -> Decoder_1[None]:
                return decoder_compressed_column(cell_table, table, column_index)

            arg_5: Decoder_1[Array[None]] = arrayi(_arrow2838)
