...
class LDLabProtocol:
    ...
    def try_get_intended_use_as_defined_term(lp: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> LDNode | None:
        ...
        if match_value is not None:
            def _arrow1546(__unit: None=None) -> bool:
    ...
    def get_components(lp: LDNode, graph: LDGraph | None=None, context: LDContext | None=None) -> Array[LDNode]:
        def _arrow1547(__unit: None=None) -> IEnumerable_1[LDNode]:
    ...
    def gen_id(name: str | None=None, process_name: str | None=None, assay_name: str | None=None, study_name: str | None=None) -> str:
        def _arrow1552(__unit: None=None) -> FSharpList[str]:
            def _arrow1551(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow1550(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow1549(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow1548(__unit: None=None) -> IEnumerable_1[str]:
                            return singleton_1(value(study_name)) if (study_name is not None) else empty()

                        return append(singleton_1(value(assay_name)) if (assay_name is not None) else empty(), delay(_arrow1548))

                    return append(singleton_1(value(process_name)) if (process_name is not None) else empty(), delay(_arrow1549))

                return append(singleton_1(value(name)) if (name is not None) else empty(), delay(_arrow1550))

            vals: FSharpList[str] = to_list(delay(_arrow1551))
            return singleton(create_missing_identifier()) if is_empty(vals) else vals

        return clean(join("_", append_1(singleton("#Protocol"), _arrow1552())))
...
LDLabProtocol_reflection = _expr1553
