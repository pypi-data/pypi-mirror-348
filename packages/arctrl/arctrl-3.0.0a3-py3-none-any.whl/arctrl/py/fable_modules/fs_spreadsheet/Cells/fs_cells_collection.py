...
def FsCellsCollection__Add_2E78CE33(this: FsCellsCollection, row: int, column: int, cell: FsCell) -> None:
    ...
    def _arrow156(__unit: None=None, this: Any=this, row: Any=row, column: Any=column, cell: Any=cell) -> Any:
    ...
    add_to_dict(_arrow156(), column, cell)
...
def FsCellsCollection__RemoveCellAt_Z37302880(this: FsCellsCollection, row: int, column: int) -> None:
    ...
    if (row == this._maxRowUsed) if row_removed else False:
        ...
        this._maxRowUsed = (max(this._rowsUsed.keys(), ObjectExpr157()) if (not is_empty(this._rowsUsed.keys())) else 0) or 0
    ...
    if (column == this._maxColumnUsed) if column_removed else False:
        ...
        this._maxColumnUsed = (max(this._columnsUsed.keys(), ObjectExpr158()) if (not is_empty(this._columnsUsed.keys())) else 0) or 0
    ...
    if match_value is None:
        ...
        def _arrow159(__unit: None=None, this: Any=this, row: Any=row, column: Any=column) -> bool:
...
def FsCellsCollection__GetCells_Z6C21C500(this: FsCellsCollection, row_start: int, column_start: int, row_end: int, column_end: int) -> IEnumerable_1[FsCell]:
    final_row: int = (this._maxRowUsed if (row_end > this._maxRowUsed) else row_end) or 0
    final_column: int = (this._maxColumnUsed if (column_end > this._maxColumnUsed) else column_end) or 0
    def _arrow167(__unit: None=None, this: Any=this, row_start: Any=row_start, column_start: Any=column_start, row_end: Any=row_end, column_end: Any=column_end) -> IEnumerable_1[FsCell]:
        def _arrow166(ro: int) -> IEnumerable_1[FsCell]:
            match_value: Any | None = Dictionary_tryGet(ro, this._rowsCollection)
            if match_value is None:
                return empty()
                ...
                columns_collection: Any = match_value
                def _arrow165(co: int) -> IEnumerable_1[FsCell]:
                    match_value_1: FsCell | None = Dictionary_tryGet(co, columns_collection)
                    if match_value_1 is not None:
                ...
                return collect(_arrow165, range_big_int(column_start, 1, final_column))


        return collect(_arrow166, range_big_int(row_start, 1, final_row))

    return delay(_arrow167)
...
def FsCellsCollection__GetFirstAddress(this: FsCellsCollection) -> FsAddress:
    if True if is_empty(this._rowsCollection) else is_empty(this._rowsCollection.keys()):
        ...
        def _arrow172(__unit: None=None, this: Any=this) -> int:
            def projection(d: Any) -> int:
                ...
                return min(d.keys(), ObjectExpr169())
            ...
            d_1: Any = min_by(projection, this._rowsCollection.values(), ObjectExpr170())
            class ObjectExpr171:
                @property
                def Compare(self) -> Callable[[int, int], int]:
                    return compare_primitives

            return min(d_1.keys(), ObjectExpr171())

        return FsAddress__ctor_Z37302880(min(this._rowsCollection.keys(), ObjectExpr168()), _arrow172())
