...
def to_fs_columns(dc: Array[DataContext]) -> FSharpList[FSharpList[FsCell]]:
    ...
    class ObjectExpr1339:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow1338(x: str, y: str) -> bool:
                return x == y
    ...
    comment_keys: FSharpList[str] = to_list(distinct(collect(mapping_1, dc), ObjectExpr1339()))
    ...
    def _arrow1351(__unit: None=None, dc: Any=dc) -> IEnumerable_1[FSharpList[FsCell]]:
        def _arrow1350(__unit: None=None) -> IEnumerable_1[FSharpList[FsCell]]:
            def _arrow1349(dc_4: DataContext) -> FSharpList[FsCell]:
                dc_3: DataContext = dc_4
                def _arrow1348(__unit: None=None) -> IEnumerable_1[FsCell]:
                    def _arrow1340(__unit: None=None) -> FSharpList[FsCell]:
                                ...
                                def _arrow1344(__unit: None=None) -> IEnumerable_1[FsCell]:
                                    def _arrow1343(__unit: None=None) -> IEnumerable_1[FsCell]:
                                        def _arrow1342(__unit: None=None) -> IEnumerable_1[FsCell]:
                                            ...
                                            return append(create_text(DataContext__get_Label(dc_3)), delay(_arrow1341))

                                        return append(create_text(DataContext__get_GeneratedBy(dc_3)), delay(_arrow1342))

                                    return append(create_text(DataContext__get_Description(dc_3)), delay(_arrow1343))

                                return append(create_term(DataContext__get_ObjectType(dc_3)), delay(_arrow1344))

                            return append(create_term(DataContext__get_Unit(dc_3)), delay(_arrow1345))

                        return append(create_term(DataContext__get_Explication(dc_3)), delay(_arrow1346))

                    return append(_arrow1340(), delay(_arrow1347))

                return to_list(delay(_arrow1348))

            return map_1(_arrow1349, dc)

        return append(singleton_1(headers), delay(_arrow1350))

    return transpose(to_list(delay(_arrow1351)))
