...
def from_string(pub_med_id: str | None, doi: str | None, author: str | None, title: str | None, status: str | None, status_term_source_ref: str | None, status_term_accession_number: str | None, comments: Array[Comment]) -> Publication:
    ...
    def _arrow941(s: str, pub_med_id: Any=pub_med_id, doi: Any=doi, author: Any=author, title: Any=title, status: Any=status, status_term_source_ref: Any=status_term_source_ref, status_term_accession_number: Any=status_term_accession_number, comments: Any=comments) -> str:
        return URIModule_fromString(s)

    pub_med_id_1: str | None = map(_arrow941, pub_med_id)
...
def from_sparse_table(matrix: SparseTable) -> FSharpList[Publication]:
    if (length(matrix.CommentKeys) != 0) if (matrix.ColumnCount == 0) else False:
        ...
        def _arrow942(__unit: None=None, matrix: Any=matrix) -> Publication:
            return_val: Publication = Publication.create()
            return_val.Comments = comments
            return return_val

        return singleton(_arrow942())

    else: 
        def _arrow943(i: int, matrix: Any=matrix) -> Publication:
        ...
        return initialize(matrix.ColumnCount, _arrow943)
...
def to_sparse_table(publications: FSharpList[Publication]) -> SparseTable:
    ...
    class ObjectExpr945:
        ...
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow944(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr945())), matrix.ColumnCount)
