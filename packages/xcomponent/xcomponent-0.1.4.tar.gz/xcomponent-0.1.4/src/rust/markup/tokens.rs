...
impl ToHtml for XFragment {
    fn to_html<'py>(
        ...
        for child in self.children() {
            result.push_str(child.to_html(py, catalog, params.clone())?.as_str())
        }
        Ok(result)
    }
}
        ...
        match catalog.get(py, self.name()) {
            Some(py_template) => {
                ...
                if self.children().len() > 0 {
                    ...
                    for child in self.children() {
                        childchildren.push_str(child.to_html(py, catalog, params.clone())?.as_str())
                    }
                    node_attrs.set_item("children", childchildren)?;
                }

                result.push_str(catalog.render_node(py, &node, node_attrs)?.as_str())
            }
            None => {
...
impl ToHtml for XNode {
    fn to_html<'py>(
        ...
        match self {
            XNode::Fragment(f) => f.to_html(py, catalog, params),
            XNode::Element(e) => e.to_html(py, catalog, params),
            XNode::DocType(d) => d.to_html(py, catalog, params),
            XNode::Text(t) => t.to_html(py, catalog, params),
            XNode::Comment(c) => c.to_html(py, catalog, params),
            XNode::Expression(e) => e.to_html(py, catalog, params),
        }
    }
}
