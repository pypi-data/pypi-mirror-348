...
from gpil.lib.pymdp.vfg.pymdp_vfg_model import PymdpModel, MissingControlState
from gpil.api.types import ActionSelectionRequest
from gpil.api.action_selection import process_action_selection_request
...
def test_validation_dual_role_conflict(insulin_pymdp_pomdp_model: PymdpModel):
    errors = insulin_pymdp_pomdp_model.validate(raise_exceptions=False)
    assert len(errors) == 1, "There should be one error"
    error = errors[0]
    assert isinstance(error, MissingControlState)
    assert error.to_dict()["parameters"]["variable"] == "uncontrolled_glucose_dynamics"

    corrected, errors = insulin_pymdp_pomdp_model.correct(raise_exceptions=False)

    assert len(errors) == 0, "There should be no errors after correction"
    assert (
        corrected.variables["glucose_utilization"].root.role
        == pyvfg.VariableRole.Latent
    )
    assert (
        DUMMY_CONTROL_STATE_NAME in corrected.variables
    ), "Control state variable should have been added"
    assert (
        corrected.variables[DUMMY_CONTROL_STATE_NAME].root.role
        == pyvfg.VariableRole.ControlState
    )

    # Verify that the corrected model is valid
    ...
    response = process_action_selection_request(corrected, request)
    assert response.success, "The request should succeed"
    assert response.result.action_data is not None, "There should be an action"
    assert list(response.result.action_data.keys()) == [
        "basal_insulin"
    ], "There should be one action"


def test_validation_missing_dummy_control_via_action_selection(insulin_pymdp_vfg: VFG):
    ...
    try:
        response = process_action_selection_request(insulin_pymdp_vfg, request)
        assert False, "The request should fail"
    except ValidationErrors as e:
        assert len(e.errors) == 1, "There should be one error"
        error = e.errors[0]
        assert isinstance(error, MissingControlState)
        error_dict = error.to_dict()
        assert error_dict["parameters"]["variable"] == "uncontrolled_glucose_dynamics"

        corrected = insulin_pymdp_vfg.apply_patches(e.patches)
        import rich

        rich.print(
            [
                f.variables
                for f in corrected.factors
                if f.role == pyvfg.FactorRole.Transition
            ]
        )
        errors = corrected.validate(raise_exceptions=False)
        assert not errors, "There should be no errors after correction"

        corrected, errors = insulin_pymdp_vfg.correct(raise_exceptions=False)
        assert not errors, "There should be no errors after correction"
