...
from gpil.lib.pymdp.vfg.pymdp_vfg_model import PymdpModel
...
def test_validation_dual_role_conflict(insulin_pymdp_vfg: PymdpModel):
    errors = insulin_pymdp_vfg.validate(raise_exceptions=False)
    assert len(errors) == 1, "There should be one error"
    error = errors[0]
    assert isinstance(error, pyvfg.ConflictingVariableRoles)
    assert error.to_dict()["parameters"]["variable"] == "glucose_utilization"
    assert error.to_dict()["parameters"]["roles"] == [
