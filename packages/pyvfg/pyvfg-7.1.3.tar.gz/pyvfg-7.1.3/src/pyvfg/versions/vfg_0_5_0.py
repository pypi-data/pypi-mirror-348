...
class VFG(BaseModel):
    ...
    def validate_as(
        ...
        match model_type:
            case ModelType.BayesianNetwork:
                return BayesianNetwork.model_validate(self.model_dump()).validate(
                    raise_exceptions,
                )
            case ModelType.Pomdp:
                return POMDP.model_validate(self.model_dump()).validate(
                    raise_exceptions
                )
            case ModelType.MarkovRandomField:
                return MarkovRandomField.model_validate(self.model_dump()).validate(
                    raise_exceptions,
                )
    ...
    def model_is_one_of(
        ...
        for mt in allowed_model_types:
            if len(self.validate_as(mt, raise_exceptions=False).model_type_errors) == 0:
                return True
