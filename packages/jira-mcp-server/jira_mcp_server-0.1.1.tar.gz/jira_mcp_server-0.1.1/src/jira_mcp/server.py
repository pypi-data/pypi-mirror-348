...
async def jira_transition_issue(
    ...
    try:
        # The jira library's transition_issue handles ID or name lookup
        success = await jira_client.transition_issue(
            issue_key,
            transition,
        ...
        if success:
             ...
             # Transition might fail without raising JIRAError if transition is invalid for current status etc.
             return {"error": f"Failed to transition issue {issue_key} using '{transition}'. Check if the transition is valid for the issue's current status.", "success": False}
