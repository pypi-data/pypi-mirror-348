...
class SafeAreaInsets:
    ...
    def from_json(cls, json: T_JSON_DICT) -> SafeAreaInsets:
        return cls(
            ...
            top_max=(
                int(json["topMax"]) if json.get("topMax", None) is not None else None
            ),
            left=int(json["left"]) if json.get("left", None) is not None else None,
            left_max=(
                int(json["leftMax"]) if json.get("leftMax", None) is not None else None
            ),
            bottom=(
                int(json["bottom"]) if json.get("bottom", None) is not None else None
            ),
            bottom_max=(
                int(json["bottomMax"])
