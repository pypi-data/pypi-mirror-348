...
def test_deps(setup):
    ...
    with pytest.raises(DepError):
        merge(["test/test3", "test/test5", "test/test6"])
...
def test_rebuilt(setup):
    """Tests that packages can be rebuilt even if nothing changes"""
    merge(["test/test4"])
    merge(["test/test4"])
