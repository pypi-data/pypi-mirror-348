...
def содержит(что, где, игнорировать_регистр=False):
    ...
    if type(где) not in [str, list, tuple]:
         ошибочка = f'Переменная "где" может быть только текстом, списком или кортежем. По факту у неё тип {type(где)}'
         raise TypeError(печать(ошибочка, красный=True, стиль= "жч", не_печатать=True))
    if type(игнорировать_регистр) != bool:
         ошибочка = f'Переменная "игнорировать_регистр" может быть только True или False. По факту у неё тип {type(игнорировать_регистр)}'
         raise TypeError(печать(ошибочка, красный=True, стиль= "жч", не_печатать=True))
    
    if type(что) != str:
         for элемент in что:
               if type(элемент) != str:
                    ошибка = f'В переменной "что" может быть только True или False. По факту у неё тип {type(игнорировать_регистр)}'
                    raise TypeError(печать(ошибка, красный=True, стиль= "жч", не_печатать=True))
    if type(где) != str:
          for элемент in где:
                    if type(элемент) != str:
                         ошибка = f'В переменной "где" может быть только True или False. По факту у неё тип {type(игнорировать_регистр)}'
                         raise TypeError(печать(ошибка, красный=True, стиль= "жч", не_печатать=True))
     ...
     # согласно этому принципу нужно сначала написать тесты, которые проверят каждый аспект работы функции. Каждый раз при доработки этой функции необходимо заново запускать тесты - если они все пройдены, значит функция точно работает правильно.
     # при разработки тестов недопустимо использование вычисляемых переменных - тестируемая функция должа принять харанее подготовленные данные, а тест проверяет, согласуется ли её ответ с ожидаемым.
...
def тесты():
    ...
    тест("что", "где", "текст", "ошибка", "неправильный тип - игнорировать регистнр")
    тест(["текст", "не цифра", 69], "где", False, "ошибка", "в переменной 'что' есть не str")
    тест(
         что="что",
         где=["текст", "не цифра", 69],
