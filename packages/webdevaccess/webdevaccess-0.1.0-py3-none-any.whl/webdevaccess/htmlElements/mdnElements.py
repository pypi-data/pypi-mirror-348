""" /src/webdevaccess/htmlElements/mdnElements.py
...
def filterAnchor(anchor: Tag) -> bool:
    ...
    There are also a few places where an element reference is used along with an inpage directive, e.g., '#scope'.  filter that out.
    This filter is to ensure we get only the anchors referencing HTML elements
    """
    href = anchor.attrs.get("href")
    if not href or ElementsReferencePath not in href or "contributors.txt" in href or '#' in href:
        return False
