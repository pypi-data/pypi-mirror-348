...
class PageConfiguration(Dict[EditorName,MacroPyConfig]):
    ...
    def __init__(self, env):
        ...
        """ Secondary counter, to know the order of the elements in one group. """
    ...
    def get_run_group_data(
        ...
        * By default, any new element is alone in its own group.
        ...
        if group_id != self.last_group_id:
            raise BuildError(
                f"Suspicious RUN_GROUP name: {group_id}. This group already exists, but some other groups "
                f"got registered in between. This is not allowed.{ self.env.file_location(all_in=True) }"
            )
        ...
        if order in group:
            ...
            raise BuildError(
                ...
                f"Invalid group: RUN_GROUP={group_id!r}.{ self.env.file_location(all_in=True) }"
            )
        group.add(order)
