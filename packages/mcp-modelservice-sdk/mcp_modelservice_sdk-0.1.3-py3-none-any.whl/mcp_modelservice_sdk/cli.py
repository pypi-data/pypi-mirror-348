...
try:
    ...
    from mcp_modelservice_sdk.src.core import (
        ...
        build_mcp_package,
        TransformationError,
        _setup_logging,  # Import for use in CLI commands
    )
except ImportError as e:
    sys.stderr.write(
        "Failed to import core modules from mcp_modelservice_sdk.src.core. Ensure the package is installed correctly or PYTHONPATH is set up.\n"
    )
    sys.stderr.write(f"Error details: {e}\n")
    # Fallback for local development if src is in PYTHONPATH, not recommended for distribution
    try:
...
app = typer.Typer(
    help="MCP Modelservice SDK CLI: Create, run, and package MCP services from your Python code.",
...
def run(
    ...
    try:
        ...
        uvicorn_log_level = common_opts.log_level.lower()
        if uvicorn_log_level not in [
            ...
            cli_logger.warning(
                f"Invalid log level '{common_opts.log_level}' for Uvicorn. Defaulting to 'info'."
            )
...
def package(
    ...
    package_host: Annotated[
        str,
        typer.Option(
            help="Host to configure in the packaged main.py.",
            rich_help_panel="Packaging Configuration",
        ),
    ] = "0.0.0.0",
    package_port: Annotated[
        int,
        typer.Option(
            help="Port to configure in the packaged main.py.",
            rich_help_panel="Packaging Configuration",
        ),
    ] = 8080,
    ...
    """
    Package an MCP service into a zip file for deployment.
    """
    ...
    cli_logger.info(
        f"Packaging MCP service into '{package_name}.zip' from source: {common_opts.source_path}"
    )
    ...
    try:
        build_mcp_package(
            ...
            mcp_server_root_path=common_opts.server_root,
            mcp_service_base_path=common_opts.mcp_base,
            log_level=common_opts.log_level,  # Passed for generated main.py and for cli_logger to build_mcp_package
            cors_enabled=common_opts.cors_enabled,
            cors_allow_origins=processed_cors_origins,
        ...
        cli_logger.info(f"Successfully packaged MCP service into '{package_name}.zip'.")
