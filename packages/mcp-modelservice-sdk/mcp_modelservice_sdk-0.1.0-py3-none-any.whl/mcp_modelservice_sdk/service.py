...
async def call_external_service_as_agent(
    ...
    logger.info(f"call_agent invoked with input: {workflow_input}, max_retries: {max_retries}, try_count: {try_count}")
    # Simulate some processing or calling the actual service
    # For demonstration, we'll just return a success message with the inputs.
    # In a real scenario, this would involve more complex logic, possibly retries, etc.
    await call_external_service(workflow_input) # Or some other logic
    return {"status": "success", "message": "Agent call processed (simulated).", "received_input": workflow_input.model_dump(), "max_retries": max_retries, "try_count": try_count}
...
def run_server(host: str = "0.0.0.0", port: int = 8080, log_level: str = "info"):
    """
    ...
    uvicorn.run(app="mcp_modelservice_sdk.service:app", host=host, port=port, log_level=log_level.lower(), reload=True) 