...
from scikit_ptm_fs.utils import get_matrix_in_format, matrix_creation_function_for_format

from scipy.sparse import issparse, csr_matrix

import numpy as np

class ProblemTransformationBase(MLSelectorBase):
    """Base class providing common functions for multi-label feature selection methods
    that follow the proablem transformation approach.
    ...
    Parameters
    ----------
    Selector : scikit selector type
        The base selector that will be used in a class, will be automagically put under self.classifier for future access.
    require_dense : boolean (default is False)
        Whether the base slector requires input as dense arrays.
    """

    def __init__(self, selector=None, require_dense=None):
        super(ProblemTransformationBase, self).__init__()

        self.copyable_attrs = ["selector", "require_dense"]
        ...
        if require_dense is not None:
            ...
            if isinstance(self.selector, MLSelectorBase):
