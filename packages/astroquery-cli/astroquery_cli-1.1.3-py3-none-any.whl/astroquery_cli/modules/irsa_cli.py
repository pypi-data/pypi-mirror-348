...
app = typer.Typer(
    name="irsa",
    help=_("Query NASA/IPAC Infrared Science Archive (IRSA)."),
...
def query_gator(
    catalog: str = typer.Argument(..., help=_("Name of the IRSA catalog (e.g., 'allwise_p3as_psd').")),
    coordinates: str = typer.Argument(..., help=_("Coordinates (e.g., '10.68h +41.26d', 'M51').")),
    radius: str = typer.Argument(..., help=_("Search radius (e.g., '10arcsec', '0.5deg').")),
...
@app.command(name="list-gator-catalogs", help=_("List available catalogs in IRSA Gator for a mission."))
def list_gator_catalogs(
    mission: Optional[str] = typer.Option(None, help=_("Filter catalogs by mission code (e.g., 'WISE', 'SPITZER').")),
):
...
@app.command(name="query-region", help=_("Perform a cone search across multiple IRSA collections."))
def query_region(
    coordinates: str = typer.Argument(..., help=_("Coordinates (e.g., '10.68h +41.26d', 'M31').")),
    radius: str = typer.Argument(..., help=_("Search radius (e.g., '10arcsec', '0.5deg').")),
    collection: Optional[str] = typer.Option(None, help=_("Specify a collection (e.g., 'allwise', '2MASS'). Leave blank for a general search.")),
    output_file: Optional[str] = common_output_options["output_file"],
    output_format: Optional[str] = common_output_options["output_format"],
    max_rows_display: int = typer.Option(20, help=_("Maximum number of rows to display. Use -1 for all rows.")),
    show_all_columns: bool = typer.Option(False, "--show-all-cols", help=_("Show all columns in the output table."))
