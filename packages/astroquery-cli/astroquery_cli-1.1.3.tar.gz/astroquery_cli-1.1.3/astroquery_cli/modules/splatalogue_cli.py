...
app = typer.Typer(
    name="splatalogue",
    help=_("Query the Splatalogue spectral line database."),
...
@app.command(name="query-lines", help=_("Query spectral lines from Splatalogue."))
def query_lines(
    frequency_range: str = typer.Argument(..., help=_("Frequency range (e.g., '100GHz-110GHz', '2100MHz-2200MHz').")),
    chemical_name: Optional[str] = typer.Option(None, "--chemical", help=_("Chemical name pattern (e.g., 'CO', '%H2O%').")),
    energy_max: Optional[float] = typer.Option(None, help=_("Maximum energy in K (E_upper).")),
    energy_type: Optional[str] = typer.Option("el", help=_("Energy type ('el' or 'eu' for E_lower or E_upper).")),
    line_strengths: Optional[str] = typer.Option(None, help=_("Line strength units (e.g., 'ls1', 'ls2', 'ls4', 'ls5' for CDMS/JPL or TopModel).")),
    exclude: Optional[List[str]] = typer.Option(None, "--exclude", help=_("Species to exclude (e.g., 'HDO').")),
...
@app.command(name="get-species-table", help=_("Get the table of NRAO recommended species."))
