...
app = typer.Typer(
    name="vizier",
    help=_("Query the VizieR database of astronomical catalogs. 📖"),
...
@app.command(name="find-catalogs", help=_("Find VizieR catalogs based on keywords, UCDs, or source names."))
def find_catalogs(
    keywords: Optional[List[str]] = typer.Option(None, "--keyword", "-k", help=_("Keyword(s) to search for in catalog descriptions.")),
    ucd: Optional[str] = typer.Option(None, help=_("UCD (Unified Content Descriptor) to filter catalogs.")),
    source_name: Optional[str] = typer.Option(None, "--source", help=_("Source name or pattern (e.g., 'Gaia DR3', '2MASS').")),
    max_catalogs: int = typer.Option(20, help=_("Maximum number of catalogs to list.")),
    ...
    vizier_server: str = typer.Option(
        "vizier_cds",
        help=_("VizieR server to use. Choices: {server_list}").format(server_list=list(VIZIER_SERVERS.keys())),
    ...
    if not query_params:
        ...
        console.print(_("Example: `aq vizier find-catalogs --keyword photometry --keyword M31`"))
        raise typer.Exit(code=1)
...
@app.command(name="query-object", help=_("Query catalogs around an object name or specific coordinates."))
def query_object(
    target: str = typer.Argument(..., help=_("Object name (e.g., 'M31') or coordinates (e.g., '10.68h +41.26d' or '160.32 41.45').")),
    radius: str = typer.Option("2arcmin", help=_("Search radius (e.g., '5arcmin', '0.1deg').")),
    catalogs: List[str] = typer.Option(..., "--catalog", "-c", help=_("VizieR catalog identifier(s) (e.g., 'I/261/gaiadr3', 'J/ApJ/710/1776'). Can be specified multiple times.")),
    columns: Optional[List[str]] = typer.Option(None, "--col", help=_("Specific columns to retrieve (e.g., 'RAJ2000', 'DEJ2000', 'pmRA'). Use 'all' for all columns. Can be specified multiple times.")),
    column_filters: Optional[List[str]] = typer.Option(None, "--filter", help=_("Column filters (e.g., 'Imag<15', 'B-V>0.5'). Can be specified multiple times. Format: 'column_name<operator>value'.")),
    row_limit: int = typer.Option(vizier_conf.row_limit, help=_("Maximum number of rows to return per catalog.")),
    max_rows_display: int = typer.Option(20, help=_("Maximum number of rows to display per table. Use -1 for all rows.")),
    show_all_columns: bool = typer.Option(False, "--show-all-cols", help=_("Show all columns in the output table.")),
    vizier_server: str = typer.Option(
        "vizier_cds",
        help=_("VizieR server to use. Choices: {server_list}").format(server_list=list(VIZIER_SERVERS.keys())),
...
@app.command(name="query-region", help=_("Query catalogs within a sky region (cone or box)."))
def query_region(
    coordinates: str = typer.Argument(..., help=_("Central coordinates for the region (e.g., '10.68h +41.26d' or '160.32 41.45').")),
    radius: Optional[str] = typer.Option(None, help=_("Cone search radius (e.g., '5arcmin', '0.1deg'). Use if not specifying width/height.")),
    width: Optional[str] = typer.Option(None, help=_("Width of a box region (e.g., '10arcmin', '0.5deg'). Requires --height.")),
    height: Optional[str] = typer.Option(None, help=_("Height of a box region (e.g., '10arcmin', '0.5deg'). Requires --width.")),
    catalogs: List[str] = typer.Option(..., "--catalog", "-c", help=_("VizieR catalog identifier(s). Can be specified multiple times.")),
    columns: Optional[List[str]] = typer.Option(None, "--col", help=_("Specific columns to retrieve. Use 'all' for all columns. Can be specified multiple times.")),
    column_filters: Optional[List[str]] = typer.Option(None, "--filter", help=_("Column filters (e.g., 'Imag<15'). Can be specified multiple times.")),
    row_limit: int = typer.Option(vizier_conf.row_limit, help=_("Maximum number of rows to return per catalog.")),
    max_rows_display: int = typer.Option(20, help=_("Maximum number of rows to display per table. Use -1 for all rows.")),
    show_all_columns: bool = typer.Option(False, "--show-all-cols", help=_("Show all columns in the output table.")),
    vizier_server: str = typer.Option(
...
@app.command(name="query-constraints", help=_("Query catalogs based on specific column constraints or keywords."))
def query_constraints(
    catalogs: List[str] = typer.Option(..., "--catalog", "-c", help=_("VizieR catalog identifier(s). Can be specified multiple times.")),
    constraints: Optional[List[str]] = typer.Option(None, "--constraint", help=_("Constraints on column values (e.g., 'Vmag=<10', 'B-V=0.5..1.0'). Can be specified multiple times. Format: 'column_name=condition'.")),
    keywords: Optional[List[str]] = typer.Option(None, "--keyword", "-k", help=_("Keywords to filter results within the catalog (different from finding catalogs).")),
    columns: Optional[List[str]] = typer.Option(None, "--col", help=_("Specific columns to retrieve. Use 'all' for all columns. Can be specified multiple times.")),
    row_limit: int = typer.Option(vizier_conf.row_limit, help=_("Maximum number of rows to return per catalog.")),
    max_rows_display: int = typer.Option(20, help=_("Maximum number of rows to display per table. Use -1 for all rows.")),
    show_all_columns: bool = typer.Option(False, "--show-all-cols", help=_("Show all columns in the output table.")),
