def generate_trigger_sql(
    ...
    if track_only is not None and exclude_fields is not None:
        raise ValueError(f"Cannot specify both track_only and exclude_fields for table {table_name}")
    ...
    if track_only is not None:
        ...
        ignored_keys_expr = f"(SELECT array_agg(key) FROM jsonb_object_keys(to_jsonb(NEW)) AS key WHERE key NOT IN {ignored_keys})"
