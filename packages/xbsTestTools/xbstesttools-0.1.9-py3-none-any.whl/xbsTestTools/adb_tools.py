...
class AdbTools:
    ...
    def wifi_assertion_internet(self):
        ...
        try:
            ...
            if response.returncode != 0 or response.stderr:
                log.error(f"{self.dev} 网络互联网异常: {response.stderr}")
                return False
            ...
            if not match:
                log.error(f"{self.dev} 无法解析ping结果: {ping_data}")
                return False

            packet_loss = float(match.group(1))

            if packet_loss <= 20:
                log.debug(f"{self.dev} 网络正常，丢包率: {packet_loss}%")
                return True
            elif packet_loss >= 80:
                log.debug(f"{self.dev} 设备Ping 网络丢包较高: {packet_loss}%")
                return True
            elif packet_loss == 100:
                log.error(f"{self.dev} 设备Ping 网络丢包100%，网络不可上网")
                return False
            else:
                log.debug(f"{self.dev} 设备Ping 网络丢包率: {packet_loss}%")
                return packet_loss < 50  # 根据丢包率决定返回值
        except Exception as e:
            log.error(f"{self.dev} 网络检测过程出错: {str(e)}")
            return False
    ...
    def dow_screen(self):
        ...
        if not response.stderr:
            if "mWakefulness=Awake" not in response.stdout:
                ...
                log.debug(f"{self.dev} 屏幕唤醒成功")
                return True
        log.error(f"{self.dev} 获取屏幕状态信息异常,屏幕唤醒失败")
    ...
    def assertion_dev_launcher(self, Launcher_name, timeout_=60):
        ...
        for t in range((timeout_ // 2)):
            ...
            if response.stderr:
                log.warning(f"{self.dev} 获取ADB设备Launcher失败 重新获取中")
                sleep(2)
                continue
            launcher_info = str(response.stdout).strip()
            log.debug(f"{self.dev} {launcher_info}")
    ...
    def assertion_wifi_type(self, type_=""):
        ...
        for t in range(3):
            response = subprocess.run(f"adb -s {self.dev} shell settings get global wifi_on", stdout=subprocess.PIPE,
                                      stderr=subprocess.PIPE, text=True)
            if response.stderr:
                log.warning(f"{self.dev} 获取ADB设备WIFI开关状态失败 重新获取中")
                sleep(2)
                continue
            wifi_onoff_info = str(response.stdout).strip()
            log.debug(f"{self.dev} WIFI 开关状态 {wifi_onoff_info}")
            if type_:
    ...
    def onoff_wifi(self, type_=False):
        ...
        for t in range(3):
            if type_:
                ...
                if response.stderr:
                    log.warning(f"{self.dev} 打开WIFI开关失败 重新打开中")
                    sleep(2)
                    continue
                if self.assertion_wifi_type("1"):
                    log.debug(f"{self.dev} WIFI开关打开")
                    return True
                log.error(f"{self.dev} WIFI开关打开失败")
            else:
                response = subprocess.run(f"adb -s {self.dev} shell cmd wifi set-wifi-enabled disabled")
                if response.stderr:
                    log.warning(f"{self.dev} 关闭WIFI开关失败 重新关闭中")
                    sleep(2)
                    continue
                if self.assertion_wifi_type("0"):
                    log.error(f"{self.dev} WIFI开关 关闭")
                    return True
                log.error(f"{self.dev} WIFI开关关闭失败")
            break
    ...
    def select_packages(self, type_="ALL", package_name=None):
        ...
        if type_ == "ALL":
            ...
            log.error(f"获取包名参数错误 {type_} 只能为一下分类['三方','系统','当前','ALL']")
            return
        ...
        log.debug(f"[{len(package_list)}个] - {package_list}")
        return package_list
    ...
    def select_size(self):
        ...
        if result.returncode != 0:
            log.error(f"{self.dev} 获取设备分辨率失败")
            return
    ...
    def swipe(self, start_x, start_y, end_x, end_y, duration_ms=300):
        ...
        if not width or not height:
            log.error(f"{self.dev}滑动失败")
            return

        if "%" in str(start_x) and "%" in str(start_y) and "%" in str(end_x) and "%" in str(end_y):
            # 计算百分比坐标
            log.debug(f"{self.dev} 滑动百分比 {start_x} {start_y} {end_x} {end_y}")
        ...
        log.debug(f"{self.dev} 滑动成功 {start_x} {start_y} {end_x} {end_y} {duration_ms}")
        return True
    ...
    def click(self, start_x, end_x, duration_ms=300):
        ...
        if "%" in str(start_x) and "%" in str(end_x):
            # 计算百分比坐标
            log.debug(f"{self.dev} 点击百分比 {start_x} {end_x}")
            start_x = int(width * int(start_x.replace('%', '')) / 100)
            end_x = int(width * int(end_x.replace('%', '')) / 100)

        command = f"adb -s {self.dev} shell input tap {start_x} {end_x} {duration_ms}"
        subprocess.run(command)
        log.debug(f"{self.dev} 点击成功 {start_x} {end_x} {duration_ms}")
        return True
