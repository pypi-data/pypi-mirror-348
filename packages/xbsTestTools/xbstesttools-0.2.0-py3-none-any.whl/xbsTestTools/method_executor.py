...
class MethodExecutor:
    ...
    def execute(self, stop_on_error: bool = True) -> bool:
        ...
        for i, method_info in enumerate(self.methods):
            ...
            try:
                log.debug(f"开始执行第 {i + 1} 个方法: {method.__name__}")
                ...
                log.debug(f"方法 {method.__name__} 执行完成，耗时: {execution_time}秒")

                if not result:
                    log.warning(f"方法 {method.__name__} 执行失败")
                    success = False
                    if stop_on_error:
                        # 执行过程失败停止
                        break
                ...
                log.warning(f"执行方法 {method.__name__} 时发生错误: {str(e)}")
                success = False
                if stop_on_error:
                    break
