...
from ngcbase.constants import STAGING_ENV, UMASK_GROUP_OTHERS_READ_EXECUTE
...
class AsyncFileExistCheckWorkerPool(AsyncTransferWorkerPoolBase):
    ...
    async def process_workload(self, trace_scope):
        ...
        for workload in workload_list:
            ...
            assert file_node
            if file_node.hash in hash_exists:
                await self.progress.advance(file_node.size, 1)
            else:
                cs, _ = self.c_table.calculate_checked_file_completion(workload[0])
                await self.progress.advance(cs, 0)
...
async def aiohttp_session_context():
    ...
    _connector = aiohttp.TCPConnector(
        ssl=_ssl_context, limit=NGC_CLI_MAX_CONCURRENCY, force_close=True, ttl_dns_cache=NGC_CLI_TRANSFER_TIMEOUT * 10
    )
        ...
        all_files = [
            {
                ...
                "filePath": os.path.relpath(file_node.file_path, self.source_dir),
                "sha256": file_node.hash,
            }
        ...
        try:
            ...
            if upload_manager.c_table.storage_version == "V2":
                ...
                if (
                    ...
                    # upload_manager.request_commit_files(file_paths, base_version)

                    if base_version:
                        upload_manager.request_commit_files([], base_version)
                    for i in range(0, len(file_paths), 100):
                        upload_manager.request_commit_files(file_paths[i : i + 100])  # noqa: E203
