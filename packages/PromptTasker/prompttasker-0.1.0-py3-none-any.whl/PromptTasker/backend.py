...
def formatfile(inputfilepath,outputfilepath): 
    ...
    try:
        ...
        formatted_output = subprocess.check_output(
            ["npx", "-y", "prettier@3.4.2", inputfilepath],text=True
        )
...
def logs(inputfilepath,outputfilepath,count):
    ...
    try:
        for i in range(0,count):
            with open(inputfilepath+sorted_files[i][0], 'r') as file:
                print('success')
                x = file.readline() 
...
def similar(inputfilepath,outputfile):
    if not os.path.exists(inputfilepath):
        return 'no_i'
    try:
...
def run_task(task: str): 
    ...
    if "choices" in result and result["choices"]:
        ...
        return 'Maybe the task you wanted is not available. Please check.'
    file_keys = {"inputfile", "inputfilepath", "inputpath", "dbpath", "filepath", "fpath","url"}
    result, outputpath = globals()[funv["name"]](**funv["arguments"])
    
    if result =='no_i':
        return ' the input file path is not valid one.'
    elif result == 'no_task':
        return 'The task you said cannot be performed on the file path or url mentioned.'
    else:
