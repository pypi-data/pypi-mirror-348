...
class MagiqueWorker(NetworkObject):
    ...
    async def handle_request(self, message: bytes, websocket):
        ...
        elif action == "reverse_invoke_result":
            await self.handle_reverse_invoke_result(request)
        else:
            logger.error(f"Unknown action: {action}")

    async def handle_reverse_invoke_result(self, request: dict):
