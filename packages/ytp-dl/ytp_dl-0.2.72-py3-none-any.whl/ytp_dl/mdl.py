...
def main():
    ...
    with tempfile.NamedTemporaryFile(delete=False, dir='/tmp', suffix=f'.{args.extension}' if args.extension else '.mp4') as tmpfile:
    ...
    try:
        run_command(ytdlp_cmd)
        if os.path.exists(output_path):
            print(f"DOWNLOADED_FILE:{output_path}")
        else:
