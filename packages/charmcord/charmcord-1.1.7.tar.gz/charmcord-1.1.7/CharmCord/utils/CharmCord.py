...
class CharmCord:
    ...
    def on_message(self, code=None):
        ...
        async def on_message(msg: discord.Message):
            options['onMessage']["author"] = msg.author.name
            options['onMessage']["channelid"] = msg.channel.id
            options['onMessage']["guildid"] = msg.guild.id
            for attr in options['onMessage'].keys():
                try:
                    value = getattr(msg, attr)
                    options['onMessage'][attr] = value
                except AttributeError:
                    continue

            if code is not None:
                final_code = await no_arguments(code, TotalFuncs, msg)
                await find_bracket_pairs(final_code, TotalFuncs, msg)
            return
