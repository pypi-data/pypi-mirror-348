...
def show_somma_bernoulli(p, n, num_exp):
    ...
    plt.plot(x, y, 'r-', linewidth=2, label='Approssimazione normale teorica')

    # Statistiche
    media_empirica = np.mean(dati)
    std_empirica = np.std(dati)

    # Aggiungere informazioni statistiche

    # Dettagli
    plt.title(f'Distribuzione binomiale B({n}, {p:.2f}) vs approssimazione normale', fontsize=14)
