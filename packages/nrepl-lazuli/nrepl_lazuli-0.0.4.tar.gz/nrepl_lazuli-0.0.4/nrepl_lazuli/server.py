...
from nrepl_lazuli.bencode import Bencode

class NREPL:
    def __init__(self, host='127.0.0.1', port=7889, debug=False, frame=None, root_dir=None):
        self.host = host
        self.port = port
        self.debug = debug
    ...
    @staticmethod
    def bind(frame, host='127.0.0.1', port=7889, debug=False, root_dir=None):
        NREPL(host, port, debug, frame, root_dir).start()

    @staticmethod
    def spawn(host='127.0.0.1', port=7889, debug=False, frame=None, root_dir=None):
        server = []
        def start(server):
            server.append(NREPL(host, port, debug, frame, root_dir))
            server[0].start()
    ...
    def handle_client(self, client_socket):
        try:
            while True:
                ...
                # Decode the bencoded message
                i, request = Bencode.decode(self.buffer)
                if(not request): continue
                self.buffer = self.buffer[i-1:-1]
    ...
    def handle_eval(self, client_socket, request):
        ...
        try:
            ...
            __lazuli__placeholder__row = "\n" * __lazuli__file__row
            if self._need_to_run_as_exec(__lazuli__last__code):
    ...
    def handle_clone(self, client_socket, request):
        ...
        # Encode the response as bencode and send it back to the client
