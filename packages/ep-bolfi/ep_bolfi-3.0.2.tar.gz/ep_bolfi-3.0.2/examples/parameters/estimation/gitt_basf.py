...
if 'overpotential' not in globals():
    overpotential = True
    print("Default: subtracts OCV from simulator data.")

if 'three_electrode' not in globals():
    three_electrode = None
    dimensionless_reference_electrode_location = 0.5
    print("Default: no three-electrode measurement setup.")

if 'dimensionless_reference_electrode_location' not in globals():
    dimensionless_reference_electrode_location = 0.5
    print("Default: reference electrode placed in the separator center.")
...
if 'transform_parameters' not in globals():
    if soc_dependent_estimation:
        transform_parameters = {
            ...
            "Negative particle diffusivity [m2.s-1]": "log",
            "Positive particle diffusivity [m2.s-1]": "log",
        }
    else:
...
gitt.indices = [index / 2 for index, _ in enumerate(gitt.indices)]
...
def simulator(trial_parameters):
    ...
    for cycle in solution.cycles:
        ...
        sim_data[2].append(cycle["Current [A]"].entries)
