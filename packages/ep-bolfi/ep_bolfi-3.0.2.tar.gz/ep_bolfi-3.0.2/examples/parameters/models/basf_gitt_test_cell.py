...
def neg_ele_exc_cur_den(cₑ, cₙ, cₙ_max, T):
    return 3.67e-6 * cₑ ** αₚₙ * cₙ ** αₙₙ * (cₙ_max - cₙ) ** αₚₙ
...
def pos_ele_exc_cur_den(cₑ, cₚ, cₚ_max, T):
    return 5.06e-6 * cₑ ** αₙₚ * cₚ ** αₙₙ * (cₚ_max - cₚ) ** αₙₚ
...
def neg_ele_exc_cur_den_der(cₑ, cₙ, cₙ_max, T):
    return 3.67e-6 * αₚₚ * cₑ ** (αₚₚ - 1) * cₙ**αₚₙ * (cₙ_max - cₙ) ** αₚₚ


parameters[
    "Negative electrode exchange-current density partial derivative "
    "by electrolyte concentration [A.m.mol-1]"
] = neg_ele_exc_cur_den_der


def pos_ele_exc_cur_den_der(cₑ, cₚ, cₚ_max, T):
    return 5.06e-6 * αₚₚ * cₑ ** (αₚₚ - 1) * cₚ**αₙₚ * (cₚ_max - cₚ) ** αₚₚ


parameters[
    "Positive electrode exchange-current density partial derivative "
    "by electrolyte concentration [A.m.mol-1]"
] = pos_ele_exc_cur_den_der
