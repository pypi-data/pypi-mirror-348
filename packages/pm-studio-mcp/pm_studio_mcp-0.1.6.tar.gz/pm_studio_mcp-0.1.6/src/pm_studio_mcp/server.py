...
async def generate_markdown_tool(content: str, filename: str):
    ...
    """
    return FileUtils.generate_markdown_tool(content, filename, config.WORKING_PATH)
...
async def generate_pie_chart_tool(data: Dict[str, int]):
    ...
    """
    return DataVisualizationUtils.generate_pie_chart_tool(data, config.WORKING_PATH)
...
@mcp.tool()
async def search_table_metadata_tool(table_name: str):
    """
...
@mcp.tool()
async def generate_sql_from_template_tool(template_name: str, filter_values: dict = None):
    """
