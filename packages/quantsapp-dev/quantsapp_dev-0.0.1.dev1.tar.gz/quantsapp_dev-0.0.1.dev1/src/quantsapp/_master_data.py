...
class MasterData:
    ...
    master_data: generic_types.MasterData = None # type: ignore
    ...
    def _get_master_data(self) -> generic_types.MasterData | None:
    ...
    def _get_master_data_from_local_file(self) -> generic_types.MasterData:
        """Get the master data from the local cache file"""

        _cache_master_data = generic_utils.get_local_file_cache_data(self._cache_file_name)
    ...
    @staticmethod
    def parse_master_data_from_api_response(master_data_api_response: generic_types.MasterDataApiResponse) -> None:
    ...
    @staticmethod
    def is_valid_instr(instr: generic_models.Instrument) -> bool:

        if not MasterData.is_valid_symbol(symbol=instr.symbol):
            return False

        if not MasterData.is_valid_expiry(symbol=instr.symbol, expiry=instr.expiry):
            return False
