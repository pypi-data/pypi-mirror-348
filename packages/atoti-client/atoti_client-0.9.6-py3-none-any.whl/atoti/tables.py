...
from ._graphql_client import CreateTableColumnInput, CreateTableInput, DeleteTableInput
...
class Tables(
    ...
    def _get_unambiguous_keys(self, *, key: TableName | None) -> list[TableName]:
        ...
        if key is None:
            output = graphql_client.get_tables(
            ...
            return [table.name for table in data_model.database.tables]

        output = graphql_client.find_table(  # type: ignore[assignment] # See https://github.com/python/mypy/issues/12968.
            data_model_transaction_id=data_model_transaction_id,
            table_name=key,
        )
        data_model = check_data_model_not_none(
        ...
        table = data_model.database.table  # type: ignore[attr-defined]
    ...
    def _update_delegate(
        ...
        for index, (table_name, definition) in enumerate(other.items()):
            columns = {
                column_name: CreateTableColumnInput(
                    column_name=column_name, data_type=data_type_to_graphql(data_type)
                )
            ...
            mutation_input = CreateTableInput(
            ...
            if definition.partitioning is not None:
                mutation_input.partitioning = definition.partitioning
            output_fields = Mutation.create_table(mutation_input).alias(
                f"createTable_{index}"
            )
    ...
    def _delete_delegate_keys(self, keys: AbstractSet[TableName], /) -> None:
        ...
        for index, key in enumerate(keys):
            output_fields = Mutation.delete_table(
                DeleteTableInput(
                    data_model_transaction_id=data_model_transaction_id, table_name=key
                )
    ...
    def schema(self) -> object:
        ...
        output = graphql_client.get_database_schema(
            data_model_transaction_id=data_model_transaction_id
        )
        data_model = check_data_model_not_none(
            output.data_model, data_model_transaction_id=data_model_transaction_id
