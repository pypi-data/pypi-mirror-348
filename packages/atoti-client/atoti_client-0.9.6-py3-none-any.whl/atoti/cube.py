...
class Cube(HasIdentifier[CubeIdentifier], ReprJsonable):
    ...
    def _fact_table_identifier(self) -> TableIdentifier | None:
        ...
        output = graphql_client.get_cube_fact_table(
            cube_name=self.name,
        ...
        return (
            None if cube.fact_table is None else TableIdentifier(cube.fact_table.name)
        )
    ...
    def query_cube_ids(self) -> AbstractSet[str]:
        ...
        return (
            frozenset()
            if cube.cluster is None
            else frozenset(node.name for node in cube.cluster.nodes)
        )
    ...
    def aggregate_cache(self) -> AggregateCache | None:
        ...
        output = graphql_client.get_aggregate_cache(
            cube_name=self.name, data_model_transaction_id=data_model_transaction_id
        )
        data_model = check_data_model_not_none(
            output.data_model, data_model_transaction_id=data_model_transaction_id
        ...
        aggregate_cache = cube.aggregate_cache
    ...
    def aggregate_cache(self, aggregate_cache: AggregateCache, /) -> None:
        ...
        mutation_input = UpdateCubeInput(
        ...
        graphql_client.update_cube(mutation_input)
    ...
    def updateable_cells(self) -> UpdateableCells | None:
        ...
        output = graphql_client.get_updateable_cells(
            cube_name=self.name, data_model_transaction_id=data_model_transaction_id
        )
        data_model = check_data_model_not_none(
            output.data_model,
        ...
        updateable_cells = cube.updateable_cells

        if updateable_cells is None:
            return None
        return UpdateableCells(
    ...
    def create_parameter_hierarchy_from_members(
        ...
        *,
        ...
        create_table_output_fields._subfields.append(
