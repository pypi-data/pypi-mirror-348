...
def reduced_graph():
    ...
    assert (0, 1, 4) in steps
    assert (3, 1, 4) in steps
    return graph


def test_number_of_edges(reduced_graph):
    assert reduced_graph.number_of_edges == 6, tuple(reduced_graph.edges())


def test_number_of_nodes(reduced_graph):
    assert reduced_graph.number_of_nodes == 5
...
def test_get_edge_operator_expression(reduced_graph):
    ...
    assert tuple(reduced_graph.get_edge_operator_expression(3, 4)) == ("*", "C", "D")
    assert tuple(reduced_graph.get_edge_operator_expression(0, 4)) == ("*", "A", "D")
...
def test_in_degree(reduced_graph):
    ...
    assert reduced_graph.in_degree(4) == 2
...
def test_out_degree(reduced_graph):
    ...
    assert reduced_graph.out_degree(3) == 3
    assert reduced_graph.out_degree(4) == 0
