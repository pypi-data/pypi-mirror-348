...
class CCD(CCDCamera, Image):
    ...
    unscaled x and y coordinate arrays used are :attr:`.xdata` and :attr:`.ydata`,
    respectively. Note that this is just the intensity at the points (x,y), not an
    integrated power over some finite pixel size.
...
class CCDScanLine(CCDCamera, ScanLine):
    ...
    slice. Where the unscaled x and y coordinate arrays used are :attr:`.xdata` and
    :attr:`.ydata`, respectively. Note that this is just the intensity at the points
...
class CCDPixel(CCDCamera, Pixel):
    ...
    point. Where the unscaled x and y coordinate used is :attr:`.xdata` and
    :attr:`.ydata`, respectively. Note that this is just the intensity at (x,y), not an
    integrated power over some finite pixel dimension.
