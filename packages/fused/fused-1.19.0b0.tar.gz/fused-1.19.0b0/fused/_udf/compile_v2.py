...
def compile_udf_and_run_with_kwargs(udf: GeoPandasUdfV2, input, **kwargs):
    with ExitStack() as stack:
        ...
        _fn = _compile_and_get_local(udf)

        kwargs_to_pass = {**udf.parameters, **kwargs}
        if input != []:
            if not hasattr(input, "as_udf_args"):
                input = MockUdfInput(input[0])
            # this gets handled by the entrypoint wrapper returned by _compile_and_get_local
            kwargs_to_pass["input"] = input
        args_to_pass = []

        output = _fn(*args_to_pass, **kwargs_to_pass)
