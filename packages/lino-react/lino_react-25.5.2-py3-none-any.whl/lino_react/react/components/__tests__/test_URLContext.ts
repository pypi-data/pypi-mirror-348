    ...
    it("trading.InvoicesByJournal workflow button", async () => {
        ...
        const clone = await page.evaluate(() => {
            return window.App.URLContext.actionHandler.parser.stringify({
                clone: {
                    params: {path: "/api/trading/InvoicesByJournal/91", mk: 1, mt: 66},
                    runnable: {actorId: "trading.InvoicesByJournal", an: "wf2", sr: 91},
                }
            }, true);
        });
        ...
        await page.goto(`${global.SERVER_URL}/#/api/trading/InvoicesByJournal/91?${clone}`);
        ...
        await page.evaluate(() => {
            window.App.URLContext.actionHandler.runAction({
                actorId: "trading.InvoicesByJournal", an: "wf1", sr: 91
            });
        });
        await page.waitForNetworkIdle();
        wfState = await page.evaluate(() => {
