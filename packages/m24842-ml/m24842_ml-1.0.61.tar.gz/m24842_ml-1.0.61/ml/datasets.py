...
class ThePile(Dataset):
    def __init__(
        ...
        # prepare cache dir
        cache_dir = os.path.join(f"{root}/ThePile", f"{split}_sharded_cache")
        os.makedirs(cache_dir, exist_ok=True)

        # find or build shards
        shard_dirs = sorted(
            [
        ...
        if not shard_dirs:
            # first time: split raw into shards, tokenize & save
            if split == "train":
        ...
        # now build length index for on-demand loading
        self.shard_paths = shard_dirs
        self.shard_lens = []
        for p in self.shard_paths:
            ds = load_from_disk(p, in_memory=False)  # Arrow‐mmapped
        ...
        # for caching the last‐used shard
    ...
    def step(self):
        # increase sequence length per epoch
