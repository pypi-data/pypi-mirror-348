...
class ThePile(Dataset):
    def __init__(self, root, split, tokenizer, min_len=1, max_len=1000, warmup_epochs=0, num_proc=4):
        """
        ...
        if split == 'train':
            ...
            self.data = load_dataset('json', data_files=f'{root}/val.jsonl', split='train')
        else:
            self.data = load_dataset('json', data_files=f'{root}/test.jsonl', split='train')
