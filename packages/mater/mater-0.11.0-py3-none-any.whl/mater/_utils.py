...
def _log_normal(
    ...
    log_normal = np.exp(
        ...
        / 2
        * (
            (np.log(time).subtract(life_time[k], axis=0)).div(
                standard_deviation[k], axis=0
            )
        )
        ** 2
...
def _exponential_decay(
    life_time: DataFrame, k: datetime.datetime, offset: datetime.datetime
):
    r"""
    ...
    time = _time_df(life_time, k, offset)
    exponential_decay = (
        np.exp(-time.div(life_time[k], axis=0))
...
def _time_df(life_time: DataFrame, k: datetime.datetime, offset: datetime.datetime):
    ...
    time = pd.DataFrame(
        data=[t] * len(life_time.index), index=life_time.index, columns=col
    )
...
def _groupby_sum_empty(df: pd.DataFrame | pd.Series, group_index: list):
    if df.empty:
        ...
        return df.groupby(
            level=group_index
        ).sum()  # may be parameterized if we need something else than sum()
...
def _loc_dynamic(df: pd.DataFrame | pd.Series, level_slice: Dict[str, List[str]]):
    ...
    filtered_df = (
        df.loc[idx[tuple(slice_obj)], :]
        if isinstance(df, pd.DataFrame)
        else df.loc[idx[tuple(slice_obj)]]
    )

    return filtered_df


def _loc_dynamic_direct(
    df: pd.DataFrame | pd.Series, level_slice: Dict[str, List[str]]
):
    """
    ...
    loc_args = tuple(
        df.index.get_level_values(level).intersection(level_slice[level])
        if level in level_slice
        else slice(None)
...
def read_json(
    ...
    if folder == "input_data":
        ...
        dict_dfs = {
            var: df[df["variable"] == var].copy().drop(["variable", "scenario"], axis=1)
...
def input_completion(
    ...
    aspect_property = (
        variable_aspects[["dimension", "property"]]
        .set_index("dimension")
        .T.to_dict("list")
    )
