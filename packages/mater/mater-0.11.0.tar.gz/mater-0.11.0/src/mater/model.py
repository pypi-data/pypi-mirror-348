...
class Mater(_MaterProcessNonPhysical):
    ...
    def parameter_from_json(
        ...
        """Returns the parameters of the mater compiler and a dictionary with all variables by loading data from the json files orders in a data directory, as defined by the `mater-data-providing` package.
        ...
        dimension["parents_values"] = dimension["parents_values"].apply(
            lambda x: x.get("default")
            if isinstance(x, dict) and "default" in x
            else np.nan
        )
        ...
        for input_name, input in inputs_dict.items():
            completed_input = input_completion(
                input_name, input, ds, variable_dimension
            )
            input_dict[input_name] = self.input_formatting(completed_input)
        # keep only keys that are in Parameter dataclass
        keys = np.intersect1d(
            list(input_dict.keys()), list(Parameter.__annotations__.keys())
        )
    ...
    def set_time(
        ...
        simulation_start_time: int | str | pd.DatetimeIndex,
        simulation_end_time: int | str | pd.DatetimeIndex,
        time_frequency: Literal[
            "YS", "QS", "MS", "W", "D", "h", "min", "s", "ms", "us", "ns"
        ],
    ):
