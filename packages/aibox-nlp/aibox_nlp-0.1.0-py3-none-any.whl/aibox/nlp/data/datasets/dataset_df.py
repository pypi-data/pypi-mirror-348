...
class DatasetDF(Dataset):
    def __init__(
        ...
        Args:
            df (pd.DataFrame, Pathlike, str): Dataframe com os dados ou caminho
                para CSV.
            ...
            target_mapper (Callable[[Any], int | float]): função de mapeamento
                para coluna target. Deve ser utilizada caso algum pré-processamento
                deva ser aplicado para converter a coluna para valores numéricos (
                default=None, não realiza mapeamento).
        """
    ...
    def load_from_kaggle(
        ...
        *files_to_load: str,
        ...
        Args:
            ...
            *files_to_load (str): uma ou mais strings com quais
                arquivos devem ser carregados (o DataFrame final
                é uma concatenação linha a linha).
            drop_others (bool): se demais colunas devem ser
                removidas (default=False).
            target_mapper (Callable[[Any], int | float]): função
                de mapeamento da coluna target.

        Returns:
            DatasetDF: instância com os dados do dataset do
                Kaggle.
        """
        import kagglehub
