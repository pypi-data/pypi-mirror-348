...
def add_entrypoint_and_running_env(step_to_upload: dict, clarity_section: dict, default_entrypoint: str, default_running_env: str) -> dict:
...
def create_step_metadata_from_poetry_v2(pyproject_data: dict, clarity_section: dict, default_entrypoint: str, default_running_env: str) -> dict:
    ...
    add_entrypoint_and_running_env(step_to_upload, clarity_section, default_entrypoint, default_running_env)
...
def create_step_metadata_from_poetry_v1(pyproject_data: dict, clarity_section: dict, default_entrypoint: str, default_running_env: str) -> dict:
...
def get_step_to_upload(pyproject_data: dict, clarity_section: dict, default_entrypoint: str, default_running_env: str) -> tuple[dict, dict]:
    # Check if the TOML has v2 format by looking for specific v2 fields
    is_v2 = "project" in pyproject_data
    if is_v2:
        step_to_upload, clarity_section = create_step_metadata_from_poetry_v2(pyproject_data, clarity_section, default_entrypoint, default_running_env)
    else:
        step_to_upload, clarity_section = create_step_metadata_from_poetry_v1(pyproject_data, clarity_section, default_entrypoint, default_running_env)
    return step_to_upload, clarity_section


def handle_upload_url_options(step_id: str, domain: str, headers: dict, step_to_upload: dict, pyproject_data: dict, is_override: bool = False) -> tuple[str, str]:
    url_to_upload = out.run_sync_function("Getting url to upload component", get_upload_url, step_id=step_id, domain=domain, headers=headers, is_override=is_override)
...
def parse_and_upload_poetry_package(package_dir: str, workspace_id: str, domain: str, auth_token: str, default_entrypoint: str, default_running_env: str, pyc: bool) -> str:
    """
    ...
    step_to_upload, clarity_section = get_step_to_upload(pyproject_data, clarity_section, default_entrypoint, default_running_env)

    out.vprint("Creating step")
    step_id, success = out.run_sync_function("Creating step", create_step, step_to_upload=step_to_upload, domain=domain, headers=headers)
    if not success:
        answer = input.ask_for_confirmation("Could not create step, would you like to bump the version?")
        if answer:
            step_id, pyproject_data = handle_new_version(step_to_upload, step_id, domain, headers, pyproject_data)
        else:
            input.ask_for_confirmation("would you like to override the component?", hard_stop=True)
    if not step_id:
        raise StopCommand("Couldn't create/update step, please check your project name and version in pyproject.toml file")
    ...
    with open(pyproject_path, "w") as f:
        ...
        f.write(toml_str)
    upload_response_data, new_step_id, pyproject_data = handle_upload_url_options(step_id, domain, headers, step_to_upload, pyproject_data, is_override=(not success))
