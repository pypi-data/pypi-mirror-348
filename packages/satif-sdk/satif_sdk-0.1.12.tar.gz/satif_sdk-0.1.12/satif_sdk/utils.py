    ...
    try:
        with open(file_path, "rb") as fb:
            ...
            if best_guess and best_guess.get("encoding"):
                ...
                # Consider logging a warning here if no encoding found, before defaulting
                # For now, consistent with original behavior of raising if not found by charset_detect
                raise ValueError(
        ...
        # Log other errors, then re-raise as RuntimeError for consistent error handling upstream
        # logger.error(f"Error during encoding detection for {file_path.name}: {e}", exc_info=True)
        raise RuntimeError(
...
def detect_csv_delimiter(sample_text: str) -> str:
    ...
    try:
        ...
        # logger.error(f"Unexpected error during CleverCSV delimiter detection: {e}", exc_info=True)
        raise RuntimeError(
