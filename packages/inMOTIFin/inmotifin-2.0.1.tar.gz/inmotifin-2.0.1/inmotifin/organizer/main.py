...
from which motifs are sampled. Default: 1,1,1,1")
@click.option(
    "--num_motifs", type=int,
...
JASPAR motif IDs.")
@click.option(
    "--dimerisation_rules", type=click.Path(),
    help="Path to the dimerisation rule tsv files. \
...
def dimers(
        ...
        dimerisation_rules,
        seed):
    """ Dimerizing motifs given motifs and distances"""
    click.echo('Dimerizing motifs given motifs and distances')
    ctx = read_config(ctx=ctx, config=config)
    ctx, basic_params = assign_basic_params(
        ctx=ctx, title=title, workdir=workdir, seed=seed)
    ...
    if jaspar_db_version not in ctx.default_map:
        ctx.default_map['jaspar_db_version'] = None
    if dimerisation_rules is not None:
        ctx.default_map['dimerisation_rules'] = dimerisation_rules
    if ctx.default_map['dimerisation_rules'] is None:
        raise ValueError("Dimerisation rules should be provided.")
    dimer_params = DimerParams(
        motif_files=ctx.default_map["motif_files"],
        jaspar_db_version=ctx.default_map["jaspar_db_version"],
        dimerisation_rule_path=ctx.default_map["dimerisation_rules"])
    controller = Controller(basic_params=basic_params)
    controller.create_dimers(dimer_params=dimer_params)
...
@click.option(
    "--b_length", type=int,
    help="Length of background sequences to simulate. Default: 50")
@click.option(
    "--b_alphabet", type=str,
...
@click.option(
    "--shuffle", type=str,
    help="Type of shuffling of the background sequences. Supported types \
are: none, random_nucl_only, random_nucl_addon. none: No shuffling, \
random_nucl_only: Nucleotide shuffling and only the shuffled sequences \
are returned. random_nucl_addon: Nucleotide shuffling and both the \
original and the shuffled sequences are returned. Default: none")
@click.option(
...
@click.option(
    "--num_topics", type=int,
    help="Number of topics into which motifs are assigned.  If = 1 \
all motifs are assigned to a single topic. Default: 1")
@click.option(
    "--max_topic_size", type=int,
    help="Maximum size of each topic. It cannot be smaller than the \
number of motifs. Each topic size is sampled from binomial distribution \
with number of trials = max_topic_size and success = topic_size_p. \
Default: inf")
@click.option(
    "--topic_size_p", type=float,
    help="This parameter controls the expected size of each group. \
Each topic size is sampled from binomial distribution with number of \
trials = max_topic_size and success = topic_size_p. Default: 1")
@click.option(
    "--topic_motif_assignment_file", type=click.Path(),
    help="Path to the motif to topic asisgnment file in two column \
tsv format. The first column is the topic IDs, and the second column \
lists the motfIDs that are assigned to the corresponding topic.")
@click.option(
    "--topic_freq_type", type=str,
    help="The method of selecting topic background frequencies.\
Values: uniform, random. Where uniform means each topic has an \
equal chance to be selected. Random means each topic is assigned \
a probability of being selected. The difference between a \
frequent and rare topic is controlled by the --topic_freq_range \
parameter. Default: uniform")
@click.option(
    "--topic_freq_range", type=int,
...
@click.option(
    ...
    help="The preference of motifs to share the same topic when \
selected for insertion. Value between 0 and 1. Default: 1")
@click.option(
    "--topic_topic_type", type=str,
    help="The method of selecting topic-topic co-occurence \
probabilities. Values: uniform, random. Where uniform \
means any two topics are equally probable of co-occuring. \
Random means topic pairs are assigned a random probability \
of co-occurence. Default: uniform")
@click.option(
    "--topic_freq_file", type=click.Path(),
...
@click.option(
    "--topic_topic_file", type=click.Path(),
    help="Tsv file including the topic-topic cooccurence \
probability matrix")
@click.option(
...
replacing existing bases. Default: central")
@click.option(
...
@click.option(
    "--num_topics_per_seq", type=int,
    help="Number of topics to sample (with replacement) per sequence. \
Default: 1")
@click.option(
...
def motif_in_seq(
    ...
    ctx, topic_params, freq_params = assign_topic_freq_params(
        ctx=ctx,
        num_topics=num_topics,
        max_topic_size=max_topic_size,
        topic_size_p=topic_size_p,
        topic_motif_assignment_file=topic_motif_assignment_file,
        topic_freq_type=topic_freq_type,
        topic_freq_range=topic_freq_range,
        motif_freq_type=motif_freq_type,
        motif_freq_range=motif_freq_range,
        concentration_factor=concentration_factor,
        topic_topic_type=topic_topic_type,
        topic_freq_file=topic_freq_file,
        motif_freq_file=motif_freq_file,
        topic_topic_file=topic_topic_file)
    ctx, positions_params, sampling_params = assign_position_sampling_params(
        ...
        num_topics_per_seq=num_topics_per_seq,
