...
def assign_topic_freq_params(
        ...
        motif_freq_file,
        topic_topic_file):
    """ Helper function to overwrite topic and frequency params \
        in config when applicable """
    if num_topics is not None:
        ctx.default_map['num_topics'] = num_topics
    if "num_topics" not in ctx.default_map:
        ctx.default_map['num_topics'] = None
    if max_topic_size is not None:
        ctx.default_map['max_topic_size'] = max_topic_size
    if "max_topic_size" not in ctx.default_map:
        ctx.default_map['max_topic_size'] = None
    if topic_size_p is not None:
        ctx.default_map['topic_size_p'] = topic_size_p
    if "topic_size_p" not in ctx.default_map:
        ctx.default_map['topic_size_p'] = None
    if topic_motif_assignment_file is not None:
        ctx.default_map['topic_motif_assignment_file'] = \
                topic_motif_assignment_file
    if "topic_motif_assignment_file" not in ctx.default_map:
        ctx.default_map['topic_motif_assignment_file'] = None

    topic_params = TopicParams(
        number_of_topics=ctx.default_map['num_topics'],
        max_topic_size=ctx.default_map['max_topic_size'],
        topic_size_binom_p=ctx.default_map['topic_size_p'],
        topic_motif_assignment_file=ctx.default_map[
            'topic_motif_assignment_file'])

    if topic_freq_type is not None:
        ctx.default_map['topic_freq_type'] = topic_freq_type
    if "topic_freq_type" not in ctx.default_map:
        ctx.default_map['topic_freq_type'] = None
    if topic_freq_range is not None:
        ctx.default_map['topic_freq_range'] = topic_freq_range
    if "topic_freq_range" not in ctx.default_map:
        ctx.default_map['topic_freq_range'] = None
    ...
    if topic_topic_type is not None:
        ctx.default_map['topic_topic_type'] = topic_topic_type
    if "topic_topic_type" not in ctx.default_map:
        ctx.default_map['topic_topic_type'] = None
    if topic_freq_file is not None:
        ctx.default_map['topic_freq_file'] = topic_freq_file
    if "topic_freq_file" not in ctx.default_map:
        ctx.default_map['topic_freq_file'] = None
    ...
    if topic_topic_file is not None:
        ctx.default_map['topic_topic_file'] = topic_topic_file
    if "topic_topic_file" not in ctx.default_map:
        ctx.default_map['topic_topic_file'] = None

    freq_params = FreqParams(
        topic_frequency_type=ctx.default_map['topic_freq_type'],
        topic_frequency_range=ctx.default_map['topic_freq_range'],
        motif_frequency_type=ctx.default_map['motif_freq_type'],
        motif_frequency_range=ctx.default_map['motif_freq_range'],
        topic_topic_type=ctx.default_map['topic_topic_type'],
        concentration_factor=ctx.default_map['concentration_factor'],
        topic_freq_file=ctx.default_map['topic_freq_file'],
        motif_freq_file=ctx.default_map['motif_freq_file'],
        topic_topic_file=ctx.default_map['topic_topic_file'])

    return ctx, topic_params, freq_params
...
def assign_position_sampling_params(
    ...
    if num_topics_per_seq is not None:
        ctx.default_map['num_topics_per_sequence'] = \
            num_topics_per_seq
    if "num_topics_per_sequence" not in ctx.default_map:
        ctx.default_map['num_topics_per_sequence'] = None
    ...
    sampling_params = SamplingParams(
        ...
        num_topics_per_sequence=ctx.default_map['num_topics_per_sequence'],
