"""Assign motifs to topics
author: Kata Ferenc
email: katalitf@uio.no
"""
from typing import List, Dict, Set
import numpy as np
from inmotifin.utils.paramsdata.topicparams import TopicParams
from inmotifin.modules.data.topics import Topics
from inmotifin.utils.fileops.reader import Reader
from inmotifin.utils.fileops.writer import Writer


class Topicer:
    """ Class to select motif-topic

    Class parameters
    ----------------
    title: str
        Title of the analysis
    params: TopicParams
        Dataclass storing number_of_topics, max_topic_size, \
        topic_size_binom_p and topic_motif_assignment_file
    motif_ids: List[str]
        Motif IDs
    reader: Reader
        File reader class to read in sequences if necessary
    writer: Writer
        instance of the writer class
    topics: Topics
        Topics with names (key) and list of motifs within them
    rng: np.random.Generator
        Random generator for group sizes and motif sampling
    """

    def __init__(
            self,
            params: TopicParams,
            motif_ids: List[str],
            reader: Reader,
            writer: Writer,
            rng: np.random.Generator) -> None:
        """Constructor for topic simulator

        Parameters
        ----------
        params: TopicParams
            Dataclass storing number_of_topics, max_topic_size, \
            topic_size_binom_p and topic_motif_assignment_file
        motif_ids: List[str]
            Motif IDs
        reader: Reader
            File reader class to read in sequences if necessary
        writer: Writer
            instance of the writer class
        rng: np.random.Generator
            Random generator for group sizes and motif sampling
        """
        self.title = writer.get_title()
        self.params = params
        self.rng = rng
        self.motif_ids = motif_ids
        self.reader = reader
        self.writer = writer
        self.topics = None

    def get_topics(self) -> Topics:
        """ Getter for topics

        Return
        ------
        topics: Topics
            Topics with names (key) and list of motifs within them
        """
        return self.topics

    def select_group_sizes_binomial(self) -> List[int]:
        """ Helper function to select sizes of groups

        Return
        ------
        adjusted_sizes: List[int]
            List of sizes of groups
        """
        # sample sizes from a binomial distribution
        group_sizes = self.rng.binomial(
            n=self.params.max_topic_size,
            p=self.params.topic_size_binom_p,
            size=self.params.number_of_topics)
        group_sizes_int = [int(group) for group in group_sizes]
        adjusted_sizes = []
        for group in group_sizes_int:
            adjusted_sizes.append(min(group, len(self.motif_ids)))
        if sum(adjusted_sizes) < len(self.motif_ids):
            print("Not all motifs can be assigned.")
            print("Consider raising topic sizes.")

        return adjusted_sizes

    def membership_assignment(
            self,
            assignees: Set[str],
            group_sizes: List[int]) -> Dict[str, List[str]]:
        """General function to assign membership of one list to another

        Parameters
        ----------
        assignees: Set[str]
            Set of instances that should be assigned to groups
        group_sizes: List[int]
            List of sizes of groups

        Return
        ------
        group_assignee_membership: Dict[str, List[str]]
            Dictionary of memberships of assignees within groups
            Key: group id, Value: list of the id of the asignees
        """
        if max(group_sizes) > len(assignees):
            message = f"Not enough elements to assign ({len(assignees)}) "
            message += f"given the group size ({group_sizes})"
            raise AssertionError(message)
        group_assignee_membership = {}
        # sample assignees
        assignees_left = set(assignees.copy())
        for group_idx, group_size in enumerate(group_sizes):
            if group_size == 0:
                continue
            topic_name = self.title + "_topic_" + str(group_idx)
            group_assignee_membership[topic_name] = []
            if len(assignees_left) < group_size:
                # if not enough assignees left, assign all to group
                # and reset them to all minus the latest selection
                group_assignee_membership[topic_name] = list(assignees_left)
                group_size = group_size - len(assignees_left)
                assignees_left = set(assignees.copy()).difference(
                    assignees_left)
            selection = self.rng.choice(
                sorted(assignees_left),
                group_size,
                replace=False)
            selected_assignees = [str(mid) for mid in selection]
            group_assignee_membership[topic_name] += selected_assignees
            group_assignee_membership[topic_name].sort()
            assignees_left = assignees_left.difference(selected_assignees)

        if len(list(group_assignee_membership.keys())) == 0:
            message = "No topics contain motifs."
            message += "Consider increasing the size of topics"
            raise AssertionError(message)

        return group_assignee_membership

    def assign_motifs_to_topics(
            self,
            group_sizes: List[int]) -> Dict[str, List[str]]:
        """Assign each motif to a topic

        Parameters
        ----------
        group_sizes: List[int]
            List of sizes of groups

        Return
        ------
        motif_topic_membership: Dict[str, List[str]]
            Dictionary of the topic IDs and the list of motifs within
        """
        motif_topic_membership = {}
        if self.params.number_of_topics == 1:
            # if only one topic, assign all motifs to that topic
            topic_name = self.title + "_topic_" + str(0)
            motif_topic_membership[topic_name] = self.motif_ids
        else:
            # otherwise, assign motifs to topics
            motif_topic_membership = self.membership_assignment(
                assignees=self.motif_ids,
                group_sizes=group_sizes)
        return motif_topic_membership

    def simulate_topics(self) -> None:
        """ Simulate topic sizes and memberships """
        if self.params.number_of_topics == 1:
            group_sizes = len(self.motif_ids)
        else:
            group_sizes = self.select_group_sizes_binomial()
        topics = self.assign_motifs_to_topics(
            group_sizes=group_sizes)
        self.topics = Topics(
            topics=topics)

    def read_topics(self) -> None:
        """ Read in topic sizes and memberships from file"""
        topics = self.reader.read_twocolumn_tsv(
            twocolumns_tsv_path=self.params.topic_motif_assignment_file)
        if len(set(list(topics.topics.values()))) < len(self.motif_ids):
            print("Not all motifs can be assigned.")
            print("Consider raising topic sizes.")

        self.topics = Topics(
            topics=topics)

    def create_topics(self) -> None:
        """ Create topic sizes and memberships """
        if self.params.topic_motif_assignment_file is not None:
            self.read_topics()
        else:
            self.simulate_topics()
            self.writer.dict_to_tsv(
                data_dict=self.topics.topics,
                filename="motif_topic_membership")
