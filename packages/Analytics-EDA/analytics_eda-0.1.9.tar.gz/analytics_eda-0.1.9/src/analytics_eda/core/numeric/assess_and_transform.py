...
def assess_and_transform(
    ...
    """
    Decide if a univariate series needs a power transform, perform it if so,
    ...
    if needs_transform:
        candidate_transforms = select_candidate_transforms(statistics)
        ...
        for name in candidate_transforms:
            ...
            try:
                ...
                if 'shapiro' in norm_t:
                    ...
                    p_val = norm_t.get('jarque_bera', {}).get('p_value', 0.0)
                # Track best
...
def select_candidate_transforms(statistics: dict) -> list[str]:
    """
