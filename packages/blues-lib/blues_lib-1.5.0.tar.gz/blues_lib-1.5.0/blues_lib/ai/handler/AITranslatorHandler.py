...
class AITranslatorHandler():
  ...
  def translate(self):
    ...
    # {str} 
    question = AIHandleQuestion.get_translate_q(self.article)
    # {AIQA}
    ai = AIQAFactory.create(self.ai_name,question)
    return self.__retry(ai,rule)
    
  def article_translate(self):
    '''
    Translate a article and offer a title
    Returns {dict} 
      - {str} title
      - {str} content
    '''
    self.retry_count = 0
    # {AIHandleRule}
    rule = AIHandleRuleFactory.create('translate_article')
    # {str} 
    question = AIHandleQuestion.get_translate_article_q(self.article,rule)
    # {AIQA}
    ai = AIQAFactory.create(self.ai_name,question)
    return self.__retry(ai,rule)
