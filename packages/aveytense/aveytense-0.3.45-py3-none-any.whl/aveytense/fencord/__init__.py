...
```ts
module aveytense.fencord
```
Since 0.3.25 this module is called `aveytense.fencord` instead of `aveytense.core`. \\
...
from aveytense import Tense as _Tense, Color as _Color
...
class Fencord:
    ...
    def __init__(self, intents: _discord.Intents = ..., presences: bool = False, members: bool = False, messageContent: bool = True):
        ...
        if not _Tense.isBoolean(messageContent):
        ...
        if _Tense.versionInfo < (0, 3, 40) and False: # < 0.3.40
    ...
    def commandInvoked(name: str, author: _tt.Union[_discord.Interaction, _discord.Message], /, parameters: _tt.Optional[dict[str, str]] = None, error: _tt.Optional[str] = None):
        ...
        if error is None:
            ...
            if isinstance(author, _discord.Message):
                t = (_Color(_Tense.fencordFormat(), 4, 0) % _Color.BOLD) + " " + (_Color("INVOCATION", 8, 99) % _Color.BOLD) + f" Invoked message command '{name.lower()}' by '{Fencord.returnName(author, name = True)}'"
                
            else:
                t = (_Color(_Tense.fencordFormat(), 4, 0) % _Color.BOLD) + " " + (_Color("INVOCATION", 8, 99) % _Color.BOLD) + f" Invoked slash command '{name.lower()}' by '{Fencord.returnName(author, name = True)}'"
            
        else:
            
            if isinstance(author, _discord.Message):
                t = (_Color(_Tense.fencordFormat(), 4, 0) % _Color.BOLD) + " " + (_Color("INVOCATION ERROR", 8, 9) % _Color.BOLD) + f" Attempt to invoke message command '{name.lower()}' by '{Fencord.returnName(author, name = True)}'"
                
            else:
                t = (_Color(_Tense.fencordFormat(), 4, 0) % _Color.BOLD) + " " + (_Color("INVOCATION ERROR", 8, 9) % _Color.BOLD) + f" Attempt to invoke slash command '{name.lower()}' by '{Fencord.returnName(author, name = True)}'"
        ...
        if error is not None:
            t += ";" + (_Color(error) % _Color.UNDERLINE)
