...
def distance(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
    ...
    return r * acos(sin(ry1) * sin(ry2) + cos(ry1) * cos(ry2) * cos(rx1 - rx2))
...
def tsp(distances: DistanceDict, depot: Key = 0, max_iterations: int = DEFAULT_MAX_ITERATIONS) -> list[Key]:  # type: ignore[misc]
    ...
    locations = {depot: m.add_depot(x=0, y=0, name=str(depot))}
    ...
    key_type = type(depot)
    return [depot, *(key_type(m.locations[i].name) for i in route.visits())]
