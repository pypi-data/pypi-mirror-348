...
def init_sentence_transformer(model_name: str = "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2", 
                             set_as_default: bool = True) -> None:
    """
    SentenceTransformer 모델 초기화 및 등록
    ...
    register_embedding_model("sentence_transformer", model, set_as_default)

def init_openai(api_key: str = None, model: str = "text-embedding-3-small", 
               set_as_default: bool = True) -> None:
    """
    OpenAI 임베딩 모델 초기화 및 등록
    ...
    Args:
        ...
        set_as_default: 기본 모델로 설정할지 여부
    """
    model = OpenAIEmbedding(api_key, model)
    register_embedding_model("openai", model, set_as_default) 