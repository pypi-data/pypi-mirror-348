...
def display_evaluation_status(evaluation_id: str):
    ...
    if response.status_code == 200:
        ...
        eval_info.add_row("Status", data.get('status') or 'Pending')
        ...
        if data.get('evaluation_results_info'):
            try:
                ...
                results_table = Table(title="Evaluation Results", box=box.ROUNDED, show_lines=True, width=200)
                results_table.add_column("Checkpoint", style="magenta")
                results_table.add_column("Dataset", style="blue")
                results_table.add_column("Status", style="green")
                results_table.add_column("Endpoint URL", style="yellow", overflow="fold")
                ...
                for ckpt_path, dataset_list in results_info.items():
                    ...
                    for dataset_info in dataset_list:
                        if len(dataset_info) >= 4:
                            ...
                            # Add status-based color
                            status_style = "green" if status.lower() == "completed" else "yellow" if status.lower() == "processing" else "red"
