...
#calculation of the mean and standard deviation which will be used to adjust the threshold using cosinus
def calculer_stats(similarities):
    ...
    """
    valeurs = [sim['similarite'] for sim in similarities]
...
def fusionner_groupes(groupes, groupe1, groupe2):
    ...
    """
    if groupe1 != groupe2 and groupe1 <= len(groupes) and groupe2 <= len(groupes):
...
def sorted_cosinus_distance(similarities, image_folder, seuil_type="strict"):
    ...
    for sim in sorted(similarities, key=lambda x: x.get('similarite',0)): 
        #Checks if the key exists and is not empty.
        if not isinstance(sim, dict):
            logging.error(f"Invalid input detected : {sim}")
            continue  
        if not all(k in sim for k in ["image_1", "visage_1", "image_2", "visage_2", "similarite"]):
            logging.error(f"Malformed dictionary : {sim}")
            continue 
