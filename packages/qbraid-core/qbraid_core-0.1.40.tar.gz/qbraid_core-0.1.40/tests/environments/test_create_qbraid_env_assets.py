...
def test_create_qbraid_env_assets(tmp_path, img_data_url):  # pylint: disable=too-many-locals
    ...
    with (
        ...
        mock_update_state_json.assert_called_once_with(
            slug_path, 0, 0, env_name=env_config.shell_prompt
        )
        ...
        mock_to_yaml.assert_called_with(
            os.path.join(slug_path, "kernels", f"python3_{slug}", "qbraid.yaml")
        )
