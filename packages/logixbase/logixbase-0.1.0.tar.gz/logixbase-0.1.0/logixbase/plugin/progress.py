...
class ProgressPlugin(BasePlugin):
    ...
    def on_init(self):
        """初始化进度插件"""
        if self.logger:
            self.logger.INFO("Initializing progress plugin")
    ...
    def on_start(self):
        ...
        if self.refresh_thread and self.refresh_thread.is_alive():
            if self.logger:
                self.logger.WARNING("Progress monitor thread already running")
            return
            ...
            for task_id, task in list(self.tasks.items()):
                ...
                if task["status"] in ["completed", "failed", "cancelled"]:
                    if "end_time" in task and time.time() - task["end_time"] > 3600:  # 1小时后清理
                        ...
                        if self.logger:
                            self.logger.DEBUG(f"Removed completed task {task_id} from tracking")
    ...
    def update_progress(self, task_id: str, current: Optional[int] = None, 
        ...
        with self.lock:
            if task_id not in self.tasks:
                if self.logger:
                    self.logger.WARNING(f"Trying to update unknown task: {task_id}")
                return None
