...
class LogMonitorPlugin(BasePlugin):
    ...
    def get_log_content(self, log_name: str, max_lines: int = 100, offset: int = 0) -> List[str]:
        ...
        try:
            ...
            if self.logger:
                self.logger.ERROR(f"Error reading log file {log_name}: {e}")
            return []
        ...
        for log_file in log_files:
            ...
            if file_age_days > keep_days:
                try:
                    ...
                    deleted += 1
                    if self.logger:
                        self.logger.INFO(f"Deleted old log file: {os.path.basename(log_file)}")
                except Exception as e:
                    if self.logger:
                        self.logger.ERROR(f"Error deleting log file {os.path.basename(log_file)}: {e}")
    ...
    def _monitor_logs_task(self):
        ...
        while self.monitor_active:
            try:
                ...
                if self.logger:
                    self.logger.ERROR(f"Error in log monitoring task: {e}")
                time.sleep(self.monitor_interval)
