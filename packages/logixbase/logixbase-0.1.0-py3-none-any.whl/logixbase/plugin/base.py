...
class BasePlugin(PluginProtocol):
    ...
    def on_exception(self, exc):
        ...
        """
        if self.logger:
            self.logger.ERROR(f"Plugin {self.name} error: {str(exc)}")
    ...
    def start(self):
        ...
        if not self.enabled:
            if self.logger:
                self.logger.WARNING(f"Plugin {self.name} is disabled, skipping start")
            return False
        ...
        try:
            ...
            self.status = "RUNNING"
            if self.logger:
                self.logger.INFO(f"Plugin {self.name} started")
            return True
        except Exception as e:
            self.status = "ERROR"
            if self.logger:
                self.logger.ERROR(f"Error starting plugin {self.name}: {str(e)}")
            return False
    ...
    def stop(self):
        ...
        try:
            ...
            self.status = "STOPPED"
            if self.logger:
                self.logger.INFO(f"Plugin {self.name} stopped")
        except Exception as e:
            self.status = "ERROR"
            if self.logger:
                self.logger.ERROR(f"Error stopping plugin {self.name}: {str(e)}")
