...
class BaseEngine(ABC):
    ...
    def on_exception(self, exc):
        ...
        for plugin_id, plugin in self.plugin_manager.get_all_plugins().items():
            try:
                ...
                self.logger.ERROR(f"Error in plugin '{plugin_id}' exception handler: {e}")
        self.status = "ERROR"
    ...
    def register_component(self, component_id, component):
        ...
        self.logger.INFO(f"Component {component_id} registered")
        return component
        ...
        if component_id:
            if component_id in self.components:
                try:
                    ...
                    self.logger.ERROR(f"Error getting status of component {component_id}: {e}")
    ...
    def _handle_exit(self, signum, frame):
        self.logger.WARNING(f"Received signal {signum}, exiting...")
        self.on_stop()
        self.on_exit()
        sys.exit(0)
    ...
    def run(self, mode="cli"):
        try:
            ...
            self.logger.ERROR("Unhandled exception during engine run")
