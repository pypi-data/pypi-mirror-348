...
def handle_streamed_response(streamed_lines):
    # Imagine streamed_lines is a generator yielding lines from the API response
    full_response = ""
    for line in streamed_lines:
        ...
        print(chunk, end="", flush=True)  # Print the incremental text
        full_response += chunk
...
def tutorial():
    print(f"""
Welcome to {Colors.BOLD}NodeRoom{Colors.END}! Let's get you started with creating your first node.
...
def get_ollama_response(prompt, model="phi", stream=True):
...
def nodes_vote_to_respond(nodes, active_nodes, user_input):
    # Simple voting system: nodes score their willingness to respond (fake scoring for demo)
    votes = {}
