...
openai_message = [
    {
        "role": "system",
        "content": [{"type": "text", "text": "你是一个擅长解读媒体文件的助手，会帮我解读里面的内容并且完整的列出来，最后对列出来的内容进行分析和总结"}]
    },
    {
        "role": "user",
        "content": []
...
def send_qwen_media_message(data, type, apikey):
    if data is not None and len(data) > 0:
        if type == 'image':
            ...
            openai_message[1]["content"].append({"type": "text", "text": "解读里面的内容并且完整的列出来，然后对列出来的内容进行分析和总结，最后再给我一个内容解读方向"})
        elif type == 'video':
            openai_message[1]["content"].append({"type": "video_url", "video_url": {"url": data}})
            openai_message[1]["content"].append({"type": "text", "text": "解读里面的内容并且完整的列出来，最后对列出来的内容进行分析和总结，最后再给我一个内容解读方向"})
    else:
        return False
    client = OpenAI(
        api_key=apikey,
...
async def video_interpretation(images: str):
    if not qwen_api_key or len(qwen_api_key) < 10:
        raise ValueError("请配置QWEN_API_KEY")
    if len(images) == 0:
        raise ValueError("图片链接为空")
    result = send_qwen_media_message(images, "video", qwen_api_key)
    return result
