...
def db_get_param(key: DbParam,
    ...
    if curr_engine:
        result = _get_param(engine=curr_engine,
                            param=key)
    return result


def db_get_params(engine: DbEngine = None) -> dict[str, Any] | None:
    """
    ...
    if curr_engine:
        data: dict[DbParam, Any] = _DB_CONN_DATA.get(curr_engine)
        if data:
            result = dict_jsonify(source=data.copy(),
                                  jsonify_keys=True,
                                  jsonify_values=True)
    return result
