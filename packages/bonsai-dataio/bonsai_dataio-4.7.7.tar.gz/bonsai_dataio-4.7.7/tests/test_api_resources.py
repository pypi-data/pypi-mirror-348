...
from dataio.schemas.bonsai_api.admin import DataResource  # Adjust import if your DataResource is elsewhere
...
class TestAPIResourceRepository(unittest.TestCase):
    ...
    def test_add_to_resource_list_api_success(self, mock_post):
        ...
        self.assertEqual(call_args[0], "https://lca.aau.dk/api/version-dataio/")  # endpoint
    ...
    def test_add_to_resource_list_api_bad_request(self, mock_post):
        ...
        resource = DataResource(name="Bad Resource", schema_name="BadSchema",task_name="test", stage="test",data_version="v0.0.0", api_endpoint="testing/")
    ...
    def test_add_to_resource_list_api_unauthorized(self, mock_post):
        ...
        resource = DataResource(name="Test Resource", schema_name="MySchema",task_name="test",stage="test",data_version="v0.0.0", api_endpoint="testing/")
    ...
    @patch.object(ResourceRepository, 'update_resource_list', return_value="UPDATED_UUID")
    ...
    def test_add_or_update_resource_list_update(
        ...
        resource = DataResource(name="ResourceToUpdate", schema_name="SomeSchema",task_name="test", stage="test",data_version="v0.0.0")
    ...
    @patch.object(ResourceRepository, 'update_resource_list', return_value="UPDATED_UUID")
    @patch.object(ResourceRepository, 'add_to_resource_list', return_value="NEW_UUID")
    def test_add_or_update_resource_list_add(
        self, mock_add, mock_update, mock_exists
    ):
        """
        ...
        resource = DataResource(name="ResourceToAdd", schema_name="SomeSchema",task_name="test", stage="test",data_version="v0.0.0")
    ...
    def test_update_resource_list_api_patch_error(self, mock_patch, mock_get):
        ...
        mock_patch_response = MagicMock()
        mock_patch_response.raise_for_status.side_effect = requests.exceptions.HTTPError("PATCH failed")
        mock_patch.return_value = mock_patch_response
        ...
        resource = DataResource(name="WillFail", schema_name="FailSchema",task_name="test", stage="test",data_version="v0.0.0")
    ...
    def test_add_to_resource_list_local_override(self, mock_to_csv):
        ...
        repo = ResourceRepository(db_path="some/path", storage_method="api", API_token="FAKE_TOKEN_VALUE")
        ...
        # 3) Create a resource
        resource = DataResource(name="LocalOverrideResource", schema_name="LocalSchema", task_name="test", data_version="v0.0.0", stage="test")
        ...
        self.assertFalse(mock_to_csv.called is False, "Expected local CSV write, but .to_csv wasn't called?")
        self.assertTrue(mock_to_csv.called, "Expected .to_csv to have been called for local mode override")
        self.assertEqual(returned_uuid, "", "Local add_to_resource_list typically returns empty string")
        # The appended row now should exist in available_resources
        self.assertEqual(len(repo.available_resources), 1)
        self.assertEqual(repo.available_resources.iloc[0]["name"], "LocalOverrideResource")
    ...
    def test_add_to_resource_list_api_override(self, mock_post):
        ...
        """
        repo = ResourceRepository(db_path="some/path", storage_method="local")  # default local
        # But we override in the call
        mock_response = MagicMock()
        mock_response.raise_for_status.return_value = None
        mock_post.return_value = mock_response

        resource = DataResource(name="APIOverride", schema_name="OverrideSchema", task_name="test", stage="test", data_version="v0.0.0", api_endpoint="testing")
    ...
    def test_get_resource_info_local_override(self, mock_get):
        ...
        """
        repo = ResourceRepository(db_path="some/path", storage_method="api", API_token="FAKE_TOKEN")
        # Mock local data
        df = pd.DataFrame([
        ...
        self.assertFalse(mock_get.called, "Did not expect an API call for local override")
    ...
    def test_list_available_resources_api_http_error(self, mock_get):
        ...
        mock_response.raise_for_status.side_effect = requests.exceptions.HTTPError("HTTP 500 Internal Server Error")
        mock_get.return_value = mock_response
        ...
        self.assertIn("Failed to retrieve list of available resources from API", str(context.exception))
    ...
    def test_get_dataframe_for_resource_api(self, mock_load_api):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            repo = ResourceRepository(db_path="some/path", storage_method="api", API_token="TEST_TOKEN",cache_dir=tmpdir)
            ...
            # 5) Assertions
            mock_load_api.assert_called_once_with(ANY,resource,tmpdir,MAX_CACHE_FILES=3)
            # Check the returned DataFrame is the same we mocked
            pd.testing.assert_frame_equal(result_df, df_mock)

    @patch("requests.Session.post")  # Patch the Session.post used in add_to_resource_list
    @patch("dataio.resources.save_to_api")  # If you also want to mock the code in save_to_api
    @patch.object(ResourceRepository, "resource_exists", return_value=False)
    def test_write_dataframe_for_resource_api_new_resource(
        ...
        repo.write_dataframe_for_resource(df, resource, overwrite=True, storage_method="api")
    ...
    def test_write_dataframe_for_resource_api_resource_exists_no_overwrite(
        ...
        df = pd.DataFrame()
        repo = ResourceRepository(db_path="some/path", storage_method="api", API_token="FAKE_TOKEN")
        resource = DataResource(name="ExistingAPIResource", 
                                schema_name="ProductionVolumes_uncertainty",
                                task_name="test",
                                stage="test",
                                code_version="v0.0.1",
        ...
        with self.assertRaises(FileExistsError):
            repo.write_dataframe_for_resource(df, resource, overwrite=False, storage_method="api")
    ...
    def test_write_dataframe_for_resource_invalid_storage_method(self):
        ...
        df = pd.DataFrame()
        repo = ResourceRepository(db_path="some/path", storage_method="api", API_token="FAKE_TOKEN")
        resource = DataResource(name="InvalidResource", 
                                schema_name="ProductionVolumes_uncertainty",
                                task_name="test",
                                stage="test",
                                code_version="v0.0.1",
    ...
    def test_get_dataframe_for_resource_api_cache(self, mock_get):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(len(csv_files), 1, "There should be exactly 1 cached CSV file.")
            ...
            self.assertEqual(len(csv_files_after), 1, "Cache should still have exactly 1 CSV file.")
    ...
    def test_cache_eviction_of_old_files(self, mock_get):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                len(csv_files_after), 3,
                "Cache should only keep at most 3 CSV files."
            )
