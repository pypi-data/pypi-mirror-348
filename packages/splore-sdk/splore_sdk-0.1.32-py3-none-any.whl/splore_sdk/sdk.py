...
class AgentSDK(BaseSDK):
    ...
    def extract(
        self, file_path: Optional[str] = None, file_stream: Optional[IO] = None, max_poll_timeout: Optional[float] = 1200
    ) -> Dict:
        """Backward compatibility method for extraction"""
        if file_path is None and file_stream is None:
            raise ValueError("One of file_path or file_stream must be provided.")
        return self.extraction.extract(file_path=file_path, file_stream=file_stream, max_poll_timeout=max_poll_timeout)
