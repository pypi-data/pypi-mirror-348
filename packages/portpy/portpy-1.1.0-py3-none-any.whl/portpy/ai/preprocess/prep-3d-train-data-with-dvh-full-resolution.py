...
def process_case(in_dir, out_dir, case):
	...
	print(ct.shape, oar.shape, dose.shape)
	num_ptv = np.sum(ptv)
	dose_copy = dose.copy()
	dose_copy *= ptv
...
# # out_dir = '../datasets/msk-echo-3d-dvh-better-cropped/train'
# # if not os.path.exists(out_dir):
# # 	os.makedirs(out_dir)
#
# case_file = '../resources/test_echo_dose.txt'
# out_dir = '../datasets/msk-echo-3d-dvh-better-cropped/test'

in_dir = r'\\pisidsmph\NadeemLab\Gourav\lung-manual-dicomextracted'

case_file = r'.\resources\train_manual_imrt_dose.txt'
# out_dir = '/nadeem_lab/Gourav/datasets/msk-manual-3d-imrt-sparse-seperate-ptv/train'
out_dir = r'\\pisidsmph\NadeemLab\Gourav\datasets\msk-manual-3d-imrt-sparse-seperate-ptv-full-resolution\train'
if not os.path.exists(out_dir):
	os.makedirs(out_dir)
