...
portConnectionHeaders = ['<th><input type="checkbox" id="selectAllPortsCheckboxes" name="selectAllPorts"></th>',
                         '<th>Src-Ports</th>',
                         '<th>Connected-To</th>',
                         '<th>Remote-Port</th>',
                         '<th>Port-Groups</th>',
                         '<th>Port-Type&emsp;<i class="fa-solid fa-arrow-down-long"></i></th>',
                         '<th>VLAN&emsp;<i class="fa-solid fa-arrow-down-long"></i></th>',
                         '<th>PG-Multi-Tenant&emsp;<i class="fa-solid fa-arrow-down-long"></i></th>',
                         '<th>Optic-Mode&emsp;<i class="fa-solid fa-arrow-down-long"></i></th>'
                        ]
    ...
    try:
        ...
        if data:
            ...
            if userRole != 'engineer':
                ...
                #vlanDropdownAddVlanId += '<option label="VLAN" selected value></option>'
            ...
            html += '<div class="row">'
            #html += '<table id="portMgmtTable" class="tableFixHead2 table-condensed"><thead><tr>'
            html += '<table id="portMgmtTable" class="tableFixHead3"><thead><tr>'
            ...
            html += '<th>Connected-To</th>'
            html += '<th>Remote-Port</th>'
            ...
            # Include additional table headers users created
            if data['additionalKeys'] != "None": 
                for additionalKey in data['additionalKeys']:
                    additionalKeysForJS.append(f'{additionalKey}-additionalKey')
                    html += f'<th>{additionalKey}</th>'
            ...
            for index, portDetails in enumerate(portMgmtSortedList):
                ...
                # For each addtional field, add the value for each port
                for keyName in data['additionalKeys']:
                    if keyName in portDetails['additionalKeyValues'].keys():
                        keyValue = portDetails['additionalKeyValues'][keyName]['value']
                        valueType  = portDetails['additionalKeyValues'][keyName]['type']
                        if 'options' in portDetails['additionalKeyValues'][keyName].keys():
                            options = portDetails['additionalKeyValues'][keyName]['options']
                        
                        if valueType == 'boolean':
                            valueValue = convertStrToBoolean(keyValue)
                        ...
                        if valueType == 'boolean':
                            ...
                            if ',' in options:
                                ...
                                for item in options.split(','):
                                    dropdown += f'<li class="mainFontSize textBlack paddingLeft20px" device={deviceName} port={portDetails["port"]} additionalKey="{keyName}" valueType="{valueType}">{item}</li>'
                            else:
                                # Single option
                                dropdown += f'<li class="mainFontSize textBlack paddingLeft20px" device={deviceName} port={portDetails["port"]} additionalKey="{keyName}" valueType="{valueType} >{options}</li>'
                                                                                              
                            dropdown += '</ul></div>'
                            html += f'<td>{dropdown}</td>'                    
                              
                        else:
                            html += f'<td class="paddingLeft5px"><a class="editingAdditionalKeys textBlack" id="editPortAddtionalKeyValue" href="#" role="button" domain="{domain}" deviceName="{deviceName}" port={portDetails["port"]} additionalKey="{keyName}" valueType="{valueType}" onclick="editPortAdditionalField(this)">{keyValue}</a></td>'
...
class ModifyPortAdditionalDropdownKeyValue(APIView):
    ...
    def post(self, request):
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            response, errorMsg , status = executeRestApiOnRemoteController('post', remoteControllerIp, restApi, params, 
                                                                           user, webPage=Vars.webpage, action='ModifyPortAdditionalDropdownKeyValue')
        else:   
            try:
                ...
                if portMgmtData:
                    for index, portMgmt in enumerate(portMgmtData['portMgmt']):
                        if port == portMgmt['port']:
                            ...
                            if portMgmt['additionalKeyValues'][key]['type'] == 'boolean':
                                value = convertStrToBoolean(value)
                         
                            portMgmtData['portMgmt'].pop(index)
                            portMgmtData['portMgmt'].insert(index, portMgmt)
                            ...
                            if portMgmtDBResult:   
                                SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='modifyPortAdditionalDropdownKeyValue', msgType='Success',
                                                          msg=f'Device:{device}  Port:{port}  Key:{key} Value:{value} ', forDetailLogs='')
                            else:
                                SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='modifyPortAdditionalDropdownKeyValue', msgType='Failed',
                                                          msg=f'Device:{device}  Port:{port}  Key:{key} Value:{value}', forDetailLogs='')                
                
            except Exception as errMsg:
                status = "failed"
                errorMsg = str(errMsg)
                SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='ModifyPortAdditionalDropdownKeyValue', msgType='Error',
                                          msg=errorMsg, forDetailLogs=traceback.format_exc(None, errMsg)) 
...
class SelectPortGroupToAddPorts(APIView):
    #@verifyUserRole(webPage=Vars.webpage, action='addPortsToPortGroup', exclude=['engineer'])
    def post(self, request):
        """
                ...
                def isPortValidForPortGroup():
                    ...
                    for port in portsToBeAdded:
                        for eachPort in portMgmtData['portMgmt']:
                            ...
                            #  'multiTenant': False, 'opticMode': None, 'vlanTagId': None, 'autoNeg': True, 'speed': '1G', 'reserved': 'available'}                             
...
class RemovePortsFromPortGroup(APIView):
    #@verifyUserRole(webPage=Vars.webpage, action='removePortsFromPortGroup', exclude=['engineer'])
    def post(self, request):
        """
...
class ConnectToLinkDevice(APIView):
    #@verifyUserRole(webPage=Vars.webpage, action='ConnectToLinkDevice', exclude=['engineer'])
    def post(self, request):
        """
...
class SetOpticMode(APIView):
    #@verifyUserRole(webPage=Vars.webpage, action='SetOpticMode', exclude=['engineer'])
    def post(self, request):
        """
...
class SetVlanId(APIView):
    #@verifyUserRole(webPage=Vars.webpage, action='SetVlanId', exclude=['engineer'])
    def post(self, request):
        """
...
class PortConnectionAddKey(APIView):
    #@verifyUserRole(webPage=Vars.webpage, action='PortConnectionAddKey', exclude=['engineer'])
    def post(self, request):
        """
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            try:
                portMgmtData = DB.name.getOneDocument(collectionName=Vars.labInventory, fields={'domain': domain, 'name': device})
                if portMgmtData:
                    # Verify if the field name exists
                    if portMgmtData['additionalKeys'] != 'None':
                        for existingField in portMgmtData['additionalKeys']:
                        ...
                        portMgmtData['additionalKeys'].append(keyName)
                    
                    else:
                        portMgmtData['additionalKeys'] = [keyName]
                    
                    # Add the key/value to all the ports
                    for index, portMgmt in enumerate(portMgmtData['portMgmt']):  
                        if valueType == 'string':
                            portMgmt['additionalKeyValues'].update({keyName: {'type': valueType, 'value': str(keyValue)}})
                            
                        elif valueType == 'integer':
                            portMgmt['additionalKeyValues'].update({keyName: {'type': valueType, 'value': int(keyValue)}})
                            
                        # elif valueType == 'list':
                        #     portMgmt['additionalKeyValues'].update({keyName: {'type': valueType, 'value': keyValue}})
                            ...
                            portMgmt['additionalKeyValues'].update({keyName: {'type': valueType, 'value': defaultValue, 'options': keyValue}})
                                                        
                        elif valueType in ['boolean: true', 'boolean: false']:
                            booleanValue = valueType.split(' ')[-1]
                            value = convertStrToBoolean(booleanValue)
                            portMgmt['additionalKeyValues'].update({keyName: {'type': 'boolean', 'value': value}})
                    ...
                    result = DB.name.updateDocument(collectionName=Vars.labInventory,
                                                    queryFields={'domain': domain,
                                                                 'name':   device},
                                                    updateFields={'additionalKeys': portMgmtData['additionalKeys'],
                                                                  'portMgmt': portMgmtData['portMgmt']},
                                                    appendToList=False)
    ...
    def post(self, request):
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            try:
                ...
                if portMgmtData:
                    if portMgmtData['additionalKeys'] != "None":
                        for additionalKey in portMgmtData['additionalKeys']:
                            tableRowsHtml += f'<tr>'
                            tableRowsHtml += f'<td><input type="checkbox" name="removeKeyCheckboxes" device="{device}" domain="{domain}" key="{additionalKey}"></td>'
                ...
                errorMsg = str(errMsg)
                tableRowsHtml = ''
                SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='PortConnectionGetRemoveKeysTable', msgType='Error',
                                          msg=errorMsg,
                                          forDetailLogs=traceback.format_exc(None, errMsg)) 
...
class PortConnectionRemoveKeys(APIView):
    #@verifyUserRole(webPage=Vars.webpage, action='PortConnectionRemoveKeys', exclude=['engineer'])
    def post(self, request):
        """
        Remove fields
        """
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            response, errorMsg , status = executeRestApiOnRemoteController('post', remoteControllerIp, restApi, params, 
                                                                           user, webPage=Vars.webpage, action='PortConnectionGetRemoveKeys')
        else:        
            try:
                portMgmtData = DB.name.getOneDocument(collectionName=Vars.labInventory, fields={'domain': domain, 'name': device})
                ...
                for key in keys:
                    key = key[2]
                    if portMgmtData:            
                        if key in portMgmtData['additionalKeys'] != "None":
                            index = portMgmtData['additionalKeys'].index(key)
                            portMgmtData['additionalKeys'].pop(index)
                ...
                status = "failed"
                errorMsg = str(errMsg)
                SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='PortConnectionGetRemoveKeys', msgType='Error',
