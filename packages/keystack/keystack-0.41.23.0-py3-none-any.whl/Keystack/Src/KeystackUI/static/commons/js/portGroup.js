...
const getPortGroupTable = async () => {
    ...
    if (data.status == 'failed') {
        ...
        /*
        // The Port's column is a table of all the ports in the Port-Group
        for (var row=1; row < portGroupTable.rows.length; row++) {
            portGroupTable.rows[row].cells[portsCellIndex].addEventListener('click', event => {
                getPortConfigurationTable(event);
            })

            portGroupTable.rows[row].cells[reservationMgmtCellIndex].addEventListener('click', event => {
                getActiveUserMgmtTable(event);
                getWaitListUserMgmtTable(event);
            })
        }
        */
