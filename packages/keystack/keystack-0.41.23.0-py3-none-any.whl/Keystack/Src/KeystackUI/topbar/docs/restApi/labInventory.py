...
from db import DB
from commonLib import logDebugMsg, netcat, getSortedPortList2
...
from keystackUtilities import readYaml, removeFile, makeFolder, execSubprocess, execSubprocessInShellMode, convertStrToBoolean, convertNoneStringToNoneType, getDictIndexFromList
...
def addDeviceHelper(domain, user, deviceKeyValues):
    try:
        ...
        deviceKeyValues.update(additionalPortKeys)
        deviceKeyValues.update({'additionalKeys': "None"})
...
def addToKeystackMisc(key, value):
    ...
    """
    if value != 'None':
        data = DB.name.getOneDocument(collectionName=Vars.keystackMisc, fields={'name': key}, includeFields=None)
                ...
                if data and count > 0:
                    ...
                    for index, device in enumerate(data[indexStart:indexEnd]):
                        ...
                        if device['notes'] == 'None':
                            ...
                            html += f'<td class="paddingLeft5px"><a class="textBlack" role="button" field="notes" currentNotes="{device["notes"]}" deviceName="{device["name"]}" href="#" onclick="editDeviceField(this)">View/Edit</a></td>'
                        ...
                        html += f"""<div class="flexRowInlineBlockLeft marginBottom10px">
                                        ...
                                        <div id={addRemoveKeysId}><a href="#" domain="{domain}" device="{device["name"]}" class="textBlack noTextDecoration paddingTop10px" data-bs-toggle="modal" data-bs-target="#addRemoveKeysModal">Add/Remove Keys</a></div>&emsp;&emsp;
...
class AddDeviceLocation(APIView):
    #@verifyUserRole(webPage=Vars.webpage, action='AddDeviceLocation', exclude=['engineer'])
    def post(self, request):
        """ 
    ...
    def post(self, request):
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            try:
                ...
                deviceKeyValues = addDeviceHelper(domain, user, deviceKeyValues)
                if deviceKeyValues == "portPrefixInconsistency":
                ...
                if deviceKeyValues == "portRangeError":            
                    SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='AddDevice', msgType='Error',
                ...
                elif deviceKeyValues is not None:
                    response = DB.name.insertOne(collectionName=Vars.webpage, data=deviceKeyValues)
                    if response.acknowledged:
                        SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='AddDevice', msgType='Success',
                                                  msg=deviceKeyValues['name'],
                                                  forDetailLogs='')
                    else:
                        status = "failed"
                        errorMsg = f'Failed to add a new port device profile: {deviceKeyValues["name"]} in MongoDB.'
                        SystemLogsAssistant().log(user=user, webPage=Vars.webpage, action='AddDevice', msgType='Failed',
                                                msg=errorMsg, forDetailLogs=traceback.format_exc(None, errMsg))
                    ...
                    for device in data:
                        ...
                        for pg in deviceAttachedPortGroups:    
                            ...
                            if data:
                                ...
                                # If there is no more devices in the domain, remove the domain
                                # if len(data['ports'].keys()) == 0:
                                #     data['ports'].pop(domain, None)
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            try:
                ...
                data = DB.name.getDocuments(collectionName=Vars.webpage,
                                            fields={'domain': domain},
                                            includeFields={'_id':0, 'additionalKeys':0, 'portMgmt':0},
                                            sortBy=[('name', 1)],
                                            limit=None)
                countX = deepcopy(data)
                count = len(list(countX))
                ...
                with open(exportCsvFileFullPath, 'w') as outfile:
                    fields = ['domain', 'name', 'deviceType', 'vendor', 'model', 'serialNumber',
                              'location', 'ipAddress', 'ipPort', 'loginName', 'password',
                              'connectionProtocol', 'ports', 'notes']
...
class ImportCSV(APIView):
    #@verifyUserRole(webPage=Vars.webpage, action='ImportCSV', exclude=['engineer'])
    def post(self, request):
        """
        ...
        if remoteControllerIp and remoteControllerIp != mainControllerIp:
            ...
            try:
                ...
                with open(tempFileFullPath, mode ='r') as fileObj:
                    csvObj = csv.DictReader(fileObj, fieldnames=getDeviceFieldsList())
                    ...
                    for index, csvRow in enumerate(csvObj):
                        ...
                        totalHeaderColumns = len(getDeviceFieldsList())
...
class GetDevicePassword(APIView):
    #@verifyUserRole(webPage=Vars.webpage, action='GetDevicePassword', exclude=['engineer'])
    def post(self, request):
        """
