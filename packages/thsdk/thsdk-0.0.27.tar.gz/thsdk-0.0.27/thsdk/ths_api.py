...
class Response:
    ...
    def __repr__(self) -> str:
        return f"Response(err_code={self.err_code}, err_message={self.err_message}, payload={self.payload})"
...
class THS:
    ...
    def history_minute_time_data(self, code: str, date: str, fields: Optional[List[str]] = None) -> Response:
        ...
        if fields:
            response.payload.data = [entry for entry in response.payload.data if all(field in entry for field in fields)]
        return response
                    ...
                    if isinstance(var, str):
                        ...
                        if len(var) == 8:
                            ...
                            dt = dt.replace(hour=9 if name == "start" else 15,
                                            minute=15 if name == "start" else 30, tzinfo=tz)
                            locals()[name] = _time2int(dt)
