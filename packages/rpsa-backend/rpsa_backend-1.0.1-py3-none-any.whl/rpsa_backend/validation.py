...
2. Instantiate with a cached model object (TorchScript, ONNX, TF).
...
def validate_strategy_script(
    *,
    ...
    2. Instantiates the `strategy` with a pre-loaded model object (if given).
    ...
    try:
        ...
        # instantiate: prefer object-first, but fall back to legacy path
        try:
        ...
        # -- 3) smoke-test via a quick game ---------------------------
        test_game = Game(strat_inst, RandomStrategy())
        t0 = time.perf_counter()
        wins, losses, ties = test_game.play_rounds(200)
        ...
        if elapsed > MAX_STRATEGY_RUNTIME:
            raise StrategyValidationError(
                f"Strategy too slow ({elapsed:.3f}s for 200 rounds); "
