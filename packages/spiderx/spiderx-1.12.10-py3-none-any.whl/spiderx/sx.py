    ...
    try:
        if ip_port:
            ...
            if 开启:
                ...
                lst = re.search('http=(.*?):(\d+)',proxy_server[0])
                proxy_ip=lst.group(1)
                proxy_port=lst.group(2)
                proxy_address=f'{proxy_ip}:{proxy_port}'
                return {'http': f'http://{proxy_address}', 'https': f'http://{proxy_address}'} if lst else None
            else:
                return None
    except:
        return None
