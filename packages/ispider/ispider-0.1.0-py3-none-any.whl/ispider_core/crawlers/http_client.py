...
async def fetch(reqA, client, mod):
    metadata = {}
...
async def async_main(reqsA, mod=0, headers={}):
    """
    settings: required WEBSITES_MAX_DEPTH
    reqsA: -- to be updated
        url = reqA[0]
        request_discriminator = reqA[1]
        dom_tld = reqA[2]
    ...
    async with httpx.AsyncClient(timeout=timeout, limits=limits, follow_redirects=True, headers=headers) as client:
        try:
            tasks = [asyncio.create_task(fetch(reqA, client, mod)) for reqA in reqsA]
