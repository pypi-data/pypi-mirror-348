...
from ispider_core.crawlers import stage1_crawl_helpers
...
def call_and_manage_resps(
    ...
    for resp in resps:
        ...
        if status_code in settings.CODES_TO_RETRY and retries <= settings.MAXIMUM_RETRIES:
            ...
            qout.put((url, rd, dom_tld, retries+1, depth))
            continue
        ...
        # ALL - LEVEL 1
        try:
            stage1_crawl_helpers.generic_crawl(
                resp, lock, exclusion_list, fetch_controller, qout)
        except Exception as e:
