...
def invalid_h5_2d(tmp_path):
    """
    Create a temporary HDF5 file with a 2D dataset to trigger ValueError.
    """
...
def test_tiff_files_invalid_dimensions(tmp_path: PathLike, invalid_h5_2d):
    """
    Ensure TiffFiles.run raises ValueError for non-3D datasets.
    """
    output = tmp_path / "PROCESSED_DATA" / "A" / "B" / "C"
    task = TiffFiles(
        inputs={
