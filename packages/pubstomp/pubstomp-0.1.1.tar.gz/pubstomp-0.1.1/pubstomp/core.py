...
async def main():
    ...
    async with aiohttp.ClientSession(cookies=cookies, headers={'User-Agent': 'Mozilla/5.0 (compatible; SimpleSpider/1.0)'}) as session:
        ...
        if ip and not args.nonmap:
                ...
                logger.info(f"Running nmap --script vuln -A {ip}")
                print(f"{Fore.GREEN}▶️ sudo nmap --script vuln -A {ip} → {nmap_file}{Style.RESET_ALL}")
                try:
                    await asyncio.to_thread(lambda: subprocess.run(
                        ["sudo", "nmap", "--script", "vuln", "-A", ip],
        ...
        if args.wordlist:
            ...
            if robots_paths:
                choice = input(f"{Fore.YELLOW}Include {len(robots_paths)} robots.txt paths in fuzzing? [Y/n] {Style.RESET_ALL}").strip().lower()
                if choice in ("", "y", "yes"):
