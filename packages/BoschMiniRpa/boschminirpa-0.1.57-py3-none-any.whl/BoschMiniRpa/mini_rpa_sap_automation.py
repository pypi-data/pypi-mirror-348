...
class MiniRpaSapAutomation:
    ...
    def expand_report_fully(self, shortcut_list):
        ...
        while True:
            ...
            try:
                self.web_rpa.browser.find_elements(By.CSS_SELECTOR, EXPAND_REPORT_SECTION_ICON_CSS_SELECTOR)
            except NoSuchElementException:
                break
            else:
                sleep(2)

    def select_y01f_report_left_menu_item(self, report_name, no_data_message):
        """ This function is used to select report left menu item.

        Args:
            report_name(str): This is the name of report left menu item
            no_data_message(str): This is the message of no data
        """
        sleep(1)
        self.web_rpa.wait_invisibility_of_loading_window()
        sleep(1)
        ...
        for report_menu_item in report_menu_items:
            if report_name == report_menu_item.text.strip():
                ...
                try:
                    message_element = self.web_rpa.browser.find_element(By.CSS_SELECTOR, "span.lsMessageBar__text")
                except NoSuchElementException:
                    pass
                else:
                    if message_element.text.strip() == no_data_message:
                        self.skip_download_process = True
