...
def create_app(df: DataFrame, files_list: list[FileInfo] | None = None) -> FastAPI:
                    ...
                    for key in label_keys:
                        ...
                        for value in label_values:
                            if str(label_value).lower() == str(value).lower():
                                print(
                                    f"✅ MATCH FOUND! Key={key}, Value={value}, Label={label_dict}"
                                )
                                return True
    ...
    async def post_data(request: Request, filter_request: FilterRequest):
        ...
        try:
            ...
            try:
                ...
                # Filter input and output fields if specified
                if filter_request.input_fields or filter_request.output_fields:
                    for item in processed_items:
                        # Filter input fields
                        if (
    ...
    async def process_multiple_files(
        ...
        try:
            ...
            # Apply input/output field filtering
            if filter_request.input_fields or filter_request.output_fields:
                for item in processed_items:
                    # Filter input fields
                    if (
