...
class CommandWrapper:
    ...
    def __init__(self, obj, name="", parent=None, allow_eval=False, include_subparser_name=False, 
                 validate_options=False, collapsed=False):
        self._obj = obj
        self._name = name
        self._parent = parent
        ...
        # Store subparser objects for collapsed mode
        self._subparser_objects = self._get_subparser_objects(obj) if collapsed else {}
    ...
    def _get_commands_for_obj(self, obj: Any) -> dict:
        ...
        # If collapsed mode is enabled, include commands from subparsers
        if hasattr(self, '_collapsed') and self._collapsed:
            subparser_commands = self._get_collapsed_commands(obj)
    ...
    def _get_collapsed_commands(self, obj: Any) -> dict:
        """Get all commands from subparsers when in collapsed mode."""
        collapsed_commands = {}
                ...
                for subparser_key, subparser_obj in subparser_items.items():
                    ...
                    for name, member in inspect.getmembers(subparser_obj):
                        if hasattr(member, '_is_command'):
                            ...
                            collapsed_commands[name] = member
    ...
    def _get_subparser_objects(self, obj: Any) -> dict:
        """Get all subparser objects for collapsed mode."""
        subparser_objects = {}
    ...
    def _find_target(self, path: str) -> tuple[Any, str, Any]:
        ...
        if self._collapsed and current_obj == self._obj:
    ...
    def getCompletionTree(self, base_path: str = "") -> Dict:
        ...
        for name, method in commands.items():
            ...
            if hasattr(method, '_completions'):
                # For collapsed mode, we need to use the correct target object
                target_obj = self._subparser_objects.get(name, current_obj) if self._collapsed else current_obj
        ...
        # Add subparsers to tree (skip in collapsed mode since commands are already flattened)
    ...
    def parseCommand(self, cmd_str: str) -> Any:
        ...
        # Handle collapsed mode for simple commands
        if self._collapsed and '.' not in cmd_path and cmd_path in self._subparser_objects:
...
def commandParser(cls=None, /, *, allow_eval: bool = False, subparsers: Optional[List[str]] = None, 
                  include_subparser_name: bool = False, collapsed: bool = False):
    ...
    def wrap(cls):
        ...
        cls._collapsed = collapsed  # Store the collapsed setting
