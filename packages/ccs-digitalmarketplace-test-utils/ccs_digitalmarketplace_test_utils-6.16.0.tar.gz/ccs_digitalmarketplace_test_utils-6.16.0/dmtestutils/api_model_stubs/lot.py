...
def cloud_lots(has_tac=False):
    if has_tac:
        return [
            ...
            LotStub(
                lot_id=17, slug="isaas", name="Infrastructure Software as a Service (iSaaS)", number="2a", has_tac=True
            ).response(),
            LotStub(lot_id=1, slug="saas", name="Software as a Service (SaaS)", number="2b", has_tac=True).response(),
            LotStub(lot_id=11, slug="cloud-support", name="Cloud support", number="3", has_tac=True).response(),
        ]
