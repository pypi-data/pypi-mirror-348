...
def call_chatgpt(prompt: str, n: int = 1, temperature: float = 1.0) -> Tuple[Dict, Dict]:
    ...
    client = OpenAI(api_key='sk-proj-kekyzbZhkOphaf5cO2tET3BlbkFJOarc1bjU7CZzLxUi6R5o')
...
def find_segments(sentence, k):
    ...
    result = []
    s = split_and_concatenate(sentence)
    for i, candidate in enumerate(s):
        if i == len(s) - 1: # last one, always assume good sentence
        ...
        if 'yes' in responses[0].lower():
            result.append(candidate)
    # print(result)
    print(extract_new_phrases(result))
