from setuptools import setup, find_packages

setup(
    name="grammatical-phrase-splitter",
    version="0.1.0",
    packages=find_packages(include=['grammatical_phrase_splitter', 'grammatical_phrase_splitter.*']),
    install_requires=[
        "openai",       
        "spacy",        
        "promptengine", 
    ],
    entry_points={
        'console_scripts': [
            'split-phrase=grammatical_phrase_splitter.grammatical_phrase_splitter:find_segments', 
        ],
    },
    include_package_data=True,
    description="A package to split sentences into grammatical segments using OpenAI API",
    author="Ziwei Gu",
    author_email="ziweigu@g.harvard.edu",
    url="https://github.com/ZiweiGu/GP-TSM", 
    classifiers=[
        'Programming Language :: Python :: 3',
        'License :: OSI Approved :: MIT License',
        'Operating System :: OS Independent',
    ],
)
