...
class JointGravComp:
    def __init__(self,joint_controller=LeftRightArmController([i for i in range(7)],[i for i in range(7)]),urdf_path="models/urdf/lus1_fix_leg_gripper.urdf"):
        ...
        self.force_controller=JntImpedance(urdf_path=urdf_path,b=0.5,k=0.1)
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                print(f"当下力矩：{torque}")
                control_word_par=[{"tor":torque[i],"res1":0,"res2":0.5} for i in range(self.arm_joint_controller.joint_num)]
                with self.lock:
                    self.arm_joint_controller.joint_ctrl(tar_pos,[3]*self.arm_joint_controller.joint_num,control_word_par)
                self.arm_joint_controller.rate_ctl.sleep()
        except:
                self.arm_joint_controller.reset_200()