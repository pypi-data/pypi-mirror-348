...
async def search_google_jobs_serper(
    ...
    async with aiohttp.ClientSession() as session:
        while len(collected) < number_of_results:
            ...
            jobs = data.get("jobs") or data.get("job_results") or data.get("jobs_results") or []
            if not jobs:
                break
    ...
    serialised = [json.dumps(_normalise_job_result(j)) for j in collected[:number_of_results]]
