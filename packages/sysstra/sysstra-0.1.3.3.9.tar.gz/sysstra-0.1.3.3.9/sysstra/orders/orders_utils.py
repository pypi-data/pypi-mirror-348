...
def check_existing_order(strike_price, option_type, exit_type, orders_list, entry_time=None):
    """ Function to check existing order on defined symbol """
    try:
        for order in orders_list:
            if not entry_time:
                if order["strike_price"] == strike_price and order["option_type"] == option_type and order["exit_type"] == exit_type:
                    return True
            else:
                try:
                ...
                if order["strike_price"] == strike_price and order["option_type"] == option_type and order["exit_type"] == exit_type and ot_time >= et_time:
                    return True
        return False
    except Exception as e:
