                        ...
                        doc = self.contents_collection.find_one_and_update(
                            {
                                ...
                                "base.value.status.value.currentStatus.value": {"$in": [None, ""]} # 값이 null이거나 빈 문자열("")인 경우에만 DeployStatus.NOTDEPLOYED.value로 업데이트
                            }, 
                            {"$set": {"base.value.status.value.currentStatus.value": DeployStatus.NOTDEPLOYED.value}},
                            return_document=ReturnDocument.AFTER
                        )
                        meta = {
