                    ...
                    # 승인 처리 중인 경우
                        ...
                        doc = self.contents_collection.find_one_and_update(
                            ...
                            [
                                {
                                    "$set": {
                                        "base.value.status.value.currentStatus.value": {
                                            "$cond": {
                                                "if": {
                                                    "$or": [
                                                        {"$eq": ["$base.value.status.value.currentStatus.value", None]},
                                                        {"$eq": ["$base.value.status.value.currentStatus.value", ""]}
                                                    ]
                                                },
                                                "then": DeployStatus.NOTDEPLOYED.value, # 값이 null이거나 빈 문자열("")인 경우에만 DeployStatus.NOTDEPLOYED.value로 업데이트
                                                "else": DeployStatus.DEPLOYED.value # 아니면 DeployStatus.DEPLOYED.value로 업데이트
                                            }
                                        }
                                    }
                                }
