...
static PyObject*
type_adapter_vector_call(PyObject* callable,
                         PyObject* const* args,
                         size_t nargsf,
                         PyObject* kwn)
{
...
TypeAdapter_Create(PyObject* cls,
                   ...
                   Inspector inspector)
{
    if (ob_str == NULL && ob_repr == NULL) {
        PyErr_SetString(PyExc_ValueError,
                        "It is not possible to convert"
                        " the validator to a string");
        return NULL;
    }
    TypeAdapter* validator;
    validator = (TypeAdapter*)TypeAdapterType.tp_alloc(&TypeAdapterType, 0);
    if (ob_repr == NULL) {
        ob_repr = TypeAdapter_Base_Repr;
    }
    validator->conv = conv;
    validator->ob_repr = ob_repr;
    validator->cls = Py_NewRef(cls);
    validator->inspector = inspector;
    validator->args = Py_XNewRef(args);
    validator->ob_str = Py_XNewRef(ob_str);
    return validator;
}
...
PyObject*
Not_Converter(TypeAdapter* self, ValidateContext* ctx, PyObject* val)
{
    return NULL;
}
...
int
Inspector_Any(TypeAdapter* self, PyObject* val)
{
    return 1;
}

int
Inspector_No(TypeAdapter* self, PyObject* val)
{
    return 0;
}
...
_parse_hint(PyObject* hint, PyObject* tp)
{
    if (PyTyping_Is_TypedDict(hint)) {
        ...
        return TypeAdapter_Create_Primitive(hint, tp);
...
ParseHint(PyObject* hint, PyObject* meta)
{
    if (DateTime_Is_TimeType((PyTypeObject*)hint)) {
        ...
        return TypeAdapter_Create_Primitive(hint, meta);
...
validator_setup(void)
{
    ...
    none_type_adapter =
      TypeAdapter_Create_Primitive((PyObject*)PyNone_Type, NULL);
