...
class MCPWrapper:
    ...
    def _get_http_base_url(self, url):
        """
        Normalize HTTP URLs to proper format for MCP SDK.
        The SDK may expect different formats for different versions.
        """
        parsed = urllib.parse.urlparse(url)
        
        # Check if there's a path component - the MCP HTTP client may have different behaviors
        # depending on SDK version. Some versions ignore path components entirely.
        if parsed.path and parsed.path != '/':
            print(f"Intelli MCPWrapper: URL contains path component: {parsed.path}")
            print(f"Note: Some MCP SDK versions may ignore path components.")
            
            # For most consistent behavior, return just the base URL without path
            # The SDK will handle endpoint construction
            ...
            print(f"Intelli MCPWrapper: Using base URL: {base_url}")
            print(f"If connection fails, try configuring server to mount at root path")
            return base_url
    ...
    def execute_tool(self, name, arguments):
        ...
        try:
            ...
            # Convert arg_* parameters to regular parameters
            converted_args = {}
            for k, v in filtered_args.items():
                if k.startswith('arg_'):
                    # Strip the arg_ prefix
                    converted_args[k[4:]] = v
                    print(f"Converting parameter '{k}' to '{k[4:]}', value: {v}")
                else:
                    converted_args[k] = v
