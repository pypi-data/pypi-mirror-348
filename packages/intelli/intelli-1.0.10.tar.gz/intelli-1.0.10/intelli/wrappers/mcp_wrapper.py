...
class MCPWrapper:
    ...
    def _get_http_base_url(self, url):
        ...
        The MCP HTTP client expects only the base URL (server:port).
        """
        parsed = urllib.parse.urlparse(url)
        
        # Ensure we only use the scheme and netloc (server:port)
        # This is most compatible with all MCP SDK versions
        ...
        print(f"Intelli MCPWrapper: Using base URL: {base_url} for HTTP connection")
        return base_url
