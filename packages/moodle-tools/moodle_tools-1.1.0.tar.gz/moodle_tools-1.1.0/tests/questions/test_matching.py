...
class TestMatching:
    def test_yml_parsing_strict(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        sys.argv = ["make-questions", "-i", "examples/matching.yaml"]
    ...
    def test_yml_parsing_non_strict(self, capsys: pytest.CaptureFixture[str]) -> None:
        # Simulate command-line arguments
        sys.argv = ["make-questions", "-i", "examples/matching.yaml", "-s"]
        ...
        assert "<text>Simple matching question</text>" in captured.out
        assert captured.err == ""
