...
class TestDescription:
    def test_yml_parsing_non_strict(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        sys.argv = ["make-questions", "-i", "examples/description.yaml", "-s"]
        ...
        assert '<question type="description">' in captured.out
        assert "<text>a description</text>" in captured.out
        assert captured.err == ""
    ...
    def test_e2e_cli_make_question(self, tmp_path: Path) -> None:
        ...
        with (test_resources_dir / "description.xml").open(encoding="utf-8") as f:
