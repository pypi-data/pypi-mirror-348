...
class TestDragDropMissingWords:
    def test_yml_parsing_strict(self, capsys: pytest.CaptureFixture[str]) -> None:
        # Simulate command-line arguments
        sys.argv = ["make-questions", "-i", "examples/dragdrop_missing-words.yaml"]
        ...
        assert "type: dragdrop_missing_words" in captured.err

    def test_yml_parsing_non_strict(self, capsys: pytest.CaptureFixture[str]) -> None:
        # Simulate command-line arguments
        sys.argv = ["make-questions", "-i", "examples/dragdrop_missing-words.yaml", "-s"]
        ...
        # Assert the output is as expected
        assert '<question type="ddwtos">' in captured.out
        assert "<text>Simple Drag-Drop missing words question</text>" in captured.out
        assert captured.err == ""
    ...
    def test_e2e_cli_make_question(self, tmp_path: Path) -> None:
        ...
        with (test_resources_dir / "dragdrop_missing-wordsRef.xml").open(encoding="utf-8") as f:
        ...
        sys.argv = [
            "make-questions",
            "-i",
            "examples/dragdrop_missing-words.yaml",
            "-o",
