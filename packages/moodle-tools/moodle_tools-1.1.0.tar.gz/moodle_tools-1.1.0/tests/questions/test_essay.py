...
class TestEssay:
    def test_yml_parsing_strict(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        sys.argv = ["make-questions", "-i", "examples/essay.yaml"]
        ...
        assert '<question type="essay">' in captured.out
    ...
    def test_yml_parsing_non_strict(self, capsys: pytest.CaptureFixture[str]) -> None:
        # Simulate command-line arguments
        sys.argv = ["make-questions", "-i", "examples/essay.yaml", "-s"]
        ...
        # Assert the output is as expected
        assert '<question type="essay">' in captured.out
        assert "<text>Essay question</text>" in captured.out
        assert captured.err == ""
    ...
    def test_e2e_cli_make_question(self, tmp_path: Path) -> None:
        ...
        with (test_resources_dir / "essayRef.xml").open(encoding="utf-8") as f:
