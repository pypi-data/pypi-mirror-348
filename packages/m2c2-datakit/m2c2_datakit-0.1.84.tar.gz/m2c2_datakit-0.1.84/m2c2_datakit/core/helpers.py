...
def export_dataframe(df, file_name, format=".csv", table_name="my_table", **kwargs):
    ...
    try:
        ...
        def export_csv(): return df.to_csv(file_name_with_extension, index=False, **kwargs)
        def export_json(): return df.to_json(file_name_with_extension, orient="records", **kwargs)
        def export_xlsx(): return df.to_excel(file_name_with_extension, index=False, **kwargs)
        def export_parquet(): return df.to_parquet(file_name_with_extension, index=False, **kwargs)
        def export_html(): return df.to_html(file_name_with_extension, index=False, **kwargs)
        def export_pkl(): return df.to_pickle(file_name_with_extension, **kwargs)
        def export_txt(): return df.to_csv(file_name_with_extension, index=False, sep="\t", **kwargs)

        def export_rdata():
            try:
                import pyreadr
                return pyreadr.write_rdata(file_name_with_extension, df, df_name=os.path.basename(file_name))
            except ImportError:
                raise ImportError("`pyreadr` is required for exporting to .rdata. Please install it.")
        ...
        def export_rds():
            try:
                ...
                raise ImportError("`pyreadr` is required for exporting to .rds. Please install it.")
        ...
        exporters[format]()
        print(f"[EXPORT] DataFrame successfully exported to: {file_name_with_extension}")
