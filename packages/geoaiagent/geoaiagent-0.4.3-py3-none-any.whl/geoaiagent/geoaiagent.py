...
#     save_uri: str = Tool(name="save_uri", description="要保存的文档路径，请使用uri形式输入，且仅支持file://协议，如file:///D:/test.docx",inputSchema={"type": "string"}),
#     content: str = Tool(name="content", description="要保存的文档内容",inputSchema={"type": "string"}),
# ) -> str:
#     """
#     将输入的数据生成为word文档，并且保存在本地
#     如果你先前没有使用read_grain_data等其它工具获得准确的信息，请不要有限使用本工具。
#     输入的content为你报告的完整内容，包括标题、摘要、正文、参考文献、附录等，请使用符合.docx格式的文本内容，。
#     输入的save_uri为你要保存的文档路径，请使用uri形式输入，且仅支持file://协议，如file:///D:/test.docx
...
#     train_uri: str = Tool(name="train_uri", description="训练集路径，请使用uri形式输入，且仅支持file://协议，如file:///D:/train.csv",inputSchema={"type": "string"}),
