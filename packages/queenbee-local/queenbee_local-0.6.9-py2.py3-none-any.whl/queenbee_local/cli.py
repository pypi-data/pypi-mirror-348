...
@click.option(
    '-c', '--container', help='A flag for running the commands inside the container.',
...
@click.pass_context
def run_recipe(
    ctx, recipe, project_folder, inputs, workers, env, name, container, debug, force
        ):
    ...
    if container:
        input_values['__run_inside_container__'] = True
