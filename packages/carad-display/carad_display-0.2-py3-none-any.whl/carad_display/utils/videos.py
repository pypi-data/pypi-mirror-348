...
from utils.json import read_data, write_data
from utils.gps import is_location_in_poly

FOLDER_PATH = '/tmp/carad_videos/'
WINDOW_NAME = "Fullscreen Video"

def get_availible_videos(cur_date: date):
	response = requests.get('http://carad.tech/videos')
...
def get_videos(location: list[float, float]) -> list[str]:
	...
	if geopolygons['0']:
		...
		return [get_path_to_default_video(), False]
