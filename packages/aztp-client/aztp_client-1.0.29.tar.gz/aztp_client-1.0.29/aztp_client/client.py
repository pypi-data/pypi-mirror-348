...
class Aztp:
    ...
    async def link_identities(
        ...
        if not source_identity.startswith("aztp://"):
            raise ValueError("source_identity must be a full AZTP ID starting with 'aztp://'")
        if not target_identity.startswith("aztp://"):
            raise ValueError("target_identity must be a full AZTP ID starting with 'aztp://'")
            
        # Validate relationship type
        if relationship_type not in ["linked", "parent"]:
            raise ValueError("relationship_type must be either 'linked' or 'parent'")
