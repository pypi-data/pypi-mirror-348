...
def pull_strings(path: Path, decode_depth:int, budget:int) -> list[str]:
    if budget is None or budget <= 0:
        budget = sys.maxsize  # effectively unlimited
            ...
            for fn in DECODER_REGISTRY:
                for tag,out in fn(s):
                    if out not in all_seen:
                        ...
                        if len(all_seen) >= budget: 
                            print(f"[yellow]⚠ Budget limit reached ({budget} strings). Use --budget to increase.[/yellow]")
                            break
...
def run(target:Path, depth:int=2, stride:int=3, out_dir:Path|None=None,
    ...
    for grp in clusters_raw:
        if not grp:
            continue  # safety – skip empty clusters to avoid crash
