...
from fleet_control.utils.python_utils import format_time, load_json, ask_confirmation
...
def clone(source: str, targets: tuple, exclude: str | None = None):
    ...
    for target_id in targets:
        if not BOT_ID_REGEX.match(target_id):
            logger.info(f"\033[93mFleet {target_id} selected as target!\033[0m")
...
def schedule_update(
    json_file: str, date: str, date_format: str, tz: str, targets: tuple, fleet: str, release: str
) -> None:
...
def rename(bot_id: str, new_id: str, version: str | None = None) -> None:
    ...
    if not BOT_ID_REGEX.match(bot_id):
        logger.info(f"ERROR: invalid target ID format: {bot_id}")
        exit(1)
    if not BOT_ID_REGEX.match(new_id):
        logger.info(f"ERROR: invalid new ID format: {new_id}")
        exit(1)
    ...
    if get_device_by_id(new_id) is not None:
        logger.info(f"ERROR: Bot with id {new_id} already exists")
        exit(1)
    ...
    if click.confirm(
        ...
        logger.info(f"Device {bot_id} successfully renamed to {new_id}")
