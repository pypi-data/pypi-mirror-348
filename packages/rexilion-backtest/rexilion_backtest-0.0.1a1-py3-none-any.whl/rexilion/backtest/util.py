...
def generate_report(df, param1, param2, fees, sr_multiplier):
    ...
    report = {
        ...
        "Trades": trades_count,
        "AR": ar,
        "Trades Ratio": (trades_count - param1) / len(df),
    }
    return report
...
def enrich_data_with_candle_and_metrics(
    data_csv: str,
    candle_csv: str,
    output_csv: str,
    ...
    df_candle = pd.read_csv(candle_csv)
    df = pd.merge(df_data, df_candle[[time_col, 'close']], on=time_col, how='left')
    ...
    for col in cols_to_enrich:
        ...
        # one–off metrics
        all_metrics[f'{col}_diff']       = formula.diff(s)
        all_metrics[f'{col}_pct_change'] = s.pct_change(1)
        # only log of positive values; NaN otherwise
        all_metrics[f'{col}_log']        = np.log(s.where(s > 0))
        # all_metrics[f'{col}_sigmoid']     = formula.sigmoid(s)
        ...
        for w in windows:
            ...
            # overall z-score at each point
            all_metrics[f'{col}_zscore_{w}']   = formula.rolling_zscore(s,w)
            # rolling mean of that z-score
