...
def _create_hale_from_data(hale_data: dict) -> Optional["HalE"]:
    ...
    """
    if 'accessType' in hale_data.get('appConfig', {}):
        hale_data['access_rights'] = hale_data['appConfig']['accessType']

    if ('appConfig' in hale_data and 'appParams' in hale_data['appConfig']
            and 'sourcePath' in hale_data['appConfig']['appParams']):
        hale_data['template_board'] = hale_data['appConfig']['appParams']['sourcePath']

    if 'name' in hale_data.get('appConfig', {}):
        hale_data['name'] = hale_data['appConfig']['name']

    hale_data_model = HalEDataModel(**hale_data)
...
class HalE:
    ...
    def __repr__(self):
        return (f"HalE(name='{self.name}', access_rights='{self.access_rights}', "
                f"template_board='{self.template_board}')")
