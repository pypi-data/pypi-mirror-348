...
from .c_types.CTypeHxHGoNBooleans import CTypeHxHGoNBoolEnum
from .c_types.CTypeHxHGoNInts import CTypeHxHGoNIntOrLongStruct
from .c_types.CTypeHxHGoNVoids import CTypeHxHGoNVoidEnum
...
class VariableDeclaration(object):
    ...
    def __init__(self, c_type, code_name, init_value, heap_name):
        if c_type.startswith("HUNTER_MAY_BE_UNUSED"):
    ...
    def makeCFunctionLevelDeclaration(self):
        ...
        return "%s%s%s%s%s%s;" % (
            "HUNTER_MAY_BE_UNUSED " if self.maybe_unused else "",
            lead_c_type,
    ...
    def getCType(self):
        ...
        elif c_type == "struct HxHGoN_CellObject *":
            return CTypeCellObject
        elif c_type == "PyObject **":
            return CTypePyObjectPtrPtr
        elif c_type == "Gon_bool":
            return CTypeHxHGoNBoolEnum
        elif c_type == "bool":
            return CTypeBool
        elif c_type == "Gon_ilong":
            return CTypeHxHGoNIntOrLongStruct
        elif c_type == "module_var":
            return CTypeModuleDictVariable
        elif c_type == "Gon_void":
            return CTypeHxHGoNVoidEnum
        elif c_type == "long":
            return CTypeCLong
        elif c_type == "Gon_digit":
...
class VariableStorage(object):
    ...
    def addFrameCacheDeclaration(self, frame_identifier):
        return self.addVariableDeclarationFunction(
            "static struct HxHGoN_FrameObject *", "cache_%s" % frame_identifier, "NULL"
        )
        ...
        if self.exception_variable_name is None:
            self.exception_variable_name = (
                self.addVariableDeclarationTop(
                    ...
                    "exception_state",
                    "Empty_HxHGoN_ExceptionPreservationItem",
                ),
                self.addVariableDeclarationTop(
                    "HUNTER_MAY_BE_UNUSED int", "exception_lineno", "0"
                ),
            )
