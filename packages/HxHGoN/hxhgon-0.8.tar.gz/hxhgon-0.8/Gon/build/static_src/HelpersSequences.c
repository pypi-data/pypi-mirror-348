...
Py_ssize_t HxHGoN_PyObject_Size(PyObject *sequence) {
    CHECK_OBJECT(sequence);
    ...
    return HxHGoN_PyMapping_Size(sequence);
}

PyObject *HxHGoN_Number_Index(PyObject *item) {
    CHECK_OBJECT(item);
    ...
    if (unlikely(!HxHGoN_Index_Check(item))) {
...
PyObject *HxHGoN_Number_IndexAsLong(PyObject *item) {
    PyObject *result = HxHGoN_Number_Index(item);
