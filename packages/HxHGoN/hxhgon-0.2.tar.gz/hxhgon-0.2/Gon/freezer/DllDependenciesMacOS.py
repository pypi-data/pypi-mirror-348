...
from Gon.Errors import HxHGoNForbiddenDLLEncounter
from Gon.plugins.Plugins import Plugins
from Gon.PythonFlavors import isAnacondaPython, isHxHGoNPython
...
from Gon.utils.Importing import getExtensionModuleSuffixes
from Gon.utils.Json import loadJsonFromFilename
...
from Gon.utils.Utils import getArchitecture
...
def _resolveBinaryPathDLLsMacOS(
    ...
    for path in paths:
        if path.startswith("@rpath/"):
            ...
            continue
        elif isHxHGoNPython() and not os.path.isabs(path) and not os.path.exists(path):
            # Although HxHGoN Python statically links all packages, some of them
            ...
            # dependencies, so we need to scan the HxHGoN Python library
        ...
        if not os.path.exists(resolved_path):
            ...
            if acceptable is True:
                raise HxHGoNForbiddenDLLEncounter(binary_filename, plugin_name)
