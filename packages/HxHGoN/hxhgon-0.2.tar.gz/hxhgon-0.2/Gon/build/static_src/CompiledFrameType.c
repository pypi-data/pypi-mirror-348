...
#if _DEBUG_REFCOUNTS
int count_active_HxHGoN_Frame_Type = 0;
int count_allocated_HxHGoN_Frame_Type = 0;
int count_released_HxHGoN_Frame_Type = 0;
#endif
...
static PyMemberDef HxHGoN_Frame_members[] = {
...
#else
#define HxHGoN_Frame_members 0
#endif

#if PYTHON_VERSION < 0x300

static PyObject *_HxHGoN_Frame_get_exc_traceback(PyObject *self, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));

    struct HxHGoN_FrameObject *frame = (struct HxHGoN_FrameObject *)self;
...
static int _HxHGoN_Frame_set_exc_traceback(PyObject *self, PyObject *traceback, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_HxHGoN_Frame_get_exc_type(PyObject *self, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _HxHGoN_Frame_set_exc_type(PyObject *self, PyObject *exception_type, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_HxHGoN_Frame_get_exc_value(PyObject *self, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _HxHGoN_Frame_set_exc_value(PyObject *self, PyObject *exception_value, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_HxHGoN_Frame_get_restricted(PyObject *self, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static PyObject *_HxHGoN_Frame_get_locals(PyObject *self, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    struct HxHGoN_FrameObject *Gon_frame = (struct HxHGoN_FrameObject *)self;
    if (Gon_frame->m_type_description == NULL) {
#if PYTHON_VERSION < 0x3b0
        PyFrameObject *locals_owner = &Gon_frame->m_frame;
#else
        _PyInterpreterFrame *locals_owner = &Gon_frame->m_interpreter_frame;
#endif
        ...
        PyObject **var_names = HxHGoN_GetCodeVarNames(HxHGoN_GetFrameCodeObject(Gon_frame));

        char const *w = Gon_frame->m_type_description;
        char const *t = Gon_frame->m_locals_storage;

        while (*w != 0) {
            switch (*w) {
            case HUNTER_TYPE_DESCRIPTION_OBJECT:
            case HUNTER_TYPE_DESCRIPTION_OBJECT_PTR: {
            ...
            case HUNTER_TYPE_DESCRIPTION_CELL: {
                struct HxHGoN_CellObject *value = *(struct HxHGoN_CellObject **)t;
                assert(HxHGoN_Cell_Check((PyObject *)value));
                CHECK_OBJECT(value);
                ...
                t += sizeof(struct HxHGoN_CellObject *);

                break;
            }
            case HUNTER_TYPE_DESCRIPTION_NULL: {
                break;
            }
            case HUNTER_TYPE_DESCRIPTION_BOOL: {
                int value = *(int *)t;
                t += sizeof(int);
                switch ((Gon_bool)value) {
                case HUNTER_BOOL_TRUE: {
                    DICT_SET_ITEM(result, *var_names, Py_True);
                    break;
                }
                case HUNTER_BOOL_FALSE: {
...
static PyObject *_HxHGoN_Frame_get_lineno(PyObject *self, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));

    struct HxHGoN_FrameObject *frame = (struct HxHGoN_FrameObject *)self;
    return HxHGoN_PyInt_FromLong(frame->m_frame.f_lineno);
}

static PyObject *_HxHGoN_Frame_get_trace(PyObject *self, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _HxHGoN_Frame_set_trace(PyObject *self, PyObject *value, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
#if !defined(_HUNTER_DEPLOYMENT_MODE) && !defined(_HUNTER_NO_DEPLOYMENT_FRAME_USELESS_SET_TRACE)
    if (value == Py_None) {
        return 0;
    } else {
        ...
        SET_CURRENT_EXCEPTION_TYPE0_STR(
            ...
            "f_trace is not writable in HxHGoN, ignore with '--no-deployment-flag=frame-useless-set-trace'");
        return -1;
    }
#else
    return 0;
...
static PyObject *_HxHGoN_Frame_get_trace_lines(PyObject *self, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _HxHGoN_Frame_set_trace_lines(PyObject *self, PyObject *value, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "f_trace_lines is not writable in HxHGoN");
    return -1;
}

static PyObject *_HxHGoN_Frame_get_trace_opcodes(PyObject *self, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
...
static int _HxHGoN_Frame_set_trace_opcodes(PyObject *self, PyObject *value, void *data) {
    assert(HxHGoN_Frame_CheckExact(self));
    CHECK_OBJECT(self);
    assert(_PyObject_GC_IS_TRACKED(self));
    ...
    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "f_trace_opcodes is not writable in HxHGoN");
    return -1;
}
#endif

static PyGetSetDef HxHGoN_Frame_tp_getset[] = {
    {(char *)"f_locals", _HxHGoN_Frame_get_locals, NULL, NULL},
    {(char *)"f_lineno", _HxHGoN_Frame_get_lineno, NULL, NULL},
    {(char *)"f_trace", _HxHGoN_Frame_get_trace, _HxHGoN_Frame_set_trace, NULL},
#if PYTHON_VERSION < 0x300
    {(char *)"f_restricted", _HxHGoN_Frame_get_restricted, NULL, NULL},
    {(char *)"f_exc_traceback", _HxHGoN_Frame_get_exc_traceback, _HxHGoN_Frame_set_exc_traceback, NULL},
    {(char *)"f_exc_type", _HxHGoN_Frame_get_exc_type, _HxHGoN_Frame_set_exc_type, NULL},
    {(char *)"f_exc_value", _HxHGoN_Frame_get_exc_value, _HxHGoN_Frame_set_exc_value, NULL},
#endif
#if PYTHON_VERSION >= 0x370
    {(char *)"f_trace_lines", _HxHGoN_Frame_get_trace_lines, _HxHGoN_Frame_set_trace_lines, NULL},
    {(char *)"f_trace_opcodes", _HxHGoN_Frame_get_trace_opcodes, _HxHGoN_Frame_set_trace_opcodes, NULL},
#endif
    {NULL}};

// tp_repr slot, decide how a function shall be output
static PyObject *HxHGoN_Frame_tp_repr(struct HxHGoN_FrameObject *Gon_frame) {
    assert(HxHGoN_Frame_CheckExact((PyObject *)Gon_frame));
    CHECK_OBJECT((PyObject *)Gon_frame);
    assert(_PyObject_GC_IS_TRACKED(Gon_frame));

#if PYTHON_VERSION >= 0x370
    PyCodeObject *code_object = HxHGoN_GetFrameCodeObject(Gon_frame);
    return HxHGoN_String_FromFormat("<compiled_frame at %p, file %R, line %d, code %S>", Gon_frame,
                                    code_object->co_filename, HxHGoN_GetFrameLineNumber(Gon_frame),
                                    code_object->co_name);
#elif _DEBUG_FRAME || _DEBUG_REFRAME || _DEBUG_EXCEPTIONS
    PyCodeObject *code_object = HxHGoN_GetFrameCodeObject(Gon_frame);
    return HxHGoN_String_FromFormat("<compiled_frame object for %s at %p>",
                                    HxHGoN_String_AsString(code_object->co_name), Gon_frame);
#else
    return HxHGoN_String_FromFormat("<compiled_frame object at %p>", Gon_frame);
#endif
}

static void HxHGoN_Frame_tp_clear(struct HxHGoN_FrameObject *frame) {
    if (frame->m_type_description) {
        ...
        while (*w != 0) {
            ...
            case HUNTER_TYPE_DESCRIPTION_OBJECT:
            case HUNTER_TYPE_DESCRIPTION_OBJECT_PTR:
            case HUNTER_TYPE_DESCRIPTION_NILONG: {
                PyObject *value = *(PyObject **)t;
                CHECK_OBJECT_X(value);
...
static struct HxHGoN_FrameObject *free_list_frames = NULL;
static int free_list_frames_count = 0;

static void HxHGoN_Frame_tp_dealloc(struct HxHGoN_FrameObject *Gon_frame) {
#if _DEBUG_REFCOUNTS
    count_active_HxHGoN_Frame_Type -= 1;
    count_released_HxHGoN_Frame_Type += 1;
#endif
    ...
    HxHGoN_GC_UnTrack(Gon_frame);

    PyFrameObject *frame = &Gon_frame->m_frame;
#if PYTHON_VERSION < 0x3b0
    ...
    assert(HxHGoN_GC_IS_TRACKED_X((PyObject *)frame->f_back));
    ...
    HxHGoN_Frame_tp_clear(Gon_frame);

    if (Py_REFCNT(Gon_frame) > 0) {
        Py_SET_REFCNT(Gon_frame, Py_REFCNT(Gon_frame) - 1);
        if (Py_REFCNT(Gon_frame) >= 1) {
    ...
    // it.
    Py_SET_SIZE(Gon_frame, Gon_frame->m_ob_size);
#endif

    releaseToFreeList(free_list_frames, Gon_frame, MAX_FRAME_FREE_LIST_COUNT);
...
static int HxHGoN_Frame_tp_traverse(struct HxHGoN_FrameObject *frame, visitproc visit, void *arg) {
    assert(HxHGoN_Frame_CheckExact((PyObject *)frame));
    ...
    while (w != NULL && *w != 0) {
        ...
        default:
            HUNTER_CANNOT_GET_HERE("invalid type description");
            assert(false);
        }
...
static PyObject *HxHGoN_GetFrameGenerator(struct HxHGoN_FrameObject *Gon_frame) {
#if PYTHON_VERSION < 0x3b0
    return Gon_frame->m_frame.f_gen;
#else
    return Gon_frame->m_generator;
#endif
}

static PyObject *HxHGoN_Frame_clear(struct HxHGoN_FrameObject *frame, PyObject *unused) {
    assert(HxHGoN_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));
    ...
    if (HxHGoN_Frame_IsExecuting(frame)) {
    ...
    PyObject *f_gen = HxHGoN_GetFrameGenerator(frame);
    if (f_gen != NULL) {
        CHECK_OBJECT(f_gen);
        ...
        if (HxHGoN_Generator_Check(f_gen)) {
            struct HxHGoN_GeneratorObject *generator = (struct HxHGoN_GeneratorObject *)f_gen;
            HxHGoN_SetFrameGenerator(frame, NULL);

            close_exception = !_HxHGoN_Generator_close(tstate, generator);
        }
#if PYTHON_VERSION >= 0x350
        else if (HxHGoN_Coroutine_Check(f_gen)) {
            struct HxHGoN_CoroutineObject *coroutine = (struct HxHGoN_CoroutineObject *)f_gen;
            HxHGoN_SetFrameGenerator(frame, NULL);

            close_exception = !_HxHGoN_Coroutine_close(tstate, coroutine);
        }
#endif
#if PYTHON_VERSION >= 0x360
        else if (HxHGoN_Asyncgen_Check(f_gen)) {
            struct HxHGoN_AsyncgenObject *asyncgen = (struct HxHGoN_AsyncgenObject *)f_gen;
            HxHGoN_SetFrameGenerator(frame, NULL);

            close_exception = !_HxHGoN_Asyncgen_close(tstate, asyncgen);
        }
#endif
        else {
...
static inline Py_ssize_t HxHGoN_Frame_GetSize(struct HxHGoN_FrameObject *frame) {
    assert(HxHGoN_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));
...
static PyObject *HxHGoN_Frame_sizeof(struct HxHGoN_FrameObject *frame, PyObject *unused) {
    assert(HxHGoN_Frame_CheckExact((PyObject *)frame));
    CHECK_OBJECT((PyObject *)frame);
    assert(_PyObject_GC_IS_TRACKED(frame));

    return PyInt_FromSsize_t(sizeof(struct HxHGoN_FrameObject) + Py_SIZE(frame));
}

static PyMethodDef HxHGoN_Frame_methods[] = {
#if PYTHON_VERSION >= 0x300
    {"clear", (PyCFunction)HxHGoN_Frame_clear, METH_NOARGS, "F.clear(): clear most references held by the frame"},
#endif
    {"__sizeof__", (PyCFunction)HxHGoN_Frame_sizeof, METH_NOARGS, "F.__sizeof__() -> size of F in memory, in bytes"},
    {NULL, NULL}};

PyTypeObject HxHGoN_Frame_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_frame",
    sizeof(struct HxHGoN_FrameObject),
    1,
    (destructor)HxHGoN_Frame_tp_dealloc,     // tp_dealloc
    0,                                       // tp_print
    0,                                       // tp_getattr
    0,                                       // tp_setattr
    0,                                       // tp_compare
    (reprfunc)HxHGoN_Frame_tp_repr,          // tp_repr
    0,                                       // tp_as_number
    0,                                       // tp_as_sequence
    0,                                       // tp_as_mapping
    0,                                       // tp_hash
    ...
    0,                                       // tp_doc
    (traverseproc)HxHGoN_Frame_tp_traverse,  // tp_traverse
    (inquiry)HxHGoN_Frame_tp_clear,          // tp_clear
    0,                                       // tp_richcompare
    0,                                       // tp_weaklistoffset
    0,                                       // tp_iter
    0,                                       // tp_iternext
    HxHGoN_Frame_methods,                    // tp_methods
    HxHGoN_Frame_members,                    // tp_members
    HxHGoN_Frame_tp_getset,                  // tp_getset
    0,                                       // tp_base
    0,                                       // tp_dict
};

void _initCompiledFrameType(void) {
    assert(HxHGoN_Frame_Type.tp_doc != PyFrame_Type.tp_doc || PyFrame_Type.tp_doc == NULL);
    assert(HxHGoN_Frame_Type.tp_traverse != PyFrame_Type.tp_traverse);
    assert(HxHGoN_Frame_Type.tp_clear != PyFrame_Type.tp_clear || PyFrame_Type.tp_clear == NULL);
    assert(HxHGoN_Frame_Type.tp_richcompare != PyFrame_Type.tp_richcompare || PyFrame_Type.tp_richcompare == NULL);
    assert(HxHGoN_Frame_Type.tp_weaklistoffset != PyFrame_Type.tp_weaklistoffset ||
           PyFrame_Type.tp_weaklistoffset == 0);
    assert(HxHGoN_Frame_Type.tp_iter != PyFrame_Type.tp_iter || PyFrame_Type.tp_iter == NULL);
    assert(HxHGoN_Frame_Type.tp_iternext != PyFrame_Type.tp_iternext || PyFrame_Type.tp_iternext == NULL);
    assert(HxHGoN_Frame_Type.tp_methods != PyFrame_Type.tp_methods);
    assert(HxHGoN_Frame_Type.tp_members != PyFrame_Type.tp_members);
    assert(HxHGoN_Frame_Type.tp_getset != PyFrame_Type.tp_getset);

    assert(HxHGoN_Frame_Type.tp_descr_get != PyFrame_Type.tp_descr_get || PyFrame_Type.tp_descr_get == NULL);

    assert(HxHGoN_Frame_Type.tp_descr_set != PyFrame_Type.tp_descr_set || PyFrame_Type.tp_descr_set == NULL);
    assert(HxHGoN_Frame_Type.tp_dictoffset != PyFrame_Type.tp_dictoffset || PyFrame_Type.tp_dictoffset == 0);
    // TODO: These get changed and into the same thing, not sure what to compare against, project something
    // assert(HxHGoN_Frame_Type.tp_dict != PyFrame_Type.tp_dict);
    // assert(HxHGoN_Frame_Type.tp_init != PyFrame_Type.tp_init || PyFrame_Type.tp_init == NULL);
    // assert(HxHGoN_Frame_Type.tp_alloc != PyFrame_Type.tp_alloc || PyFrame_Type.tp_alloc == NULL);
    // assert(HxHGoN_Frame_Type.tp_new != PyFrame_Type.tp_new || PyFrame_Type.tp_new == NULL);
    // assert(HxHGoN_Frame_Type.tp_free != PyFrame_Type.tp_free || PyFrame_Type.tp_free == NULL);
    // assert(HxHGoN_Frame_Type.tp_weaklist != PyFrame_Type.tp_weaklist);
    assert(HxHGoN_Frame_Type.tp_bases != PyFrame_Type.tp_bases);
    assert(HxHGoN_Frame_Type.tp_mro != PyFrame_Type.tp_mro);
    assert(HxHGoN_Frame_Type.tp_cache != PyFrame_Type.tp_cache || PyFrame_Type.tp_cache == NULL);
    assert(HxHGoN_Frame_Type.tp_subclasses != PyFrame_Type.tp_subclasses || PyFrame_Type.tp_cache == NULL);
    assert(HxHGoN_Frame_Type.tp_del != PyFrame_Type.tp_del || PyFrame_Type.tp_del == NULL);
#if PYTHON_VERSION >= 0x300
    assert(HxHGoN_Frame_Type.tp_finalize != PyFrame_Type.tp_finalize || PyFrame_Type.tp_finalize == NULL);
#endif
    HxHGoN_PyType_Ready(&HxHGoN_Frame_Type, &PyFrame_Type, true, true, false, false, false);

    // These are to be used interchangeably. Make sure that's true.
    assert(offsetof(struct HxHGoN_FrameObject, m_frame) == 0);
}

static struct HxHGoN_FrameObject *_MAKE_COMPILED_FRAME(PyCodeObject *code, PyObject *module, PyObject *f_locals,
...
#if _DEBUG_REFCOUNTS
    count_active_HxHGoN_Frame_Type += 1;
    count_allocated_HxHGoN_Frame_Type += 1;
#endif
    ...
    struct HxHGoN_FrameObject *result;

    // Macro to assign result memory from GC or free list.
    allocateFromFreeList(free_list_frames, struct HxHGoN_FrameObject, HxHGoN_Frame_Type, locals_size);
...
#if PYTHON_VERSION >= 0x300
    HxHGoN_SetFrameGenerator(result, NULL);

    HxHGoN_Frame_MarkAsNotExecuting(result);
#endif
...
struct HxHGoN_FrameObject *MAKE_MODULE_FRAME(PyCodeObject *code, PyObject *module) {
...
struct HxHGoN_FrameObject *MAKE_FUNCTION_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
                                               Py_ssize_t locals_size) {
    PyObject *f_locals;
...
struct HxHGoN_FrameObject *MAKE_CLASS_FRAME(PyThreadState *tstate, PyCodeObject *code, PyObject *module,
...
PyCodeObject *makeCodeObject(PyObject *filename, int line, int flags, PyObject *function_name,
    ...
    assert(HxHGoN_StringOrUnicode_CheckExact(filename));

    CHECK_OBJECT(function_name);
    assert(HxHGoN_String_CheckExact(function_name));
    ...
    if (empty_code == NULL) {
        ...
        HUNTER_MAY_BE_UNUSED PyObject *module =
            PyImport_ExecCodeModule("Gon_empty_function", empty_code_module_object);
        CHECK_OBJECT(module);
        ...
        HUNTER_MAY_BE_UNUSED bool bool_res = HxHGoN_DelModuleString(tstate, "Gon_empty_function");
    ...
    if (result == NULL) {
        PyErr_PrintEx(0);
        HUNTER_CANNOT_GET_HERE("Failed to create code object");
    }
...
void HxHGoN_Frame_AttachLocals(struct HxHGoN_FrameObject *frame_object, char const *type_description, ...) {
    assert(HxHGoN_Frame_CheckExact((PyObject *)frame_object));
    ...
    while (*w != 0) {
        ...
        case HUNTER_TYPE_DESCRIPTION_CELL: {
            struct HxHGoN_CellObject *value = va_arg(ap, struct HxHGoN_CellObject *);
            assert(HxHGoN_Cell_Check((PyObject *)value));
            CHECK_OBJECT(value);
            CHECK_OBJECT_X(value->ob_ref);

            memcpy(t, &value, sizeof(struct HxHGoN_CellObject *));
            ...
            // Py_SET_REFCNT((struct HxHGoN_CellObject *)t, 1);
            Py_INCREF(value);
        ...
        case HUNTER_TYPE_DESCRIPTION_NULL: {
            HUNTER_MAY_BE_UNUSED void *value = va_arg(ap, struct HxHGoN_CellObject *);
        ...
        case HUNTER_TYPE_DESCRIPTION_NILONG: {
            Gon_ilong value = va_arg(ap, Gon_ilong);
    ...
    assert(t - frame_object->m_locals_storage <= HxHGoN_Frame_GetSize(frame_object));
}
...
void dumpFrameStack(void) {
    ...
    while (current) {
        ...
        PyObject *code_repr = PyObject_Str((PyObject *)HxHGoN_InterpreterFrame_GetCodeObject(current->current_frame));
#endif

        PRINT_FORMAT("Frame stack %d: %s %d %s\n", total--, HxHGoN_String_AsString(current_repr), Py_REFCNT(current),
                     HxHGoN_String_AsString(code_repr));
...
static void PRINT_UNCOMPILED_FRAME(char const *prefix, PyFrameObject *frame) {
    ...
    if (frame) {
        ...
        PyObject *code_object_str = PyObject_Repr((PyObject *)HxHGoN_Frame_GetCodeObject(frame));
...
void PRINT_COMPILED_FRAME(char const *prefix, struct HxHGoN_FrameObject *frame) {
    return PRINT_UNCOMPILED_FRAME(prefix, &frame->m_frame);
}

void PRINT_INTERPRETER_FRAME(char const *prefix, HxHGoN_ThreadStateFrameType *frame) {
#if PYTHON_VERSION < 0x3b0
    ...
    if (frame) {
        ...
        PyObject *code_object_str = PyObject_Repr((PyObject *)HxHGoN_InterpreterFrame_GetCodeObject(frame));
        PRINT_ITEM(code_object_str);
        Py_DECREF(code_object_str);
    } else {
        PRINT_STRING("<NULL> no frame");
