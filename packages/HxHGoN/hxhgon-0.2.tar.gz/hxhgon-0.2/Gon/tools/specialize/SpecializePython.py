...
import Gon.code_generation.BinaryOperationHelperDefinitions
import Gon.code_generation.CodeGeneration
import Gon.code_generation.ComparisonCodes
import Gon.code_generation.Namify
import Gon.nodes.NetworkxNodes
import Gon.nodes.PackageMetadataNodes
import Gon.nodes.PackageResourceNodes
import Gon.nodes.SideEffectNodes
import Gon.nodes.TensorflowNodes
...
import Gon.specs.BuiltinTypeOperationSpecs
import Gon.specs.HardImportSpecs
import Gon.tree.Building
from Gon.containers.OrderedDicts import OrderedDict
from Gon.containers.OrderedSets import OrderedSet
from Gon.nodes.ImportNodes import hard_modules_non_stdlib
from Gon.nodes.NodeMetaClasses import NodeCheckMetaClass
...
from Gon.utils.Jinja2 import getTemplate
...
def _getMixinForShape(shape):
    ...
    if shape is tshape_str:
        return "Gon.nodes.ExpressionShapeMixins.ExpressionStrShapeExactMixin"
    elif shape is tshape_list:
        return "Gon.nodes.ExpressionShapeMixins.ExpressionListShapeExactMixin"
    elif shape is tshape_tuple:
        return "Gon.nodes.ExpressionShapeMixins.ExpressionTupleShapeExactMixin"
    elif shape is tshape_int:
        return "Gon.nodes.ExpressionShapeMixins.ExpressionIntShapeExactMixin"
    elif shape is tshape_bool:
        return "Gon.nodes.ExpressionShapeMixins.ExpressionBoolShapeExactMixin"
    elif shape is tshape_none:
        return "Gon.nodes.ExpressionShapeMixins.ExpressionNoneShapeExactMixin"
    elif shape is tshape_dict:
        return "Gon.nodes.ExpressionShapeMixins.ExpressionDictShapeExactMixin"
    elif shape is tshape_bytes:
        return "Gon.nodes.ExpressionShapeMixins.ExpressionBytesShapeExactMixin"
    else:
        assert False, shape
...
processTypeShapeAttribute(
    "tshape_type",
    Gon.specs.BuiltinTypeOperationSpecs,
    python2_type_methods,
    python3_type_methods,
)
...
def emitGenerationWarning(emit, doc_string, template_name):
    ...
    for spec_descriptions in getSpecVersions(Gon.specs.HardImportSpecs):
...
def makeAttributeNodes():
    filename_python = "Gon/nodes/AttributeNodesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit("from Gon.specs.BuiltinParameterSpecs import extractBuiltinArgs")

        emit("from Gon.nodes.ConstantRefNodes import makeConstantRefNode")
        emit(
            """\
from Gon.nodes.NodeMakingHelpers import (
    ...
    makeRaiseExceptionReplacementExpression
        ...
        emit(
            "from Gon.nodes.KeyValuePairNodes import makeKeyValuePairExpressionsFromKwArgs"
        )

        emit("from Gon.nodes.AttributeNodes import makeExpressionAttributeLookup")

        emit("from Gon.PythonVersions import getDictFromkeysNoArgErrorMessage")
...
def makeBuiltinOperationNodes():
    filename_python = "Gon/nodes/BuiltinOperationNodeBasesGenerated.py"
...
def makeChildrenHavingMixinNodes():
    ...
    filename_python = "Gon/nodes/ChildrenHavingMixins.py"
    filename_python2 = "Gon/nodes/ExpressionBasesGenerated.py"
    filename_python3 = "Gon/nodes/StatementBasesGenerated.py"
    ...
    with withFileOpenedAndAutoFormatted(
        ...
        emit1(
            """
from Gon.nodes.Checkers import (
    checkStatementsSequenceOrNone,
...
def makeHardImportNodes():
    ...
    filename_python = "Gon/nodes/HardImportNodesGenerated.py"
