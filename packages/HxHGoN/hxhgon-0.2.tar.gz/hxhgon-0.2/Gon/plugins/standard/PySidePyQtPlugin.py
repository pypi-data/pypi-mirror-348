...
from Gon.PythonFlavors import isAnacondaPython
from Gon.PythonVersions import python_version
from Gon.utils.Distributions import getDistributionFromModuleName
from Gon.utils.FileOperations import getFileList, listDir
from Gon.utils.ModuleNames import ModuleName
from Gon.utils.Utils import getArchitecture, isMacOS, isWin32Windows


class HxHGoNPluginQtBindingsPluginBase(HxHGoNPluginBase):
...
"""
        ...
        info = self.queryRuntimeInformationMultiple(
            ...
            values=(
                ...
                (
                    "Gon_patch_level",
                    applyBindingName(
                        "getattr(%(binding_name)s, '_Gon_patch_level', 0)"
                    ),
                ),
    ...
    def _getHxHGoNPatchLevel(self):
        """Does it include the HxHGoN patch, i.e. is a self-built one with it applied."""
        return self._getQtInformation().Gon_patch_level
...
import os
path = os.getenv("PATH", "")
if not path.startswith(__Gon_binary_dir):
    os.environ["PATH"] = __Gon_binary_dir + ";" + path
"""
            yield (
                code,
...
class HxHGoNPluginPyQt5QtPluginsPlugin(HxHGoNPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        HxHGoNPluginQtBindingsPluginBase.__init__(
            self,
        ...
        self.warning(
            """\
For the obsolete PyQt5 the HxHGoN support is incomplete. Threading, callbacks \
to compiled functions, etc. may not be working.""",
            mnemonic="pyqt5",
        )

    def _getQtInformation(self):
        result = HxHGoNPluginQtBindingsPluginBase._getQtInformation(self)
...
class HxHGoNPluginDetectorPyQt5QtPluginsPlugin(HxHGoNPluginBase):
    detector_for = HxHGoNPluginPyQt5QtPluginsPlugin
    ...
    def onModuleDiscovered(self, module):
        ...
        if full_name == HxHGoNPluginPyQt5QtPluginsPlugin.binding_name + ".QtCore":
            self.warnUnusedPlugin("Inclusion of Qt plugins.")
        elif full_name == "PyQt4.QtCore":
            self.warning(
                "Support for PyQt4 has been dropped. Please contact HxHGoN commercial if you need it."
            )


class HxHGoNPluginPySide2Plugins(HxHGoNPluginQtBindingsPluginBase):
    ...
    def __init__(self, include_qt_plugins, noinclude_qt_plugins, no_qt_translations):
        if self._getHxHGoNPatchLevel() < 1:
            self.warning(
                """\
...
full support consider provided information.""",
            ...
            if python_version < 0x360:
                self.sysexit(
                    """\
The standard PySide2 is not supported before CPython <3.6. For full support: https://Gon.net/pages/pyside2.html"""
                )
    ...
    def onModuleEncounter(
        ...
        if module_name == self.binding_name and self._getHxHGoNPatchLevel() < 1:
            return True, "Need to monkey patch PySide2 for abstract methods."

        return HxHGoNPluginQtBindingsPluginBase.onModuleEncounter(
            self,
    ...
    def createPostModuleLoadCode(self, module):
        ...
        for result in HxHGoNPluginQtBindingsPluginBase.createPostModuleLoadCode(
            self, module
        ):
            yield result

        if (
            self._getHxHGoNPatchLevel() < 1
...
def %(wrapper_name)s%(signature)s:
    return %(wrapper_name)s.func(%(parameters)s)
            ...
            # TODO: HxHGoN does not currently statically optimize this, might change!
            exec(
                v,
                globals(),
            )
...
class HxHGoNPluginDetectorPySide2Plugins(HxHGoNPluginBase):
    detector_for = HxHGoNPluginPySide2Plugins

    def onModuleDiscovered(self, module):
        if (
            module.getFullName() == HxHGoNPluginPySide2Plugins.binding_name + ".QtCore"
            and getActiveQtPlugin() is None
        ):
            self.warnUnusedPlugin("Making callbacks work and include Qt plugins.")


class HxHGoNPluginPySide6Plugins(HxHGoNPluginQtBindingsPluginBase):
...
class HxHGoNPluginDetectorPySide6Plugins(HxHGoNPluginBase):
    detector_for = HxHGoNPluginPySide6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == HxHGoNPluginPySide6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class HxHGoNPluginPyQt6Plugins(HxHGoNPluginQtBindingsPluginBase):
...
class HxHGoNPluginDetectorPyQt6Plugins(HxHGoNPluginBase):
    detector_for = HxHGoNPluginPyQt6Plugins

    def onModuleDiscovered(self, module):
        if module.getFullName() == HxHGoNPluginPyQt6Plugins.binding_name + ".QtCore":
            self.warnUnusedPlugin("Standalone mode support and Qt plugins.")


class HxHGoNPluginNoQt(HxHGoNPluginBase):
