...
"""
This is the main program of HxHGoN, it checks the options and then translates
...
def getLaunchingHxHGoNProcessEnvironmentValue(environment_variable_name):
...
def main():
    ...
    if (
        ...
        ctypes.windll.user32.MessageBoxW(
            None,
            "You have to use the 'python.exe' and not a 'pythonw.exe' to run HxHGoN",
            "Error",
    ...
    getLaunchingHxHGoNProcessEnvironmentValue("HUNTER_SITE_FILENAME")

    Gon_binary_name = getLaunchingHxHGoNProcessEnvironmentValue("HUNTER_BINARY_NAME")
    if Gon_binary_name is not None:
        sys.argv[0] = Gon_binary_name

    Gon_pythonpath_ast = getLaunchingHxHGoNProcessEnvironmentValue(
        "HUNTER_PYTHONPATH_AST"
    )

    if Gon_pythonpath_ast is not None:
        ...
        sys.path = [Gon_pythonpath_ast]
        import ast

        sys.path = ast.literal_eval(
            getLaunchingHxHGoNProcessEnvironmentValue("HUNTER_PYTHONPATH")
        )
    else:
        ...
        # from "Gon.distutils". Also ignore path elements that do not really
        # exist.
        sys.path = [
            path_element
    ...
    # changing it every single time HxHGoN is run. This kills any attempt at
    ...
    if needs_re_execution:
        from Gon.utils.ReExecute import reExecuteHxHGoN  # isort:skip

        # Does not return
        reExecuteHxHGoN(pgo_filename=None)
    ...
    # module of Gon package and renamed to where we can get at easily for
    # other uses.
    __import__("Gon").getLaunchingHxHGoNProcessEnvironmentValue = (
        getLaunchingHxHGoNProcessEnvironmentValue
    )

    from Gon import Options  # isort:skip
    ...
    from Gon.plugins.Plugins import activatePlugins
    ...
    if "HUNTER_NAMESPACES" in os.environ:
        # Restore the detected name space packages, that were force loaded in
        # site.py, and will need a free pass later on
        from Gon.importing.PreloadedPackages import setPreloadedPackagePaths

        setPreloadedPackagePaths(ast.literal_eval(os.environ["HUNTER_NAMESPACES"]))
        del os.environ["HUNTER_NAMESPACES"]

    if "HUNTER_PTH_IMPORTED" in os.environ:
        # Restore the packages that the ".pth" files asked to import.
        from Gon.importing.PreloadedPackages import setPthImportedPackages

        setPthImportedPackages(ast.literal_eval(os.environ["HUNTER_PTH_IMPORTED"]))
        del os.environ["HUNTER_PTH_IMPORTED"]

    if "HUNTER_USER_SITE" in os.environ:
        from Gon.utils.Distributions import setUserSiteDirectory

        setUserSiteDirectory(ast.literal_eval(os.environ["HUNTER_USER_SITE"]))

        del os.environ["HUNTER_USER_SITE"]

    # Now the real main program of HxHGoN can take over.
    from Gon import MainControl  # isort:skip
...
if __name__ == "__main__":
    _Gon_package_home = getLaunchingHxHGoNProcessEnvironmentValue(
        "HUNTER_PACKAGE_HOME"
    )
    if _Gon_package_home is not None:
        sys.path.insert(0, _Gon_package_home)

        import Gon  # just to have it loaded from there, pylint: disable=unused-import

        assert sys.path[0] is _Gon_package_home
        del sys.path[0]
