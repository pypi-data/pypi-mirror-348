...
from Gon.utils.ThreadedExecutor import RLock
...
class HxHGoNProgressBar(object):
...
def setupProgressBar(stage, unit, total, min_total=0):
    ...
    if use_progress_bar:
        Tracing.progress = HxHGoNProgressBar(
            iterable=None,
            stage=stage,
            total=total,
            min_total=min_total,
...
@contextmanager
def withHxHGoNDownloadProgressBar(*args, **kwargs):
    if not use_progress_bar or _getTqdmModule() is None:
        yield
    else:

        class HxHGoNDownloadProgressBar(tqdm):
        ...
        with HxHGoNDownloadProgressBar(*args, **kwargs) as progress_bar:
