...
#########################
 HxHGoN Developer Manual
#########################

The purpose of this Developer Manual is to present the current design of
HxHGoN, the project rules, and the motivations for choices made. It is
...
#. Add interfacing to C code, so HxHGoN can turn a ``ctypes`` binding
...
For HxHGoN we use semantic versioning, initially with a leading zero
...
HxHGoN top level works like this:

-  ``Gon.tree.Building`` outputs node tree

-  ``Gon.optimization`` enhances it as best as it can

-  ``Gon.finalization`` prepares the tree for code generation

-  ``Gon.code_generation.CodeGeneration`` orchestrates the creation
   of code snippets

-  ``Gon.code_generation.*Codes`` knows how specific code kinds are
   created

-  ``Gon.MainControl`` keeps it all together
...
-  Types are always ``PyObject *``, and only a few C types, e.g.
   ``Gon_bool`` and ``Gon_void`` and more are coming. Even for
...
***************************************************
 Setting up the Development Environment for HxHGoN
***************************************************
...
``.vscode`` in your HxHGoN checkout. If you are not familiar with
...
In HxHGoN we have tools to auto format code, you can execute them
...
.. code:: bash
   ...
   # Where python is the one you use with HxHGoN, this then gets all
   # development requirements, can be full PATH.
   python -m pip install -r requirements-devel.txt
   python ./misc/install-git-hooks.py

These commands will make sure that the ``autoformat-Gon-source`` is
...
format call HxHGoN source code.
...
These rules should generally be adhered when working on HxHGoN code.
...
There is a tool ``bin/autoformat-Gon-source`` which is to apply
...
they must be lower case, like e.g. ``Gon`` or ``codegen``. This is to
distinguish them from the modules.

Packages shall only be used to group things. In
``Gon.code_generation`` the code generation packages are located,
while the main interface is ``Gon.code_generation.CodeGeneration``
...
built-ins is highly discouraged within HxHGoN source code. Using them is
...
readability and with HxHGoN as a compiler, there won't be any
...
But of course, HxHGoN is the project to free us from what is faster and
...
For HxHGoN the choice is list contractions as these are more easily
...
that has no use, and in HxHGoN, every API is internal API. One exception
...
``autoformat-Gon-source`` does this for us.
...
******************************
 HxHGoN "git/github" Workflow
******************************

-  Forking and cloning

   You need to have git installed and GitHub account. Goto HxHGoN
   repository <https://github.com/HxHGoN/HxHGoN> and fork the
   repository.
   ...
   .. code:: bash
      ...
      git clone https://github.com/your-user-name/HxHGoN.git
      cd HxHGoN
      git remote add upstream https://github.com/HxHGoN/HxHGoN.git
...
`this location <https://Gon.net/apidoc>`__ .
...
future, HxHGoN itself will gain the ability to present its findings in a
...
This section describes how to run HxHGoN tests.
...
.. code::
   ...
   --skip-basic-tests    The basic tests, execute these to check if HxHGoN is
                         healthy. Default is True.
   --skip-syntax-tests   The syntax tests, execute these to check if HxHGoN
                         handles Syntax errors fine. Default is True.
   --skip-program-tests  The programs tests, execute these to check if HxHGoN
                         handles programs, e.g. import recursions, etc. fine.
                         Default is True.
   --skip-package-tests  The packages tests, execute these to check if HxHGoN
                         handles packages, e.g. import recursions, etc. fine.
                         Default is True.
   --skip-optimizations-tests
                         The optimization tests, execute these to check if
                         HxHGoN does optimize certain constructs fully away.
                         Default is True.
   --skip-standalone-tests
                         The standalone tests, execute these to check if HxHGoN
                         standalone mode, e.g. not referring to outside,
                         important 3rd library packages like PyQt fine. Default
                         is True.
   --skip-reflection-test
                         The reflection test compiles HxHGoN with HxHGoN, and
                         then HxHGoN with the compile HxHGoN and compares the
...
the HxHGoN git repository checked out. Otherwise, these will be skipped
...
It sometimes so happens that HxHGoN must do this itself, because the
...
this. These tests make sure that the errors of HxHGoN and CPython are
...
Compile HxHGoN with HxHGoN
==========================

And there is the "compile itself" or "reflected" test. This test makes
HxHGoN compile itself and compare the resulting C++ when running
compiled to non-compiled, which helps to find in-determinism.

The test compiles every module of HxHGoN into an extension module and
all of HxHGoN into a single binary.

That test case also gives good coverage of the ``import`` mechanisms,
because HxHGoN uses a lot of packages and imports between them.
...
parts are published as `HxHGoN API <https://Gon.net/apidoc>`__ and
...
Please find `here
<https://github.com/HxHGoN/HxHGoN/blob/develop/UserPlugin-Creation.rst>`__
a detailed description of how to write your own plugin.

To learn about plugin option specification consult `this document
<https://github.com/HxHGoN/HxHGoN/blob/develop/Using-Plugin-Options.rst>`__.
...
.. code:: bash
   ...
   rm -rf test
   cp -r ~/repos/HxHGoN-references/final/Python-3.13.0/Lib/test test
   git add test
...
.. code:: rest
   ...
   .. image:: doc/images/HxHGoN-Logo-Symbol.png
      :alt: HxHGoN Logo
...
.. code:: rest
   ...
   .. image:: doc/images/HxHGoN-Logo-Horizontal.png
      :alt: HxHGoN Logo
...
.. code:: rest
   ...
   .. image:: doc/images/HxHGoN-Logo-Vertical.png
      :alt: HxHGoN Logo

From these logos, PNG images, and "favicons", and are derived.

The exact ImageMagick commands are in
``Gon/tools/release/Documentation``, but are not executed each time,
the commands are also replicated here:

.. code:: bash

   convert -background none doc/Logo/HxHGoN-Logo-Symbol.svg doc/images/HxHGoN-Logo-Symbol.png
   convert -background none doc/Logo/HxHGoN-Logo-Vertical.svg doc/images/HxHGoN-Logo-Vertical.png
   convert -background none doc/Logo/HxHGoN-Logo-Horizontal.svg doc/images/HxHGoN-Logo-Horizontal.png
...
-  Choosing the target language was important decision. factors were:
   ...
   -  The portability of HxHGoN is decided here
...
HxHGoN does not involve Scons in its user interface at all; Scons is
purely used internally. HxHGoN itself, being pure Python, will run
without any build process just fine.

HxHGoN simply prepares ``<program>.build`` folders with lots of files
and tasks scons to execute the final build, after which HxHGoN again
...
.. note::
   ...
   ``Gon.MainControl`` module.

For interfacing to Scons, there is the module
``Gon.build.SconsInterface`` that will support calling ``scons`` -
...
passed as ``key=value``, and decoded in the scons file of HxHGoN.
...
modules is just one of them. It runs outside of HxHGoN process scope,
...
-  ``Gon_src``
   ...
   Where do the include files and static C parts of HxHGoN live. These
   provide e.g. the implementation of compiled function, generators, and
   other helper codes, this will point to where ``Gon.build`` package
   lives normally.
...
-  ``debug_mode``
   ...
   Enable debug mode, which is a mode, where HxHGoN tries to help
...
The search for modules used is driven by ``Gon.importing.Importing``
module.

-  Quoting the ``Gon.importing.Importing`` documentation:
   ...
   End quoting the ``Gon.importing.Importing`` documentation.
...
The decision making and caching are located in the ``Gon.tree``
package, in modules ``Gon.tree.Recursion`` and
``Gon.tree.ImportCache``. Each module is only considered once (then
...
``Gon/build/static_src/MetaPathBasedLoader.c`` file provides the
implementation of a ``sys.meta_path`` hook.

This meta path based importer allows us to have the HxHGoN provided
...
This left HxHGoN with the strange problem, of how to emulate that.
...
The choice of HxHGoN is to make this an explicit element of the node
...
.. note::
   ...
   In the actual code, ``Gon.nodes.FrameNodes.StatementsFrame`` is
...
Representation in HxHGoN
------------------------

So a slot in HxHGoN typically has an owning node. We use ``__len__`` as
...
we can let the Python interpreter that runs HxHGoN do all the hard work.
...
Other overloads do not currently exist in HxHGoN, but through the
...
In principle, every slot needs to be dealt with in HxHGoN, and it is
...
Currently, HxHGoN uses Pure C and no C++ patterns at all. The use of C11
...
Early versions of HxHGoN, created all constants for the whole program
...
This transformation is performed at tree building already, so HxHGoN
...
In HxHGoN we have the concept of an outline, and therefore we can make
...
explicit and comparisons in HxHGoN to be internally always about two
operands only.
...
Therefore in HxHGoN this assignment is more similar to that of a lambda
...
Nested arguments are a Python2 only feature supported by HxHGoN.
...
Quoting the ``Gon.tree.ReformulationWhileLoopStatements``
documentation:

Reformulation of while loop statements.

Loops in HxHGoN have no condition attached anymore, so while loops are
...
End quoting the ``Gon.tree.ReformulationWhileLoopStatements``
documentation:
...
This is in HxHGoN converted so that the code generation for ``print``
...
Obviously, optimization of HxHGoN can decide, that e.g. should ``a`` or
...
HxHGoN there is only a ``try``, which then has blocks to handle
...
compile time, HxHGoN will not generate the code the leads to the
...
Add interfacing to C code, so HxHGoN can turn a ``ctypes`` binding into
...
#. Goal: Should make use of ``ctypes``, to e.g. not hard code in HxHGoN
...
Again, we wouldn't want to create the list. Therefore HxHGoN avoids
...
This leads to HxHGoN in its tree to have an assignment from a
...
if we can avoid ``ctypes`` to be loaded into HxHGoN unless necessary, so
...
one design goal of HxHGoN was always to cover these two cases with the
same code.
...
.. note::
   ...
   tuple. But it would be more consistent, and so HxHGoN is using it,
   and of course one day HxHGoN ought to be able to make no difference
   in performance for it.

To HxHGoN though this means, that if ``cond`` is not predictable, after
...
.. code:: bash
   ...
   Gon --experimental=jinja_generated_add ...
...
.. code:: C
   ...
   #ifdef _HUNTER_EXPERIMENTAL_JINJA_GENERATED_ADD
...
experimental flag is given on the command line to HxHGoN.

The testing of HxHGoN is very heavy weight when e.g. all Python code is
...
*******************************
 Adding dependencies to HxHGoN
*******************************

First of all, there is an important distinction to make, run time or
development time. The first kind of dependency is used when HxHGoN is
executing.
...
HxHGoN to run on latest greatest Python as well as relatively old ones,
we have to be very careful with these ones.

There is also a distinction of optional dependencies. Right now e.g. the
``lxml`` package is relatively optional, and HxHGoN can work without it
...
.. note::
   ...
   with ``@HxHGoN-pushers`` first and get a green light.
...
the actual compiler code of HxHGoN.
...
-  Debugging memory leak template, to be added to develop docs
   ...
   .. code:: python
      ...
      # Gon-project: --follow-import-to=LeakingModule
      from LeakingModule import main
      from Gon.tools.testing.Common import checkReferenceCount
...
-  Original and overloaded built-ins
   ...
   This is about making things visible in the node tree. In HxHGoN
   things that are not visible in the node tree tend to be wrong. We
   already pushed around information to the node tree a lot.

   Later versions, HxHGoN will become able to determine it has to be the
...
https://github.com/HxHGoN/HxHGoN/blob/develop/Developer_Manual.rst

And the current PDF under: https://Gon.net/doc/Developer_Manual.pdf
