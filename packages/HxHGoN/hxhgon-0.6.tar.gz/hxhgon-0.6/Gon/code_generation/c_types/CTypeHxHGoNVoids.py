...
class CTypeHxHGoNVoidEnum(CTypeNotReferenceCountedMixin, CTypeBase):
    c_type = "Gon_void"
    ...
    def emitValueAssertionCode(cls, value_name, emit):
        emit("assert(%s == HUNTER_VOID_OK);" % value_name)

    @classmethod
    def emitReInitCode(cls, value_name, emit):
        emit("%s = HUNTER_VOID_OK;" % value_name)
    ...
    def getExceptionCheckCondition(cls, value_name):
        return "%s == HUNTER_VOID_EXCEPTION" % value_name
