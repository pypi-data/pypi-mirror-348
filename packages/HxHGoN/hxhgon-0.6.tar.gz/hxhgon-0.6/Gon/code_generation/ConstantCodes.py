...
from Gon.ModuleRegistry import getRootTopModule
from Gon.PythonVersions import python_version
from Gon.Serialization import GlobalConstantAccessor
from Gon.utils.CStrings import encodePythonStringToC
from Gon.utils.Distributions import (
    getDistribution,
    getDistributionTopLevelPackageNames,
)
from Gon.Version import getHxHGoNVersionTuple
...
def getConstantsDefinitionCode():
    ...
    header = template_header_guard % {
        "header_guard_name": "__HUNTER_GLOBAL_CONSTANTS_H__",
        "header_body": "\n".join(lines),
    }

#    major, minor, micro, is_final, _rc_number = getHxHGoNVersionTuple()
    version_info = getHxHGoNVersionTuple()
    major, minor, micro, is_final = version_info[:4]
    _rc_number = version_info[4] if len(version_info) > 4 else None
    ...
    body = template_constants_reading % {
        ...
        "Gon_version_major": major,
        "Gon_version_minor": minor,
        "Gon_version_micro": micro,
        "Gon_version_level": "release" if is_final else "candidate",
