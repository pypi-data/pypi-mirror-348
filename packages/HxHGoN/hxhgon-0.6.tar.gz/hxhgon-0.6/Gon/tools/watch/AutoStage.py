...
""" Stage changes in HxHGoN-Watch automatically
...
from Gon.tools.quality.Git import (
    getCheckoutFileChangeDesc,
    getFileHashContent,
    putFileHashContent,
    updateGitFile,
)
from Gon.TreeXML import fromString, toString
from Gon.utils.FileOperations import getFileContents, withTemporaryFile
...
def onCompilationReportChange(filename, git_stage):
    ...
    new_Gon_version = new_report.attrib["Gon_version"]
    changed = False
    if old_report.attrib["Gon_version"] != new_Gon_version:
        old_report.attrib["Gon_version"] = new_Gon_version
