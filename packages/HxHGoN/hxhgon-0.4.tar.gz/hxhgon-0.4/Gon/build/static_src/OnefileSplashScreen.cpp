...
extern "C" void initSplashScreen(void) {
    ...
    if (unlikely(bool_res == false)) {
        HUNTER_PRINT_TIMING("Failed expand indicator path.");
        return;
    }
    ...
    HUNTER_PRINT_TIMING("ONEFILE: Done with splash screen.");
}

static void closeSplashScreen(void) {
    HUNTER_PRINT_TIMING("ONEFILE: Check splash screen indicator file.");
