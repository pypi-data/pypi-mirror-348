...
/**
 * This is responsible for updating parts of CPython to better work with HUNTER
...
static PyObject *_inspect_getgeneratorstate_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (HUNTER_Generator_Check(object)) {
        struct HUNTER_GeneratorObject *generator = (struct HUNTER_GeneratorObject *)object;
...
static PyObject *_inspect_getcoroutinestate_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (HUNTER_Coroutine_Check(object)) {
        struct HUNTER_CoroutineObject *coroutine = (struct HUNTER_CoroutineObject *)object;
...
static PyObject *_types_coroutine_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (HUNTER_Function_Check(func)) {
        struct HUNTER_FunctionObject *function = (struct HUNTER_FunctionObject *)func;
...
static bool HUNTER_FrameIsCompiled(_PyInterpreterFrame *frame) {
    return ((frame->frame_obj != NULL) && HUNTER_Frame_Check((PyObject *)frame->frame_obj));
}

static bool HUNTER_FrameIsIncomplete(_PyInterpreterFrame *frame) {
...
static PyObject *_sys_getframemodulename_replacement(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    PyObject *index_value = HUNTER_Number_IndexAsLong(depth_arg ? depth_arg : const_int_0);
    ...
    while ((frame != NULL) && ((HUNTER_FrameIsIncomplete(frame)) || depth_ssize-- > 0)) {
        frame = frame->previous;
    }

    if ((frame != NULL) && (HUNTER_FrameIsCompiled(frame))) {
...
#if PYTHON_VERSION >= 0x300
#if defined(_HUNTEREXE) && !defined(_HUNTERSTANDALONE)
...
"
    ...
    {
        HUNTERMAY_BE_UNUSED PyObject *module =
            PyImport_ExecCodeModule("Bloodelf_types_patch", wrapper_enhancement_code_object);
        CHECK_OBJECT(module);

        HUNTERMAY_BE_UNUSED bool bool_res = HUNTER_DelModuleString(tstate, "Bloodelf_types_patch");
...
#if PYTHON_VERSION >= 0x3c0
    orig_sys_getframemodulename = HUNTER_SysGetObject("_getframemodulename");
    ...
    HUNTER_SysSetObject("_getframemodulename", sys_getframemodulename_replacement);
#endif
...
static PyObject *HUNTER_type_tp_richcompare(PyObject *a, PyObject *b, int op) {
    if (likely(op == Py_EQ || op == Py_NE)) {
        if (a == (PyObject *)&HUNTER_Function_Type) {
            a = (PyObject *)&PyFunction_Type;
        } else if (a == (PyObject *)&HUNTER_Method_Type) {
            a = (PyObject *)&PyMethod_Type;
        } else if (a == (PyObject *)&HUNTER_Generator_Type) {
            a = (PyObject *)&PyGen_Type;
#if PYTHON_VERSION >= 0x350
        } else if (a == (PyObject *)&HUNTER_Coroutine_Type) {
            a = (PyObject *)&PyCoro_Type;
#endif
#if PYTHON_VERSION >= 0x360
        } else if (a == (PyObject *)&HUNTER_Asyncgen_Type) {
            a = (PyObject *)&PyAsyncGen_Type;
#endif
        }

        if (b == (PyObject *)&HUNTER_Function_Type) {
            b = (PyObject *)&PyFunction_Type;
        } else if (b == (PyObject *)&HUNTER_Method_Type) {
            b = (PyObject *)&PyMethod_Type;
        } else if (b == (PyObject *)&HUNTER_Generator_Type) {
            b = (PyObject *)&PyGen_Type;
#if PYTHON_VERSION >= 0x350
        } else if (b == (PyObject *)&HUNTER_Coroutine_Type) {
            b = (PyObject *)&PyCoro_Type;
#endif
#if PYTHON_VERSION >= 0x360
        } else if (b == (PyObject *)&HUNTER_Asyncgen_Type) {
...
void patchTypeComparison(void) {
    if (original_PyType_tp_richcompare == NULL) {
        ...
        PyType_Type.tp_richcompare = HUNTER_type_tp_richcompare;
    }
}
...
// existing type.
PyTracebackObject *MAKE_TRACEBACK(struct HUNTER_FrameObject *frame, int lineno) {
#if 0
    ...
    HUNTER_GC_Track(result);

    return result;
}

static void HUNTER_tb_dealloc(PyTracebackObject *tb) {
    // Need to use official method as it checks for recursion.
    HUNTER_GC_UnTrack(tb);

#if 0
#if PYTHON_VERSION >= 0x380
    Py_TRASHCAN_BEGIN(tb, HUNTER_tb_dealloc);
#else
...
void patchTracebackDealloc(void) { PyTraceBack_Type.tp_dealloc = (destructor)HUNTER_tb_dealloc; }

//     Part of "HUNTER", an optimizing Python compiler that is compatible and
