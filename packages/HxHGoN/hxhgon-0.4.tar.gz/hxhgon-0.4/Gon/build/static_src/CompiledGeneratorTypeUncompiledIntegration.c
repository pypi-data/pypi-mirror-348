...
#if PYTHON_VERSION >= 0x300
static PyBaseExceptionObject *HxHGoN_BaseExceptionSingleArg_new(PyThreadState *tstate, PyTypeObject *type,
                                                                PyObject *arg) {
...
static PyObject *HxHGoN_CreateStopIteration(PyThreadState *tstate, PyObject *value) {
    if (value == Py_None) {
        value = NULL;
    }

    PyStopIterationObject *result =
        (PyStopIterationObject *)HxHGoN_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_StopIteration, value);
...
static void HxHGoN_SetStopIterationValue(PyThreadState *tstate, PyObject *value) {
    ...
    if (likely(!PyTuple_Check(value) && !PyExceptionInstance_Check(value))) {
        ...
        struct HxHGoN_ExceptionPreservationItem exception_state = {Py_NewRef(PyExc_StopIteration),
                                                                   HxHGoN_CreateStopIteration(tstate, value)};

        RESTORE_ERROR_OCCURRED_STATE(tstate, &exception_state);
    }
#else
    struct HxHGoN_ExceptionPreservationItem exception_state = {HxHGoN_CreateStopIteration(tstate, value)};
...
#else
    struct HxHGoN_ExceptionPreservationItem exception_state = {HxHGoN_CreateStopIteration(tstate, NULL)};
...
#if PYTHON_VERSION >= 0x3c0
static PyObject *HxHGoN_CreateStopAsyncIteration(PyThreadState *tstate) {
    return (PyObject *)HxHGoN_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_StopAsyncIteration, NULL);
}
#endif
...
#else
    struct HxHGoN_ExceptionPreservationItem exception_state = {HxHGoN_CreateStopAsyncIteration(tstate)};
...
#if PYTHON_VERSION >= 0x3c0
static PyObject *HxHGoN_CreateGeneratorExit(PyThreadState *tstate) {
    return (PyObject *)HxHGoN_BaseExceptionSingleArg_new(tstate, (PyTypeObject *)PyExc_GeneratorExit, NULL);
}
#endif
...
#else
    struct HxHGoN_ExceptionPreservationItem exception_state = {HxHGoN_CreateGeneratorExit(tstate)};
...
#if PYTHON_VERSION >= 0x300
static bool HxHGoN_PyGen_FetchStopIterationValue(PyThreadState *tstate, PyObject **pvalue) {
#if PYTHON_VERSION < 0x3c0
...
static inline void HxHGoN_PyGen_exc_state_clear(_PyErr_StackItem *exc_state) {
...
#if PYTHON_VERSION < 0x3b0
static inline bool HxHGoN_PyFrameHasCompleted(PyFrameObject *const frame) {
...
static PyObject *HxHGoN_PyGen_Send(PyThreadState *tstate, PyGenObject *gen, PyObject *arg) {
#if PYTHON_VERSION >= 0x3a0
    PyObject *result;
...
#if PYTHON_VERSION < 0x3b0
    if (f == NULL || HxHGoN_PyFrameHasCompleted(f)) {
#else
...
#else
        HxHGoN_PyGen_exc_state_clear(&gen->gi_exc_state);
#endif
...
#if PYTHON_VERSION >= 0x300
#define HUNTER_UNCOMPILED_THROW_INTEGRATION 1
#endif
...
static PyObject *HxHGoN_PyGen_gen_close(PyThreadState *tstate, PyGenObject *gen, PyObject *args);
static int HxHGoN_PyGen_gen_close_iter(PyThreadState *tstate, PyObject *yf);
...
const uint8_t HxHGoN_PyOpcode_Deopt[256] = {
...
PyObject *HxHGoN_PyGen_yf(PyGenObject *gen) {
    ...
    if (gen->gi_frame_state < FRAME_CLEARED) {
        ...
        if (HxHGoN_PyOpcode_Deopt[_Py_OPCODE(next)] != RESUME || _Py_OPARG(next) < 2) {
            return NULL;
        }
...
static PyFrameObject *_HxHGoN_PyFrame_New_NoTrack(PyCodeObject *code) {
...
// Also not exported.
static PyFrameObject *_HxHGoN_PyFrame_MakeAndSetFrameObject(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    assert(frame->frame_obj == NULL);

    struct HxHGoN_ExceptionPreservationItem saved_exception_state;
    FETCH_ERROR_OCCURRED_STATE(tstate, &saved_exception_state);

    PyFrameObject *f = _HxHGoN_PyFrame_New_NoTrack(frame->f_code);
...
static inline PyFrameObject *_HxHGoN_PyFrame_GetFrameObject(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    ...
    return _HxHGoN_PyFrame_MakeAndSetFrameObject(tstate, frame);
}

// Also not exported, taking over a frame object.
static void _HxHGoN_take_ownership(PyThreadState *tstate, PyFrameObject *f, _PyInterpreterFrame *frame) {
    assert(frame->owner != FRAME_OWNED_BY_FRAME_OBJECT);
    assert(frame->owner != FRAME_CLEARED);
    ...
    if (prev) {
        ...
        // TODO: HxHGoN doesn't do that for compiled frames ever, is that really needed?
        PyFrameObject *back = _HxHGoN_PyFrame_GetFrameObject(tstate, prev);
    ...
    if (!_PyObject_GC_IS_TRACKED((PyObject *)f)) {
        HxHGoN_GC_Track((PyObject *)f);
    }
}

// Cleanup up the frame is also not exported.
static void _HxHGoN_PyFrame_Clear(PyThreadState *tstate, _PyInterpreterFrame *frame) {
    ...
    if (frame->frame_obj) {
        ...
        if (Py_REFCNT(f) > 1) {
            _HxHGoN_take_ownership(tstate, f, frame);
            Py_DECREF(f);
...
static PySendResult HxHGoN_PyGen_gen_send_ex2(PyThreadState *tstate, PyGenObject *gen, PyObject *arg,
...
#if PYTHON_VERSION < 0x3c0
    _HxHGoN_PyFrame_Clear(tstate, frame);
#endif
...
static PyObject *HxHGoN_PyGen_gen_send_ex(PyThreadState *tstate, PyGenObject *gen, PyObject *arg, int exc,
                                          int closing) {
    PyObject *result;

    if (HxHGoN_PyGen_gen_send_ex2(tstate, gen, arg, &result, exc, closing) == PYGEN_RETURN) {
...
//   be normalized.
static PyObject *HxHGoN_UncompiledGenerator_throw(PyThreadState *tstate, PyGenObject *gen, int close_on_genexit,
                                                  struct HxHGoN_ExceptionPreservationItem *exception_state) {
#if _DEBUG_GENERATOR
    PRINT_STRING("HxHGoN_UncompiledGenerator_throw: Enter ");
    PRINT_ITEM((PyObject *)gen);
    PRINT_EXCEPTION_STATE(exception_state);
    PRINT_NEW_LINE();
#endif

    PyObject *yf = HxHGoN_PyGen_yf(gen);
    ...
    if (yf != NULL) {
        ...
        if (close_on_genexit && EXCEPTION_STATE_MATCH_BOOL_SINGLE(tstate, exception_state, PyExc_GeneratorExit)) {
            ...
            int err = HxHGoN_PyGen_gen_close_iter(tstate, yf);
            ...
            if (err < 0) {
                ...
                return HxHGoN_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 0);
            }
        ...
        if (PyGen_CheckExact(yf) || PyCoro_CheckExact(yf)) {
            ...
            // Handing exception ownership to "HxHGoN_UncompiledGenerator_throw".
            ret = HxHGoN_UncompiledGenerator_throw(tstate, (PyGenObject *)yf, close_on_genexit, exception_state);
            gen->gi_frame_state = state;
        ...
        if (ret == NULL) {
            ...
            if (HxHGoN_PyGen_FetchStopIterationValue(tstate, &val)) {
                ret = HxHGoN_PyGen_gen_send_ex(tstate, gen, val, 0, 0);
                Py_DECREF(val);
            } else
#endif
            {
                ret = HxHGoN_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 0);
            }
        }
...
throw_here:
    ...
    return HxHGoN_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 1);
...
static PyObject *HxHGoN_PyGen_yf(PyGenObject *gen) {
    PyFrameObject *f = gen->gi_frame;
...
static PyObject *HxHGoN_UncompiledGenerator_throw(PyThreadState *tstate, PyGenObject *gen, int close_on_genexit,
    ...
    if (yf != NULL) {
        ...
        if (ret == NULL) {
            ...
            if (HxHGoN_PyGen_FetchStopIterationValue(tstate, &exception_state->exception_value)) {
                ret = HxHGoN_PyGen_gen_send_ex(tstate, gen, exception_state->exception_value, 0, 0);
...
static int HxHGoN_PyGen_gen_close_iter(PyThreadState *tstate, PyObject *yf) {
    ...
    if (PyGen_CheckExact(yf)
        ...
        assert(false);
        retval = HxHGoN_PyGen_gen_close(tstate, (PyGenObject *)yf, NULL);
...
static PyObject *HxHGoN_PyGen_gen_close(PyThreadState *tstate, PyGenObject *gen, PyObject *args) {
    ...
    if (yf != NULL) {
        ...
        gen->gi_running = 1;
#endif
        err = HxHGoN_PyGen_gen_close_iter(tstate, yf);
    ...
    PyObject *retval = HxHGoN_PyGen_gen_send_ex(tstate, gen, Py_None, 1, 1);
