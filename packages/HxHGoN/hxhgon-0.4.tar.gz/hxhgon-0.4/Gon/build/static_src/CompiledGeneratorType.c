...
#if _DEBUG_REFCOUNTS
int count_active_HxHGoN_Generator_Type;
int count_allocated_HxHGoN_Generator_Type;
int count_released_HxHGoN_Generator_Type;
#endif
...
#if _DEBUG_GENERATOR
HUNTER_MAY_BE_UNUSED static void _PRINT_GENERATOR_STATUS(char const *descriptor, char const *context,
                                                         struct HxHGoN_GeneratorObject *generator) {
    char const *status;
...
HUNTER_MAY_BE_UNUSED static void PRINT_COROUTINE_VALUE(char const *name, PyObject *value) {
    PRINT_STRING(name);
    PRINT_STRING("=");
    PRINT_ITEM(value);
    if (value) {
...
HUNTER_MAY_BE_UNUSED static void PRINT_COROUTINE_STRING(char const *name, char const *value) {
    PRINT_STRING(name);
    PRINT_STRING("=");
    PRINT_STRING(value);
    PRINT_NEW_LINE();
...
static void HxHGoN_MarkGeneratorAsFinished(struct HxHGoN_GeneratorObject *generator) {
...
static void HxHGoN_MarkGeneratorAsRunning(struct HxHGoN_GeneratorObject *generator) {
    generator->m_running = 1;

    if (generator->m_frame) {
        HxHGoN_Frame_MarkAsExecuting(generator->m_frame);
    }
}

static void HxHGoN_MarkGeneratorAsNotRunning(struct HxHGoN_GeneratorObject *generator) {
    generator->m_running = 0;

    if (generator->m_frame) {
        HxHGoN_Frame_MarkAsNotExecuting(generator->m_frame);
    }
}

static PyObject *HxHGoN_Generator_tp_repr(struct HxHGoN_GeneratorObject *generator) {
#if PYTHON_VERSION < 0x300
    return HxHGoN_String_FromFormat("<compiled_generator object %s at %p>", HxHGoN_String_AsString(generator->m_name),
                                    generator);
#elif PYTHON_VERSION < 0x350
    return HxHGoN_String_FromFormat("<compiled_generator object %U at %p>", generator->m_name, generator);

#else
    return HxHGoN_String_FromFormat("<compiled_generator object %U at %p>", generator->m_qualname, generator);
#endif
}

static long HxHGoN_Generator_tp_traverse(struct HxHGoN_GeneratorObject *generator, visitproc visit, void *arg) {
...
static void HxHGoN_Generator_release_closure(struct HxHGoN_GeneratorObject *generator) {
    for (Py_ssize_t i = 0; i < generator->m_closure_given; i++) {
...
static PyObject *_HxHGoN_CallGeneratorCodeC(PyThreadState *tstate, struct HxHGoN_GeneratorObject *generator,
                                            PyObject *send_value) {

#if PYTHON_VERSION < 0x300
    return ((generator_code)generator->m_code)(tstate, generator, send_value);
#else
    struct HxHGoN_ExceptionStackItem enter_exception = GET_CURRENT_EXCEPTION(tstate);
...
static PyObject *HxHGoN_CallGeneratorThrowMethod(PyObject *throw_method,
...
static PyObject *_HxHGoN_Generator_throw2(PyThreadState *tstate, struct HxHGoN_GeneratorObject *generator,
                                          struct HxHGoN_ExceptionPreservationItem *exception_state);
...
static PyObject *_HxHGoN_YieldFromPassExceptionTo(PyThreadState *tstate, PyObject *value,
                                                  struct HxHGoN_ExceptionPreservationItem *exception_state) {
    ...
    if (PyGen_CheckExact(value)
        ...
        PyObject *result = HxHGoN_UncompiledGenerator_throw(tstate, gen, 1, exception_state);

        return result;
    }
#endif

    if (HxHGoN_Generator_Check(value)) {
        struct HxHGoN_GeneratorObject *gen = ((struct HxHGoN_GeneratorObject *)value);

        return _HxHGoN_Generator_throw2(tstate, gen, exception_state);
    }

#if PYTHON_VERSION >= 0x350
    if (HxHGoN_Coroutine_Check(value)) {
        struct HxHGoN_CoroutineObject *coro = ((struct HxHGoN_CoroutineObject *)value);
        // Handing exception ownership over.
        return _HxHGoN_Coroutine_throw2(tstate, coro, true, exception_state);
    }

    if (HxHGoN_CoroutineWrapper_Check(value)) {
        struct HxHGoN_CoroutineObject *coro = ((struct HxHGoN_CoroutineWrapperObject *)value)->m_coroutine;
        // Handing exception ownership over.
        return _HxHGoN_Coroutine_throw2(tstate, coro, true, exception_state);
    }
#endif
    ...
    if (throw_method != NULL) {
        PyObject *result = HxHGoN_CallGeneratorThrowMethod(throw_method, exception_state);
...
static PyObject *_HxHGoN_YieldFromGeneratorCore(PyThreadState *tstate, struct HxHGoN_GeneratorObject *generator,
...
static PyObject *HxHGoN_YieldFromGeneratorCore(PyThreadState *tstate, struct HxHGoN_GeneratorObject *generator,
                                               PyObject *send_value) {
    CHECK_OBJECT(generator);
    ...
    PyObject *yielded = _HxHGoN_YieldFromGeneratorCore(tstate, generator, yield_from, send_value);
    ...
    if (yielded == NULL) {
        ...
        if (generator->m_returned != NULL) {
            ...
            yielded = _HxHGoN_CallGeneratorCodeC(tstate, generator, yield_from_result);
        } else {
            assert(HAS_ERROR_OCCURRED(tstate));
            yielded = _HxHGoN_CallGeneratorCodeC(tstate, generator, NULL);
        }
...
static PyObject *HxHGoN_YieldFromGeneratorNext(PyThreadState *tstate, struct HxHGoN_GeneratorObject *generator) {
    CHECK_OBJECT(generator);

    // Coroutines are already perfect for yielding from.
#if PYTHON_VERSION >= 0x350
    if (PyCoro_CheckExact(generator->m_yield_from) || HxHGoN_Coroutine_Check(generator->m_yield_from)) {
    ...
    return HxHGoN_YieldFromGeneratorCore(tstate, generator, Py_None);
}

static PyObject *HxHGoN_YieldFromGeneratorInitial(PyThreadState *tstate, struct HxHGoN_GeneratorObject *generator,
                                                  PyObject *send_value) {
    PyObject *result = HxHGoN_YieldFromGeneratorCore(tstate, generator, send_value);
...
static void _HxHGoN_GeneratorPopFrame(PyThreadState *tstate, HxHGoN_ThreadStateFrameType *return_frame) {
#if PYTHON_VERSION < 0x3b0
...
static void RAISE_RUNTIME_ERROR_RAISED_STOP_ITERATION(PyThreadState *tstate, char const *message) {
    ...
    struct HxHGoN_ExceptionPreservationItem new_exception_state;
    ...
    HxHGoN_Exception_SetContext(new_exception_state.exception_value, saved_exception_state.exception_value);
...
static PyObject *_HxHGoN_Generator_send(PyThreadState *tstate, struct HxHGoN_GeneratorObject *generator,
                                        PyObject *value, struct HxHGoN_ExceptionPreservationItem *exception_state) {
    CHECK_OBJECT(generator);
    assert(HxHGoN_Generator_Check((PyObject *)generator));
    ...
    if (generator->m_status != status_Finished) {
        ...
        HxHGoN_MarkGeneratorAsRunning(generator);
        ...
        if (generator->m_yield_from == NULL) {
            yielded = _HxHGoN_CallGeneratorCodeC(tstate, generator, value);
        } else {
            // This does not release the value if any, so we need to do it afterwards.
            yielded = HxHGoN_YieldFromGeneratorInitial(tstate, generator, value);
            Py_XDECREF(value);
        }
#else
        yielded = _HxHGoN_CallGeneratorCodeC(tstate, generator, value);
        ...
        while (yielded == NULL && generator->m_yield_from != NULL) {
            yielded = HxHGoN_YieldFromGeneratorNext(tstate, generator);
        }
#endif
        HxHGoN_MarkGeneratorAsNotRunning(generator);
        ...
        if (yielded == NULL) {
            ...
            PRINT_NEW_LINE();
#endif
            HxHGoN_MarkGeneratorAsFinished(generator);

            if (generator->m_frame != NULL) {
#if PYTHON_VERSION >= 0x300
                HxHGoN_SetFrameGenerator(generator->m_frame, NULL);
#endif
                Py_DECREF(generator->m_frame);
                generator->m_frame = NULL;
            }

            HxHGoN_Generator_release_closure(generator);
            ...
            if (generator->m_returned) {
                if (generator->m_returned != Py_None) {
                    HxHGoN_SetStopIterationValue(tstate, generator->m_returned);
                }
...
#if _HUNTER_MAINTAIN_SYS_EXC_VARS
...
static PyObject *HxHGoN_Generator_send(struct HxHGoN_GeneratorObject *generator, PyObject *value) {
    CHECK_OBJECT(value);
    ...
    PyObject *result = _HxHGoN_Generator_send(tstate, generator, value, &exception_state);
...
static PyObject *HxHGoN_Generator_tp_iternext(struct HxHGoN_GeneratorObject *generator) {
#if _DEBUG_GENERATOR
    PRINT_GENERATOR_STATUS("Enter", generator);
    PRINT_CURRENT_EXCEPTION();
    PRINT_NEW_LINE();
    ...
    return _HxHGoN_Generator_send(tstate, generator, Py_None, &exception_state);
}

/* Our own qiter interface, which is for quicker simple loop style iteration,
   that does not send anything in. */
PyObject *HxHGoN_Generator_qiter(PyThreadState *tstate, struct HxHGoN_GeneratorObject *generator, bool *finished) {
    Py_INCREF_IMMORTAL(Py_None);

    struct HxHGoN_ExceptionPreservationItem exception_state;
    INIT_ERROR_OCCURRED_STATE(&exception_state);

    PyObject *result = _HxHGoN_Generator_send(tstate, generator, Py_None, &exception_state);
...
static bool _HxHGoN_Generator_close(PyThreadState *tstate, struct HxHGoN_GeneratorObject *generator) {
#if _DEBUG_GENERATOR
    PRINT_GENERATOR_STATUS("Enter", generator);
#endif
    CHECK_OBJECT(generator);
    ...
    if (generator->m_status == status_Running) {
        ...
        PyObject *result = _HxHGoN_Generator_send(tstate, generator, NULL, &exception_state);
...
static PyObject *HxHGoN_Generator_close(struct HxHGoN_GeneratorObject *generator, PyObject *unused) {
    PyThreadState *tstate = PyThreadState_GET();

    bool r = _HxHGoN_Generator_close(tstate, generator);
...
#if PYTHON_VERSION >= 0x3c0
static bool _HxHGoN_Generator_check_throw_args(PyThreadState *tstate, PyObject **exception_type,
...
static bool _HxHGoN_Generator_make_throw_exception_state(PyThreadState *tstate,
                                                         struct HxHGoN_ExceptionPreservationItem *exception_state,
    ...
    if (_HxHGoN_Generator_check_throw_args(tstate, &exception_type, &exception_value, &exception_tb) == false) {
...
static bool HxHGoN_gen_close_iter(PyThreadState *tstate, PyObject *yield_from) {
#if _DEBUG_GENERATOR
    PRINT_STRING("HxHGoN_gen_close_iter: Enter\n");
#endif
    ...
    // generators.
    if (HxHGoN_Generator_Check(yield_from)) {
#if _DEBUG_GENERATOR
        PRINT_STRING("HxHGoN_gen_close_iter: Defer to _HxHGoN_Generator_close\n");
#endif
        return _HxHGoN_Generator_close(tstate, (struct HxHGoN_GeneratorObject *)yield_from);
    }

#if PYTHON_VERSION >= 0x350
    if (HxHGoN_Coroutine_Check(yield_from)) {
#if _DEBUG_GENERATOR
        PRINT_STRING("HxHGoN_gen_close_iter: Defer to _HxHGoN_Coroutine_close\n");
#endif
        return _HxHGoN_Coroutine_close(tstate, (struct HxHGoN_CoroutineObject *)yield_from);
    }
#endif

#if PYTHON_VERSION >= 0x360
    if (HxHGoN_Asyncgen_Check(yield_from)) {
#if _DEBUG_GENERATOR
        PRINT_STRING("HxHGoN_gen_close_iter: Defer to _HxHGoN_Asyncgen_close\n");
#endif
        return _HxHGoN_Asyncgen_close(tstate, (struct HxHGoN_AsyncgenObject *)yield_from);
    }
#endif
...
#if _DEBUG_GENERATOR
            PRINT_STRING("HxHGoN_gen_close_iter: Strange error while looking up close method.\n");
#endif
...
#if _DEBUG_GENERATOR
        PRINT_STRING("HxHGoN_gen_close_iter: Leave, has no close method.\n");
#endif
        return true;
    }
...
#if _DEBUG_GENERATOR
        PRINT_STRING("HxHGoN_gen_close_iter: Leave, exception from close.\n");
#endif
        return false;
    }
...
#if _DEBUG_GENERATOR
    PRINT_STRING("HxHGoN_gen_close_iter: Leave, closed.\n");
#endif
...
static PyObject *_HxHGoN_Generator_throw2(PyThreadState *tstate, struct HxHGoN_GeneratorObject *generator,
    ...
    if (generator->m_yield_from != NULL) {
        if (EXCEPTION_STATE_MATCH_BOOL_SINGLE(tstate, exception_state, PyExc_GeneratorExit)) {
            ...
            HxHGoN_MarkGeneratorAsRunning(generator);
            bool res = HxHGoN_gen_close_iter(tstate, generator->m_yield_from);
            HxHGoN_MarkGeneratorAsNotRunning(generator);
            ...
            // Transferred exception ownership to "_HxHGoN_Generator_send".
            return _HxHGoN_Generator_send(tstate, generator, NULL, exception_state);
        }
        ...
        if (HxHGoN_Generator_Check(generator->m_yield_from)) {
            struct HxHGoN_GeneratorObject *gen = ((struct HxHGoN_GeneratorObject *)generator->m_yield_from);
...
#endif
#if PYTHON_VERSION >= 0x350
        } else if (HxHGoN_Coroutine_Check(generator->m_yield_from)) {
            struct HxHGoN_CoroutineObject *coro = ((struct HxHGoN_CoroutineObject *)generator->m_yield_from);
            // Transferred exception ownership to "_HxHGoN_Coroutine_throw2".
            HxHGoN_MarkGeneratorAsRunning(generator);
            ret = _HxHGoN_Coroutine_throw2(tstate, coro, true, exception_state);
            HxHGoN_MarkGeneratorAsNotRunning(generator);
        } else if (HxHGoN_CoroutineWrapper_Check(generator->m_yield_from)) {
            struct HxHGoN_CoroutineObject *coro =
                ((struct HxHGoN_CoroutineWrapperObject *)generator->m_yield_from)->m_coroutine;
...
#endif
#if PYTHON_VERSION >= 0x360
        } else if (HxHGoN_AsyncgenAsend_Check(generator->m_yield_from)) {
            struct HxHGoN_AsyncgenAsendObject *asyncgen_asend =
                ((struct HxHGoN_AsyncgenAsendObject *)generator->m_yield_from);
        ...
        if (unlikely(ret == NULL)) {
            ...
            if (HxHGoN_PyGen_FetchStopIterationValue(tstate, &val)) {
                ...
                ret = _HxHGoN_Generator_send(tstate, generator, val, &no_exception_state);
            } else {
#if _DEBUG_GENERATOR
                ...
                ret = _HxHGoN_Generator_send(tstate, generator, NULL, &no_exception_state);
            }
...
throw_here:
    ...
    if (generator->m_status == status_Running) {
        // Passing exception ownership to _HxHGoN_Generator_send
        PyObject *result = _HxHGoN_Generator_send(tstate, generator, NULL, exception_state);
...
static PyObject *HxHGoN_Generator_throw(struct HxHGoN_GeneratorObject *generator, PyObject *args) {
    ...
    PyObject *result = _HxHGoN_Generator_throw2(tstate, generator, &exception_state);
...
static void HxHGoN_CallFinalizer(PyObject *self) {
...
static bool HxHGoN_CallFinalizerFromDealloc(PyObject *self) {
    assert(Py_REFCNT(self) == 0);

    // Temporarily resurrect the object, so it can be worked with.
    Py_SET_REFCNT(self, 1);

    HxHGoN_CallFinalizer(self);
...
static void HxHGoN_Generator_tp_finalizer(struct HxHGoN_GeneratorObject *generator) {
    if (generator->m_status != status_Running) {
        return;
    }
    ...
    bool close_result = _HxHGoN_Generator_close(tstate, generator);
...
static struct HxHGoN_GeneratorObject *free_list_generators = NULL;
static int free_list_generators_count = 0;

static void HxHGoN_Generator_tp_dealloc(struct HxHGoN_GeneratorObject *generator) {
#if _DEBUG_REFCOUNTS
    count_active_HxHGoN_Generator_Type -= 1;
    count_released_HxHGoN_Generator_Type += 1;
#endif

    if (generator->m_weakrefs != NULL) {
        HxHGoN_GC_UnTrack(generator);
        ...
        HxHGoN_GC_Track(generator);
    }

#if PYTHON_VERSION >= 0x300
    if (HxHGoN_CallFinalizerFromDealloc((PyObject *)generator) == false) {
        return;
    }
#else
    // Revive temporarily.
...
static long HxHGoN_Generator_tp_hash(struct HxHGoN_GeneratorObject *generator) { return generator->m_counter; }

static PyObject *HxHGoN_Generator_get_name(PyObject *self, void *data) {
    struct HxHGoN_GeneratorObject *generator = (struct HxHGoN_GeneratorObject *)self;
...
#if PYTHON_VERSION >= 0x350
static int HxHGoN_Generator_set_name(PyObject *self, PyObject *value, void *data) {
...
static PyObject *HxHGoN_Generator_get_qualname(PyObject *self, void *data) {
    struct HxHGoN_GeneratorObject *generator = (struct HxHGoN_GeneratorObject *)self;
...
static int HxHGoN_Generator_set_qualname(PyObject *self, PyObject *value, void *data) {
    // Cannot be deleted, not be non-unicode value.
    if (unlikely((value == NULL) || !PyUnicode_Check(value))) {
        PyThreadState *tstate = PyThreadState_GET();
...
static PyObject *HxHGoN_Generator_get_yield_from(PyObject *self, void *data) {
    struct HxHGoN_GeneratorObject *generator = (struct HxHGoN_GeneratorObject *)self;
...
static PyObject *HxHGoN_Generator_get_code(PyObject *self, void *data) {
    struct HxHGoN_GeneratorObject *generator = (struct HxHGoN_GeneratorObject *)self;
...
static int HxHGoN_Generator_set_code(PyObject *self, PyObject *value, void *data) {
    PyThreadState *tstate = PyThreadState_GET();

    SET_CURRENT_EXCEPTION_TYPE0_STR(tstate, PyExc_RuntimeError, "gi_code is not writable in HxHGoN");
    return -1;
}

static PyObject *HxHGoN_Generator_get_frame(PyObject *self, void *data) {
    PyObject *result;
...
static int HxHGoN_Generator_set_frame(PyObject *self, PyObject *value, void *data) {
    PyThreadState *tstate = PyThreadState_GET();
...
static PyObject *HxHGoN_Generator_get_running(PyObject *self, void *data) {
    PyObject *result;

    struct HxHGoN_GeneratorObject *generator = (struct HxHGoN_GeneratorObject *)self;
/* The type of "gi_running" changed in Python3. */
#if PYTHON_VERSION < 0x300
    result = HxHGoN_PyInt_FromLong(generator->m_running);
#else
...
static int HxHGoN_Generator_set_running(PyObject *self, PyObject *value, void *data) {
#if PYTHON_VERSION < 0x300
...
#if !defined(_HUNTER_FULL_COMPAT) || PYTHON_VERSION >= 0x3a0
...
static PyGetSetDef HxHGoN_Generator_tp_getset[] = {
#if PYTHON_VERSION < 0x350
    {(char *)"__name__", HxHGoN_Generator_get_name, NULL, NULL},
#else
    {(char *)"__name__", HxHGoN_Generator_get_name, HxHGoN_Generator_set_name, NULL},
    {(char *)"__qualname__", HxHGoN_Generator_get_qualname, HxHGoN_Generator_set_qualname, NULL},
    {(char *)"gi_yieldfrom", HxHGoN_Generator_get_yield_from, NULL, NULL},
#endif
    {(char *)"gi_code", HxHGoN_Generator_get_code, HxHGoN_Generator_set_code, NULL},
    {(char *)"gi_frame", HxHGoN_Generator_get_frame, HxHGoN_Generator_set_frame, NULL},
    {(char *)"gi_running", HxHGoN_Generator_get_running, HxHGoN_Generator_set_running, NULL},

    {NULL}};

static PyMethodDef HxHGoN_Generator_methods[] = {{"send", (PyCFunction)HxHGoN_Generator_send, METH_O, NULL},
                                                 {"throw", (PyCFunction)HxHGoN_Generator_throw, METH_VARARGS, NULL},
                                                 {"close", (PyCFunction)HxHGoN_Generator_close, METH_NOARGS, NULL},
                                                 {NULL}};

// This is only used.
#if PYTHON_VERSION >= 0x3a0
static PyAsyncMethods HxHGoN_Generator_as_async = {
    NULL, /* am_await */
    NULL, /* am_aiter */
    NULL, /* am_anext */
    // TODO: have this too, (sendfunc)_HxHGoN_Generator_am_send
    NULL /* am_send */
};
#endif

PyTypeObject HxHGoN_Generator_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "compiled_generator", // tp_name
    sizeof(struct HxHGoN_GeneratorObject),               // tp_basicsize
    sizeof(struct HxHGoN_CellObject *),                  // tp_itemsize
    (destructor)HxHGoN_Generator_tp_dealloc,             // tp_dealloc
    0,                                                   // tp_print
    0,                                                   // tp_getattr
    0,                                                   // tp_setattr
#if PYTHON_VERSION < 0x3a0
    0, // tp_as_async
#else
    &HxHGoN_Generator_as_async, // tp_as_async
#endif
    (reprfunc)HxHGoN_Generator_tp_repr, // tp_repr
    0,                                  // tp_as_number
    0,                                  // tp_as_sequence
    0,                                  // tp_as_mapping
    (hashfunc)HxHGoN_Generator_tp_hash, // tp_hash
    0,                                  // tp_call
    0,                                  // tp_str
    ...
    // tp_flags
    0,                                                   // tp_doc
    (traverseproc)HxHGoN_Generator_tp_traverse,          // tp_traverse
    0,                                                   // tp_clear
    0,                                                   // tp_richcompare
    offsetof(struct HxHGoN_GeneratorObject, m_weakrefs), // tp_weaklistoffset
    0,                                                   // tp_iter (PyObject_SelfIter)
    (iternextfunc)HxHGoN_Generator_tp_iternext,          // tp_iternext
    HxHGoN_Generator_methods,                            // tp_methods
    NULL,                                                // tp_members
    HxHGoN_Generator_tp_getset,                          // tp_getset
    0,                                                   // tp_base
    0,                                                   // tp_dict
    0,                                                   // tp_descr_get
    0,                                                   // tp_descr_set
...
#if PYTHON_VERSION >= 0x300
    ,
    (destructor)HxHGoN_Generator_tp_finalizer // tp_finalize
#endif
};
...
void _initCompiledGeneratorType(void) {
    HxHGoN_PyType_Ready(&HxHGoN_Generator_Type, &PyGen_Type, true, false, true, false, false);

    // Be a paranoid subtype of uncompiled function, we want nothing shared.
    assert(HxHGoN_Generator_Type.tp_doc != PyGen_Type.tp_doc || PyGen_Type.tp_doc == NULL);
    assert(HxHGoN_Generator_Type.tp_traverse != PyGen_Type.tp_traverse);
    assert(HxHGoN_Generator_Type.tp_clear != PyGen_Type.tp_clear || PyGen_Type.tp_clear == NULL);
    assert(HxHGoN_Generator_Type.tp_richcompare != PyGen_Type.tp_richcompare || PyGen_Type.tp_richcompare == NULL);
    assert(HxHGoN_Generator_Type.tp_iter != PyGen_Type.tp_iter || PyGen_Type.tp_iter == PyObject_SelfIter);
    assert(HxHGoN_Generator_Type.tp_iternext != PyGen_Type.tp_iternext || PyGen_Type.tp_iternext == NULL);
#if PYTHON_VERSION >= 0x350
    assert(HxHGoN_Generator_Type.tp_as_async != PyGen_Type.tp_as_async || PyGen_Type.tp_as_async == NULL);
#endif
    assert(HxHGoN_Generator_Type.tp_methods != PyGen_Type.tp_methods);
    assert(HxHGoN_Generator_Type.tp_members != PyGen_Type.tp_members);
    assert(HxHGoN_Generator_Type.tp_getset != PyGen_Type.tp_getset);
    assert(HxHGoN_Generator_Type.tp_base != PyGen_Type.tp_base);
    assert(HxHGoN_Generator_Type.tp_dict != PyGen_Type.tp_dict);
    assert(HxHGoN_Generator_Type.tp_descr_get != PyGen_Type.tp_descr_get || PyGen_Type.tp_descr_get == NULL);

    assert(HxHGoN_Generator_Type.tp_descr_set != PyGen_Type.tp_descr_set || PyGen_Type.tp_descr_set == NULL);
    assert(HxHGoN_Generator_Type.tp_dictoffset != PyGen_Type.tp_dictoffset || PyGen_Type.tp_dictoffset == 0);
    // TODO: These get changed and into the same thing, not sure what to compare against, project something
    // assert(HxHGoN_Generator_Type.tp_init != PyGen_Type.tp_init || PyGen_Type.tp_init == NULL);
    // assert(HxHGoN_Generator_Type.tp_alloc != PyGen_Type.tp_alloc || PyGen_Type.tp_alloc == NULL);
    // assert(HxHGoN_Generator_Type.tp_new != PyGen_Type.tp_new || PyGen_Type.tp_new == NULL);
    // assert(HxHGoN_Generator_Type.tp_free != PyGen_Type.tp_free || PyGen_Type.tp_free == NULL);
    assert(HxHGoN_Generator_Type.tp_bases != PyGen_Type.tp_bases);
    assert(HxHGoN_Generator_Type.tp_mro != PyGen_Type.tp_mro);
    assert(HxHGoN_Generator_Type.tp_cache != PyGen_Type.tp_cache || PyGen_Type.tp_cache == NULL);
    assert(HxHGoN_Generator_Type.tp_subclasses != PyGen_Type.tp_subclasses || PyGen_Type.tp_cache == NULL);
    assert(HxHGoN_Generator_Type.tp_weaklist != PyGen_Type.tp_weaklist);
    assert(HxHGoN_Generator_Type.tp_del != PyGen_Type.tp_del || PyGen_Type.tp_del == NULL);
#if PYTHON_VERSION >= 0x300
    assert(HxHGoN_Generator_Type.tp_finalize != PyGen_Type.tp_finalize || PyGen_Type.tp_finalize == NULL);
#endif
...
PyObject *HxHGoN_Generator_New(generator_code code, PyObject *module, PyObject *name,
#if PYTHON_VERSION >= 0x350
                               PyObject *qualname,
#endif
...
#if _DEBUG_REFCOUNTS
    count_active_HxHGoN_Generator_Type += 1;
    count_allocated_HxHGoN_Generator_Type += 1;
#endif

    struct HxHGoN_GeneratorObject *result;
    ...
    allocateFromFreeList(free_list_generators, struct HxHGoN_GeneratorObject, HxHGoN_Generator_Type, full_size);
...
#if PYTHON_VERSION >= 0x300
    result->m_resume_exception = HxHGoN_ExceptionStackItem_Empty;
#endif

    static long HxHGoN_Generator_counter = 0;
    result->m_counter = HxHGoN_Generator_counter++;

    HxHGoN_GC_Track(result);
    return (PyObject *)result;
}

static PyObject *_EMPTY_GENERATOR_CONTEXT(PyThreadState *tstate, struct HxHGoN_GeneratorObject *generator,
                                          PyObject *yield_return_value) {
    return NULL;
}

PyObject *HxHGoN_Generator_NewEmpty(PyObject *module, PyObject *name,
#if PYTHON_VERSION >= 0x350
                                    PyObject *qualname,
#endif
                                    PyCodeObject *code_object, struct HxHGoN_CellObject **closure,
                                    Py_ssize_t closure_given) {
    return HxHGoN_Generator_New(_EMPTY_GENERATOR_CONTEXT, module, name,
#if PYTHON_VERSION >= 0x350
                                qualname,
#endif
