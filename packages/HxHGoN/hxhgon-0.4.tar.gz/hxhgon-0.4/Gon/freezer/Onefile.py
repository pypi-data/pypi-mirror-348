...
from Gon.Options import getOnefileTempDirSpec, isOnefileTempDirMode
from Gon.OutputDirectories import getResultFullpath
from Gon.plugins.Plugins import Plugins
from Gon.PostProcessing import executePostProcessingResources
from Gon.PythonVersions import (
    getZstandardSupportingVersions,
    python_version,
)
from Gon.Tracing import onefile_logger, postprocessing_logger
from Gon.utils.Execution import withEnvironmentVarsOverridden
...
from Gon.utils.SharedLibraries import cleanupHeaderForAndroid
from Gon.utils.Signing import addMacOSCodeSignature
...
from Gon.utils.WindowsResources import RT_RCDATA, addResourceToFile
...
def _runOnefileScons(onefile_compression, onefile_archive):
    ...
    env_values["_HUNTER_ONEFILE_TEMP_SPEC"] = getOnefileTempDirSpec()
    env_values["_HUNTER_ONEFILE_COMPRESSION_BOOL"] = "1" if onefile_compression else "0"
    env_values["_HUNTER_ONEFILE_ARCHIVE_BOOL"] = "1" if onefile_archive else "0"
...
def runOnefileCompressor(
    ...
    if compressor_python is None or areSamePaths(
        ...
        from Gon.tools.onefile_compressor.OnefileCompressor import (
        ...
        mapping = {
            "HUNTER_PACKAGE_HOME": os.path.dirname(
                os.path.abspath(sys.modules["Gon"].__path__[0])
            )
        }

        mapping["HUNTER_PROGRESS_BAR"] = "1" if Options.shallUseProgressBar() else "0"
