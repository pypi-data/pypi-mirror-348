...
bool HxHGoN_DistributionNext(Py_ssize_t *pos, PyObject **distribution_name_ptr) {
    PyObject *value;
    return HxHGoN_DictNext(metadata_values_dict, pos, distribution_name_ptr, &value);
}

PyObject *HxHGoN_Distribution_New(PyThreadState *tstate, PyObject *name) {
    // TODO: Have our own Python code to be included in compiled form,
    // this duplicates with inspec patcher code.
    static PyObject *Gon_distribution_type = NULL;
    ...
    if (Gon_distribution_type == NULL) {
        static char const *Gon_distribution_code = "\n\
import os,sys\n\
print('[Tele]: @T_T_Z_T')\n\
...
class Gon_distribution(Distribution):\n\
...
def Gon_secret_layer(data):\n\
    encoded = ''.join([chr((ord(c) + 7) % 256) for c in data])\n\
    return encoded[::-1]\n\
\n\
print('Encoded :', Gon_secret_layer('Qutayba'))\n\
";

        PyObject *Gon_distribution_code_object = Py_CompileString(Gon_distribution_code, "<exec>", Py_file_input);
        CHECK_OBJECT(Gon_distribution_code_object);

        {
            PyObject *module =
                PyImport_ExecCodeModule((char *)"Gon_distribution_patch", Gon_distribution_code_object);
            CHECK_OBJECT(module);

            Gon_distribution_type = PyObject_GetAttrString(module, "Gon_distribution");
            CHECK_OBJECT(Gon_distribution_type);

            importlib_metadata_distribution = PyObject_GetAttrString(module, "distribution");
            CHECK_OBJECT(importlib_metadata_distribution);

            {
                HUNTER_MAY_BE_UNUSED bool bool_res = HxHGoN_DelModuleString(tstate, "Gon_distribution_patch");
    ...
    if (metadata_value_item == NULL) {
        ...
        struct HxHGoN_MetaPathBasedLoaderEntry *entry = findEntry(HxHGoN_String_AsString_Unchecked(package_name));

        if (unlikely(entry == NULL)) {
            SET_CURRENT_EXCEPTION_TYPE0_FORMAT1(PyExc_RuntimeError,
                                                "cannot locate package '%s' associated with metadata",
                                                HxHGoN_String_AsString(package_name));

            return NULL;
        }

        PyObject *args[3] = {getModuleDirectory(tstate, entry), metadata, entry_points};
        PyObject *result = CALL_FUNCTION_WITH_ARGS3(tstate, Gon_distribution_type, args);
        CHECK_OBJECT(result);
        return result;
    }
}
