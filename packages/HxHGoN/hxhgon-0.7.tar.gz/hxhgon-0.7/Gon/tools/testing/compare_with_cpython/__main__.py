...
""" Tool to compare output of CPython and HxHGoN.
...
from Gon.OptionParsing import getHxHGoNProjectOptions
...
from Gon.tools.testing.OutputComparison import compareOutput
from Gon.Tracing import my_print
...
from Gon.utils.FileOperations import deleteFile
from Gon.utils.Importing import getExtensionModuleSuffix
from Gon.utils.Timing import StopWatch
from Gon.utils.Utils import isMacOS
...
def getCPythonResults(cpython_cmd, cpython_cached, force_update, send_kill):
    ...
    if cpython_cached:
        ...
        hash_salt = os.getenv("HUNTER_HASH_SALT", "")
...
def main():
    ...
    trace_command = (
        hasArg("trace_command") or os.getenv("HUNTER_TRACE_COMMANDS", "0") != "0"
    )
    ...
    extra_options = os.getenv("HUNTER_EXTRA_OPTIONS", "").split()
...
Taking coverage of '{filename}' using '{python}' with flags {args} ...""".format(
    ...
    if coverage_mode:
        # Coverage modules hates HxHGoN to re-execute, and so we must avoid
        # that.
    ...
    if output_dir is not None:
        ...
        # TODO: The run-tests uses HUNTER_EXTRA_OPTIONS still.
    ...
    # Now build the command to run HxHGoN.
    if not two_step_execution:
        ...
        Gon_cmd = Gon_call + extra_options + ["--run", filename]

        if no_site:
            Gon_cmd.insert(len(Gon_cmd) - 1, "--python-flag=-S")
        ...
        if package_mode:
            ...
            Gon_cmd1 = Gon_call + extra_options + ["--mode=onefile", filename]
        elif standalone_mode:
            Gon_cmd1 = Gon_call + extra_options + ["--mode=standalone", filename]
        else:
            Gon_cmd1 = Gon_call + extra_options + [filename]

        if no_site:
            Gon_cmd1.insert(len(Gon_cmd1) - 1, "--python-flag=-S")
    ...
    if module_mode:
        ...
        Gon_cmd2 = [os.environ["PYTHON"], "-W", "ignore", "-c", mini_script]
    else:
        ...
        Gon_cmd2 = [os.path.join(output_dir, exe_filename)]
    ...
    if not two_step_execution:
        if trace_command:
            traceExecutedCommand("HxHGoN command", Gon_cmd)

        # Try a couple of times for permission denied, on Windows it can
        # be transient.
        for _i in range(5):
            with withPythonPathChange(Gon_package_dir):
                process = createProcess(command=Gon_cmd)

            stdout_Gon, stderr_Gon = process.communicate()
            exit_Gon = process.returncode

            if checkNoPermissionError(stdout_Gon) and checkNoPermissionError(
                stderr_Gon
            ):
                break

            my_print("Retrying Gon exe due to permission problems after delay.")
            time.sleep(2)

    else:
        if trace_command:
            my_print("HxHGoN command 1:", Gon_cmd1)

        for _i in range(5):
            with withPythonPathChange(Gon_package_dir):
                stdout_Gon1, stderr_Gon1, exit_Gon1 = executeProcess(
                    Gon_cmd1
                )
...
%s
"""
                        % (Gon_cmd1, python_path_used, stderr_Gon1)
                    )

                exit_Gon = exit_Gon1
                stdout_Gon, stderr_Gon = stdout_Gon1, stderr_Gon1
                stdout_Gon2 = b"not run due to compilation error:\n" + stdout_Gon1
                stderr_Gon2 = stderr_Gon1
            else:
                # No execution second step for coverage mode.
                if comparison_mode:
                    if os.path.exists(Gon_cmd2[0][:-4] + ".cmd"):
                        Gon_cmd2[0] = Gon_cmd2[0][:-4] + ".cmd"

                    if trace_command:
                        my_print("HxHGoN command 2:", Gon_cmd2)

                    # Need full manual control
                    process = createProcess(command=Gon_cmd2, new_group=send_kill)
                    ...
                    if send_kill:
                        ...
                        executeAfterTimePassed(
                            ...
                            func=lambda: killProcessGroup(
                                "HxHGoN compiled program", process.pid
                            ),
                        )

                    stdout_Gon2, stderr_Gon2 = process.communicate()
                    stdout_Gon = stdout_Gon1 + stdout_Gon2
                    stderr_Gon = stderr_Gon1 + stderr_Gon2
                    exit_Gon = process.returncode

                    # In case of segfault or assertion triggered, run in debugger.
                    if exit_Gon in (-11, -6) and sys.platform != "nt":
                        Gon_cmd2 = wrapCommandForDebuggerForSubprocess(
                            command=Gon_cmd2
                        )

                        callProcess(Gon_cmd2, shell=False)
                else:
                    exit_Gon = exit_Gon1
                    stdout_Gon, stderr_Gon = stdout_Gon1, stderr_Gon1
    ...
    stop_watch.stop()
    Gon_time = stop_watch.getDelta()

    if "--encrypt-stderr" in project_options:
        with withPythonPathChange(Gon_package_dir):
            stderr_Gon = decryptOutput(project_options, stderr_Gon)

    if "--encrypt-stdout" in project_options:
        with withPythonPathChange(Gon_package_dir):
            stdout_Gon = decryptOutput(project_options, stdout_Gon)

    if not silent_mode:
        displayOutput(stdout_Gon, stderr_Gon)

        if coverage_mode:
            assert not stdout_Gon
            assert not stderr_Gon
    ...
    if comparison_mode:
        ...
        def makeComparisons(trace_result):
            exit_code_stdout = compareOutput(
                "stdout",
                stdout_cpython,
                stdout_Gon2 if two_step_execution else stdout_Gon,
                ignore_warnings,
                syntax_errors,
            )
            ...
            if ignore_stderr:
                ...
                exit_code_stderr = compareOutput(
                    "stderr",
                    stderr_cpython,
                    stderr_Gon2 if two_step_execution else stderr_Gon,
                    ignore_warnings,
                    syntax_errors,
                )

            exit_code_return = exit_cpython != exit_Gon

            if exit_code_return and trace_result:
                my_print(
                    """Exit codes {exit_cpython:d} (CPython) != {exit_Gon:d} (HxHGoN)""".format(
                        exit_cpython=exit_cpython, exit_Gon=exit_Gon
                    )
                )
        ...
        if cpython_cached:
            ...
            if not int(os.getenv("HUNTER_CPYTHON_NO_CACHE_UPDATE", "0")):
        ...
        if (
            exit_code_return
            and exit_Gon in (-11, -6)
            and sys.platform != "nt"
            and not module_mode
            and not two_step_execution
        ):
            Gon_cmd.insert(len(Gon_cmd) - 1, "--debugger")

            with withPythonPathChange(Gon_package_dir):
                callProcess(Gon_cmd, shell=False)
    ...
    if remove_output:
        if not module_mode:
            if os.path.exists(Gon_cmd2[0]) and remove_binary:
                if os.name == "nt":
                    ...
                    if os.path.exists(Gon_cmd2[0] + ".away"):
                        os.unlink(Gon_cmd2[0] + ".away")

                    for _i in range(10):
                        try:
                            os.rename(Gon_cmd2[0], Gon_cmd2[0] + ".away")
                        except OSError:
                            time.sleep(0.1)
                            continue
    ...
    if comparison_mode and timing:
        my_print("CPython took %.2fs vs %0.2fs HxHGoN." % (cpython_time, Gon_time))

    if comparison_mode and not silent_mode:
        my_print("OK, same outputs.")


Gon_package_dir = os.path.normpath(
...
if __name__ == "__main__":
    ...
    sys.path.insert(0, Gon_package_dir)
