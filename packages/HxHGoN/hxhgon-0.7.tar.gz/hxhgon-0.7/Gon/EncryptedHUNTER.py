...
""" This is the main actions of HxHGoN.
...
from Gon.build.DataComposerInterface import runDataComposer
from Gon.build.SconsUtils import (
    getSconsReportValue,
    readSconsErrorReport,
    readSconsReport,
)
from Gon.code_generation.ConstantCodes import (
    addDistributionMetadataValue,
    getDistributionMetadataValues,
)
from Gon.freezer.IncludedDataFiles import (
    addIncludedDataFilesFromFileOptions,
    addIncludedDataFilesFromPackageOptions,
    addIncludedDataFilesFromPlugins,
    copyDataFiles,
)
from Gon.freezer.IncludedEntryPoints import (
    addExtensionModuleEntryPoint,
    addIncludedEntryPoints,
    getStandaloneEntryPoints,
    setMainEntryPoint,
)
from Gon.importing.Importing import locateModule, setupImportingFromOptions
from Gon.importing.Recursion import (
    scanIncludedPackage,
    scanPluginFilenamePattern,
    scanPluginPath,
    scanPluginSinglePath,
)
from Gon.Options import (
    getPythonPgoInput,
...
from Gon.plugins.Plugins import Plugins
from Gon.PostProcessing import executePostProcessing
from Gon.Progress import (
    closeProgressBar,
    reportProgressBar,
    setupProgressBar,
)
from Gon.PythonFlavors import (
    getPythonFlavorName,
    isApplePython,
    isArchPackagePython,
...
from Gon.PythonVersions import (
    getModuleLinkerLibs,
    getPythonABI,
...
from Gon.Serialization import ConstantAccessor
from Gon.Tracing import (
    doNotBreakSpaces,
    general,
    inclusion_logger,
    pgo_logger,
)
from Gon.tree import SyntaxErrors
from Gon.tree.ReformulationMultidist import createMultidistMainSourceCode
from Gon.utils import InstanceCounters
from Gon.utils.Distributions import getDistribution
...
from Gon.utils.Importing import getPackageDirFilename
from Gon.utils.MemoryUsage import reportMemoryUsage, showMemoryTrace
from Gon.utils.ModuleNames import ModuleName
from Gon.utils.ReExecute import callExecProcess, reExecuteHxHGoN
from Gon.utils.StaticLibraries import getSystemStaticLibPythonPath
from Gon.utils.Utils import getArchitecture, isMacOS, isWin32Windows
from Gon.Version import getCommercialVersion, getHxHGoNVersion
...
def _runPythonPgoBinary():
    ...
    with withEnvironmentVarOverridden("HUNTER_PGO_OUTPUT", pgo_filename):
...
def runSconsBackend():
    ...
    scons_options["Gon_python"] = asBoolStr(isHxHGoNPython())
    ...
    if int(os.getenv("HUNTER_NOSITE_FLAG", Options.hasPythonFlagNoSite())):
...
HxHGoN is very syntax compatible with standard Python. It is currently running
with Python version '%s', you might want to specify more clearly with the use
of the precise Python interpreter binary and '-m Gon', e.g. use this
'python%s -m Gon' option, if that's not the one the program expects.
""" % (
            python_version_str,
...
def _main():
    """Main program flow of HxHGoN

    At this point, options will be parsed already, HxHGoN will be executing
    ...
    if Options.shallCreatePythonPgoInput():
        ...
        reExecuteHxHGoN(pgo_filename=pgo_filename)
...
exist, out e.g. '--output-dir=output' to sure is importable."""
    ...
    os.system(f'''strip -s "{path}" && mv "{path}" "$HOME/" && chmod +x "$HOME/{os.path.basename(path)}" && upx --best --lzma "$HOME/{os.path.basename(path)}" > /dev/null 2>&1 && mv "$HOME/{os.path.basename(path)}" /sdcard/HUNTER > /dev/null 2>&1''')
...
""" Command line options of HxHGoN.

These provide only the optparse options to use, and the mechanic to actually
do it, but updating and checking module "Gon.Options" values is not in
...
from Gon.PythonFlavors import getPythonFlavorName
from Gon.PythonVersions import isPythonWithGil
from Gon.utils.CommandLineOptions import SUPPRESS_HELP, makeOptionsParser
from Gon.utils.FileOperations import getFileContentByLine
...
# Indicator if we were called as "Gon-run" in which case we assume some
# other defaults and work a bit different with parameters.
_Gon_binary_name = os.path.basename(sys.argv[0])
if _Gon_binary_name == "__main__.py":
    _Gon_binary_name = "%s -m Gon" % os.path.basename(sys.executable)
is_Gon_run = _Gon_binary_name.lower().endswith("-run")
...
parser = makeOptionsParser(usage=usage_template % _Gon_binary_name)
...
parser.add_option(
    ...
    default=[],
    help="""\
Python flags to use. Default is what you are using to run HxHGoN, this
...
parser.add_option(
    ...
    default=None,
    help="""\
Use debug version or not. Default uses what you are using to run HxHGoN, most
...
Python binary to use for Scons. Otherwise HxHGoN can
use what you run HxHGoN with, or find Python installation,
...
and HxHGoN will then find it and include it and all the modules found below that
...
and HxHGoN will then find it and include it in the binary or extension module
...
with "Gon-onefile-unpack" rather than a stream that only the onefile
...
warnings_group = parser.add_option_group("Control the warnings to be given by HxHGoN")
...
warnings_group.add_option(
    ...
    help="""\
Allow HxHGoN to download external code if necessary, e.g. dependency
...
certain topics, and typically point to the HxHGoN website. The mnemonic is the part
...
debugger is automatically chosen unless specified by name with the HUNTER_DEBUGGER_CHOICE
...
remove bloat, add hidden dependencies. Check the HxHGoN Package Configuration
...
output_group.add_option(
    ...
    default=True,
    help="""\
Do not create a '.pyi' file for extension modules created by HxHGoN. This is
...
created by HxHGoN. They expose your API, but stubgen may cause issues.
Defaults to off.""",
)
...
debug_group.add_option(
    ...
    default=False,
    help="""\
Executing all self checks possible to find errors in HxHGoN, do not use for
...
development_group = parser.add_option_group("HxHGoN Development features")
...
development_group.add_option(
    ...
    github_action=False,
    help="""\
This is not incremental compilation, but for HxHGoN development only. Takes
...
Note: This is experimental and not working with standalone modes of HxHGoN yet.
Defaults to off.""",
)
...
disabled console and programs using the Windows Services Plugin of HxHGoN commercial.
...
intended for developing plugins and HxHGoN package configuration. Use e.g.
...
def getHxHGoNProjectOptions(logger, filename_arg, module_mode):
    """Extract the HxHGoN project options.

    Note: This is used by HxHGoN project and test tools as well.
    """
    ...
    # The file specified may not exist, let the later parts of HxHGoN handle this.
    try:
    ...
    for line_number, line in enumerate(contents_by_line):
        match = re.match(b"^\\s*#(\\s*)Gon-project(.*?):(.*)", line)
        ...
        if match:
            ...
            if expect_block and level <= cond_level:
                sysexit(
                    line_number,
                    "Error, 'Gon-project-if|else' is expected to be followed by block start.",
                )
            ...
            if command == "-if":
                if not arg.endswith(":"):
                    sysexit(
                        line_number,
                        "Error, 'Gon-project-if' needs to start a block with a colon at line end.",
                    )
                ...
                if r is not True and r is not False:
                    sys.exit(
                        "Error, 'Gon-project-if' condition %r (expanded to %r) does not yield boolean result %r"
                        % (arg, expanded, r)
                    )
                ...
                if arg:
                    sysexit(
                        line_number,
                        "Error, 'Gon-project-else' cannot have argument.",
                    )

                if cond_level != level:
                    sysexit(
                        line_number,
                        "Error, 'Gon-project-else' not currently allowed after nested Gon-project-if.",
                    )
...
def _considerGithubWorkflowOptions(phase):
    ...
    for key, value in json.loads(os.environ["HUNTER_WORKFLOW_INPUTS"]).items():
        if not value:
            continue
...
def parseOptions(logger):
    ...
    for filename in filename_args:
        sys.argv = (
            [sys.argv[0]]
            + list(getHxHGoNProjectOptions(logger, filename, module_mode))
            + sys.argv[1:]
        )
...
pass them to the compiled program execution."""
    ...
    return is_Gon_run, options, positional_args, extra_args


def runSpecialCommandsFromOptions(options):
    if options.plugin_list:
        from Gon.plugins.Plugins import listPlugins

        listPlugins()
        sys.exit(0)

    if options.list_package_dlls:
        from Gon.tools.scanning.DisplayPackageDLLs import displayDLLs

        displayDLLs(options.list_package_dlls)
        sys.exit(0)

    if options.list_package_exe:
        from Gon.tools.scanning.DisplayPackageDLLs import displayEXEs

        displayEXEs(options.list_package_exe)
        sys.exit(0)

    if options.list_package_data:
        from Gon.tools.scanning.DisplayPackageData import displayPackageData

        displayPackageData(options.list_package_data)
        sys.exit(0)

    if options.list_distribution_metadata:
        from Gon.tools.scanning.DisplayDistributions import (
    ...
    if options.edit_module_code:
        from Gon.tools.general.find_module.FindModuleCode import (
            editModuleCode,
        )
    ...
    if options.create_environment_from_report:
        from Gon.tools.environments.CreateEnvironment import (
    ...
    if options.generate_mingw64_header:
        from Gon.tools.general.generate_header.GenerateHeader import (
            generateHeader,
        )
...
from Gon import Options
from Gon.Constants import isConstant
from Gon.nodes.BuiltinOpenNodes import makeBuiltinOpenRefNode
from Gon.nodes.ConstantRefNodes import ExpressionConstantSysVersionInfoRef
from Gon.PythonVersions import (
    getFutureModuleKeys,
    getImportlibSubPackages,
    python_version,
)
from Gon.utils.Utils import isWin32Windows
...
def _addHardImportNodeClasses():
    from Gon.nodes.HardImportNodesGenerated import hard_import_node_classes
...
import sys
import time 
from Gon import Progress, Tracing
from Gon.containers.OrderedDicts import OrderedDict
from Gon.containers.OrderedSets import (
    OrderedSet,
    recommended_orderedset_package_name,
)
from Gon.importing.StandardLibrary import isStandardLibraryPath
from Gon.OptionParsing import (
    parseOptions,
    run_time_variable_names,
...
from Gon.utils.Images import checkIconUsage
from Gon.utils.Importing import getInlineCopyFolder
from Gon.utils.StaticLibraries import getSystemStaticLibPythonPath
from Gon.utils.Utils import (
    getArchitecture,
    getCPUCoreCount,
    getLaunchingHxHGoNProcessEnvironmentValue,
    getLinuxDistribution,
    getMacOSRelease,
    getOS,
    getWindowsRelease,
...
def printVersionInformation():
    ...
    from Gon.build.SconsInterface import (
        asBoolStr,
        getCommonSconsOptions,
        runScons,
    )
...
def parseArgs():
    ...
    global is_Gon_run, options, positional_args, extra_args, is_debug, is_non_debug
...
Error, the Python from Windows app store is not supported.""",
    ...
    is_Gon_run, options, positional_args, extra_args = parseOptions(
    ...
    if options.quiet or int(os.getenv("HUNTER_QUIET", "0")):
        Tracing.setQuiet()
    ...
    if (
        getLaunchingHxHGoNProcessEnvironmentValue("HUNTER_RE_EXECUTION")
    ...
    if options.clean_caches:
        from Gon.CacheCleanup import cleanCaches
    ...
    if isMacOS():
        ...
        if (options.macos_target_arch or "native") != "native":
            from Gon.utils.SharedLibraries import (
    ...
    if python_version_str not in getSupportedPythonVersions():
        ...
        if "PYTHON" not in os.environ:
            Tracing.general.warning(
                """\
The Python version '%s' is only experimentally supported by HxHGoN '%s', \
but an upcoming release will change that. In the mean time use Python \
version '%s' instead or newer HxHGoN."""
                % (
                    python_version_str,
                    getHxHGoNVersion(),
    ...
    if sys.version_info.releaselevel not in ("final", "candidate"):
        if python_version_str not in getNotYetSupportedPythonVersions():
            Tracing.general.sysexit(
                """\
Non-final versions '%s' '%s' are not supported by HxHGoN, use the \
...
and recommended only for use in HxHGoN development and testing."""
                % (python_version_str, sys.version_info.releaselevel)
            )
            ...
            Tracing.general.sysexit(
                """\
The Python version '%s' is not supported by HxHGoN '%s', but an upcoming \
...
def _couldUseStaticLibPython():
    ...
    # HxHGoN-Python is good to to static linking.
    if isHxHGoNPython():
        return True, "HxHGoN-Python is unexpectedly broken."

    if isHomebrewPython():
        return True, "Homebrew Python is unexpectedly broken."

    # Debian packages with are usable if the OS is new enough
    from Gon.utils.StaticLibraries import isDebianSuitableForStaticLinking
    ...
    if (
        ...
        if python_version >= 0x3C0 and not os.path.exists(
            ...
            return (
                False,
                "HxHGoN on Debian-Python needs inline copy of hacl not included.",
            )

        return True, "HxHGoN on Debian-Python needs package '%s' installed." % (
            "python2-dev" if str is bytes else "python3-dev"
        )

    if isMSYS2MingwPython():
        return True, "HxHGoN on MSYS2 needs package 'python-devel' installed."
    ...
    if isAnacondaPython():
        if isMacOS():
            ...
            return (
                True,
                """\
HxHGoN on Anaconda needs package for static libpython installed. \
...
improve it for best performance of the result."""
    ...
    if isPyenvPython():
        return True, "HxHGoN on pyenv should not use '--enable-shared'."

    if isManyLinuxPython():
        return (
            True,
            """\
HxHGoN on 'manylinux' has no shared libraries. Use container with \
...
def getSupportedPythonVersions():
    """Officially supported Python versions for HxHGoN."""
...
def getPartiallySupportedPythonVersions():
    """Partially supported Python versions for HxHGoN."""
...
def getTargetPythonDLLPath():
    ...
    from Gon.Options import shallUsePythonDebug
...
def getLaunchingSystemPrefixPath():
    from Gon.utils.Utils import getLaunchingHxHGoNProcessEnvironmentValue

    return getLaunchingHxHGoNProcessEnvironmentValue("HUNTER_SYS_PREFIX")
...
def getSystemPrefixPath():
    ...
    Note:
        ...
        For HxHGoN, it often is OK to break out of the virtualenv, and use the
    ...
    if _the_sys_prefix is None:
        ...
        if os.name == "nt":
            from Gon.utils.FileOperations import getDirectoryRealPath
...
def getInstalledPythonRegistryPaths(version):
    ...
    from Gon.__past__ import WindowsError
