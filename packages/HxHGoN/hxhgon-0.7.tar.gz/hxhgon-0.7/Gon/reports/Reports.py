...
from Gon import TreeXML
from Gon.__past__ import unicode
from Gon.build.DataComposerInterface import getDataComposerReportValues
from Gon.build.SconsUtils import readSconsErrorReport
from Gon.code_generation.ConstantCodes import getDistributionMetadataValues
from Gon.containers.OrderedSets import OrderedSet
from Gon.freezer.IncludedDataFiles import getIncludedDataFiles
from Gon.freezer.IncludedEntryPoints import getStandaloneEntryPoints
from Gon.freezer.Standalone import getRemovedUsedDllsInfo
from Gon.importing.Importing import getPackageSearchPath
from Gon.importing.Recursion import getRecursionDecisions
...
from Gon.OutputDirectories import (
    getResultRunFilename,
    getSourceDirectoryPath,
    hasMainModule,
)
from Gon.plugins.Plugins import getActivePlugins
...
from Gon.Tracing import ReportingSystemExit, reports_logger
...
from Gon.utils.MemoryUsage import getMemoryInfos
...
def _getReportInputData(aborted):
    ...
    Gon_version = getHxHGoNVersion()
    Gon_commercial_version = getCommercialVersion() or "not installed"

    Gon_aborted = aborted

    Gon_exception = sys.exc_info()
...
def writeCompilationReport(report_filename, report_input_data, diffable):
    ...
    exit_message = None
    if not report_input_data["Gon_aborted"]:
        completion = "yes"
    elif report_input_data["Gon_exception"][0] is KeyboardInterrupt:
        completion = "interrupted"
    elif report_input_data["Gon_exception"][0] is SystemExit:
        completion = "error exit (%s)" % report_input_data["Gon_exception"][1].code
    elif report_input_data["Gon_exception"][0] is ReportingSystemExit:
        completion = (
            "error exit message (%s)" % report_input_data["Gon_exception"][1].code
        )
        exit_message = report_input_data["Gon_exception"][1].exit_message
    else:
        completion = "exception"

    root = TreeXML.Element(
        "Gon-compilation-report",
        Gon_version=report_input_data["Gon_version"],
        Gon_commercial_version=report_input_data["Gon_commercial_version"],
...
_crash_report_filename = "Gon-crash-report.xml"
...
def writeCompilationReports(aborted):
    ...
    if (
        ...
        # of reporting exit, these do not constitute definitive bugs of HxHGoN but
