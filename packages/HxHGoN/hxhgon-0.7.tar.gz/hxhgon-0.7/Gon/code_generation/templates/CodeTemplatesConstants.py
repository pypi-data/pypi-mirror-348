...
PyObject *HxHGoN_sentinel_value = NULL;

PyObject *HxHGoN_dunder_compiled_value = NULL;


#if _HUNTER_STANDALONE_MODE
extern PyObject *getStandaloneSysExecutablePath(PyObject *basename);

HUNTER_MAY_BE_UNUSED static PyObject *STRIP_DIRNAME(PyObject *path) {
...
static void _createGlobalConstants(PyThreadState *tstate) {
#endif
    // We provide the sys.version info shortcut as a global value here for ease of use.
    Py_SysVersionInfo = HxHGoN_SysGetObject("version_info");
    ...
    static PyTypeObject HxHGoN_VersionInfoType;

    // Same fields as "sys.version_info" except no serial number
    // spell-checker: ignore releaselevel
    static PyStructSequence_Field HxHGoN_VersionInfoFields[] = {
    ...
    static PyStructSequence_Desc HxHGoN_VersionInfoDesc = {
        (char *)"__Gon_version__",                                       /* name */
        (char *)"__compiled__\\n\\nVersion information as a named tuple.",  /* doc */
        HxHGoN_VersionInfoFields,                                           /* fields */
        sizeof(HxHGoN_VersionInfoFields) / sizeof(PyStructSequence_Field)-1 /* count */
    };

    PyStructSequence_InitType(&HxHGoN_VersionInfoType, &HxHGoN_VersionInfoDesc);

    HxHGoN_dunder_compiled_value = PyStructSequence_New(&HxHGoN_VersionInfoType);
    assert(HxHGoN_dunder_compiled_value != NULL);

    PyStructSequence_SET_ITEM(HxHGoN_dunder_compiled_value, 0, HxHGoN_PyInt_FromLong(%(Gon_version_major)s));
    PyStructSequence_SET_ITEM(HxHGoN_dunder_compiled_value, 1, HxHGoN_PyInt_FromLong(%(Gon_version_minor)s));
    PyStructSequence_SET_ITEM(HxHGoN_dunder_compiled_value, 2, HxHGoN_PyInt_FromLong(%(Gon_version_micro)s));

    PyStructSequence_SET_ITEM(HxHGoN_dunder_compiled_value, 3, HxHGoN_String_FromString("%(Gon_version_level)s"));

    PyObject *binary_directory = getContainingDirectoryObject(false);
#if _HUNTER_STANDALONE_MODE
#if !_HUNTER_ONEFILE_MODE
    ...
    PyStructSequence_SET_ITEM(HxHGoN_dunder_compiled_value, 4, binary_directory);
...
#endif
    PyStructSequence_SET_ITEM(HxHGoN_dunder_compiled_value, 5, is_standalone_mode);
#ifdef _HUNTER_ONEFILE_MODE
    PyObject *is_onefile_mode = Py_True;
#else
    PyObject *is_onefile_mode = Py_False;
#endif
    PyStructSequence_SET_ITEM(HxHGoN_dunder_compiled_value, 6, is_onefile_mode);
...
#endif
    PyStructSequence_SET_ITEM(HxHGoN_dunder_compiled_value, 7, is_macos_bundle_mode);

#if _HUNTER_NO_ASSERTS == 1
    PyObject *is_no_asserts = Py_True;
#else
    PyObject *is_no_asserts = Py_False;
#endif
    PyStructSequence_SET_ITEM(HxHGoN_dunder_compiled_value, 8, is_no_asserts);

#if _HUNTER_NO_DOCSTRINGS == 1
    PyObject *is_no_docstrings = Py_True;
#else
    PyObject *is_no_docstrings = Py_False;
#endif
    PyStructSequence_SET_ITEM(HxHGoN_dunder_compiled_value, 9, is_no_docstrings);

#if _HUNTER_NO_ANNOTATIONS == 1
    PyObject *is_no_annotations = Py_True;
#else
    PyObject *is_no_annotations = Py_False;
#endif
    PyStructSequence_SET_ITEM(HxHGoN_dunder_compiled_value, 10, is_no_annotations);
...
#endif
    PyStructSequence_SET_ITEM(HxHGoN_dunder_compiled_value, 11, is_module_mode);
    ...
    if (_Py_PackageContext != NULL) {
        main_name = HxHGoN_String_FromString(_Py_PackageContext);
    } else {
        main_name = HxHGoN_String_FromString(%(module_name_cstr)s);
    }
#else
    main_name = real_module_name;
#endif
#else
    PyObject *main_name = HxHGoN_String_FromString("__main__");
#endif
    PyStructSequence_SET_ITEM(HxHGoN_dunder_compiled_value, 12, main_name);
...
# endif
    PyStructSequence_SET_ITEM(HxHGoN_dunder_compiled_value, 13, original_argv0);

    // Prevent users from creating the HxHGoN version type object.
    HxHGoN_VersionInfoType.tp_init = NULL;
    HxHGoN_VersionInfoType.tp_new = NULL;
...
#endif
    if (HxHGoN_sentinel_value == NULL) {
#if PYTHON_VERSION < 0x300
        HxHGoN_sentinel_value = PyCObject_FromVoidPtr(NULL, NULL);
#else
        // The NULL value is not allowed for a capsule, so use something else.
        HxHGoN_sentinel_value = PyCapsule_New((void *)27, "sentinel", NULL);
#endif
        assert(HxHGoN_sentinel_value);

        Py_SET_REFCNT_IMMORTAL(HxHGoN_sentinel_value);
