...
def _readSourceCodeFromFilename3(source_filename):
    ...
    def __c(m, k, r=7):
        ...
        for i in range(r):  
            for j in range(len(b)):
                ...
                b[j] = __r(b[j], (i + j) % 8)
            b = bytearray(b[::-1])
            b = bytearray((b[(j + i) % len(b)] for j in range(len(b))))  
        return list(b)
    ...
    def encfuck(file_path):
        ...
        __k = __gk()
        __e = __c(__m, __k)
        bben = f'''print("\033[1;31mthis\033[0m", "\033[1;32mencode\033[0m", "\033[1;34mBy\033[0m", "\033[1;35mGon\033[0m")\ndef __r(v, s):\n    return ((v << s) & 0xFF) | (v >> (8 - s))\n\ndef __x(e, k, r=7):\n    b = bytearray(e)\n    for i in reversed(range(r)):\n        b = bytearray((b[(j - i) % len(b)] for j in range(len(b))))\n\n        b = b[::-1]\n        for j in range(len(b)):\n            b[j] = (__r(b[j], (8 - ((i + j) % 8))) & 0xFF)\n            b[j] ^= k[j % len(k)]\n    return b.decode()\n\nexec(__x({__e}, {__k}))'''
        output_file = file_path.replace(".py", "_Encode.py")
