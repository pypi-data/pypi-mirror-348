...
static PyObject *HxHGoN_CapsuleNew(void *pointer) {
...
#define HxHGoN_CapsuleGetPointer(capsule) (((HxHGoN_PyCapsule *)(capsule))->pointer)

#else
#define HxHGoN_CapsuleGetPointer(capsule) (PyCObject_AsVoidPtr(capsule))
#endif

#if PYTHON_VERSION >= 0x3a0
PyTypeObject *HxHGoN_PyUnion_Type;
#endif

static PyObject *_makeDeepCopyFunctionCapsule(copy_func func) { return HxHGoN_CapsuleNew((void *)func); }
...
static void _initDeepCopy(void) {
...
#if PYTHON_VERSION >= 0x3a0
    {
        ...
        HxHGoN_PyUnion_Type = Py_TYPE(union_value);

        PyDict_SetItem(_deep_copy_dispatch, (PyObject *)HxHGoN_PyUnion_Type, _deep_noop);
...
static PyObject *DEEP_COPY_ITEM(PyThreadState *tstate, PyObject *value, PyTypeObject **type, copy_func *copy_function) {
    *type = Py_TYPE(value);
    ...
    if (unlikely(dispatcher == NULL)) {
        HUNTER_CANNOT_GET_HERE("DEEP_COPY encountered unknown type");
    }
    ...
    if (dispatcher == Py_None) {
        ...
        *copy_function = (copy_func)(HxHGoN_CapsuleGetPointer(dispatcher));
...
PyObject *DEEP_COPY(PyThreadState *tstate, PyObject *value) {
    ...
    if (dispatcher == Py_None) {
        ...
        copy_func copy_function = (copy_func)(HxHGoN_CapsuleGetPointer(dispatcher));
        return copy_function(tstate, value);
    }
...
Py_hash_t DEEP_HASH(PyThreadState *tstate, PyObject *value) {
    ...
    if (PyType_Check(value)) {
        ...
        while (HxHGoN_DictNext(value, &pos, &key, &dict_value)) {
...
#endif
#if PYTHON_VERSION >= 0x3a0
    } else if (Py_TYPE(value) == HxHGoN_PyUnion_Type) {
        Py_hash_t result = DEEP_HASH_INIT(tstate, value);
        ...
        HUNTER_CANNOT_GET_HERE("Unknown type hashed");
...
void CHECK_OBJECT_DEEP(PyObject *value) {
    ...
    if (PyTuple_Check(value)) {
        ...
        while (HxHGoN_DictNext(value, &pos, &dict_key, &dict_value)) {
...
static PyObject *_DEEP_COPY_ELEMENT_GUIDED(PyThreadState *tstate, PyObject *value, char const **guide) {
    ...
    *guide += 1;
    ...
    default:
        HUNTER_CANNOT_GET_HERE("Illegal type guide");
        abort();
    }
}
