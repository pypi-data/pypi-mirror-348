...
#if HUNTER_FLOAT_HAS_FREELIST

static PyFloatObject *_HxHGoN_AllocatePyFloatObject(PyThreadState *tstate) {
    ...
    if (result_float) {
        ...
        *free_list = (PyFloatObject *)Py_TYPE(result_float);
        ...
        result_float = (PyFloatObject *)HxHGoNObject_Malloc(sizeof(PyFloatObject));
    }

    Py_SET_TYPE(result_float, &PyFloat_Type);
    HxHGoN_Py_NewReference((PyObject *)result_float);
...
PyObject *MAKE_FLOAT_FROM_DOUBLE(double value) {
    ...
    PyFloatObject *result = _HxHGoN_AllocatePyFloatObject(tstate);
