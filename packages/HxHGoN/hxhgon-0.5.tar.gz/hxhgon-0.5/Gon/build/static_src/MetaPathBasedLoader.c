...
extern PyTypeObject HxHGoN_Loader_Type;

struct HxHGoN_LoaderObject {
...
#elif _HUNTER_SYSFLAG_VERBOSE
static inline bool isVerbose(void) { return true; }
#else
static inline bool isVerbose(void) { return false; }
#endif

static struct HxHGoN_MetaPathBasedLoaderEntry *loader_entries = NULL;
...
#if defined(_WIN32) && _HUNTER_STANDALONE_MODE

static void appendModuleNameAsPathW(wchar_t *buffer, PyObject *module_name, size_t buffer_size) {
    Py_ssize_t size;
    wchar_t const *module_name_wstr = HxHGoN_UnicodeAsWideString(module_name, &size);
...
HUNTER_MAY_BE_UNUSED static PyObject *MAKE_RELATIVE_PATH_FROM_NAME(char const *name, bool is_package, bool dir_only) {
    ...
    PyObject *module_path_entry_base = HxHGoN_String_FromString(buffer);
...
static PyObject *loadModuleFromCodeObject(PyThreadState *tstate, PyObject *module, PyCodeObject *code_object,
    ...
    {
        HUNTER_MAY_BE_UNUSED bool b_res = HxHGoN_SetModuleString(name, module);
        assert(b_res != false);
    }
    ...
    PyObject *module_path_name = HxHGoN_String_FromString(buffer);
    ...
    module = PyImport_ExecCodeModuleEx((char *)name, (PyObject *)code_object, HxHGoN_String_AsString(module_path));
...
static struct HxHGoN_MetaPathBasedLoaderEntry *findEntry(char const *name) {
    struct HxHGoN_MetaPathBasedLoaderEntry *current = loader_entries;
    assert(current);

    while (current->name != NULL) {
        if ((current->flags & HUNTER_TRANSLATED_FLAG) != 0) {
            current->name = UN_TRANSLATE(current->name);
            current->flags -= HUNTER_TRANSLATED_FLAG;
        }
...
#if !_HUNTER_STANDALONE_MODE
static struct HxHGoN_MetaPathBasedLoaderEntry *findContainingPackageEntry(char const *name) {
    struct HxHGoN_MetaPathBasedLoaderEntry *current = loader_entries;
    ...
    while (current->name != NULL) {
        ...
        if ((current->flags & HUNTER_PACKAGE_FLAG) != 0) {
...
static bool scanModuleInPackagePath(PyThreadState *tstate, PyObject *module_name, char const *parent_module_name) {
    PyObject *sys_modules = HxHGoN_GetSysModules();
    ...
    char const *module_relative_name_str = HxHGoN_String_AsString(module_name) + strlen(parent_module_name) + 1;
    ...
    for (Py_ssize_t i = 0; i < parent_path_size; i += 1) {
        ...
        for (Py_ssize_t j = 0; j < filenames_list_size; j += 1) {
            ...
            if (HxHGoN_String_CheckExact(filename)) {
                char const *filename_str = HxHGoN_String_AsString(filename);
    ...
    PRINT_STRING(HxHGoN_String_AsString(module_name));
    ...
    for (Py_ssize_t i = 0; i < PyList_GET_SIZE(suffix_list); i += 1) {
        ...
        char const *suffix_str = HxHGoN_String_AsString(suffix);
        ...
        for (Py_ssize_t j = 0; j < PyList_GET_SIZE(candidates); j += 1) {
            ...
            char const *candidate_str = HxHGoN_String_AsString(candidate);
...
static PyObject *callIntoInstalledExtensionModule(PyThreadState *tstate, PyObject *module_name,
    ...
    wchar_t const *extension_module_filename_str = HxHGoN_UnicodeAsWideString(extension_module_filename, NULL);
#else
    char const *extension_module_filename_str = HxHGoN_String_AsString(extension_module_filename);
#endif

    // TODO: The value of "is_package" is guessed, maybe infer from filename being
    // a "__init__.so" and the like.
    return callIntoExtensionModule(tstate, HxHGoN_String_AsString(module_name), extension_module_filename_str, false);
}

#endif

static char const *getEntryModeString(struct HxHGoN_MetaPathBasedLoaderEntry const *entry) {
    char const *mode = "compiled";

    if ((entry->flags & HUNTER_EXTENSION_MODULE_FLAG) != 0) {
        mode = "extension";
    } else if ((entry->flags & HUNTER_BYTECODE_FLAG) != 0) {
        mode = "bytecode";
    }
...
static PyObject *_Gon_loader_find_module(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    char const *name = HxHGoN_String_AsString(module_name);

    if (isVerbose()) {
        PySys_WriteStderr("import %s # considering responsibility (find_module)\n", name);
    }

    struct HxHGoN_MetaPathBasedLoaderEntry *entry = findEntry(name);
    ...
    if (entry) {
        ...
        PyObject *metapath_based_loader = (PyObject *)&HxHGoN_Loader_Type;
...
static PyObject *_Gon_loader_get_data(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *filename;
...
static void setModuleFileValue(PyThreadState *tstate, PyObject *module, filename_char_t const *filename) {
    ...
    PyObject *new_file_value = HxHGoN_String_FromFilename(filename);
...
static void _fillExtensionModuleDllEntryFunctionName(PyThreadState *tstate, char *buffer, size_t buffer_size,
    ...
    PyObject *name_obj = BYTES_DECODE2(tstate, name_bytes_obj, HxHGoN_String_FromString("utf8"));
...
#if PYTHON_VERSION >= 0x3c0 && defined(_HUNTER_USE_UNEXPOSED_API)
extern _Thread_local const char *pkgcontext;
#endif

static const char *HxHGoNImport_SwapPackageContext(const char *new_context) {
...
static PyObject *callIntoExtensionModule(PyThreadState *tstate, char const *full_name, const filename_char_t *filename,
...
#ifndef _HUNTER_EXPERIMENTAL_DEBUG_STANDALONE
    ...
    if (dlopenflags_object == NULL) {
        dlopenflags_object = CALL_FUNCTION_NO_ARGS(tstate, HxHGoN_SysGetObject("getdlopenflags"));
    }
    ...
    char const *old_context = HxHGoNImport_SwapPackageContext(package);
    ...
    if (base_name != NULL) {
        base_name_obj = HxHGoN_String_FromString(base_name + 1);
        preserved_basename_module = HxHGoN_GetModule(tstate, base_name_obj);
        Py_XINCREF(preserved_basename_module);

        prefix_name_obj = HxHGoN_String_FromStringAndSize(full_name, base_name - full_name + 1);
    }
#endif
    ...
    HxHGoN_DelModuleString(tstate, full_name);
    ...
    HxHGoNImport_SwapPackageContext(old_context);

#if PYTHON_VERSION < 0x300
    PyObject *module = HxHGoN_GetModuleString(tstate, full_name);
#endif
    ...
    if (Py_TYPE(module) == &PyModuleDef_Type) {
        ...
        PyObject *full_name_obj = HxHGoN_String_FromString(full_name);

        PyObject *origin = HxHGoN_String_FromFilename(filename);
        ...
        HxHGoN_SetModule(full_name_obj, module);
        Py_DECREF(full_name_obj);

#if _HUNTER_EXPERIMENTAL_OLD_EXTENSION_MODULE_LOADING
        ...
        if (package != NULL) {
            ...
            if (package_name == Py_None) {
                ...
                PyObject *package_name_obj = HxHGoN_String_FromString(package2);
    ...
    PyObject *filename_obj = HxHGoN_String_FromFilename(filename);
...
// the package context.
...
#if _HUNTER_EXPERIMENTAL_DEBUG_EXTENSION_MODULE_PRESERVATION_HACK
        PRINT_STRING("Restoring preserved module: ");
        PRINT_ITEM(base_name_obj);
        if (HxHGoN_HasModule(tstate, base_name_obj)) {
            PRINT_STRING(" changes ");
            PRINT_ITEM(HxHGoN_GetModule(tstate, base_name_obj));
        }
        PRINT_STRING(" -> ");
        PRINT_ITEM_LINE(preserved_basename_module);
#endif
        HxHGoN_SetModule(base_name_obj, preserved_basename_module);
    ...
    if (base_name_obj != NULL) {
        ...
        {
            PyObject *modules_dict = HxHGoN_GetSysModules();
...
#endif
            while (HxHGoN_DictNext(modules_dict, &pos, &key, &value)) {
                // TODO: Should have Gon_bool return values for these as well maybe.
        ...
        for (Py_ssize_t i = 0; i < n; i++) {
            ...
            PyObject *module_to_correct = HxHGoN_GetModule(tstate, module_to_correct_name);
...
#endif
            HxHGoN_SetModule(correct_module_name, module_to_correct);

            HxHGoN_DelModule(tstate, module_to_correct_name);
        }
    ...
    int res = _PyImport_FixupExtensionObject(module, full_name_obj, filename_obj
                                             ...
                                             ,
                                             HxHGoN_GetSysModules()
#endif
    );
#endif

#if PYTHON_VERSION >= 0x3d0
    HxHGoN_SetModuleString(full_name, module);
#endif
...
static void loadTriggeredModule(PyThreadState *tstate, char const *name, char const *trigger_name) {
    ...
    struct HxHGoN_MetaPathBasedLoaderEntry *entry = findEntry(trigger_module_name);
    ...
    if (entry != NULL) {
        ...
        if (unlikely(HAS_ERROR_OCCURRED(tstate))) {
            if ((entry->flags & HUNTER_ABORT_MODULE_FLAG) != 0) {
                printf("Critical error loading %s.\n", trigger_module_name);
                abort();
            } else {
                PyObject *trigger_module_name_str = HxHGoN_String_FromString(trigger_module_name);
...
static PyObject *loadModule(PyThreadState *tstate, PyObject *module, PyObject *module_name,
                            struct HxHGoN_MetaPathBasedLoaderEntry const *entry) {
#if _HUNTER_STANDALONE_MODE
    if ((entry->flags & HUNTER_EXTENSION_MODULE_FLAG) != 0) {
        bool is_package = (entry->flags & HUNTER_PACKAGE_FLAG) != 0;
...
#endif
        if ((entry->flags & HUNTER_BYTECODE_FLAG) != 0) {
        ...
        return loadModuleFromCodeObject(tstate, module, code_object, entry->name,
                                        (entry->flags & HUNTER_PACKAGE_FLAG) != 0);
    } else {
        assert((entry->flags & HUNTER_EXTENSION_MODULE_FLAG) == 0);
        assert(entry->python_init_func);

        {
            HUNTER_MAY_BE_UNUSED bool res = HxHGoN_SetModule(module_name, module);
            assert(res != false);
        }

        // Run the compiled module code, we get the module returned.
#if PYTHON_VERSION < 0x300
        HUNTER_MAY_BE_UNUSED
#endif
    ...
    return HxHGoN_GetModule(tstate, module_name);
}
...
static PyObject *_EXECUTE_EMBEDDED_MODULE(PyThreadState *tstate, PyObject *module, PyObject *module_name,
    ...
    if (entry != NULL) {
#ifdef _HUNTER_EXPERIMENTAL_FORCE_GC_COLLECT_ON_IMPORT
        PyGC_Collect();
#endif
    ...
    if (frozen_import) {
        ...
        if (res == 1) {
            result = HxHGoN_GetModule(tstate, module_name);
        }
    }
...
PyObject *IMPORT_EMBEDDED_MODULE(PyThreadState *tstate, char const *name) {
    PyObject *module_name = HxHGoN_String_FromString(name);

    // Check if it's already loaded, and don't do it again otherwise.
    PyObject *module = HxHGoN_GetModule(tstate, module_name);
...
#if PYTHON_VERSION < 0x350
    if (unlikely(result == NULL)) {
        HxHGoN_DelModule(tstate, module_name);
    }
#endif
...
static PyObject *_Gon_loader_load_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    PyObject *unused;
    ...
    assert(module_name);
    assert(HxHGoN_String_Check(module_name));
...
static PyObject *_Gon_loader_is_package(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    if (entry) {
        result = BOOL_FROM((entry->flags & HUNTER_PACKAGE_FLAG) != 0);
    } else {
...
static PyObject *_Gon_loader_iter_modules(struct HxHGoN_LoaderObject *self, PyObject *args, PyObject *kwds) {
    PyObject *prefix;
    ...
    while (current->name != NULL) {
        ...
        PyObject *name;
        if (self->m_loader_entry) {
            name = HxHGoN_String_FromString(current->name + strlen(s) + 1);
        } else {
            name = HxHGoN_String_FromString(current->name);
        }
        ...
        PyTuple_SET_ITEM_IMMORTAL(r, 1, BOOL_FROM((current->flags & HUNTER_PACKAGE_FLAG) != 0));
...
static PyObject *getModuleDirectory(PyThreadState *tstate, struct HxHGoN_MetaPathBasedLoaderEntry const *entry) {
    ...
    PyObject *dir_name = HxHGoNUnicode_FromWideChar(buffer, -1);
#else
    char buffer[1024];
    copyStringSafe(buffer, entry->file_path, sizeof(buffer));

    PyObject *dir_name = HxHGoN_String_FromString(dirname(buffer));
#endif
#else
    PyObject *module_name;
    if ((entry->flags & HUNTER_PACKAGE_FLAG) != 0) {
        module_name = HxHGoN_String_FromString(entry->name);
    } else {
        char buffer[1024];
        ...
        module_name = HxHGoN_String_FromString(buffer);
    }
...
static PyObject *getModuleFileValue(PyThreadState *tstate, struct HxHGoN_MetaPathBasedLoaderEntry const *entry) {
    ...
    PyObject *module_filename = HxHGoN_String_FromString(filename_buffer);
...
static PyObject *_Gon_loader_repr_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module;
    PyObject *unused;
    ...
    return PyUnicode_FromFormat("<module '%s' from %R>", PyModule_GetName(module),
                                HxHGoN_GetFilenameObject(tstate, module));
}
...
static PyObject *createModuleSpec(PyThreadState *tstate, PyObject *module_name, PyObject *origin, bool is_package) {
    ...
    PyObject *args = MAKE_TUPLE2(tstate, module_name, (PyObject *)&HxHGoN_Loader_Type);
...
static PyObject *_Gon_loader_find_spec(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    char const *full_name = HxHGoN_String_AsString(module_name);

    if (isVerbose()) {
        PySys_WriteStderr("import %s # considering responsibility (find_spec)\n", full_name);
    }

    struct HxHGoN_MetaPathBasedLoaderEntry const *entry = findEntry(full_name);
    ...
    if (isVerbose()) {
        PySys_WriteStderr("import %s # claimed responsibility (%s)\n", HxHGoN_String_AsString(module_name),
...
static PyObject *_Gon_loader_create_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *spec;
...
#if !_HUNTER_STANDALONE_MODE && !_HUNTER_EXPERIMENTAL_OLD_EXTENSION_MODULE_LOADING
    if (installed_extension_modules != NULL) {
        PyObject *extension_module_filename = DICT_GET_ITEM0(tstate, installed_extension_modules, module_name);
...
#if _HUNTER_EXPERIMENTAL_OLD_EXTENSION_MODULE_LOADING
    struct HxHGoN_MetaPathBasedLoaderEntry *entry = NULL;
#else
    struct HxHGoN_MetaPathBasedLoaderEntry *entry = findEntry(name);
#endif
    ...
    if ((entry != NULL) && ((entry->flags & HUNTER_EXTENSION_MODULE_FLAG) != 0)) {
...
static PyObject *_Gon_loader_exec_module(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module;
...
static PyObject *_Gon_loader_get_resource_reader(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *module_name;
    ...
    if (entry) {
        ...
        return HxHGoN_ResourceReader_New(entry);
    }
...
static PyObject *_Gon_loader_find_distributions(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *context;
    ...
    while (HxHGoN_DistributionNext(&pos, &distribution_name)) {
        bool include = false;
        if (name == Py_None) {
            include = true;
        } else {
            Gon_bool cmp_res = RICH_COMPARE_EQ_NBOOL_OBJECT_OBJECT(name, distribution_name);

            if (unlikely(cmp_res == HUNTER_BOOL_EXCEPTION)) {
                Py_DECREF(name);
                return NULL;
            }

            include = cmp_res == HUNTER_BOOL_TRUE;
        }

        if (include) {
            // Create a distribution object from our data.
            PyObject *distribution = HxHGoN_Distribution_New(tstate, distribution_name);
...
static PyObject *_Gon_loader_sys_path_hook(PyObject *self, PyObject *args, PyObject *kwds) {
    PyObject *path;
    ...
    struct HxHGoN_MetaPathBasedLoaderEntry *entry = loader_entries;
    assert(entry);

    PyThreadState *tstate = PyThreadState_GET();

    while (entry->name != NULL) {
        if ((entry->flags & HUNTER_TRANSLATED_FLAG) != 0) {
            entry->name = UN_TRANSLATE(entry->name);
            entry->flags -= HUNTER_TRANSLATED_FLAG;
        }
        ...
        if ((entry->flags & HUNTER_PACKAGE_FLAG) != 0) {
            ...
            Gon_bool cmp_res = compareFilePaths(tstate, module_directory, path);
            ...
            if (cmp_res == HUNTER_BOOL_TRUE) {
                return HxHGoN_Loader_New(entry);
            }
        }
...
static PyMethodDef HxHGoN_Loader_methods[] = {
    {"iter_modules", (PyCFunction)_Gon_loader_iter_modules, METH_VARARGS | METH_KEYWORDS, NULL},
    {"get_data", (PyCFunction)_Gon_loader_get_data, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"find_module", (PyCFunction)_Gon_loader_find_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"load_module", (PyCFunction)_Gon_loader_load_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"is_package", (PyCFunction)_Gon_loader_is_package, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#if PYTHON_VERSION >= 0x300
    {"module_repr", (PyCFunction)_Gon_loader_repr_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"find_spec", (PyCFunction)_Gon_loader_find_spec, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#endif
#if PYTHON_VERSION >= 0x350
    {"create_module", (PyCFunction)_Gon_loader_create_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
    {"exec_module", (PyCFunction)_Gon_loader_exec_module, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},
#endif
#if PYTHON_VERSION >= 0x370
    {"get_resource_reader", (PyCFunction)_Gon_loader_get_resource_reader, METH_STATIC | METH_VARARGS | METH_KEYWORDS,
     NULL},
#endif

    {"find_distributions", (PyCFunction)_Gon_loader_find_distributions, METH_STATIC | METH_VARARGS | METH_KEYWORDS,
     NULL},

    {"sys_path_hook", (PyCFunction)_Gon_loader_sys_path_hook, METH_STATIC | METH_VARARGS | METH_KEYWORDS, NULL},

    {NULL, NULL} // terminator
};

static PyObject *HxHGoN_Loader_tp_repr(struct HxHGoN_LoaderObject *loader) {
    if (loader->m_loader_entry == NULL) {
        // TODO: Indicate in module mode, which one it is for.
        return HxHGoN_String_FromString("<Gon_module_loader>");
    } else {
        return HxHGoN_String_FromFormat("<Gon_module_loader for '%s'>", loader->m_loader_entry->name);
    }
}
...
static struct HxHGoN_LoaderObject *free_list_loaders = NULL;
static int free_list_loaders_count = 0;

static void HxHGoN_Loader_tp_dealloc(struct HxHGoN_LoaderObject *loader) {
    HxHGoN_GC_UnTrack(loader);

    releaseToFreeList(free_list_loaders, loader, MAX_LOADER_FREE_LIST_COUNT);
}

static int HxHGoN_Loader_tp_traverse(struct HxHGoN_LoaderObject *loader, visitproc visit, void *arg) { return 0; }

static PyObject *HxHGoN_Loader_get_name(struct HxHGoN_LoaderObject *loader, void *closure) {
    PyObject *result = HxHGoN_String_FromString(loader->m_loader_entry->name);

    return result;
}
static PyObject *HxHGoN_Loader_get_path(struct HxHGoN_LoaderObject *loader, void *closure) {
    PyThreadState *tstate = PyThreadState_GET();
...
static PyObject *HxHGoN_Loader_get__module__(struct HxHGoN_LoaderObject *loader, void *closure) {
    PyObject *result = const_str_plain___Gon__;

    Py_INCREF_IMMORTAL(result);
    return result;
}

static PyGetSetDef HxHGoN_Loader_tp_getset[] = {{(char *)"__module__", (getter)HxHGoN_Loader_get__module__, NULL, NULL},
                                                {(char *)"name", (getter)HxHGoN_Loader_get_name, NULL, NULL},
                                                {(char *)"path", (getter)HxHGoN_Loader_get_path, NULL, NULL},
                                                {NULL}};

PyTypeObject HxHGoN_Loader_Type = {
    PyVarObject_HEAD_INIT(NULL, 0) "Gon_module_loader",
    sizeof(struct HxHGoN_LoaderObject),      // tp_basicsize
    0,                                       // tp_itemsize
    (destructor)HxHGoN_Loader_tp_dealloc,    // tp_dealloc
    0,                                       // tp_print
    0,                                       // tp_getattr
    0,                                       // tp_setattr
    0,                                       // tp_reserved
    (reprfunc)HxHGoN_Loader_tp_repr,         // tp_repr
    0,                                       // tp_as_number
    0,                                       // tp_as_sequence
    0,                                       // tp_as_mapping
    0,                                       // tp_hash
    ...
    0,                                       // tp_doc
    (traverseproc)HxHGoN_Loader_tp_traverse, // tp_traverse
    0,                                       // tp_clear
    0,                                       // tp_richcompare
    0,                                       // tp_weaklistoffset
    0,                                       // tp_iter
    0,                                       // tp_iternext
    HxHGoN_Loader_methods,                   // tp_methods
    0,                                       // tp_members
    HxHGoN_Loader_tp_getset,                 // tp_getset
};

/* Used by modules to register child loaders for packages. */
PyObject *HxHGoN_Loader_New(struct HxHGoN_MetaPathBasedLoaderEntry const *entry) {
    struct HxHGoN_LoaderObject *result;

    allocateFromFreeListFixed(free_list_loaders, struct HxHGoN_LoaderObject, HxHGoN_Loader_Type);
...
void registerMetaPathBasedLoader(struct HxHGoN_MetaPathBasedLoaderEntry *_loader_entries,
    ...
    if (isVerbose()) {
        PySys_WriteStderr("Setup Gon compiled module/bytecode/extension importer.\n");
    }

    loader_entries = _loader_entries;

#if _HUNTER_MODULE_MODE && PYTHON_VERSION < 0x3c0
    if (_Py_PackageContext != NULL) {
        updateMetaPathBasedLoaderModuleRoot(_Py_PackageContext);
    }
#endif

    HxHGoN_PyType_Ready(&HxHGoN_Loader_Type, NULL, true, false, false, false, false);

#if _HUNTER_EXE_MODE
    {
        HUNTER_MAY_BE_UNUSED int res =
            PyDict_SetItemString((PyObject *)dict_builtin, "__Gon_loader_type", (PyObject *)&HxHGoN_Loader_Type);
        assert(res == 0);
    }
#endif

#if PYTHON_VERSION >= 0x370
    HxHGoN_PyType_Ready(&HxHGoN_ResourceReader_Type, NULL, true, false, false, false, false);
#endif

    // Register it as a meta path loader.
    PyObject *global_loader = HxHGoN_Loader_New(NULL);

    LIST_INSERT_CONST(HxHGoN_SysGetObject("meta_path"),
#if PYTHON_VERSION < 0x300
                      0,
#else
                      2,
...
#if _HUNTER_STANDALONE_MODE
    LIST_INSERT_CONST(HxHGoN_SysGetObject("path_hooks"), 0, PyObject_GetAttrString(global_loader, "sys_path_hook"));
#else
    ...
    LIST_APPEND1(HxHGoN_SysGetObject("path_hooks"), PyObject_GetAttrString(global_loader, "sys_path_hook"));
#endif
}
...
void setEarlyFrozenModulesFileAttribute(PyThreadState *tstate) {
    ...
    PyObject *builtin_module_names = HxHGoN_SysGetObject("builtin_module_names");

    while (HxHGoN_DictNext(sys_modules, &pos, &key, &value)) {
        if (key != NULL && value != NULL && PyModule_Check(value)) {
            ...
            if (is_package || HAS_ATTR_BOOL(tstate, value, const_str_plain___file__) ||
                ...
                PyObject *file_value = MAKE_RELATIVE_PATH_FROM_NAME(HxHGoN_String_AsString(key), is_package, false);
