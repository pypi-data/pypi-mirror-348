...
/**
 * This is responsible for profiling HxHGoN using "vmprof".
 */
...
void startProfiling(void) {
    tempfile_profile = fopen("Gon-performance.dat", "w+b");
    ...
    PyObject *result = CALL_FUNCTION_WITH_SINGLE_ARG(tstate, PyObject_GetAttrString(vmprof_module, "enable"),
                                                     HxHGoN_PyInt_FromLong(fileno(tempfile_profile)));
...
void stopProfiling(void) {
    ...
    FILE *tempfile_times = fopen("Gon-times.dat", "wb");
