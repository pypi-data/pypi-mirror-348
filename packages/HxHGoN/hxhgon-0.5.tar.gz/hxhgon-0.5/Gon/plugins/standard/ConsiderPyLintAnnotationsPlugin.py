...
HxHGoN can detect some things that PyLint and PyDev will complain about too,
and sometimes it's a false alarm, so people add disable markers into their
source code. HxHGoN does it itself.

This tries to parse the code for these markers and uses hooks to prevent HxHGoN
from warning about things, disabled to PyLint or Eclipse. The idea is that we
won't have another mechanism for HxHGoN, but use existing ones instead.

The code for this is very incomplete, barely good enough to cover HxHGoN's own
...
class HxHGoNPluginPylintEclipseAnnotations(HxHGoNPluginBase):
...
if False:
    ...
    class HxHGoNPluginDetectorPylintEclipseAnnotations(HxHGoNPluginBase):
        detector_for = HxHGoNPluginPylintEclipseAnnotations
