...
""" CType classes for PyObject *, PyObject **, and struct HxHGoN_CellObject *

"""

from Gon.__past__ import iterItems, xrange
from Gon.code_generation.ErrorCodes import (
    getErrorExitBoolCode,
    getReleaseCode,
)
from Gon.code_generation.templates.CodeTemplatesVariables import (
...
from Gon.Constants import getConstantValueGuide, isMutable
...
"""
    ...
    @classmethod
    def emitAssignmentCodeToHxHGoNBool(
        cls, to_name, value_name, needs_check, emit, context
    ):
        ...
        emit(
            "%s = %s == 0 ? HUNTER_BOOL_FALSE : HUNTER_BOOL_TRUE;"
            % (to_name, truth_name)
        )
...
class CTypePyObjectPtr(CPythonPyObjectPtrBase):
    ...
    def getCellObjectAssignmentCode(cls, target_cell_code, variable_code_name, emit):
        emit("%s = HxHGoN_Cell_New0(%s);" % (target_cell_code, variable_code_name))
...
class CTypeCellObject(CTypeBase):
    c_type = "struct HxHGoN_CellObject *"
    ...
    def getInitValue(cls, init_from):
        ...
        if init_from is not None:
            return "HxHGoN_Cell_New1(%s)" % init_from
        else:
            return "HxHGoN_Cell_NewEmpty()"
    ...
    def emitValueAccessCode(cls, value_name, emit, context):
        ...
        return VariableDeclaration(
            "PyObject *", "HxHGoN_Cell_GET(%s)" % value_name, None, None
        )

    @classmethod
    def getVariableArgDeclarationCode(cls, variable_code_name):
        return "struct HxHGoN_CellObject *%s" % variable_code_name
