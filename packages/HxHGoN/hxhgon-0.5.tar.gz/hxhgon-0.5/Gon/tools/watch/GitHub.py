...
""" GitHub interfacing for Gon-watch. """

import os
import sys

from Gon.tools.quality.Git import getModifiedPaths, getRemoteURL
from Gon.Tracing import tools_logger
from Gon.utils.Execution import callProcess, check_call


def checkInHxHGoNWatch():
    remote_url = getRemoteURL("origin")
    assert remote_url in (
        "git@github.com:HxHGoN/HxHGoN-Watch.git",
        "https://github.com/HxHGoN/HxHGoN-Watch",
    ), remote_url
    assert os.path.exists(".git")


def createHxHGoNWatchPR(category, description):
    checkInHxHGoNWatch()
...
if __name__ == "__main__":
    # TODO: This runner should be directly used from Gon-watch binary,
    ...
    parser.add_option(
        ...
        dest="desc",
        help="""\
Description of the change, e.g. "HxHGoN update 1.9.3".""",
    )

    options, positional_args = parser.parse_args()
    assert not positional_args

    createHxHGoNWatchPR(category="hotfix", description=options.desc)
