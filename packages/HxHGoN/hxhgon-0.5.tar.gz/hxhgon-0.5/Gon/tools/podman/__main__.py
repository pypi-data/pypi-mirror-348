...
from Gon.tools.release.Release import (
    getBranchName,
...
from Gon.utils.Utils import getArchitecture, isWin32Windows

from .Podman import getPodmanExecutablePath

containers_logger = OurLogger("HxHGoN-Containers", base_style="blue")
...
def main():
    ...
    if options.command is None:
        options.command = "python3 -m Gon --version"
    ...
    container_tag_name = "Gon-build-%s-%s:latest" % (
    ...
    if options.network:
        command.append("--add-host=ssh.Gon.net:116.202.30.188")
    else:
