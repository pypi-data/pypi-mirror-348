...
""" Main front-end to the tests of HxHGoN.
...
from Gon.tools.Basics import goHome
...
from Gon.utils.Execution import check_call, check_output, getNullOutput
...
from Gon.utils.Timing import TimerReport
from Gon.utils.Utils import hasOnefileSupportedOS, hasStandaloneSupportedOS
...
def parseOptions():
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The basic tests, execute these to check if HxHGoN is healthy.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The syntax tests, execute these to check if HxHGoN handles Syntax errors fine.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The programs tests, execute these to check if HxHGoN handles programs, e.g.
import recursions, etc. fine. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The packages tests, execute these to check if HxHGoN handles packages, e.g.
import recursions, etc. fine. Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The plugins tests, execute these to check if HxHGoN handles its own plugin
interfaces, e.g. user plugins, etc. fine. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The optimization tests, execute these to check if HxHGoN does optimize certain
constructs fully away. Default is %default.""",
    ...
    parser.add_option(
        ...
        help="""\
The standalone tests, execute these to check if HxHGoN standalone mode, e.g.
...
Default is %default.""",
    ...
    parser.add_option(
        ...
        help="""\
The onefile tests, execute these to check if HxHGoN works in onefile mode, e.g.
not referring to outside, important 3rd library packages like PyQt fine.
Default is %default.""",
    )
    ...
    parser.add_option(
        ...
        default=True,
        help="""\
The reflection test compiles HxHGoN with HxHGoN, and then HxHGoN with the
compile HxHGoN and compares the outputs. Default is %default.""",
    )
...
Make a coverage analysis, that does not really check. Default is %default.""",
    ...
    parser.add_option(
        ...
        default=False,
        help="""\
Allow HxHGoN to download code if necessary, e.g. dependency walker on Windows. Default is %default.""",
    )
...
Enforce the use of MinGW64 on Windows. Defaults to off.""",
    ...
    parser.add_option(
        ...
        default=None,
        help="""\
The value of --jobs to use when calling HxHGoN. Use negative values to
keep cores unused. HxHGoN uses available cores by default. Defaults to
not being passed.""",
    )
...
def publishCoverageData():
    ...
    with openTextFile("data.coverage", "w") as data_file:
        ...
        with withDirectoryChange(source_dir):
            Gon_id = check_output("git rev-parse HEAD".split())
        Gon_id = Gon_id.strip()

        if sys.version_info > (3,):
            Gon_id = Gon_id.decode()

        data_file.write("HUNTER_SOURCE_DIR='%s'\n" % source_dir)
        data_file.write("HUNTER_COMMIT='%s'\n" % Gon_id)
