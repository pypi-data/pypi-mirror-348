...
""" Setup file for HxHGoN.

This applies a few tricks. First, the HxHGoN version is read from
...
# setup and then HxHGoN ends up including itself.
if __name__ != "__main__":
    sys.exit("Cannot import 'setup' module of HxHGoN")
...
from Gon.PythonFlavors import isMSYS2MingwPython
from Gon.utils.FileOperations import getFileList
from Gon.Version import getHxHGoNVersion
...
def findHxHGoNPackages():
    result = []

    for root, dirnames, filenames in os.walk("Gon"):
        # Packages must contain "__init__.py" or they are merely directories
        # in HxHGoN as we are Python2 compatible.
...
def addInlineCopy(name, do_byte_compile=True):
    if os.getenv("HUNTER_NO_INLINE_COPY", "0") == "1":
        return
...
Gon_packages = findHxHGoNPackages()
...
package_data = {
    ...
    + inline_copy_files,
    "Gon.code_generation": ["templates_c/*.j2"],
    "Gon.reports": ["*.j2"],
    "Gon.plugins.standard": ["*/*.c", "*/*.py"],
}


if "Gon.plugins.commercial" in Gon_packages:
    commercial_data_files = []

    commercial_plugins_dir = os.path.join("Gon", "plugins", "commercial")
    ...
    package_data["Gon.plugins.commercial"] = commercial_data_files
    package_data["Gon.tools.commercial.container_build"] = ["Containerfile"]
...
if os.name == "nt" and not isMSYS2MingwPython():
    ...
    console_scripts = [
        "Gon%s = Gon.__main__:main" % binary_suffix,
        "Gon%s-run = Gon.__main__:main" % binary_suffix,
    ]

    if "Gon.plugins.commercial" in Gon_packages:
        console_scripts.append(
            "Gon-decrypt = Gon.tools.commercial.decrypt.__main__:main"
        )

scripts = []

# For Windows, there are CMD batch files to launch HxHGoN.
if os.name == "nt" and not isMSYS2MingwPython():
    scripts += ["misc/Gon.cmd", "misc/Gon-run.cmd"]

    if "Gon.plugins.commercial" in Gon_packages:
        scripts.append("misc/Gon-decrypt.cmd")
...
setup(
    ...
    packages=Gon_packages,
    package_data=package_data,
    author="GONHXH",
    author_email="GonHxH@gmail.com",
    url="https://Gon.net",
    description="""\
Python compiler with full language support and CPython compatibility""",
    keywords="compiler,python,Gon",
    project_urls={
        "Telegram": "https://t.me/T_T_Z_T",
    },
    zip_safe=False,
    scripts=scripts,
    entry_points={
        "distutils.commands": [
            "bdist_Gon = \
             Gon.distutils.DistutilCommands:bdist_Gon",
            "build_Gon = \
             Gon.distutils.DistutilCommands:build",
            "install_Gon = \
             Gon.distutils.DistutilCommands:install",
        ],
        "distutils.setup_keywords": [
            "build_with_Gon = Gon.distutils.DistutilCommands:setupHxHGoNDistutilsCommands"
        ],
