...
Plugins: Welcome to HxHGoN! This is your shortest way to become part of it.

This is to provide the base class for all plugins. Some of which are part of
proper HxHGoN, and some of which are waiting to be created and submitted for
inclusion by you.
...
from Gon import Options, OutputDirectories
from Gon.__past__ import basestring, iter_modules
from Gon.build.DataComposerInterface import deriveModuleConstantsBlobName
from Gon.containers.OrderedDicts import OrderedDict
from Gon.containers.OrderedSets import OrderedSet
from Gon.Errors import HxHGoNSyntaxError
from Gon.freezer.IncludedDataFiles import IncludedDataFile
from Gon.freezer.IncludedEntryPoints import IncludedEntryPoint
from Gon.ModuleRegistry import addUsedModule
from Gon.PythonVersions import python_version
from Gon.Tracing import plugins_logger, printLine
from Gon.utils.CommandLineOptions import OurOptionGroup
...
from Gon.utils.Importing import importFileAsModule
from Gon.utils.ModuleNames import (
    ModuleName,
    checkModuleName,
...
from .PluginBase import HxHGoNPluginBase, control_tags
...
def _addActivePlugin(plugin_class, args, force=False):
    ...
    assert isinstance(plugin_instance, HxHGoNPluginBase), plugin_instance
    ...
    for item in iter_modules(scan_path):
        ...
        for detector in detectors:
            ...
            if detector.__name__.replace(
                "HxHGoNPluginDetector", ""
            ) != plugin_class.__name__.replace("HxHGoNPlugin", ""):
                plugins_logger.warning(
                    "Class names %r and %r do not match HxHGoNPlugin* and HxHGoNPluginDetector* naming convention."
...
def loadStandardPluginClasses():
    ...
    Returns:
        None
    """
    import Gon.plugins.standard

    _loadPluginClassesFromPackage(Gon.plugins.standard)

    try:
        import Gon.plugins.commercial
    except ImportError:
        pass
    else:
        _loadPluginClassesFromPackage(Gon.plugins.commercial)
...
class Plugins(object):
    ...
    def _reportImplicitImports(plugin, module, implicit_imports):
        from Gon.importing.Importing import getModuleNameAndKindFromFilename
        from Gon.importing.Recursion import decideRecursion, recurseTo
    ...
    def _createTriggerLoadedModule(cls, module, trigger_name, code, flags):
        ...
        from Gon.tree.Building import buildModule
    ...
    def onModuleUsageLookAhead(cls, module_name, module_filename, module_kind):
        ...
        if parent_package_name is not None:
            from Gon.importing.Importing import locateModule
        ...
        def getModuleSourceCode():
            ...
            from Gon.tree.SourceHandling import readSourceCodeFromFilename
    ...
    def onModuleInitialSet():
        ...
        from Gon.ModuleRegistry import addRootModule
    ...
    def onModuleCompleteSet():
        ...
        from Gon.ModuleRegistry import getDoneModules
    ...
    def writeExtraCodeFiles(onefile):
        # Circular dependency.
        from Gon.tree.SourceHandling import writeSourceCode
...
def listPlugins():
    ...
    printLine("The following plugins are available in HxHGoN".center(80))
    printLine("-" * 80)
...
def isObjectAUserPluginBaseClass(obj):
    ...
    try:
        return (
            obj is not HxHGoNPluginBase
            and issubclass(obj, HxHGoNPluginBase)
    ...
    for plugin_name, (plugin_class, plugin_detector) in sorted(
        ...
        if plugin_name in Options.getPluginsEnabled():
            ...
            if plugin_class.isRelevant():
                ...
                plugin_class.warning(
                    "Not relevant with this OS, or HxHGoN arguments given, not activated."
                )
...
def getPluginOptions(plugin_name):
    ...
    for option in plugin_options.get(plugin_name, {}):
        ...
        if "[REQUIRED]" in option.help:
            if not arg_value:
                plugins_logger.sysexit(
                    "Error, required plugin argument '%s' of HxHGoN plugin '%s' not given."
