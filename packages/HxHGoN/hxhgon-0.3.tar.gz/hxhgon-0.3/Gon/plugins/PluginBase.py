...
from Gon.__past__ import iter_modules, unicode
from Gon.containers.Namedtuples import makeNamedtupleClass
...
from Gon.ModuleRegistry import (
...
from Gon.Tracing import plugins_logger
from Gon.utils.AppDirs import getAppdirsModule
...
from Gon.utils.Importing import (
...
from Gon.utils.SharedLibraries import locateDLL, locateDLLsInDirectory
from Gon.utils.SlotMetaClasses import getMetaClassBase
...
def _getPackageData(package_name, resource, default=None):
    ...
    from Gon.utils.PackageResources import getPackageData
...
class HxHGoNPluginBase(getMetaClassBase("Plugin", require_slots=False)):
    """HxHGoN base class for all plugins.

    Derive your plugin from "HxHGoNPluginBase" please.
    For instructions, see https://github.com/HxHGoN/HxHGoN/blob/orsiris/UserPlugin-Creation.rst

    Plugins allow to adapt HxHGoN's behavior in a number of ways as explained
    ...
    A plugin in general must be enabled to be used by HxHGoN. This happens by
    specifying "--enable-plugin" (standard plugins) or by "--user-plugin" (user
    plugins) in the HxHGoN command line. However, some plugins are always enabled
    and invisible to the user.

    HxHGoN comes with a number of "standard" plugins to be enabled as needed.
    What they are can be displayed using "Gon --plugin-list file.py" (filename
    ...
    # Standard plugins must provide this as a unique string which HxHGoN
    ...
    def isAlwaysEnabled():
        ...
        Notes:
            ...
            this case, the plugin will be enabled upon HxHGoN start-up. Any
...
%(recommendation)s or uninstall it for best compatibility with pure Python execution."""
    ...
    def locateModules(module_name):
        ...
        from Gon.importing.Importing import locateModules
...
""" % {
        ...
        except HxHGoNCalledProcessError as e:
            if e.returncode == 38:
                self.warning(
