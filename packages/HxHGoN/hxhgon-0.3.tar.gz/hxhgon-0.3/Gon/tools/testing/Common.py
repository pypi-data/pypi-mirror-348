...
from Gon.__past__ import md5, subprocess
from Gon.containers.OrderedDicts import OrderedDict
from Gon.Options import getCommercialVersion
from Gon.PythonVersions import getTestExecutionPythonVersions, isDebugPython
from Gon.Tracing import OurLogger, flushStandardOutputs, my_print
...
from Gon.utils.InstalledPythons import findInstalledPython
from Gon.utils.Jinja2 import getTemplate
from Gon.utils.Utils import isFreeBSD, isLinux, isMacOS, isWin32Windows
...
def setup(suite="", needs_io_encoding=False, silent=False, go_main=True):
    ...
    os.environ["HUNTER_TEST_SUITE"] = suite
...
def _compareWithCPythonVariations(
    ...
    for test_variant_name in extra_flags.keys():
        ...
        with withEnvironmentVarOverridden("HUNTER_TEST_VARIANT", test_variant_name):
            compareWithCPython(
...
def compareWithCPython(dirname, filename, extra_flags, search_mode, on_error=None):
    ...
    if os.getenv("HUNTER_TEST_INSTALLED", "") == "1":
        command = [
            sys.executable,
            "-m",
            "Gon.tools.testing.compare_with_cpython",
            path,
            "silent",
        ]
    else:
        ...
        if os.path.exists(compare_with_cpython):
            ...
            test_logger.sysexit("Error, cannot locate HxHGoN comparison runner.")
...
def getDebugPython():
    ...
    # On Windows systems, these work. TODO: Python asserts in HxHGoN with
...
Execute all tests, continue execution even after failure of one.""",
    ...
    select_group.add_option(
        ...
        default=None,
        help="""\
Pass this as the --jobs value to HxHGoN.""",
    )
    ...
    debug_group.add_option(
        ...
        default=False,
        help="""\
Executing all self checks possible to find errors in HxHGoN, good for test coverage.
Defaults to off.""",
    )
...
Defaults to off.""",
    ...
    if options.show_commands:
        os.environ["HUNTER_TRACE_COMMANDS"] = "1"
...
def addExtendedExtraOptions(*args):
    old_value = os.getenv("HUNTER_EXTRA_OPTIONS")
    ...
    os.environ["HUNTER_EXTRA_OPTIONS"] = value
...
def withExtendedExtraOptions(*args):
    ...
    if old_value is None:
        del os.environ["HUNTER_EXTRA_OPTIONS"]
    else:
        os.environ["HUNTER_EXTRA_OPTIONS"] = old_value
...
def getTestingCPythonOutputsCacheDir():
    ...
    result = os.path.join(cache_dir, os.getenv("HUNTER_TEST_SUITE", ""))
...
def setupCacheHashSalt(test_code_path):
    ...
    os.environ["HUNTER_HASH_SALT"] = salt_value.hexdigest()
...
#     "# Gon-skip-unless-expression: expression to be evaluated"
#       OR
#     "# Gon-skip-unless-imports: module1,module2,..."
def checkTestRequirements(filename):
    ...
    for line in readSourceCodeFromFilename(None, filename).splitlines():
        if line.startswith("# Gon-skip-unless-"):
...
def extractHxHGoNVersionFromFilePath(version_filename):
    with openTextFile(version_filename, "r") as f:
        option_lines = f.readlines()

    (version_line,) = [line for line in option_lines if line.startswith("HxHGoN V")]
...
def decryptOutput(project_options, output):
    Gon_decrypt_call = [
        ...
        "-m",
        "Gon.tools.commercial.decrypt",  # Note: Needed for Python2.6
    ]

    for project_option in project_options:
        if project_option.startswith("--encryption-key="):
            Gon_decrypt_call.append("--key=" + project_option.split("=", 1)[1])

    stdout, stderr, exit_code = executeProcess(Gon_decrypt_call, stdin=output)
