...
from Gon.Options import getUserProvidedYamlFiles
...
def getYamlPackageConfiguration():
    """Get HxHGoN package configuration. Merged from multiple sources."""
    ...
    if _package_config is None:
        _package_config = parsePackageYaml(
            ...
            "standard.Gon-package.config.yml",
        )
        _package_config.update(
            parsePackageYaml(
                "Gon.plugins.standard",
                "stdlib2.Gon-package.config.yml",
            )
        )
        _package_config.update(
            parsePackageYaml(
                "Gon.plugins.standard",
                "stdlib3.Gon-package.config.yml",
            )
        )
        ...
        try:
            _package_config.update(
                parsePackageYaml(
                    "Gon.plugins.commercial", "commercial.Gon-package.config.yml"
                )
            )
        except IOError:
...
def getYamlPackageConfigurationSchemaFilename():
    """Get the filename of the schema for HxHGoN package configuration."""
    return os.path.join(
        os.path.dirname(__file__),
        "..",
        "..",
        "misc",
        "Gon-package-config-schema.json",
    )
