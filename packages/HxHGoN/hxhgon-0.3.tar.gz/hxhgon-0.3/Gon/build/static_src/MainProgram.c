...
#define HUNTER_MAIN_MODULE_NAME "__main__"
#define HUNTER_MAIN_IS_PACKAGE_BOOL false
#define _HUNTER_ATTACH_CONSOLE_WINDOW 1
#if defined(__APPLE__)
#define _HUNTER_MACOS_BUNDLE_MODE 1
#endif
#endif

// It doesn't work for MinGW64 to update the standard output handles early on,
// so make a difference here.
#if defined(HUNTER_FORCED_STDOUT_PATH) || defined(HUNTER_FORCED_STDERR_PATH)
#if defined(__MINGW64__) || defined(__MINGW32__)
#define HUNTER_STANDARD_HANDLES_EARLY 0
#else
#define HUNTER_STANDARD_HANDLES_EARLY 1
#endif
#else
#define HUNTER_STANDARD_HANDLES_EARLY 0
#endif

#if defined(_WIN32) && (defined(_HUNTER_ATTACH_CONSOLE_WINDOW) || defined(_HUNTER_HIDE_CONSOLE_WINDOW))
#include "HelpersConsole.c"
#endif

// We are open to having this defined otherwise, this is a default only.
#if defined(_WIN32) && defined(HUNTER_COMPANY_NAME) && defined(HUNTER_PRODUCT_NAME) &&                                 \
    !defined(HUNTER_APP_MODEL_USER_ID)
#define HUNTER_APP_MODEL_USER_ID L"" HUNTER_COMPANY_NAME L"." HUNTER_PRODUCT_NAME
#endif

#if defined(_WIN32) && defined(HUNTER_APP_MODEL_USER_ID)
...
static void prepareFrozenModules(void) {
    ...
    struct _frozen *merged =
        (struct _frozen *)malloc(sizeof(struct _frozen) * (_HUNTER_FROZEN + pre_existing_count + 1));
...
#include "Gon/environment_variables.h"
...
static void prepareStandaloneEnvironment(void) {
    ...
    HUNTER_PRINTF_TRACE("main(): Binary dir is %s\n", binary_directory);
...
static void restoreStandaloneEnvironment(void) {
    ...
    // HUNTER_PRINTF_TRACE("Final PySys_GetPath is 's'.\n", PySys_GetPath());
#elif PYTHON_VERSION < 0x370
    PySys_SetPath(getBinaryDirectoryWideChars(true));
    Py_SetPath(getBinaryDirectoryWideChars(true));
    // HUNTER_PRINTF_TRACE("Final Py_GetPath is '%ls'.\n", Py_GetPath());
#endif

#ifdef _HUNTER_EXPERIMENTAL_DUMP_PY_PATH_CONFIG
...
#if PYTHON_VERSION >= 0x300 && _HUNTER_NATIVE_WCHAR_ARGV == 0
...
static void PRINT_REFCOUNTS(void) {
    ...
    PRINT_FORMAT("Compiled Functions: %d | %d | %d (module/class ownership may occur)\n",
                 count_active_HxHGoN_Function_Type, count_allocated_HxHGoN_Function_Type,
                 count_released_HxHGoN_Function_Type);
    PRINT_FORMAT("Compiled Generators: %d | %d | %d\n", count_active_HxHGoN_Generator_Type,
                 count_allocated_HxHGoN_Generator_Type, count_released_HxHGoN_Generator_Type);
#if PYTHON_VERSION >= 0x350
    PRINT_FORMAT("Compiled Coroutines: %d | %d | %d\n", count_active_HxHGoN_Coroutine_Type,
                 count_allocated_HxHGoN_Coroutine_Type, count_released_HxHGoN_Coroutine_Type);
    PRINT_FORMAT("Compiled Coroutines Wrappers: %d | %d | %d\n", count_active_HxHGoN_CoroutineWrapper_Type,
                 count_allocated_HxHGoN_CoroutineWrapper_Type, count_released_HxHGoN_CoroutineWrapper_Type);

    PRINT_FORMAT("Compiled Coroutines AIter Wrappers: %d | %d | %d\n", count_active_HxHGoN_AIterWrapper_Type,
                 count_allocated_HxHGoN_AIterWrapper_Type, count_released_HxHGoN_AIterWrapper_Type);
#endif
#if PYTHON_VERSION >= 0x360
    PRINT_FORMAT("Compiled Asyncgen: %d | %d | %d\n", count_active_HxHGoN_Asyncgen_Type,
                 count_allocated_HxHGoN_Asyncgen_Type, count_released_HxHGoN_Asyncgen_Type);
    PRINT_FORMAT("Compiled Asyncgen Wrappers: %d | %d | %d\n", count_active_HxHGoN_AsyncgenValueWrapper_Type,
                 count_allocated_HxHGoN_AsyncgenValueWrapper_Type, count_released_HxHGoN_AsyncgenValueWrapper_Type);
    PRINT_FORMAT("Compiled Asyncgen Asend: %d | %d | %d\n", count_active_HxHGoN_AsyncgenAsend_Type,
                 count_allocated_HxHGoN_AsyncgenAsend_Type, count_released_HxHGoN_AsyncgenAsend_Type);
    PRINT_FORMAT("Compiled Asyncgen Athrow: %d | %d | %d\n", count_active_HxHGoN_AsyncgenAthrow_Type,
                 count_allocated_HxHGoN_AsyncgenAthrow_Type, count_released_HxHGoN_AsyncgenAthrow_Type);
#endif

    PRINT_FORMAT("Compiled Frames: %d | %d | %d (cache usage may occur)\n", count_active_HxHGoN_Frame_Type,
                 count_allocated_HxHGoN_Frame_Type, count_released_HxHGoN_Frame_Type);
    PRINT_FORMAT("Compiled Cells: %d | %d | %d (function ownership may occur)\n", count_active_HxHGoN_Cell_Type,
                 count_allocated_HxHGoN_Cell_Type, count_released_HxHGoN_Cell_Type);
...
static int HANDLE_PROGRAM_EXIT(PyThreadState *tstate) {
    ...
    if (HAS_ERROR_OCCURRED(tstate)) {
        ...
        while (tstate->curexc_traceback) {
            ...
            if (0 == strcmp(PyUnicode_AsUTF8(HxHGoN_Frame_GetCodeObject(frame)->co_filename),
...
#endif
        HUNTER_FINALIZE_PROGRAM(tstate);
...
static PyObject *EXECUTE_MAIN_MODULE(PyThreadState *tstate, char const *module_name, bool is_package) {
    HUNTER_INIT_PROGRAM_LATE(module_name);
...
#if _HUNTER_PLUGIN_WINDOWS_SERVICE_ENABLED
#include "Gon_windows_service.h"

// Callback from Windows Service logic.
void SvcStartPython(void) {
    PyThreadState *tstate = PyThreadState_GET();

    EXECUTE_MAIN_MODULE(tstate, HUNTER_MAIN_MODULE_NAME, HUNTER_MAIN_IS_PACKAGE_BOOL);

    HUNTER_PRINT_TIMING("SvcStartPython() Python exited.")

    int exit_code = HANDLE_PROGRAM_EXIT(tstate);

    // TODO: Log exception and call ReportSvcStatus

    HUNTER_PRINT_TIMING("SvcStartPython(): Calling Py_Exit.");
    Py_Exit(exit_code);
}
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (i == 1) {
            ...
            if (strcmpFilename(argv[i], FILENAME_EMPTY_STR "install") == 0) {
                HUNTER_PRINT_TRACE("main(): Calling plugin SvcInstall().");

                SvcInstall();
                HUNTER_CANNOT_GET_HERE("main(): SvcInstall must not return");
            }
#endif
        }
...
#if !defined(_HUNTER_DEPLOYMENT_MODE) && !defined(_HUNTER_NO_DEPLOYMENT_SELF_EXECUTION)
...
#if defined(_HUNTER_ONEFILE_MODE) && defined(_WIN32)

static long onefile_ppid;

DWORD WINAPI doOnefileParentMonitoring(LPVOID lpParam) {
    HUNTER_PRINT_TRACE("Onefile parent monitoring starts.");
    ...
    HUNTER_PRINT_TRACE("Onefile parent monitoring causes KeyboardInterrupt.");
    PyErr_SetInterrupt();

    Sleep(_HUNTER_ONEFILE_CHILD_GRACE_TIME_INT);

    HUNTER_PRINT_TRACE("Onefile parent monitoring hard kills after ignored KeyboardInterrupt.");
    ExitProcess(1);
...
#if HUNTER_FORCED_STDOUT_NULL_BOOL
    if (strcmp(name, "stdout") == 0) {
        return true;
    }
#endif

#if HUNTER_FORCED_STDERR_NULL_BOOL
    if (strcmp(name, "stderr") == 0) {
        return true;
    }
#elif defined(HUNTER_FORCED_STDERR_PATH) || defined(HUNTER_FORCED_STDERR_NONE_BOOL)
    if (strcmp(name, "stderr") == 0) {
        return false;
    }
#endif

    PyObject *sys_std_handle = HxHGoN_SysGetObject(name);
...
static void setStdinHandle(PyThreadState *tstate, PyObject *stdin_file) {
    ...
    HxHGoN_SysSetObject("stdin", stdin_file);
...
static void setStdoutHandle(PyThreadState *tstate, PyObject *stdout_file) {
    ...
    HxHGoN_SysSetObject("stdout", stdout_file);
...
static void setStderrHandle(PyThreadState *tstate, PyObject *stderr_file) {
    ...
    HxHGoN_SysSetObject("stderr", stderr_file);
...
#if HUNTER_STANDARD_HANDLES_EARLY == 0
#if defined(HUNTER_FORCED_STDOUT_PATH) || defined(HUNTER_FORCED_STDERR_PATH)
#ifdef _WIN32
static PyObject *getExpandedTemplatePath(wchar_t const *template_path) {
    ...
    return HxHGoNUnicode_FromWideChar(filename_buffer, -1);
}
#else
static PyObject *getExpandedTemplatePath(char const *template_path) {
    ...
    return HxHGoN_String_FromString(filename_buffer);
}
#endif
#endif
#endif
...
// reason.
#if HUNTER_STANDARD_HANDLES_EARLY == 1 && PYTHON_VERSION >= 0x370
    HUNTER_PRINT_TRACE("setInputOutputHandles(): Early handles.");
#if defined(HUNTER_FORCED_STDOUT_PATH) || defined(HUNTER_FORCED_STDERR_PATH)
    PyObject *args = MAKE_DICT_EMPTY(tstate);

    DICT_SET_ITEM(args, const_str_plain_encoding, HxHGoN_String_FromString("utf-8"));
    DICT_SET_ITEM(args, const_str_plain_line_buffering, Py_True);

#if defined(HUNTER_FORCED_STDOUT_PATH)
    HUNTER_PRINT_TRACE("setInputOutputHandles(): Forced stdout update.");
    {
        PyObject *sys_stdout = HxHGoN_SysGetObject("stdout");
...
#if defined(HUNTER_FORCED_STDERR_PATH)
    HUNTER_PRINT_TRACE("setInputOutputHandles(): Forced stderr update.");
    {
        PyObject *sys_stderr = HxHGoN_SysGetObject("stderr");
    ...
    HUNTER_PRINT_TRACE("setInputOutputHandles(): Done with early handles.");
#endif

#if HUNTER_STANDARD_HANDLES_EARLY == 0
    HUNTER_PRINT_TRACE("setInputOutputHandles(): Late handles.");
#if defined(HUNTER_FORCED_STDOUT_PATH)
    {
#if defined(_WIN32)
        PyObject *filename = getExpandedTemplatePath(L"" HUNTER_FORCED_STDOUT_PATH);
#else
        PyObject *filename = getExpandedTemplatePath(HUNTER_FORCED_STDOUT_PATH);
#endif
    ...
    {
#ifdef _WIN32
        PyObject *filename = getExpandedTemplatePath(L"" HUNTER_FORCED_STDERR_PATH);
#else
        PyObject *filename = getExpandedTemplatePath(HUNTER_FORCED_STDERR_PATH);
#endif
    ...
    {
#if defined(_WIN32)
        PyObject *devnull_filename = HxHGoN_String_FromString("NUL:");
#else
        PyObject *devnull_filename = HxHGoN_String_FromString("/dev/null");
#endif

        HUNTER_PRINT_TRACE("setInputOutputHandles(): Considering stdin.");

        if (shallSetOutputHandleToNull("stdin")) {
            HUNTER_PRINT_TRACE("setInputOutputHandles(): Set stdin to NULL file.");
        ...
        HUNTER_PRINT_TRACE("setInputOutputHandles(): Considering stdout.");

        if (shallSetOutputHandleToNull("stdout")) {
            HUNTER_PRINT_TRACE("setInputOutputHandles(): Set stdout to NULL file.");
        ...
        HUNTER_PRINT_TRACE("setInputOutputHandles(): Considering stderr.");

        if (shallSetOutputHandleToNull("stderr")) {
            HUNTER_PRINT_TRACE("setInputOutputHandles(): Set stderr to NULL file.");
...
#if HUNTER_FORCED_STDOUT_NONE_BOOL
    HUNTER_PRINT_TRACE("setInputOutputHandles(): Forcing stdout to None.");
    setStdoutHandle(tstate, Py_None);
#endif

#if HUNTER_FORCED_STDERR_NONE_BOOL
    HUNTER_PRINT_TRACE("setInputOutputHandles(): Forcing stderr to None.");
...
static void HxHGoN_Py_Initialize(void) {
#if PYTHON_VERSION > 0x350 && !defined(_HUNTER_EXPERIMENTAL_DISABLE_ALLOCATORS)
    initHxHGoNAllocators();
#endif

#if PYTHON_VERSION < 0x380 || defined(_HUNTER_EXPERIMENTAL_OLD_PY_INITIALIZE)
    Py_Initialize();
#else
    ...
    HUNTER_MAY_BE_UNUSED _PyRuntimeState *runtime = &_PyRuntime;
    ...
    // Need to disable frozen modules, HxHGoN can handle them better itself.
...
// too.
    ...
    HUNTER_PRINT_TIMING("HxHGoN_Py_Initialize(): Calling Py_InitializeFromConfig.");
...
// short lived.
#if SYSFLAG_ISOLATED
    HxHGoN_SysSetObject("path", PyList_New(0));
#endif
#endif
...
#if HUNTER_STANDARD_HANDLES_EARLY == 1
#if defined(_WIN32)
...
static void HxHGoN_at_exit(void) { HUNTER_PRINT_TIMING("HxHGoN_at_exit(): Called by C exit()"); }
#endif

#if !defined(_HUNTER_DEPLOYMENT_MODE) && !defined(_HUNTER_NO_DEPLOYMENT_SEGFAULT)
#include <signal.h>
static void Gon_segfault_handler(int sig) {
    puts("HxHGoN: A segmentation fault has occurred. This is highly unusual and can");
    puts("تم برمجة المكتبة من فريق مختص اقوى منك ومن تيمك اذا عندك تيم! ");
    puts("have multiple reasons. Please check https://Gon.net/info/segfault.html");
    puts("for solutions.");
    puts("Create By HUNTER ");
    signal(SIGSEGV, SIG_DFL);
    raise(SIGSEGV);
}
#endif

#if _HUNTER_NATIVE_WCHAR_ARGV == 1
...
PyObject *getOriginalArgv0Object(void) {
    ...
    return HxHGoN_String_FromFilename(original_argv0);
}
...
static int HxHGoN_Main(int argc, native_command_line_argument_t **argv) {
#if defined(_HUNTER_HIDE_CONSOLE_WINDOW)
    hideConsoleIfSpawned();
#endif

    // Installer a segfault handler that outputs a helpful message.
#if !defined(_HUNTER_DEPLOYMENT_MODE) && !defined(_HUNTER_NO_DEPLOYMENT_SEGFAULT)
    signal(SIGSEGV, Gon_segfault_handler);
#endif

#ifdef _HUNTER_EXPERIMENTAL_DUMP_C_TRACEBACKS
    INIT_C_BACKTRACES();
    DUMP_C_BACKTRACE();
#endif
    // Trace when the process exits.
#if defined(_HUNTER_EXPERIMENTAL_SHOW_STARTUP_TIME)
    atexit(HxHGoN_at_exit);
#endif

    // Attach to the parent console respecting redirection only, otherwise we
    // cannot even output traces.
#if defined(_WIN32) && defined(_HUNTER_ATTACH_CONSOLE_WINDOW)
    inheritAttachedConsole();
#endif

#if defined(_WIN32) && defined(HUNTER_APP_MODEL_USER_ID)
    setCurrentProcessExplicitAppUserModelID(HUNTER_APP_MODEL_USER_ID);
#endif

// Make sure, we use the absolute program path for argv[0] for standalone mode
#if _HUNTER_NATIVE_WCHAR_ARGV == 0
    original_argv0 = argv[0];
#if !defined(_HUNTER_ONEFILE_MODE)
...
#if defined(HUNTER_FORCED_STDOUT_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_OUTPUT_HANDLE, stdout, L"" HUNTER_FORCED_STDOUT_PATH);
#else
    changeStandardHandleTarget(stdout, HUNTER_FORCED_STDOUT_PATH);
#endif
#endif
#if defined(HUNTER_FORCED_STDERR_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_ERROR_HANDLE, stderr, L"" HUNTER_FORCED_STDERR_PATH);
#else
    changeStandardHandleTarget(stderr, HUNTER_FORCED_STDERR_PATH);
#endif
#endif
#if defined(HUNTER_FORCED_STDIN_PATH)
#ifdef _WIN32
    changeStandardHandleTarget(STD_INPUT_HANDLE, stdin, L"" HUNTER_FORCED_STDIN_PATH);
#else
    changeStandardHandleTarget(stdin, HUNTER_FORCED_STDIN_PATH);
#endif
#endif
#endif
    ...
    HUNTER_PRINT_TIMING("main(): Entered.");
    HUNTER_INIT_PROGRAM_EARLY(argc, argv);
...
#if _HUNTER_STANDALONE_MODE
    HUNTER_PRINT_TIMING("main(): Prepare standalone environment.");
    prepareStandaloneEnvironment();
#endif

#if _HUNTER_FROZEN > 0
    HUNTER_PRINT_TIMING("main(): Preparing frozen modules.");
...
#ifdef HUNTER_PYTHON_STATIC
    HUNTER_PRINT_TIMING("main(): Preparing static modules.");
...
#if defined(_HUNTER_ONEFILE_MODE)
    {
        environment_char_t const *parent_original_argv0 = getEnvironmentVariable("HUNTER_ORIGINAL_ARGV0");
        ...
        if (parent_original_argv0 != NULL) {
            ...
            unsetEnvironmentVariable("HUNTER_ORIGINAL_ARGV0");
        }
    }
#endif

#if PYTHON_VERSION >= 0x300 && _HUNTER_NATIVE_WCHAR_ARGV == 0
    HUNTER_PRINT_TRACE("main(): Calling convertCommandLineParameters.");
    orig_argv = convertCommandLineParameters(argc, argv);
#elif PYTHON_VERSION < 0x300 && _HUNTER_NATIVE_WCHAR_ARGV == 1
    HUNTER_PRINT_TRACE("main(): Calling getCommandLineToArgvA.");
...
// Make sure, we use the absolute program path for argv[0] for standalone mode
#if _HUNTER_NATIVE_WCHAR_ARGV == 1
    original_argv0 = argv[0];
#if PYTHON_VERSION >= 0x300 && !defined(_HUNTER_ONEFILE_MODE)
    ...
    HUNTER_PRINT_TRACE("main(): Calling setCommandLineParameters.");
    setCommandLineParameters(argc, argv);

    /* For Python installations that need the home set, we inject it back here. */
#if defined(PYTHON_HOME_PATH)
#if PYTHON_VERSION < 0x300
    HUNTER_PRINT_TRACE("main(): Prepare run environment '" PYTHON_HOME_PATH "'.");
    Py_SetPythonHome(PYTHON_HOME_PATH);
#else
    HUNTER_PRINTF_TRACE("main(): Prepare run environment '%S'.\n", L"" PYTHON_HOME_PATH);
    ...
    HUNTER_PRINT_TRACE("main(): Disabling Python warnings.");
    {
#if PYTHON_VERSION >= 0x300
...
// Workaround older Python not handling stream setup on redirected files properly.
    ...
    HUNTER_PRINT_TIMING("main(): Calling HxHGoN_Py_Initialize to initialize interpreter.");
    HxHGoN_Py_Initialize();

    PyThreadState *tstate = PyThreadState_GET();

#if _HUNTER_STANDALONE_MODE
    HUNTER_PRINT_TRACE("main(): Restore standalone environment.");
    restoreStandaloneEnvironment();
#else
    {
        environment_char_t const *python_path_cstr = getEnvironmentVariable("HUNTER_PYTHONPATH");

        if (python_path_cstr != NULL) {
            PyObject *python_path_str = HxHGoN_String_FromFilename(python_path_cstr);
#ifdef _WIN32
            ...
            HxHGoN_SysSetObject("path", python_path_list);

            unsetEnvironmentVariable("HUNTER_PYTHONPATH");
        }
    }
#endif
    ...
    PyList_SetSlice(HxHGoN_SysGetObject("path"), 0, 1, PyTuple_New(0));
#endif

    /* Initialize the built-in module tricks used and builtin-type methods */
    HUNTER_PRINT_TRACE("main(): Calling _initBuiltinModule().");
    _initBuiltinModule();

    /* Initialize the Python constant values used. This also sets
     * "sys.executable" while at it.
     */
    HUNTER_PRINT_TIMING("main(): Calling createGlobalConstants().");
    createGlobalConstants(tstate);
    HUNTER_PRINT_TIMING("main(): Returned createGlobalConstants().");

    /* Complex call helpers need "__main__" constants, even if we only
     * go into "__parents__main__" module as a start point.
     */
    HUNTER_PRINT_TIMING("main(): Calling createMainModuleConstants().");
    createMainModuleConstants(tstate);
    HUNTER_PRINT_TIMING("main(): Returned createMainModuleConstants().");

    HUNTER_PRINT_TRACE("main(): Calling _initBuiltinOriginalValues().");
...
#if PYTHON_VERSION < 0x300
    PyStructSequence_SET_ITEM(HxHGoN_SysGetObject("flags"), 9, const_int_0);
#else
    PyStructSequence_SetItem(HxHGoN_SysGetObject("flags"), 6, const_int_0);
#endif
#endif

    /* Initialize the compiled types of HxHGoN. */
    ...
    HUNTER_PRINT_TRACE("main(): Calling enhancePythonTypes().");
    enhancePythonTypes();

    HUNTER_PRINT_TRACE("main(): Calling patchTypeComparison().");
    patchTypeComparison();

    HUNTER_PRINT_TRACE("main(): Calling patchTracebackDealloc().");
    patchTracebackDealloc();

#ifndef HUNTER_USE_PYCORE_THREAD_STATE
    /* Allow to override the ticker value, to remove checks for threads in
     * CPython core from impact on benchmarks. */
    char const *ticker_value = getenv("HUNTER_TICKER");
    ...
    if (needs_stdout_attaching) {
        PyObject *filename = HxHGoN_String_FromString("CONOUT$");
        // This defaults to "utf-8" internally. We may add an argument of use
        // platform ones in the future.
        PyObject *encoding = NULL;
    ...
    if (needs_stdin_attaching) {
        PyObject *filename = HxHGoN_String_FromString("CONIN$");
        // This defaults to "utf-8" internally. We may add an argument of use
        // platform ones in the future.
        PyObject *encoding = NULL;
    ...
    HUNTER_PRINT_TRACE("main(): Setting Python input/output handles.");
    ...
    // standard loader.
    {
        HUNTER_MAY_BE_UNUSED PyObject *importlib_module = getImportLibBootstrapModule();
        CHECK_OBJECT(importlib_module);
    }
#endif

    HUNTER_PRINT_TRACE("main(): Calling setEarlyFrozenModulesFileAttribute().");

    setEarlyFrozenModulesFileAttribute(tstate);
#endif

#if _HUNTER_FROZEN > 0
    HUNTER_PRINT_TRACE("main(): Removing early frozen module table again.");
    PyImport_FrozenModules = old_frozen;
#endif

    HUNTER_PRINT_TRACE("main(): Calling setupMetaPathBasedLoader().");
...
#if NO_PYTHON_WARNINGS && PYTHON_VERSION >= 0x342 && PYTHON_VERSION < 0x3a0 && defined(_HUNTER_FULL_COMPAT)
...
#if _HUNTER_PGO_PYTHON
    // Profiling with our own Python PGO if enabled.
    PGO_Initialize();
#endif

#if PYTHON_VERSION >= 0x300
    HUNTER_PRINT_TRACE("main(): Calling patchInspectModule().");
...
// that are unexplained.
...
#if PYTHON_VERSION >= 0x300 && SYSFLAG_NO_RANDOMIZATION == 1
    HUNTER_PRINT_TRACE("main(): Reverting to initial 'PYTHONHASHSEED' value.");
    undoEnvironmentVariable(tstate, "PYTHONHASHSEED", old_env_hash_seed);
#endif

#if PYTHON_VERSION >= 0x300 && SYSFLAG_UNBUFFERED == 1
    HUNTER_PRINT_TRACE("main(): Reverting to initial 'PYTHONUNBUFFERED' value.");
    undoEnvironmentVariable(tstate, "PYTHONUNBUFFERED", old_env_unbuffered);
#endif

#if _HUNTER_STANDALONE_MODE
    // Restore the PATH, so the program can use it.
    HUNTER_PRINT_TRACE("main(): Reverting to initial 'PATH' value.");
...
#ifdef _HUNTER_PLUGIN_MULTIPROCESSING_ENABLED
    if (unlikely(is_multiprocessing_fork)) {
        HUNTER_PRINT_TRACE("main(): Calling __parents_main__.");
        EXECUTE_MAIN_MODULE(tstate, "__parents_main__", false);

        int exit_code = HANDLE_PROGRAM_EXIT(tstate);

        HUNTER_PRINT_TRACE("main(): Calling __parents_main__ Py_Exit.");
        Py_Exit(exit_code);
#if defined(_WIN32)
    } else if (unlikely(is_joblib_popen_loky_win32)) {
        HUNTER_PRINT_TRACE("main(): Calling joblib.externals.loky.backend.popen_loky_win32.");
        ...
        PyObject *argv_list = HxHGoN_SysGetObject("argv");
        ...
        if (loky_joblib_pipe_handle_arg == 0) {
            ...
            PyObject *kw_values[] = {
                HxHGoN_PyLong_FromLong(loky_joblib_pipe_handle_arg),
                HxHGoN_PyLong_FromLong(loky_joblib_parent_pid_arg),
            };
        ...
        HUNTER_PRINT_TRACE("main(): Calling 'joblib.externals.loky.backend.popen_loky_posix' Py_Exit.");
        Py_Exit(exit_code);
#else
    } else if (unlikely(is_joblib_popen_loky_posix)) {
        HUNTER_PRINT_TRACE("main(): Calling joblib.externals.loky.backend.popen_loky_posix.");
        ...
        int res = PyList_SetSlice(HxHGoN_SysGetObject("argv"), 0, 2, const_tuple_empty);
        assert(res == 0);
        ...
        HUNTER_PRINT_TRACE("main(): Launching as 'multiprocessing.resource_tracker'.");
        ...
        CALL_FUNCTION_WITH_SINGLE_ARG(tstate, main_function,
                                      HxHGoN_PyInt_FromLong(multiprocessing_resource_tracker_arg));

        int exit_code = HANDLE_PROGRAM_EXIT(tstate);

        HUNTER_PRINT_TRACE("main(): Calling 'multiprocessing.resource_tracker' Py_Exit.");
        Py_Exit(exit_code);
    } else if (unlikely(loky_resource_tracker_arg != -1)) {
        HUNTER_PRINT_TRACE("main(): Launching as 'joblib.externals.loky.backend.resource_tracker'.");
        ...
        CALL_FUNCTION_WITH_SINGLE_ARG(tstate, main_function, HxHGoN_PyInt_FromLong(loky_resource_tracker_arg));

        int exit_code = HANDLE_PROGRAM_EXIT(tstate);

        HUNTER_PRINT_TRACE("main(): Calling 'joblib.externals.loky.backend.resource_tracker' Py_Exit.");
        Py_Exit(exit_code);
        ...
        HUNTER_PRINT_TRACE("main(): Calling 'anyio.to_process' Py_Exit.");
        Py_Exit(exit_code);
    } else {
#endif
#if defined(_HUNTER_ONEFILE_MODE) && defined(_WIN32)
        {
            char buffer[128] = {0};
            DWORD size = GetEnvironmentVariableA("HUNTER_ONEFILE_PARENT", buffer, sizeof(buffer));
...
#endif
        PyDict_DelItemString(HxHGoN_GetSysModules(), HUNTER_MAIN_MODULE_NAME);
        DROP_ERROR_OCCURRED(tstate);

#if _HUNTER_PLUGIN_WINDOWS_SERVICE_ENABLED
        HUNTER_PRINT_TRACE("main(): Calling plugin SvcLaunchService() entry point.");
        SvcLaunchService();
#else
    /* Execute the "__main__" module. */
    HUNTER_PRINT_TIMING("main(): Calling " HUNTER_MAIN_MODULE_NAME ".");
    EXECUTE_MAIN_MODULE(tstate, HUNTER_MAIN_MODULE_NAME, HUNTER_MAIN_IS_PACKAGE_BOOL);
    HUNTER_PRINT_TIMING("main(): Exited from " HUNTER_MAIN_MODULE_NAME ".");
...
#if !HUNTER_MAIN_IS_PACKAGE_BOOL
    ...
    HUNTER_PRINT_TIMING("main(): Calling Py_Exit.");
    Py_Exit(exit_code);

    // The "Py_Exit()" calls is not supposed to return.
    HUNTER_CANNOT_GET_HERE("Py_Exit does not return");
}

#ifdef _HUNTER_WINMAIN_ENTRY_POINT
int __stdcall wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, wchar_t *lpCmdLine, int nCmdShow) {
    /* MSVC, MINGW64 */
    int argc = __argc;
    wchar_t **argv = __wargv;

    return HxHGoN_Main(argc, argv);
}
#else
#if defined(_WIN32)
int wmain(int argc, wchar_t **argv) { return HxHGoN_Main(argc, argv); }
#else
int main(int argc, char **argv) { return HxHGoN_Main(argc, argv); }
#endif
#endif
...
#if defined(_WIN32)
#define HUNTER_DLL_FUNCTION __declspec(dllexport)
#else
#define HUNTER_DLL_FUNCTION __attribute__((visibility("default")))
#endif

HUNTER_DLL_FUNCTION int run_code(int argc, native_command_line_argument_t **argv) { return HxHGoN_Main(argc, argv); }
#endif
