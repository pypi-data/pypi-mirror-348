...
#ifndef __HUNTER_PRELUDE_H__
#define __HUNTER_PRELUDE_H__
 ...
 */
#ifdef __GNUC__
#define HUNTER_MAY_BE_UNUSED __attribute__((__unused__))
#else
#define HUNTER_MAY_BE_UNUSED
#endif
...
#define HUNTER_USE_PYCORE_THREAD_STATE
#endif

#ifdef HUNTER_USE_PYCORE_THREAD_STATE
#undef Py_BUILD_CORE
...
static inline size_t HxHGoN_static_builtin_index_get(PyTypeObject *self) { return (size_t)self->tp_subclasses - 1; }

// Changed internal type access for Python3.13
#if PYTHON_VERSION < 0x3d0
#define managed_static_type_state static_builtin_state

static inline managed_static_type_state *HxHGoN_static_builtin_state_get(PyInterpreterState *interp,
                                                                         PyTypeObject *self) {
    return &(interp->types.builtins[HxHGoN_static_builtin_index_get(self)]);
}
#else
static inline managed_static_type_state *HxHGoN_static_builtin_state_get(PyInterpreterState *interp,
                                                                         PyTypeObject *self) {
    return &(interp->types.builtins.initialized[HxHGoN_static_builtin_index_get(self)]);
}
#endif

HUNTER_MAY_BE_UNUSED static inline managed_static_type_state *HxHGoN_PyStaticType_GetState(PyInterpreterState *interp,
                                                                                           PyTypeObject *self) {
    assert(self->tp_flags & _Py_TPFLAGS_STATIC_BUILTIN);
    return HxHGoN_static_builtin_state_get(interp, self);
}

#define _PyStaticType_GetState(interp, self) HxHGoN_PyStaticType_GetState(interp, self)
#endif
...
#define HUNTER_NO_RETURN HEDLEY_NO_RETURN

/* This is used to indicate code control flows we know cannot happen. */
#ifndef __HUNTER_NO_ASSERT__
#define HUNTER_CANNOT_GET_HERE(NAME)                                                                                   \
    PRINT_FORMAT("%s : %s\n", __FUNCTION__, #NAME);                                                                    \
    abort();
#else
#define HUNTER_CANNOT_GET_HERE(NAME) abort();
#endif

#define HUNTER_ERROR_EXIT(NAME)                                                                                        \
    PRINT_FORMAT("%s : %s\n", __FUNCTION__, #NAME);                                                                    \
    abort();
 ...
 */
#include <malloc.h>
#define HUNTER_DYNAMIC_ARRAY_DECL(VARIABLE_NAME, ELEMENT_TYPE, COUNT)                                                  \
    ELEMENT_TYPE *VARIABLE_NAME = (ELEMENT_TYPE *)_alloca(sizeof(ELEMENT_TYPE) * (COUNT));
#else
#define HUNTER_DYNAMIC_ARRAY_DECL(VARIABLE_NAME, ELEMENT_TYPE, COUNT) ELEMENT_TYPE VARIABLE_NAME[COUNT];
#endif
...
#define HxHGoN_String_AsString PyString_AsString
#define HxHGoN_String_AsString_Unchecked PyString_AS_STRING
#define HxHGoN_String_Check PyString_Check
#define HxHGoN_String_CheckExact PyString_CheckExact
HUNTER_MAY_BE_UNUSED static inline bool HxHGoN_StringOrUnicode_CheckExact(PyObject *value) {
    return PyString_CheckExact(value) || PyUnicode_CheckExact(value);
}
#define HxHGoN_StringObject PyStringObject
#define HxHGoN_String_FromString PyString_FromString
#define HxHGoN_String_FromStringAndSize PyString_FromStringAndSize
#define HxHGoN_String_FromFormat PyString_FromFormat
#define PyUnicode_CHECK_INTERNED (0)
HUNTER_MAY_BE_UNUSED static Py_UNICODE *HxHGoN_UnicodeAsWideString(PyObject *str, Py_ssize_t *size) {
    PyObject *unicode;
...
#else
#define HxHGoN_String_AsString _PyUnicode_AsString
...
#ifdef __HUNTER_NO_ASSERT__
#define HxHGoN_String_AsString_Unchecked PyUnicode_UTF8
#else
HUNTER_MAY_BE_UNUSED static char const *HxHGoN_String_AsString_Unchecked(PyObject *object) {
    char const *result = PyUnicode_UTF8(object);
    assert(result != NULL);
    return result;
}
#endif
#define HxHGoN_String_Check PyUnicode_Check
#define HxHGoN_String_CheckExact PyUnicode_CheckExact
#define HxHGoN_StringOrUnicode_CheckExact PyUnicode_CheckExact
#define HxHGoN_StringObject PyUnicodeObject
#define HxHGoN_String_FromString PyUnicode_FromString
#define HxHGoN_String_FromStringAndSize PyUnicode_FromStringAndSize
#define HxHGoN_String_FromFormat PyUnicode_FromFormat
#define HxHGoN_UnicodeAsWideString PyUnicode_AsWideCharString
#endif
...
#ifdef __HUNTER_NO_ASSERT__
#define HxHGoN_TypeLookup(x, y) _PyType_Lookup(x, y)
#else
HUNTER_MAY_BE_UNUSED static PyObject *HxHGoN_TypeLookup(PyTypeObject *type, PyObject *name) {
...
#if PYTHON_VERSION < 0x300
#define HUNTER_MODULE_ENTRY_FUNCTION void
#else
#define HUNTER_MODULE_ENTRY_FUNCTION PyObject *
#endif
 ...
 */
#define HUNTER_CROSS_MODULE
#define HUNTER_LOCAL_MODULE static
...
#define HxHGoN_GC_Track PyObject_GC_Track
#define HxHGoN_GC_UnTrack PyObject_GC_UnTrack
...
#undef _PyObject_GC_TRACK
#undef _PyObject_GC_UNTRACK
#elif _HUNTER_MODULE_MODE && PYTHON_VERSION >= 0x370 && PYTHON_VERSION < 0x380
#define HxHGoN_GC_Track PyObject_GC_Track
#define HxHGoN_GC_UnTrack PyObject_GC_UnTrack
#undef _PyObject_GC_TRACK
#undef _PyObject_GC_UNTRACK
...
#else
#define HxHGoN_GC_Track _PyObject_GC_TRACK
#define HxHGoN_GC_UnTrack _PyObject_GC_UNTRACK
#endif

#if _HUNTER_EXPERIMENTAL_FAST_THREAD_GET && PYTHON_VERSION >= 0x300 && PYTHON_VERSION < 0x370
...
#if PYTHON_VERSION < 0x300
#define HxHGoNType_HasFeatureClass(descr) (PyType_HasFeature(Py_TYPE(descr), Py_TPFLAGS_HAVE_CLASS))
#else
#define HxHGoNType_HasFeatureClass(descr) (1)
#endif
...
extern bool HxHGoN_Type_IsSubtype(PyTypeObject *a, PyTypeObject *b);

#include "Gon/allocator.h"
#include "Gon/exceptions.h"
...
typedef long Gon_digit;

#include "Gon/helpers.h"

#include "Gon/compiled_frame.h"

#include "Gon/compiled_cell.h"

#include "Gon/compiled_function.h"

/* Sentinel PyObject to be used for all our call iterator endings. */
extern PyObject *HxHGoN_sentinel_value;

/* Value to use for __compiled__ value of all modules. */
extern PyObject *HxHGoN_dunder_compiled_value;

#include "Gon/compiled_generator.h"

#include "Gon/compiled_method.h"

#if PYTHON_VERSION >= 0x350
#include "Gon/compiled_coroutine.h"
#endif

#if PYTHON_VERSION >= 0x360
#include "Gon/compiled_asyncgen.h"
#endif

#include "Gon/filesystem_paths.h"
#include "Gon/safe_string_ops.h"

#include "Gon/jit_sources.h"

#if _HUNTER_EXPERIMENTAL_WRITEABLE_CONSTANTS
#include "Gon_data_decoder.h"
#else
#define DECODE(x) assert(x)
#define UN_TRANSLATE(x) (x)
#endif

#if _HUNTER_EXPERIMENTAL_FILE_TRACING
#include "Gon_file_tracer.h"
#else
#if PYTHON_VERSION < 0x300
...
#if _HUNTER_EXPERIMENTAL_INIT_PROGRAM
#include "Gon_init_program.h"
#else
#define HUNTER_INIT_PROGRAM_EARLY(argc, argv)
#define HUNTER_INIT_PROGRAM_LATE(module_name)
#endif

#if _HUNTER_EXPERIMENTAL_EXIT_PROGRAM
#include "Gon_exit_program.h"
#else
#define HUNTER_FINALIZE_PROGRAM(tstate)
#endif
