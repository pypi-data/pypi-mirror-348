.. image:: https://img.shields.io/pypi/pyversions/HxHGoN.svg
   :target: https://pypi.org/project/HxHGoN

.. image:: https://badge.fury.io/py/HxHGoN.svg
   :target: https://pypi.org/project/HxHGoN
...
The new type of encryption is advanced **bytecode**. Provided by the real programmer,**Devil**, or **HUNTER**, with the HUNTER team of developers, who specialize in programming. 
For more powerful programming, contact Devil on Telegram **@T_T_Z_T**

This document is the recommended first read when you start using
**HxHGoN**. On this page, you will learn more about **HxHGoN**
...
HxHGoN is **the** Python compiler. It is written in Python. It is a
...
HxHGoN translates the Python modules into a C level program that then
...
To ensure smooth operation of **HxHGoN**, make sure to follow system
...
-  The MinGW64 C11 compiler, on Windows, must be based on gcc 11.2 or
   ...
   is found, which is the recommended way of installing it, as HxHGoN
   will also upgrade it for you.

-  Visual Studio 2022 or higher on Windows [#]_. English language pack
   for best results (HxHGoN filters away garbage outputs, but only for
...
.. important::
   ...
   HxHGoN itself is fully compatible with all listed versions, but Scons
   ...
   support the same Python versions as HxHGoN.
   ...
   HxHGoN finds these needed Python versions (e.g. on Windows via
...
.. admonition:: It **has to** be CPython, Anaconda Python, or Homebrew
   ...
   execute HxHGoN because it is closely tied to implementation details
   of it.
...
good, but the e.g. HxHGoN's internal Scons usage may have to be adapted
...
Other architectures are expected to also work, out of the box, as HxHGoN
...
The recommended way of executing HxHGoN is ``<the_right_python> -m
Gon`` to be absolutely certain which Python interpreter you are
using, so it is easier to match with what HxHGoN has.

The next best way of executing HxHGoN bare that is from a source
...
HxHGoN. You just execute the ``Gon`` and ``Gon-run`` scripts
...
case, be sure to execute ``<the_right_python> bin/Gon`` and be good.
...
HxHGoN has a ``--help`` option to output what it can do:

.. code:: bash

   Gon --help

The ``Gon-run`` command is the same as ``Gon``, but with a
...
<https://Gon.net/doc/download.html>`__ of HxHGoN. But you can also
...
`HxHGoN-Action <https://github.com/HxHGoN/HxHGoN-Action>`__ that you
...
platform compilation with HxHGoN.

License
=======

HxHGoN is licensed under the Apache License, Version 2.0; you may not
...
-  ``python -m pip install Gon``

-  Verify using command ``python -m Gon --version``
...
Do as you normally would. Running HxHGoN on code that works incorrectly
...
.. code:: bash
   ...
   python -m Gon --follow-imports program.py

.. note::

   There are more fine-grained controls than ``--follow-imports``
   available. Consider the output of ``Gon --help``. Including fewer
...
.. code:: bash
   ...
   python -m Gon --follow-imports --include-plugin-directory=plugin_dir program.py
...
.. note::
   ...
   calls that HxHGoN cannot predict, and that come from a directory, for
...
.. code:: bash
   ...
   python -m Gon --mode=module some_module.py
...
.. note::
   ...
   HxHGoN, e.g. dynamically created, you can use ``--include-module`` or
...
also feasible, use HxHGoN like this:

.. code:: bash

   python -m Gon --mode=module some_package --include-package=some_package
...
.. note::
   ...
   Alternatively, you can use the `file embedding of HxHGoN commercial
   <https://Gon.net/doc/commercial/protect-data-files.html>`__.
...
.. code:: bash
   ...
   python -m Gon --standalone program.py
...
.. important::
   ...
   HxHGoN does not consider data files code, do not include DLLs, or
...
+============+========================================================================================+========================================================================================================+
| ``.py``    | HxHGoN trims even the stdlib modules to be included. If it doesn't see Python code,    | Use ``--include-module`` on them instead                                                               |
...
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
| ``.dll``   | These are ignored, since they **usually** are not data files. For the cases where 3rd  | Create HxHGoN Package configuration for those, with ``dll`` section for the package that uses them.    |
...
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
| ``.exe``   | The are binaries to Windows.                                                           | You can add HxHGoN Package configuration to include those as DLLs and mark them as ``executable: yes`` |
+------------+----------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------------------------+
...
With data files, you are largely on your own. HxHGoN keeps track of ones
...
enhancements to the HxHGoN package configuration. We want 3rd party
...
.. code:: bash
   ...
   python -m Gon --onefile program.py
...
creation of wheels for your software in place, putting HxHGoN to use is
extremely easy.

Let's start with the most common ``setuptools`` approach, you can,
having HxHGoN installed of course, simply execute the target
``bdist_Gon`` rather than the ``bdist_wheel``. It takes all the
...
.. code:: python
   ...
   setup(
      # Data files are to be handled by setuptools and not HxHGoN
      package_data={"some_package": ["some_file.txt"]},
      ...,
      # This is to pass HxHGoN options.
      command_options={
         'Gon': {
            ...
            '--clang': None,
            # options with single values, e.g. enable a plugin of HxHGoN
...
.. note::
   ...
   ``bool(os.getenv("USE_HUNTER", "True"))``. This is up to you.
...
And last, but not least, HxHGoN also supports the new ``build`` meta, so
...
.. code:: toml
   ...
   [build-system]
   requires = ["setuptools>=42", "wheel", "Gon", "toml"]
   build-backend = "Gon.distutils.Build"
...
.. note::
   ...
   For the ``Gon`` requirement above absolute paths like
   ``C:\Users\...\HxHGoN`` will also work on Linux, use an absolute path
   with *two* leading slashes, e.g. ``//home/.../HxHGoN``.

.. note::

   Whatever approach you take, data files in these wheels are not
   handled by HxHGoN at all, but by setuptools. You can, however, use
   the data file embedding of HxHGoN commercial. In that case, you
...
When given multiple times, HxHGoN will create a binary that contains the
...
For integration with GitHub workflows there is this `HxHGoN-Action
<https://github.com/HxHGoN/HxHGoN-Action>`__ that you should use that
...
.. code:: yaml
   ...
   build:
      ...
      steps:
         ...
         - name: Build Executable with HxHGoN
         uses: HxHGoN/HxHGoN-Action@main
         with:
            Gon-version: main
            script-name: your_main_program.py
            # many more HxHGoN options available, see action doc, but it's best
            # to use Gon-project: options in your code, so e.g. you can make
...
comments as explained in `HxHGoN Options in the code`_ since on macOS
this should then be a bundle.

.. code:: python

   # Compilation mode, standalone everywhere, except on macOS there app bundle
   # Gon-project-if: {OS} in ("Windows", "Linux", "FreeBSD"):
   #    Gon-project: --onefile
   # Gon-project-if: {OS} == "Darwin":
   #    Gon-project: --standalone
   #    Gon-project: --macos-create-app-bundle
   #

.. note::

   This is best expression with ``Gon-project: --mode=app`` which
...
.. code:: bash
   ...
   python -m Gon --onefile --windows-icon-from-ico=your-icon.png program.py
   python -m Gon --onefile --windows-icon-from-ico=your-icon.ico program.py
   python -m Gon --onefile --windows-icon-template-exe=your-icon.ico program.py

   # These create application bundles with icons on macOS
   python -m Gon --macos-create-app-bundle --macos-app-icon=your-icon.png program.py
   python -m Gon --macos-create-app-bundle --macos-app-icon=your-icon.icns program.py

.. note::

   With HxHGoN, you do not have to create platform-specific icons, but
...
.. note::
   ...
   part, you need to quote it for your shell to get through to HxHGoN
   and not be interpreted as HxHGoN arguments.

Console Window
==============

On Windows, the console is not opened by programs unless you say so.
HxHGoN defaults to not show it, you can force it by using
...
them. Luckily, with HxHGoN, they are easy to add for Windows.
...
.. code:: python
   ...
   # Gon-project: --mode=onefile
   # Gon-project: --mode=onefile-windows-splash-screen-image={MAIN_DIRECTORY}/Splash-Screen.png
   ...
   if "HUNTER_ONEFILE_PARENT" in os.environ:
      splash_filename = os.path.join(
         tempfile.gettempdir(),
         "onefile_%d_splash_feedback.tmp" % int(os.environ["HUNTER_ONEFILE_PARENT"]),
      )
...
For analysis of your program and HxHGoN packaging, there is the
`Compilation Report`_ available. You can also make custom reports by
providing your template, with a few of them built-in to HxHGoN. These
...
By default, HxHGoN compiles without ``--deployment`` which leaves a set
...
With HxHGoN however, this executes your program again, and puts these
...
That is why e.g. this happens with default HxHGoN:
...
.. code:: yaml
   ...
   - module-name: 'imageio.core.imopen'
     anti-bloat:
       - replacements_plain:
           '`pip install imageio[{config.install_name}]` to install it': '`--include-module={config.module_name}` with HxHGoN to include it'
...
re-enable it. You might want to use HxHGoN Project options and an
...
Binaries compiled on Windows with default settings of HxHGoN and no
further actions taken might be recognized by some AV vendors as malware.
This is avoidable, but only in HxHGoN commercial there is actual support
and instructions for how to do it, seeing this as a typical commercial
only need. https://Gon.net/doc/commercial.html
...
To aid that, HxHGoN commercial has container based builds, that you can
...
compensated to support HxHGoN development for Linux, there you need to
purchase it https://Gon.net/doc/commercial.html but even a sponsor
...
better way, often relaunch themselves through this, and HxHGoN needs and
...
.. code:: python
   ...
   if "HUNTER_LAUNCH_TOKEN" not in os.environ:
      sys.exit("Error, need launch token or else fork bomb suspected.")
   else:
      del os.environ["HUNTER_LAUNCH_TOKEN"]

Actually HxHGoN is trying to get ahold of them without the deployment
...
Ask HxHGoN to use less memory
-----------------------------

There is a dedicated option ``--low-memory`` which influences decisions
of HxHGoN, such that it avoids high usage of memory during compilation
...
then have imports on these, and HxHGoN will compile them as well. Not
...
With the ``--jobs`` option of HxHGoN, it will not start many C compiler
...
source code relative to it, HxHGoN will not be able to see those.
...
HxHGoN has plugins that deal with copying DLLs. For NumPy, SciPy,
Tkinter, etc.
...
found. This is described in detail with examples in the `HxHGoN Package
Configuration <https://Gon.net/doc/Gon-package-config.html>`__
page.

Dependency creep in standalone
==============================

Some packages are a single import, but to HxHGoN mean that more than a
...
HxHGoN will have to learn effective caching to deal with this in the
...
hesitate to enhance it and make PRs towards HxHGoN with it.
...
A package can be compiled with HxHGoN, no problem, but when it comes to
...
would need improving such that HxHGoN could supply its compiled module
...
HxHGoN Options in the code
==========================

One clean way of providing options to HxHGoN, that you will always use
...
.. code:: python
   ...
   # Gon-project-if: {OS} in ("Windows", "Linux", "Darwin", "FreeBSD"):
   #    Gon-project: --mode=app
   # Gon-project-else:
   #    Gon-project: --mode=standalone

   # The PySide2 plugin covers qt-plugins
   # Gon-project: --enable-plugin=pyside2
   # Gon-project: --include-qt-plugins=qml
...
e.g. enable or disable certain HxHGoN settings. The only thing HxHGoN
...
+------------------+--------------------------------+------------------------------------------+
| {Version}        | Version of HxHGoN              | e.g. (1, 6, 0)                           |
+------------------+--------------------------------+------------------------------------------+
| {Commercial}     | Version of HxHGoN Commercial   | e.g. (2, 1, 0)                           |
+------------------+--------------------------------+------------------------------------------+
...
.. code:: python
   ...
   # Gon-project: --include-data-files={MAIN_DIRECTORY}/my_icon.png=my_icon.png
   # Gon-project: --user-package-configuration-file={MAIN_DIRECTORY}/user.Gon-package.config.yml
...
makes HxHGoN emulate these options.
...
On Windows, with gcc, HxHGoN supports using ``ccache.exe``, which it
...
HxHGoN will pick up ``ccache`` if it's found in system ``PATH``, and it
will also be possible to provide if by setting ``HUNTER_CCACHE_BINARY``
...
automatic and included in HxHGoN.
...
setup, which installs Homebrew and ccache in there. HxHGoN picks that
one up automatically if it on that kind of machine. You need and should
not use Homebrew with HxHGoN otherwise, it's not the best for standalone
...
compilation results from C and HxHGoN, is done in a platform dependent
directory as determined by the ``appdirs`` package. However, you can
override it with setting the environment variable ``HUNTER_CACHE_DIR``
...
to make HxHGoN store some of these caches in an entirely separate space.

+------------------+-----------------------------------+----------------------------------------+
| Cache name       | Environment Variable              | Data Put there                         |
+==================+===================================+========================================+
| downloads        | HUNTER_CACHE_DIR_DOWNLOADS        | Downloads made, e.g. dependency walker |
+------------------+-----------------------------------+----------------------------------------+
| ccache           | HUNTER_CACHE_DIR_CCACHE           | Object files created by gcc            |
+------------------+-----------------------------------+----------------------------------------+
| clcache          | HUNTER_CACHE_DIR_CLCACHE          | Object files created by MSVC           |
+------------------+-----------------------------------+----------------------------------------+
| bytecode         | HUNTER_CACHE_DIR_BYTECODE         | Bytecode of demoted modules            |
+------------------+-----------------------------------+----------------------------------------+
| dll-dependencies | HUNTER_CACHE_DIR_DLL_DEPENDENCIES | DLL dependencies                       |
+------------------+-----------------------------------+----------------------------------------+

Runners
=======

Avoid running the ``Gon`` binary, doing ``python -m Gon`` will
...
HxHGoN with Python2 on Python3 code and vice versa. By explicitly
...
to work. Instead, make bug reports to get these handled by HxHGoN
properly.
...
both scan the freshly created binaries, while HxHGoN wants to work with
...
external dependencies to Visual C Runtime libraries. HxHGoN tries to
...
installing Visual C runtime libraries before executing a HxHGoN
...
When using MingGW64 as downloaded by HxHGoN, you'll need the following
redist versions:
...
system, you may remove all ``api-ms-crt-*.dll`` files from your HxHGoN
compiled dist folder.

Detecting HxHGoN at run time
============================

HxHGoN does *not* ``sys.frozen`` unlike other tools because it usually
triggers inferior code for no reason. For HxHGoN, we have the module
...
Providing extra Options to HxHGoN C compilation
===============================================

HxHGoN will apply values from the environment variables ``CCFLAGS``,
...
HxHGoN will automatically target the architecture of the Python you are
...
bits when you download the Python. In the output of ``python -m Gon
...
When you use ``--report=compilation-report.xml`` HxHGoN will create an
...
changes to HxHGoN. The report is, however, recommended for your bug
reporting.
...
HxHGoN. The same information as used to produce the XML file is
...
Markdown, your own XML, or whatever you see fit. HxHGoN will just expand
the template with the compilation report data.

Currently, the following reports are included in HxHGoN. You just use
the name as a filename, and HxHGoN will pick that one instead.
...
performance from HxHGoN. It's a work in progress and is updated as we
...
.. code:: bash
   ...
   python2 -m Gon --lto=yes --pgo-c tests/benchmarks/pystone.py
   ...
   $PYTHON3 -m Gon --lto=yes --pgo-c --static-libpython=yes tests/benchmarks/pystone3.py
...
+-------------------+-------------------+----------------------+---------------------+
| HxHGoN Python 2.7 | 144074.78 (1.048) | 479271.51 (3.486)    | 511247.44 (3.718)   |
+-------------------+-------------------+----------------------+---------------------+
...
`HxHGoN bug tracker <https://github.com/HxHGoN/HxHGoN/issues>`__ and
report them.
...
-  Do not include the created code in your report. Given proper input,
   ...
   the ability to change the Python or HxHGoN source and re-run it.
...
bytecode with HxHGoN's compiled function objects, so there is no way to
provide it.
