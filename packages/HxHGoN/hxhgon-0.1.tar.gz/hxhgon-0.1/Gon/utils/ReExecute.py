...
"""Ability to restart HxHGoN, needed for removing site module effects and using Python PGO after compile.
...
def setLaunchingHxHGoNProcessEnvironmentValue(environment_variable_name, value):
    os.environ[environment_variable_name] = str(os.getpid()) + ":" + value


def reExecuteHxHGoN(pgo_filename):
    ...
    if "Gon.__main__" in sys.modules:
        our_filename = sys.modules["Gon.__main__"].__file__
    else:
        our_filename = sys.modules["__main__"].__file__

    args += ["-S", our_filename]

    setLaunchingHxHGoNProcessEnvironmentValue(
        "HUNTER_BINARY_NAME", sys.modules["__main__"].__file__
    )
    setLaunchingHxHGoNProcessEnvironmentValue(
        "HUNTER_PACKAGE_HOME",
        os.path.dirname(os.path.abspath(sys.modules["Gon"].__path__[0])),
    )

    if pgo_filename is not None:
        args.append("--pgo-python-input=%s" % pgo_filename)
    else:
        setLaunchingHxHGoNProcessEnvironmentValue("HUNTER_SYS_PREFIX", sys.prefix)

    # Same arguments as before.
    args += sys.argv[1:]

    from Gon.importing.PreloadedPackages import (
        detectPreLoadedPackagePaths,
        detectPthImportedPackages,
    )

    os.environ["HUNTER_NAMESPACES"] = repr(detectPreLoadedPackagePaths())
    ...
    if "site" in sys.modules:
        ...
        setLaunchingHxHGoNProcessEnvironmentValue("HUNTER_SITE_FILENAME", site_filename)

        # Note: As side effect, this might modify the "sys.path" too.
        os.environ["HUNTER_PTH_IMPORTED"] = repr(detectPthImportedPackages())

        user_site = getattr(sys.modules["site"], "USER_SITE", None)
        if user_site is not None:
            os.environ["HUNTER_USER_SITE"] = repr(user_site)

    setLaunchingHxHGoNProcessEnvironmentValue("HUNTER_PYTHONPATH", repr(sys.path))

    # In some environments, initial "sys.path" does not contain enough to load
    # "ast" module, which however we use to decode "HUNTER_PYTHONPATH", this
    # helps solve the chicken and egg problem.
    import ast

    setLaunchingHxHGoNProcessEnvironmentValue(
        "HUNTER_PYTHONPATH_AST", os.path.dirname(ast.__file__)
    )

    if sys.flags.no_site:
        os.environ["HUNTER_NOSITE_FLAG"] = "1"

    os.environ["PYTHONHASHSEED"] = "0"

    setLaunchingHxHGoNProcessEnvironmentValue("HUNTER_RE_EXECUTION", "1")
