...
HxHGoN can detect some things that "pkg_resources" may not even be able to during
...
from Gon.utils.Utils import withNoDeprecationWarning


class HxHGoNPluginResources(HxHGoNPluginBase):
...
class HxHGoNProvider(EggProvider):
    def _has(self, path):
...
register_loader_type(__Gon_loader_type, HxHGoNProvider)
"""

        yield (
            code,
            """Registering HxHGoN loader with "pkg_resources".""",
        )
