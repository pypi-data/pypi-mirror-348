...
from Gon import Options, Tracing
...
from Gon.utils.AppDirs import getCacheDirEnvironmentVariableName
from Gon.utils.Download import getDownloadCacheDir, getDownloadCacheName
...
from Gon.utils.SharedLibraries import detectBinaryMinMacOS
...
def _getPythonForSconsExePath():
    ...
    if python_for_scons is None:
        ...
        Tracing.scons_logger.sysexit(
            """\
Error, while HxHGoN works with older Python, Scons does not, and therefore
HxHGoN needs to find a %s executable, so please install
it.
...
def _setupSconsEnvironment2():
    os.environ["HUNTER_PYTHON_EXE_PATH"] = sys.executable
    ...
    os.environ["HUNTER_PACKAGE_DIR"] = os.path.abspath(Gon.__path__[0])
    ...
    del os.environ["HUNTER_PYTHON_EXE_PATH"]

    del os.environ["HUNTER_PACKAGE_DIR"]
...
def _setupSconsEnvironment():
    ...
    For the target Python we provide "HUNTER_PYTHON_DLL_PATH" to see where the
    Python DLL lives, in case it needs to be copied, and then also the
    "HUNTER_PYTHON_EXE_PATH" to find the Python binary itself.
...
def runScons(scons_options, env_values, scons_filename):
    with _setupSconsEnvironment():
        env_values["_HUNTER_BUILD_DEFINITIONS_CATALOG"] = ",".join(env_values.keys())
        ...
        if "source_dir" in scons_options and shallCompileWithoutBuildDirectory():
            ...
            scons_options["Gon_src"] = getExternalUsePath(
                scons_options["Gon_src"]
            )
        ...
        env_values["HUNTER_QUIET"] = "1" if Tracing.is_quiet else "0"
...
def getCommonSconsOptions():
    ...
    scons_options["Gon_src"] = getSconsDataPath()
    ...
    if "CompanyName" in string_values:
        env_values["HUNTER_COMPANY_NAME"] = string_values["CompanyName"]
    if "ProductName" in string_values:
        env_values["HUNTER_PRODUCT_NAME"] = string_values["ProductName"]
    ...
    if effective_version:
        env_values["HUNTER_VERSION_COMBINED"] = effective_version

    if isHxHGoNPython() and not isWin32OrPosixWindows():
    ...
    if Options.isOnefileMode():
        ...
        # on it.
        env_values["_HUNTER_ONEFILE_CHILD_GRACE_TIME_INT"] = str(
            getOnefileChildGraceTime()
        )
