...
from Gon.__past__ import basestring, unicode
from Gon.containers.OrderedDicts import OrderedDict
from Gon.Progress import enableThreading
from Gon.PythonFlavors import isTermuxPython
...
from Gon.utils.Utils import isLinux, isMacOS, isPosixWindows, isWin32Windows
...
def _enableFlagSettings(env, name, experimental_flags):
    for flag_name in experimental_flags:
        ...
        if value:
            env.Append(CPPDEFINES=[("_HUNTER_%s" % flag_name, value)])
        else:
            env.Append(CPPDEFINES=["_HUNTER_%s" % flag_name])
...
def createEnvironment(
    ...
    env.Gon_python = getArgumentBool("Gon_python", False)
    ...
    if env.standalone_mode:
        env.Append(CPPDEFINES=["_HUNTER_STANDALONE_MODE"])
    ...
    if env.onefile_mode:
        env.Append(CPPDEFINES=["_HUNTER_ONEFILE_MODE"])
    ...
    if env.onefile_dll_mode:
        env.Append(CPPDEFINES=["_HUNTER_ONEFILE_DLL_MODE"])
...
def provideStaticSourceFile(env, sub_path, c11_mode):
    source_filename = os.path.join(env.Gon_src, "static_src", sub_path)
...
def createDefinitionsFile(source_dir, filename, definitions):
    for env_name in os.environ["_HUNTER_BUILD_DEFINITIONS_CATALOG"].split(","):
    ...
    with openTextFile(build_definitions_filename, "w", encoding="utf8") as f:
        for key, value in sorted(definitions.items()):
            if key == "_HUNTER_BUILD_DEFINITIONS_CATALOG":
                continue
    ...
    if not _linker_arch_determined:
        if os.name == "nt":
            ...
            _linker_arch = _getBinaryArch(
                binary=os.environ["HUNTER_PYTHON_EXE_PATH"], mingw_mode=mingw_mode
            )
...
b) Using "--mingw64" lets HxHGoN download MinGW64 for you. Note: MinGW64
...
def makeResultPathFileSystemEncodable(env, result_exe):
    ...
    if os.name == "nt" and not isFilesystemEncodable(result_exe):
        result_exe = os.path.join(
            ...
            "_Gon_temp.pyd" if env.module_mode else "_Gon_temp.exe",
        )
