...
/**
 * This is responsible for collection of HxHGoN Python PGO information. It writes
...
void PGO_Initialize(void) {
    ...
    // shall be written to.
    char const *output_filename = getenv("HUNTER_PGO_OUTPUT");

    if (unlikely(output_filename == NULL)) {
        HUNTER_CANNOT_GET_HERE("HUNTER_PGO_OUTPUT needs to be set");
    }
