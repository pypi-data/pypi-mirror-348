...
#if defined(_HUNTER_CONSTANTS_FROM_LINKER)
...
#elif defined(_HUNTER_CONSTANTS_FROM_CODE)
...
#if defined(_HUNTER_CONSTANTS_FROM_INCBIN)
...
static Py_hash_t HxHGoN_FastHashBytes(const void *value, Py_ssize_t size) {
    if (unlikely(size == 0)) {
        return 0;
    }
...
static Py_hash_t our_list_hash(PyListObject *list) {
    return HxHGoN_FastHashBytes(&list->ob_item[0], Py_SIZE(list) * sizeof(PyObject *));
}
...
static Py_hash_t our_tuple_hash(PyTupleObject *tuple) {
    return HxHGoN_FastHashBytes(&tuple->ob_item[0], Py_SIZE(tuple) * sizeof(PyObject *));
}
...
static Py_hash_t our_set_hash(PyObject *set) {
    ...
    while (_PySet_Next(set, &pos, &key)) {
        result *= 1000003;
        result ^= HxHGoN_FastHashBytes(&key, sizeof(PyObject *));
    }
#else
    Py_hash_t unused;
...
static PyObject *our_set_tp_richcompare(PyObject *set1, PyObject *set2, int op) {
    ...
    if (Py_SIZE(set1) != Py_SIZE(set2)) {
        ...
        while (_PySet_Next(set1, &pos1, &key1)) {
            {
                HUNTER_MAY_BE_UNUSED int res = _PySet_Next(set2, &pos2, &key2);
                assert(res != 0);
            }
        ...
        while (_PySet_NextEntry(set1, &pos1, &key1, &unused1)) {
            {
                HUNTER_MAY_BE_UNUSED int res = _PySet_NextEntry(set2, &pos2, &key2, &unused2);
                assert(res != 0);
            }
...
static Py_hash_t our_dict_hash(PyObject *dict) {
    ...
    while (HxHGoN_DictNext(dict, &pos, &key, &value)) {
        result *= 1000003;
        result ^= HxHGoN_FastHashBytes(&key, sizeof(PyObject *));
        result *= 1000003;
        result ^= HxHGoN_FastHashBytes(&value, sizeof(PyObject *));
    }
...
static PyObject *our_dict_tp_richcompare(PyObject *a, PyObject *b, int op) {
    ...
    if (Py_SIZE(a) != Py_SIZE(b)) {
        ...
        // effect intended.
        while (HxHGoN_DictNext(a, &pos1, &key1, &value1)) {
            {
                HUNTER_MAY_BE_UNUSED int res = HxHGoN_DictNext(b, &pos2, &key2, &value2);
                assert(res != 0);
            }
...
PyObject **HxHGoN_Long_SmallValues;
#elif PYTHON_VERSION >= 0x300
PyObject *HxHGoN_Long_SmallValues[HUNTER_STATIC_SMALLINT_VALUE_MAX - HUNTER_STATIC_SMALLINT_VALUE_MIN + 1];
#endif
#endif
...
static void initCaches(void) {
    ...
    HxHGoN_Long_SmallValues = (PyObject **)_PyInterpreterState_GET()->small_ints;
#elif PYTHON_VERSION >= 0x300
    for (long i = HUNTER_STATIC_SMALLINT_VALUE_MIN; i < HUNTER_STATIC_SMALLINT_VALUE_MAX; i++) {
        // Have to use the original API here since out "HxHGoN_PyLong_FromLong"
        // would insist on using "HxHGoN_Long_SmallValues" to produce it.
        PyObject *value = PyLong_FromLong(i);
        HxHGoN_Long_SmallValues[HUNTER_TO_SMALL_VALUE_OFFSET(i)] = value;
    }
#endif
#endif
...
static PyObject *_unpackAnonValue(unsigned char anon_index) {
    ...
    default:
        ...
        HUNTER_CANNOT_GET_HERE("Corrupt constants blob");
    }
}
...
static PyObject *_HxHGoN_Unicode_ImmortalFromStringAndSize(PyThreadState *tstate, const char *data, Py_ssize_t size,
                                                           bool is_ascii) {
#if PYTHON_VERSION < 0x300
...
#if PYTHON_VERSION >= 0x3c0 && !defined(__HUNTER_NO_ASSERT__)
...
static unsigned char const *_unpackBlobConstant(PyThreadState *tstate, PyObject **output, unsigned char const *data) {
    ...
    *output = NULL;
...
#ifdef _HUNTER_EXPERIMENTAL_DEBUG_CONSTANTS
    ...
    case 'D': {
        ...
        if (size > 0) {
            HUNTER_DYNAMIC_ARRAY_DECL(keys, PyObject *, size);
            HUNTER_DYNAMIC_ARRAY_DECL(values, PyObject *, size);
            ...
            if (size == 0) {
                ...
                if (empty_frozenset == NULL) {
                    empty_frozenset = CALL_FUNCTION_WITH_SINGLE_ARG(tstate, (PyObject *)&PyFrozenSet_Type,
                                                                    HxHGoN_Bytes_FromStringAndSize("", 0));
                }
    ...
    case 'q': {
        ...
        PyObject *l = HxHGoN_LongFromCLong((c == 'l') ? ((long)value) : (-(long)value));
        assert(l != NULL);

        // Avoid the long cache, won't do anything useful for small ints
#if PYTHON_VERSION >= 0x300
        if (value < HUNTER_STATIC_SMALLINT_VALUE_MIN || value >= HUNTER_STATIC_SMALLINT_VALUE_MAX)
#endif
        {
    ...
    case 'G':
    case 'g': {
        PyObject *result = HxHGoN_PyLong_FromLong(0);

        int size = (int)_unpackVariableLength(&data);

        PyObject *shift = HxHGoN_PyLong_FromLong(31);

        for (int i = 0; i < size; i++) {
            result = PyNumber_InPlaceLshift(result, shift);

            uint64_t value = _unpackVariableLength(&data);
            PyObject *part = HxHGoN_LongFromCLong((long)value);
            assert(part != NULL);
    ...
    case 'c': {
        ...
        PyObject *b = HxHGoN_Bytes_FromStringAndSize((const char *)data, size);
        CHECK_OBJECT(b);
    ...
    case 'd': {
        ...
        *output = s;
#else
        PyObject *b = HxHGoN_Bytes_FromStringAndSize((const char *)data, 1);
        data += 1;
        *output = b;
#endif
    ...
    case 'w': {
        ...
        PyObject *u = _HxHGoN_Unicode_ImmortalFromStringAndSize(tstate, (const char *)data, 1, true);
        data += 1;
    ...
    case 'u': { // Python2 unicode, Python3 str, zero terminated.
        ...
        PyObject *u = _HxHGoN_Unicode_ImmortalFromStringAndSize(tstate, (const char *)data, size, c == 'a');
        data += size + 1;
    ...
    case 'v': {
        ...
        PyObject *u = _HxHGoN_Unicode_ImmortalFromStringAndSize(tstate, (const char *)data, size, false);
        data += size;
    ...
    case 's': {
        *output = _HxHGoN_Unicode_ImmortalFromStringAndSize(tstate, (const char *)data, 0, true);
        is_object = true;
...
#if _HUNTER_CONSTANTS_FROM_MACOS_SECTION
...
void loadConstantsBlob(PyThreadState *tstate, PyObject **output, char const *name) {
    ...
    if (init_done == false) {
        HUNTER_PRINT_TIMING("loadConstantsBlob(): One time init.");
...
#elif defined(_HUNTER_CONSTANTS_FROM_RESOURCE)
#if _HUNTER_EXE_MODE
        ...
        assert(constant_bin);
#elif _HUNTER_CONSTANTS_FROM_MACOS_SECTION
        constant_bin = findMacOSBinarySection();

        assert(constant_bin);
#endif
        HUNTER_PRINT_TIMING("loadConstantsBlob(): Found blob, decoding now.");
        DECODE(constant_bin);

        HUNTER_PRINT_TIMING("loadConstantsBlob(): CRC32 that blob for correctness.");
        ...
        HUNTER_PRINT_TIMING("loadConstantsBlob(): One time init complete.");
