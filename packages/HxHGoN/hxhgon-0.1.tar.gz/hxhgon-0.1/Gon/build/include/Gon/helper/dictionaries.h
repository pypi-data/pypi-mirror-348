...
#ifndef __HUNTER_DICTIONARIES_H__
#define __HUNTER_DICTIONARIES_H__
...
typedef PyDictEntry *HxHGoN_DictEntryHandle;

static PyDictEntry *GET_STRING_DICT_ENTRY(PyDictObject *dict, HxHGoN_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(HxHGoN_String_CheckExact(key));
...
HUNTER_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(HxHGoN_DictEntryHandle handle) { return handle->me_value; }

HUNTER_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(HxHGoN_DictEntryHandle handle, PyObject *value) {
    handle->me_value = value;
}

static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, HxHGoN_StringObject *key) {
...
typedef PyObject **HxHGoN_DictEntryHandle;

#if PYTHON_VERSION >= 0x3b0
extern Py_ssize_t HxHGoN_Py_unicodekeys_lookup_unicode(PyDictKeysObject *dk, PyObject *key, Py_hash_t hash);

extern Py_ssize_t HxHGoN_PyDictLookup(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
extern Py_ssize_t HxHGoN_PyDictLookupStr(PyDictObject *mp, PyObject *key, Py_hash_t hash, PyObject ***value_addr);
#endif

static inline Py_hash_t HxHGoN_Py_unicode_get_hash(PyObject *o) {
    assert(PyUnicode_CheckExact(o));

    return FT_ATOMIC_LOAD_SSIZE_RELAXED(((PyUnicodeObject *)o)->_base._base.hash);
}

static HxHGoN_DictEntryHandle GET_STRING_DICT_ENTRY(PyDictObject *dict, HxHGoN_StringObject *key) {
    assert(PyDict_CheckExact(dict));
    assert(HxHGoN_String_CheckExact(key));

    Py_hash_t hash = HxHGoN_Py_unicode_get_hash((PyObject *)key);
...
#else
    PyObject **value;
    HUNTER_MAY_BE_UNUSED Py_ssize_t found;

    // We cannot use HxHGoN_PyDictLookupStr in all cases, some modules misbehave
    // and put non-strings in module dictionaries.
    if (unlikely(dict->ma_keys->dk_kind == DICT_KEYS_GENERAL)) {
        found = HxHGoN_PyDictLookup(dict, (PyObject *)key, hash, &value);
    } else {
        found = HxHGoN_PyDictLookupStr(dict, (PyObject *)key, hash, &value);
    }
...
HUNTER_MAY_BE_UNUSED static PyObject *GET_DICT_ENTRY_VALUE(HxHGoN_DictEntryHandle handle) { return *handle; }

HUNTER_MAY_BE_UNUSED static void SET_DICT_ENTRY_VALUE(HxHGoN_DictEntryHandle handle, PyObject *value) {
    *handle = value;
}

HUNTER_MAY_BE_UNUSED static PyObject *GET_STRING_DICT_VALUE(PyDictObject *dict, HxHGoN_StringObject *key) {
    HxHGoN_DictEntryHandle handle = GET_STRING_DICT_ENTRY(dict, key);
...
HUNTER_MAY_BE_UNUSED static bool DICT_SET_ITEM(PyObject *dict, PyObject *key, PyObject *value) {
    CHECK_OBJECT(dict);
...
HUNTER_MAY_BE_UNUSED static bool DICT_REMOVE_ITEM(PyObject *dict, PyObject *key) {
...
#define _HUNTER_MAINTAIN_DICT_VERSION_TAG 1
#endif

#if _HUNTER_MAINTAIN_DICT_VERSION_TAG
extern uint64_t Gon_dict_version_tag_counter;
#endif

HUNTER_MAY_BE_UNUSED static void UPDATE_STRING_DICT0(PyDictObject *dict, HxHGoN_StringObject *key, PyObject *value) {
    CHECK_OBJECT(value);

    HxHGoN_DictEntryHandle entry = GET_STRING_DICT_ENTRY(dict, key);
...
#if _HUNTER_MAINTAIN_DICT_VERSION_TAG
            dict->ma_version_tag = Gon_dict_version_tag_counter++;
#endif
            CHECK_OBJECT(old);
...
HUNTER_MAY_BE_UNUSED static void UPDATE_STRING_DICT_INPLACE(PyDictObject *dict, HxHGoN_StringObject *key,
                                                            PyObject *value) {
    CHECK_OBJECT(value);
...
HUNTER_MAY_BE_UNUSED static void UPDATE_STRING_DICT1(PyDictObject *dict, HxHGoN_StringObject *key, PyObject *value) {
    CHECK_OBJECT(key);
    CHECK_OBJECT(value);
...
extern bool HxHGoN_DictNext(PyObject *dict, Py_ssize_t *pos, PyObject **key_ptr, PyObject **value_ptr);

#if PYTHON_VERSION >= 0x3a0 && !defined(_HUNTER_EXPERIMENTAL_DISABLE_FREELIST_ALL) &&                                  \
    !defined(_HUNTER_EXPERIMENTAL_DISABLE_FREELIST_DICT)
#define HUNTER_DICT_HAS_FREELIST 1

// Replacement for PyDict_New that is faster
extern PyObject *MAKE_DICT_EMPTY(PyThreadState *tstate);
#else
#define HUNTER_DICT_HAS_FREELIST 0
