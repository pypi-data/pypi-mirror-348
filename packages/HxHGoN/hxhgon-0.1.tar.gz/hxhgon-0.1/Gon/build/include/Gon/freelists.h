...
#ifndef __HUNTER_FREELISTS_H__
#define __HUNTER_FREELISTS_H__

#ifndef __cplusplus
#include <stdbool.h>
#endif

#ifdef _HUNTER_EXPERIMENTAL_DISABLE_FREELIST_ALL
    ...
    if (free_list != NULL) {                                                                                           \
        ...
        if (Py_SIZE(result) < size) {                                                                                  \
            ...
            assert(result != NULL);                                                                                    \
                                                                                                                       ...
                                                                                                                       \
        HxHGoN_Py_NewReference((PyObject *)result);                                                                    \
    } else {                                                                                                           \
        result = (object_type *)HxHGoN_GC_NewVar(&type_type, size);                                                    \
    }                                                                                                                  \
    CHECK_OBJECT(result);
...
#define allocateFromFreeListFixed(free_list, object_type, type_type)                                                   \
    if (free_list != NULL) {                                                                                           \
        ...
        result = (object_type *)HxHGoN_GC_New(&type_type);                                                             \
    }                                                                                                                  \
    CHECK_OBJECT(result);
...
HUNTER_MAY_BE_UNUSED static inline struct _Py_object_freelists *_HxHGoN_object_freelists_GET(PyThreadState *tstate) {
