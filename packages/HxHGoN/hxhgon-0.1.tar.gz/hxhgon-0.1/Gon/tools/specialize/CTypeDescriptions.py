...
from Gon.__past__ import long
from Gon.code_generation.BinaryOperationHelperDefinitions import (
    isCommutativeType,
)
from Gon.code_generation.Namify import namifyConstant
...
class TypeDescBase(getMetaClassBase("Type", require_slots=False)):
    ...
    def getIndexCheckExpression(self, operand):
        if self.hasSlot("nb_index"):
            ...
            return "HxHGoN_Index_Check(%s)" % operand
        else:
            return "0"
    ...
    def getTypeSubTypeCheckExpression(self, other, type2, type1):
        return "HxHGoN_Type_IsSubtype(%s, %s)" % (
...
return %(return_value)s;""" % {
    ...
    def getTypeComparisonSpecializationCode(
        ...
        if helper_target is target:
            ...
            return "return %s(%s, %s) ? HUNTER_BOOL_TRUE : HUNTER_BOOL_FALSE;" % (
                helper_name,
                operand1,
                operand2,
            )
    ...
    def getAssignFromLongExpressionCode(cls, result, operand):
        if cls.type_name == "object":
            ...
            return "%s = HxHGoN_PyInt_FromLong(%s);" % (result, operand)
        elif cls.type_name == "long":
            return "%s = HxHGoN_LongFromCLong(%s);" % (result, operand)
        elif cls.type_name == "nbool":
            return "%s = %s;" % (
                result,
    ...
    def getAssignFromLongConstantCode(cls, result, value):
        if cls.type_name in ("object", "long"):
            ...
            const_name3 = "HxHGoN_Long_GetSmallValue(%s)" % value
...
class LongDesc(ConcreteNonSequenceTypeBase):
    ...
    def getLongValueSizeExpression(operand):
        return "HxHGoN_LongGetSignedDigitSize(%s_long_object)" % operand

    @staticmethod
    def getLongValueIsNegativeTestExpression(operand):
        return "HxHGoN_LongIsNegative(%s_long_object)" % operand

    @staticmethod
    def getLongValueDigitCountExpression(operand):
        return "HxHGoN_LongGetDigitSize(%s_long_object)" % operand

    @staticmethod
    def getLongValueDigitExpression(operand, index):
        return "HxHGoN_LongGetDigitPointer(%s_long_object)[%s]" % (operand, index)

    @staticmethod
    def getLongValueDigitsPointerExpression(operand):
        return "HxHGoN_LongGetDigitPointer(%s_long_object)" % operand
...
class CLongDesc(ConcreteCTypeBase):
    ...
    @staticmethod
    def getAsObjectValueExpression(operand):
        return "HxHGoN_PyLong_FromLong(%s)" % operand
...
class NBoolDesc(ConcreteCTypeBase):
    ...
    @staticmethod
    def getToValueFromBoolExpression(operand):
        return "%s ? HUNTER_BOOL_TRUE : HUNTER_BOOL_FALSE" % operand

    @classmethod
    def getToValueFromObjectExpression(cls, operand):
        # TODO: Seems wrong, int return values only happen to match Gon_bool here
...
class NILongDesc(DualTypeBase):
    type_name = "nilong"
    type_desc = "HxHGoN int/long/C long value"
    type_decl = "Gon_ilong *"
