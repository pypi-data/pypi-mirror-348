...
from Gon.freezer.DependsExe import getDependsExePath, parseDependsExeOutput
...
from Gon.utils.Utils import isFreeBSD, isMacOS, isWin32Windows
...
def _getRuntimeTraceOfLoadedFilesWin32(logger, command, required):
    ...
    # TODO: Move the handling of this into Gon.tools.Execution module methods.
    try:
...
def _takeSystemCallTraceOutput(logger, path, command):
    ...
    with withEnvironmentVarOverridden("LD_PRELOAD", None):
        if os.getenv("HUNTER_TRACE_COMMANDS", "0") != "0":
