...
class TwoLayerPerceptronRegressor(CaseLevelTaskAdaptor):
    """
    A PyTorch-based 2-Layer Perceptron adaptor for regression tasks.
    Attributes:
        shot_features (np.ndarray): Few-shot feature matrix of shape (n_shots, n_features).
        shot_labels (np.ndarray): Few-shot labels.
        test_features (np.ndarray): Test feature matrix of shape (n_test_samples, n_features).
    ...
    """
    def __init__(self, shot_features, shot_labels, test_features, survival=False, hidden_dim=256, num_epochs=100, learning_rate=0.001, shot_extra_labels=None):
        super().__init__(shot_features, shot_labels, test_features, shot_extra_labels)
        self.survival = survival
