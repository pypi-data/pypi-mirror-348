...
class Source():
    """Represents a photon source.
        ...
        telescope : IACT | None, optional
            The IACT object associated with this source. If None, the source is assumed to be at a fixed position defined by `self.positions.position` otherwise the source is generated at a distance `distance` along the optical axis.
        dtype : type, optional
    ...
    def __init__(self, telescope: Optional['IACT'] = None, dtype: type = np.float32):
        ...
        self.positions: AbstractPhotonPositions = PointLikePos([0,0,5e3]) #: Photon positions generator.
    ...
    def set_target(self, target_surface: str | int | None = None, distance: float = 10000.):
        """Adjust source position to match the center of the target.
        ...
        self.positions.rotation = local_to_pointing_rotation(*self._tel.pointing) # Transform to the pointing direction
        ...
        # Rotate position into the pointing direction
        p = cp.array([0,0,distance]) @ self.positions.rotation.T
        self.positions.position = p
