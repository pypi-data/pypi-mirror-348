...
# Analysis object: /ATLAS_2024_I2765017:TYPE=BSM/sr0l_met_mono
...
ax_title  = r'0-lepton region, $\geq 1$ jet phase space'
ax_xScale = 'linear'
ax_yScale = 'log'
xLims = (200.0, 2600.0)
yLims = (9e-05, 1100.0)
...
labels = {
    'legend' : ([ r"ATLAS Data", r"NNLO QCD $\times$ nNLO EW", r"BSM+SM excl.  1\% "+"\n"+r" ( 2\% expected)" ], r""),
}
...
exec(open(prefix+'sr0l_met_mono__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'sr0l_met_mono.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'sr0l_met_mono.png'), format='PNG')
