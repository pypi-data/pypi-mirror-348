...
# Analysis object: /ATLAS_2024_I2765017:TYPE=BSM/rmiss_cr1im_dphijj_vbf
...
ax_xLabel = r'$\Delta\phi_{jj}$ [rad/$\pi$]'
ax_yLabel = r'$R_\mathrm{miss}$'
ax_title  = r'0-lepton/1-$\mu$ region ratio, VBF phase space'
ax_xScale = 'linear'
ax_yScale = 'log'
xLims = (-1.0, 1.0)
...
labels = {
    'legend' : ([ r"ATLAS Data", r"MEPS@NLO $\times$ nNLO EW", r"BSM+SM No exclusion; expected exclusion was  1\%" ], r""),
}
...
exec(open(prefix+'rmiss_cr1im_dphijj_vbf__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'rmiss_cr1im_dphijj_vbf.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'rmiss_cr1im_dphijj_vbf.png'), format='PNG')
