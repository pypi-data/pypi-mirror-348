...
# Analysis object: /ATLAS_2024_I2765017:TYPE=BSM/rmiss_cr1im_met_mono
...
ax_title  = r'0-lepton/1-$\mu$ region ratio, $\geq 1$ jet phase space'
ax_xScale = 'linear'
ax_yScale = 'log'
xLims = (200.0, 2600.0)
...
labels = {
    'legend' : ([ r"ATLAS Data", r"MEPS@NLO $\times$ nNLO EW", r"BSM+SM No exclusion; expected exclusion was 31\%" ], r""),
}
...
exec(open(prefix+'rmiss_cr1im_met_mono__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'rmiss_cr1im_met_mono.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'rmiss_cr1im_met_mono.png'), format='PNG')
