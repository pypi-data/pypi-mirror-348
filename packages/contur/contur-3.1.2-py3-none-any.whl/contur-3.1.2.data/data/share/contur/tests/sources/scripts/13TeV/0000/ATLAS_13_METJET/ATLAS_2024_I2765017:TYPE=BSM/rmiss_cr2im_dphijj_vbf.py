...
# Analysis object: /ATLAS_2024_I2765017:TYPE=BSM/rmiss_cr2im_dphijj_vbf
...
ax_title  = r'0-lepton/2-$\mu$ region ratio, VBF phase space'
ax_xScale = 'linear'
ax_yScale = 'log'
xLims = (-1.0, 1.0)
...
labels = {
    'legend' : ([ r"ATLAS Data", r"MEPS@NLO $\times$ nNLO EW", r"BSM+SM No exclusion; expected exclusion was  4\%" ], r""),
}
...
exec(open(prefix+'rmiss_cr2im_dphijj_vbf__data.py').read(), dataf)
...
plt.savefig(os.path.join(plot_outdir, 'rmiss_cr2im_dphijj_vbf.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'rmiss_cr2im_dphijj_vbf.png'), format='PNG')
