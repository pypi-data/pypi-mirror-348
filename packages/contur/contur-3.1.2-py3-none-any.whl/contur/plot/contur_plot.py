...
class ConturPlotBase(Depot):
    ...
    def plot_figures(self):
        ...
        plotBase = conturPlot(saveAxes=self.doSavePlotfile, plotTitle=self.plotTitle,
                              ...
                              style=self.style, showcls=self.showcls, show_legend=self.show_legend)
        ...
        # if we don't make a new plot object, the exp limits get added to the legend twice in the dominant pools plot
        # TODO: fix this
        plotBase = conturPlot(saveAxes=self.doSavePlotfile, iLevel=self.iLevel, iOrder=self.iOrder, cpow=self.cpow,
        ...
        for point in self.points:
            ...
            try:
                if self.omittedPools:
                    ...
                    new_grid.grid[xpos][ypos] = point.combined_exclusion_dict[stat_type]['CLs']
            except KeyError:
                cfg.contur_log.warning("No {} stat for point ({},{})".format(stat_type,xpos,ypos))
                point.combined_exclusion_dict[stat_type]['CLs'] = 0.0
        ...
        for point in self.points:
            ...
            try:
                if self.omittedPools:
                    ...
                    self.conturGrid[xpos][ypos] = point.combined_exclusion_dict[cfg.primary_stat]['CLs']
            except KeyError:
                cfg.contur_log.warning("No {} stat for point ({},{})".format(cfg.primary_stat,xpos,ypos))
                point.combined_exclusion_dict[cfg.primary_stat]['CLs'] = 0.0
...
class conturPlot(object):
    ...
    def __init__(self, saveAxes=False, plotTitle="", iLevel=3, iOrder=3, iSigma=0.75, cpow=0.1, style="DRAFT", showcls=False, simplecls=False, interactive_mode=False, primary_stat=cfg.smbg, show_legend=True):
