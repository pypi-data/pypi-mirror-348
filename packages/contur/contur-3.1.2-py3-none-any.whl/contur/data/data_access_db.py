        ...
        for stat_type in cfg.stat_types:   
            ...
            if likelihood_point.get_full_likelihood(stat_type) is not None and likelihood_point.get_full_likelihood(stat_type).getCLs(stat_type) is not None:
                ...
                for obs, exc in likelihood_point.obs_excl_dict.items():
                    c.execute("insert into obs_exclusions (id, run_id, stat_type, exclusion, mu_lower_limit, mu_upper_limit, mu_hat, histo) \
                            values (?,?,?,?,?,?,?,?);", (None, run_id, stat_type, exc[stat_type]['CLs'], exc[stat_type]['mu_lower_limit'], exc[stat_type]['mu_upper_limit'],exc[stat_type]['mu_hat'], obs))
