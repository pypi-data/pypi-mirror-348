...
def get_argparser(arg_group):
    ...
    * group **smtest** for running statistical tests on SM theory
    ...
    if arg_group == 'analysis':
        ...
        active_groups = ['io', 'grid', 'grid_run', 'dress', 'select', 'stats', 'params', 'ana_select', 'beams']
    ...
    # arguments for running on a grid 
    if "grid_run" in active_groups:
        add_grid_run_info(parser)

    # general arguments specifying results grid (for generation or analysis)
...
def add_grid_run_info(parser,default=None):
    ...
    gridruninfo.add_argument("-r", "--results", dest="RESULTS", default=None,
...
def add_grid_info(parser,default=None):
    ...
    gridinfo.add_argument('--databg', dest="DATABG", default=False, action="store_true",help="Include data-as-background analysis.")
...
def add_batch(parser):
    ...
    batch.add_argument('--template', dest='template_file',
                        default=cfg.mceg_template, help='Template generator config file.')
