...
DEFAULT_TEMPLATE_REPO = "gh:jonmatum/draftmk-copier-templates@v0.8.2"
...
def init_project(
    ...
    if internal_config.exists():
        ...
        console.print(
            "[bold yellow][!] mkdocs.internal.yml already exists.[/bold yellow]"
        )
    if public_config.exists():
        logger.info("mkdocs.public.yml already exists. Skipping.")
        console.print(
            "[bold yellow][!] mkdocs.public.yml already exists.[/bold yellow]"
        )
