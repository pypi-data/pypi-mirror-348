...
class TestMachineCodeTranspiler(unittest.TestCase):
    ...
    def test_transpile_clear_register(self):
        ...
        self.assertEqual(resultado, "MUL H1, H1 ; Registro registrado y reutilizado")
    ...
    def test_transpile_eliminate_mov_redundant(self):
        """ Prueba la eliminación de MOV innecesarios. """
        resultado = self.transpiler_x86.transpile("ALLOCATE H1 H1")
        self.assertEqual(resultado, "NOP")

    def test_transpile_register_reuse(self):
        """ Prueba la reutilización de registros en operaciones matemáticas. """
        resultado = self.transpiler_x86.transpile("ADD H1 H2")
        self.assertIn("; Registro registrado y reutilizado", resultado)

    def test_transpile_multiple_register_reuse(self):
        """ Prueba la reutilización de registros en múltiples operaciones matemáticas. """
        self.transpiler_x86.transpile("ADD H1 H2")  # Primera operación, debería marcar H1 en uso
        resultado = self.transpiler_x86.transpile("ADD H1 H3")  # Segunda operación con H1 reutilizado
        self.assertIn("; Registro reutilizado", resultado)
