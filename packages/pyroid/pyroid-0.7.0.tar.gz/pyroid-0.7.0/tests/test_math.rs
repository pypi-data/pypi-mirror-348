...
use pyroid::math::vector::Vector;
use pyroid::math::matrix::Matrix;
use pyroid::math::stats;
...
fn test_vector_operations() {
    ...
    // Test norm
    let norm = v1.norm();
    assert!((norm - (1.0f64.powi(2) + 2.0f64.powi(2) + 3.0f64.powi(2)).sqrt()).abs() < 1e-10);
}
...
fn test_stats_functions() {
    ...
    assert!((std_result - 1.5811388300841898).abs() < 1e-10);
    
    // Test variance
    let var_result = stats::variance(&data);
    assert!((var_result - 2.5).abs() < 1e-10);
