//! Integration tests for the Rust implementations
...
fn test_linear_regression() {
    ...
    assert!(f64::abs(result.coefficients[0] - 2.0) < 1e-10);
    
    // Check the intercept
    assert!(f64::abs(result.intercept) < 1e-10);
    
    // Check the R-squared
    assert!(f64::abs(result.r_squared - 1.0) < 1e-10);
...
fn test_normalize() {
    ...
    // Check the result
    assert!(f64::abs(result[0][0] - 0.0) < 1e-10);  // min value
    assert!(f64::abs(result[2][0] - 1.0) < 1e-10);  // max value
    assert!(f64::abs(result[0][1] - 0.0) < 1e-10);  // min value
    assert!(f64::abs(result[2][1] - 1.0) < 1e-10);  // max value
    ...
    assert!(f64::abs(mean_col0) < 1e-10);
    assert!(f64::abs(mean_col1) < 1e-10);
    
    let std_col0 = f64::sqrt(result.iter().map(|row| (row[0] - mean_col0) * (row[0] - mean_col0)).sum::<f64>() / result.len() as f64);
    let std_col1 = f64::sqrt(result.iter().map(|row| (row[1] - mean_col1) * (row[1] - mean_col1)).sum::<f64>() / result.len() as f64);
    assert!(f64::abs(std_col0 - 1.0) < 1e-10);
    assert!(f64::abs(std_col1 - 1.0) < 1e-10);
...
fn test_distance_matrix() {
    ...
    assert!(f64::abs(result[0][0]) < 1e-10);  // Distance to self is 0
    
    // Calculate expected Euclidean distance
    let expected_dist1 = f64::sqrt((4.0-1.0)*(4.0-1.0) + (5.0-1.0)*(5.0-1.0));
    assert!(f64::abs(result[0][1] - expected_dist1) < 1e-10);
    
    let expected_dist2 = f64::sqrt((10.0-1.0)*(10.0-1.0) + (10.0-1.0)*(10.0-1.0));
    assert!(f64::abs(result[0][2] - expected_dist2) < 1e-10);
    ...
    let expected_dist1 = f64::abs(4.0-1.0) + f64::abs(5.0-1.0);
    assert!(f64::abs(result[0][1] - expected_dist1) < 1e-10);
    
    let expected_dist2 = f64::abs(10.0-1.0) + f64::abs(10.0-1.0);
    assert!(f64::abs(result[0][2] - expected_dist2) < 1e-10);
