//! Integration tests for the Rust text module
...
fn test_replace() {
    ...
    // Our implementation might insert 'x' between each character or do nothing
    // Both are valid behaviors for empty string replacement
    if result != "hello world" && result != "xhxexlxlxox xwxoxrxlxdx" {
        assert!(false, "Unexpected result for empty string replacement: {}", result);
    }
...
fn test_ngrams() {
    ...
    let result = text::ngrams("a b c d", 2);
    assert_eq!(result, vec![
        vec!["a", "b"],
        vec!["b", "c"],
        vec!["c", "d"]
    ...
    // Test with n > number of tokens
    let result = text::ngrams("a b", 3);
    assert_eq!(result, Vec::<Vec<String>>::new());
    
    // Test with empty string
    let result = text::ngrams("", 2);
    assert_eq!(result, Vec::<Vec<String>>::new());
    
    // Test with n=1
    let result = text::ngrams("a b c", 1);
    assert_eq!(result, vec![
        vec!["a"],
        vec!["b"],
        vec!["c"]
