...
EstimationFrequency = Literal["Daily", "Weekly"]
ESTIMATION_FREQUENCY_MAP: dict[EstimationFrequency, int] = {"Daily": 1, "Weekly": 7}
...
class ExposureSettings(Settings):
    ...
    estimation_frequency: EstimationFrequency = Field(
        description="The frequency at which the exposures change.",
        default="Weekly",
        examples=["Daily", "Weekly"],
    )

    @property
    def estimation_frequency_in_days(self) -> int:
        """
        Returns the number of days for the given estimation frequency.
        """
        return ESTIMATION_FREQUENCY_MAP[self.estimation_frequency]
...
class ExposureSettingsMenu(SettingsMenu[ExposureSettings], frozen=True, extra="forbid"):
    ...
    estimation_frequencies: list[EstimationFrequency] = Field(
        description="""
        The available estimation frequencies that can be used for the factor risk model.
        """,
    )
    ...
    def describe(self, settings: ExposureSettings | None = None) -> str:
        if settings is not None:
            ...
            freq = f"\nEstimation Frequency: {settings.estimation_frequency}"
        else:
            hierarchy = self.styles
            standardize_styles = ""
            other = list(self.other_labels.values())
            freq = f"\nEstimation Frequencies: {', '.join(self.estimation_frequencies)}"
    ...
    def validate_settings(self, settings: ExposureSettings) -> None:
        ...
        if settings.estimation_frequency not in self.estimation_frequencies:
            raise ValueError(
                f"Invalid estimation frequency: {settings.estimation_frequency}"
            )
