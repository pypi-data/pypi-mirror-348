...
from fastmcp.server.openapi import RouteMap, RouteType, Tool
...
mcp = FastMCP.from_openapi(openapi_spec=spec, client=api_client, name="Closelink MCP 1.0.2", route_maps=custom_maps)


async def get_updated_tools():
...
async def prepare():
    print("Preparing MCP server..")
    print("Preparing MCP server..")
    print("Preparing MCP server..")
    mcp.tools = await get_updated_tools()
