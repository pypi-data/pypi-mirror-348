...
def check_vision(
    ...
    if isinstance(client, llama_api_client.LlamaAPIClient):
        if stream:
            result = _check_vision_llamaapi_streaming(response, test_case)
        else:
            result = _check_vision_llamaapi_non_streaming(response, test_case)
    elif isinstance(client, openai.OpenAI):
        if stream:
            result = _check_vision_openai_streaming(response, test_case)
        else:
            result = _check_vision_openai_non_streaming(response, test_case)
    else:
