...
def _check_valid_json(prediction: str, expected_output: list[BaseModel]) -> dict[str, Any]:
    try:
        ...
        return {
            "pass": any(expected_item == actual_json for expected_item in expected_output),
        }
...
def check_structured_output(
    ...
    if isinstance(client, llama_api_client.LlamaAPIClient):
        if stream:
            result = _check_structured_output_llamaapi_streaming(response, test_case)
        else:
            result = _check_structured_output_llamaapi_non_streaming(response, test_case)
    elif isinstance(client, openai.OpenAI):
        if stream:
            result = _check_structured_output_openai_streaming(response, test_case)
        else:
            result = _check_structured_output_openai_non_streaming(response, test_case)
    else:
