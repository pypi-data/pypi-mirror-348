...
class StyleRenderer(mistune.HTMLRenderer):
    def __init__(self, style: str, code_style: str = ""):
        super().__init__()
    ...
    def link(self, link, text=None, title=None):
        if self.template_dict[f"link_template"] is not None:
            return self.template_dict[f"link_template"].render(link=link, text=text)
        else:
            if text is None:
                text = link
            s = '<a href="{}"'.format(self.safe_url(link))
            if title:
    ...
    def block_code(self, code, info=None):
        if self.template_dict[f"mac_window_template"] is not None:
            from robot_basic.robot_basic.md_2_html.highlight.renderer_code import (
                renderer_by_node,
            )
