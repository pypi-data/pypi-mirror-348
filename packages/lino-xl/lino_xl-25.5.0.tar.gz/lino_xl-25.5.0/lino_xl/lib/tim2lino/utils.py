...
class TimLoader:
    ...
    def __init__(self, dbpath, languages=None, **kwargs):
        ...
        if indfile.exists():
            ...
            dd.logger.info("Last import time was %s", self.last_import_time)
        else:
    ...
    def load_jnl(self, row, **kw):
        ...
        dd.logger.warning("Ignore journal %s because voucher_type is none", row)
    ...
    def load_dbf(self, tableName, row2obj=None):
        ...
        if self.last_import_time and os.path.getmtime(fn) < self.last_import_time:
            dd.logger.info("No need to import %s", fn)
            return
        if row2obj is None:
