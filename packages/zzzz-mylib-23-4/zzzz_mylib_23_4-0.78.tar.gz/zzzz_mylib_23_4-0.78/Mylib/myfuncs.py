...
def get_weights_path_from_data_transformation_name_59(name):
    """Tìm đường dẫn đến weights (phục vụ cho weighted data transformation) từ data transformation name

    Examples:
    ```
    data transformation name = dt1_weighted_batch_50000 -> result = dt1
    ```

    """
    pattern = r"(\w+)_weighted"
    return re.findall(pattern, name)[0]


def get_batch_size_from_data_transformation_name_60(name):
    """Get batch size (phục vụ cho batche data transformation) từ data transformation name

    Examples:
    ```
    data transformation name = dt1_weighted_batch_50000 -> result = 50000
    ```
    """
    pattern = r"batch_(\d+)"
