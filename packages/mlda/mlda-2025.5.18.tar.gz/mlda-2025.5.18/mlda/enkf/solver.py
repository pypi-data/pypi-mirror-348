...
class Solver:
    ...
    def prep(self, recon_period=None, localize=True, loc_radius=2500, dist_vsf=1, dlat=1, dlon=1, loc_method='gaspari_cohn',
    ...
    def run_t(self, t, method='EnSRF', debug=False):
        ...
        if method in ['EnSRF', 'EnSRF_DASH']:
            post = utils.states2ds(S.X_updated, self.prior.ds_ann)
        elif method == 'EnOI':
            post = utils.states2ds(S.X_target_updated, self.prior_target.ds_ann)
        
        return S, post

    def run(self, method='EnSRF', debug=False, nproc=4, chunksize=10):
