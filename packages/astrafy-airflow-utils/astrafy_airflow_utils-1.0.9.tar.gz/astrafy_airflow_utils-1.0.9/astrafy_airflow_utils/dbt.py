...
def dbt(
        ...
        other_args: str = ""):
    """
    Builds the complete dbt command and the uploads the dbt result files to the GCS bucket managed folder for further reuse. It fails if no `bucket_folder_results` is set in the environment.
...
def dbt_run_only_failed(
        ...
        other_args: str = ""):
    """
    On DAG's try nº 2 it automatically downloads whatever it failed/errored on the last run from the managed folder  given bucket in order to retry to execute it, on others tries it simples runs the util's dbt command.
    ...
    if [ {{{{ ti.try_number }}}} -eq 2 ]; then
        upload_to_gcs.download_failed_models(); gcp=$?;
    else
        dbt(profile_arg,target_arg,f"build --select result:fail+ result:error+ --state /app/target",vars_arg,other_args); dbt_ret=$?;
    fi
