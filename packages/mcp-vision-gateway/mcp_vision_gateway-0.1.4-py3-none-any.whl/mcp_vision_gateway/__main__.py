...
def make_vision_api_request(
    ...
    """向视觉API发送请求，支持流式和非流式响应"""
    debug_log(f"使用模型 {model} 发送请求 (stream={stream})")
...
def vision_query_stream(
    ...
    api_base: Optional[str] = None,
    api_key: Optional[str] = None,
    chunk_delay: float = 0.05  # 每个文本块之间的延迟时间（秒）
) -> str:
    """向AI模型发送查询并以流式方式返回结果
    ...
    参数:
        ...
        api_base: API基础URL (默认使用环境变量)
        api_key: API密钥 (默认使用环境变量)
        chunk_delay: 每个文本块之间的延迟时间（秒），用于模拟打字效果
    """
    config = get_config()
    ...
    # 发送流式请求
    try:
        stream_generator = make_vision_api_request(
            messages=messages,
            model=actual_model,
            api_base=actual_api_base,
            api_key=actual_api_key,
