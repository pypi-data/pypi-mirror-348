...
class SuppressedOutput:
    try:
        def __init__(self, original_stdout, start_str="|>"):
            self.original_stdout = original_stdout
            self.start_str = start_str

        def write(self, text):
            if not text.startswith(self.start_str):
                self.original_stdout.write(text)
...
def suppress_print(func=None, *, start_str=None):
    def decorator(f):
        def wrapper(*args, **kwargs):
            original_stdout = sys.stdout
            sys.stdout = SuppressedOutput(original_stdout, start_str)
            try:
