...
async def log_middleware(request, handler):
    ...
    request_log.debug(f"收到请求: {request.method} {request.path} {headers_info}")
...
async def hd_post_stop_inference(request: web.Request):
    ...
    data = await request.post()       
    tracker.stop_inference_record(token, str(data['answer']), bool(data['is_correct']), bool(data['is_valid']))
    return web.json_response({"status": "success"})
