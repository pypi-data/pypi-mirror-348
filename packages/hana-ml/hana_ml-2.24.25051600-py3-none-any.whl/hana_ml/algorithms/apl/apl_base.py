...
class APLBase(MLBase, ModelSavingServices):
    ...
    def _create_basic_config_table(self, name=None):
        if name is None:
    ...
    @staticmethod
    def _check_valid(data, key=None, features=None, label=None, weight=None):
        """
        Checks whether key, features, label and weight are valid regarding to the given dataframe (data).
        Raises ValueError if any parameter is invalid.
        """
        if features is not None:
            if not features:
                raise ValueError('The list of features provided is empty.')
        l_given = [key, label, weight]
        if features is not None:
        ...
        if not all(chk_list):
            raise ValueError('Some of the values provided in key, label, weight, or features parameters do not match any columns in the DataFrame.')
