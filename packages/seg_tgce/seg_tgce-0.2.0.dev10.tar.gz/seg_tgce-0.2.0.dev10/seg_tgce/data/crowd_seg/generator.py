...
class CrowdSegDataGenerator(Sequence):  # pylint: disable=too-many-instance-attributes
    ...
    def _load_sample(self, filename: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        labeler_mask = np.zeros(len(self.scorers_tags), dtype=np.float32)
        ...
        for scorer_idx, scorer_dir in enumerate(self.scorers_tags):
            ...
            if os.path.exists(mask_path):
                ...
                for class_num in self.classes_definition:
                    masks[..., class_num, scorer_idx] = np.where(
                        mask == class_num, 1.0, 0.0
                    ).reshape(*self.image_size)
                labeler_mask[scorer_idx] = 1.0
        ...
        if self.use_cache:
            ...
            # Add to cache
            self._cache[filename] = (image, masks)
            self._cache_order.append(filename)
    ...
    def __data_generation(
        ...
        for batch, filename in enumerate(batch_filenames):
            ...
            # Create labeler mask based on which annotators provided masks
            labeler_mask[batch] = np.any(sample_masks, axis=(0, 1, 2))
