...
# このスクリプトが実行されていることを明確にするための出力
print("!!! SERVER.PY VERSION 0.1.7 STARTING !!!", file=sys.stderr)

# Setup logging using standard library
logging.basicConfig(level=logging.DEBUG)
logger = logging.getLogger(__name__)

logger.debug("!!! DEBUG logging is active in server.py !!!")

logger.debug(f"CWD at script start: {os.getcwd()}")
logger.debug(f"ESA_TOKEN before load_dotenv: {os.getenv('ESA_TOKEN')}")
logger.debug(f"ESA_TEAM_NAME before load_dotenv: {os.getenv('ESA_TEAM_NAME')}")

# Load environment variables from .env file
dotenv_loaded = load_dotenv()
logger.debug(f"load_dotenv() called. Found and loaded .env file: {dotenv_loaded}")
logger.debug(f"PYTHON_DOTENV_PATH environment variable: {os.getenv('PYTHON_DOTENV_PATH')}")

logger.debug(f"ESA_TOKEN after load_dotenv: {os.getenv('ESA_TOKEN')}")
logger.debug(f"ESA_TEAM_NAME after load_dotenv: {os.getenv('ESA_TEAM_NAME')}")
