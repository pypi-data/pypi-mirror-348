...
class TestPolicyTypes:
    ...
    def test_method_list(self, client: ConnecteamAPISDK) -> None:
        policy_type = client.time_off.v1.policy_types.list()
        assert_matches_type(PolicyTypeListResponse, policy_type, path=["response"])

    @pytest.mark.skip()
    @parametrize
    def test_raw_response_list(self, client: ConnecteamAPISDK) -> None:
        response = client.time_off.v1.policy_types.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        policy_type = response.parse()
        assert_matches_type(PolicyTypeListResponse, policy_type, path=["response"])

    @pytest.mark.skip()
    @parametrize
    def test_streaming_response_list(self, client: ConnecteamAPISDK) -> None:
        with client.time_off.v1.policy_types.with_streaming_response.list() as response:
...
class TestAsyncPolicyTypes:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @pytest.mark.skip()
    @parametrize
    async def test_method_list(self, async_client: AsyncConnecteamAPISDK) -> None:
        policy_type = await async_client.time_off.v1.policy_types.list()
        assert_matches_type(PolicyTypeListResponse, policy_type, path=["response"])

    @pytest.mark.skip()
    @parametrize
    async def test_raw_response_list(self, async_client: AsyncConnecteamAPISDK) -> None:
        response = await async_client.time_off.v1.policy_types.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        policy_type = await response.parse()
        assert_matches_type(PolicyTypeListResponse, policy_type, path=["response"])

    @pytest.mark.skip()
    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncConnecteamAPISDK) -> None:
        async with async_client.time_off.v1.policy_types.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
