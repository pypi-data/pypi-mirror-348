...
export class ChatContextController {
  ...
  getContextDataForApi(
    ...
    attachedContexts: Array<{ type: string; value: string[] }>;
  } {
    if (!this.notebookController) {
      console.error(
        'NotebookController not available in ChatContextController logic'
      );
      return { currentNotebook: [], selectedCells: [], attachedContexts: [] };
    }
    ...
    const finalDocuments: Array<{ type: string; value: string[] }> = [];

    const docIds = currentContexts
      .filter(ctx => ctx.type === 'document')
      .map(ctx => ctx.id);
    if (docIds.length > 0) {
      finalDocuments.push({ type: 'document', value: docIds });
    }

    const repoIds = currentContexts
      .filter(ctx => ctx.type === 'git-repo')
      .map(ctx => ctx.id);
    if (repoIds.length > 0) {
      finalDocuments.push({ type: 'git-repo', value: repoIds });
    }
    ...
    return {
      ...
      attachedContexts: finalDocuments
    };
  }

  async sendDocumentContext(
    name: string,
    file: File | null,
    link: string | null
  ): Promise<DocumentContext | void> {
    if (!file && !link) {
      console.warn('No file or link provided to sendDocumentContext.');
      return;
    }
    ...
    try {
      ...
      if (!documentName) {
        if (file) {
          documentName = file.name.split('/').pop() || file.name;
        }
        if (link && !documentName) {
          try {
            const url = new URL(link);
            documentName = url.hostname;
          } catch (e) {
            documentName = 'linked_document';
          }
        }
      }
      ...
      console.debug('Sending document context to backend:', {
        file: file?.name,
        link,
        documentName
      });

      await (client as any).sendDocumentToIndex(file, link, documentName);

      console.info('Document context sent successfully.');
      return docContext; // Return the context so UI can potentially use it
    } catch (error) {
  ...
  async getIndexedDocumentNames(): Promise<string[]> {
    try {
      const client = await getJovyanClient();
      const documentNames = await (client as any).getIndexDocumentNames(
        'document'
      );
      return documentNames || [];
    } catch (error) {
      console.error('Failed to fetch indexed document names:', error);
      return [];
    }
  }

  async getIndexedRepoNames(): Promise<string[]> {
    try {
      const client = await getJovyanClient();
      const repoNames = await (client as any).getIndexDocumentNames('git-repo');
      return repoNames || [];
    } catch (error) {
      console.error('Failed to fetch indexed repository names:', error);
      return [];
    }
  }

  async sendGitRepoContext(
    name: string,
    url: string
  ): Promise<GitRepoContext | void> {
    try {
      const client = await getJovyanClient();
      const repoContext = new GitRepoContext(url, name, url);

      console.debug('Sending Git repo context to backend:', {
        url,
        name
      });

      await (client as any).sendDocumentToIndex(null, url, name);

      console.info('Git repo context sent successfully.');
      return repoContext;
    } catch (error) {
      console.error('Failed to send Git repo context:', error);
      throw error;
    }
  }
}
