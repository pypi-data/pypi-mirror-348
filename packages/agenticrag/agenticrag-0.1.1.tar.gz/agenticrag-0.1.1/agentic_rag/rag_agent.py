...
from agentic_rag.tasks import QuestionAnsweringTask, BaseTask
...
from agentic_rag.types.core import RAGAgentResponse
...
from agentic_rag.utils.rag_agent_loader_mixin import RAGAgentLoaderMixin
...
class RAGAgent(RAGAgentLoaderMixin):
    ...
    def __init__(
        ...
        Args:
            llm (BaseChatModel): The language model used for decision-making and tool selection.
        ...
        """
        self.llm = llm
        self.persistence_dir = persistent_dir
        ...
        if not retrievers:
            ...
            retrievers = [VectorRetriever(store=self.text_store, persistent_dir=persistent_dir)]
        else:
            seen_types = set()
    ...
    def invoke(self, query: str, max_retry: int=10) -> RAGAgentResponse:
        """
        ...
        Note: Max_retry can't control how many times internal agents (retrievers or tasks) use llm, to control behavior of those you must pass them in
        constructor with default behavior set.

        Args:
            query (str): The query to be processed by the agent.
            max_retry (int, optional): The maximum number of retries for the agent. Defaults to 10.
        """
        ...
        for i in range(max_retry):
            ...
            if tool_name == "final_answer":
                ...
                return RAGAgentResponse(
                    ...
                    tasks=[task.name for task in tasks],
                    datasets=[dataset.name for dataset in datasets],
                    retrievers=[retriever.name for retriever in selected_retrievers],
                    tries=i+1
                )
