...
class RedisMsgAPI: 
    ...
    def get_message(self,new_message:bool=False):
        if new_message:
            stream_key = {self.stream_key: '>'}
        else:
            stream_key = {self.stream_key: '0'}
        try:
            messages = self.r.xreadgroup(self.group_name, self.worker_name, stream_key, count=1, block=1000)
            if messages:
