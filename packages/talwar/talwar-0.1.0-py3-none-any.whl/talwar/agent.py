...
class Agent:
    ...
    def _update_progress(self, current: int, total: int):
        """Update scan progress and log status."""
        self.scan_state['progress'] = (current / total) * 100
        logger.info(f"Scan progress: {self.scan_state['progress']:.1f}%", color='cyan')
        ...
        if os.path.exists(state_file):
            try:
                with open(state_file, 'r') as f:
                    ...
                    # Convert lists back to sets for completed_urls and pending_urls
                    if 'completed_urls' in loaded_state:
                        loaded_state['completed_urls'] = set(loaded_state['completed_urls'])
                    if 'pending_urls' in loaded_state:
                        loaded_state['pending_urls'] = set(loaded_state['pending_urls'])
                    self.scan_state = loaded_state
            except json.JSONDecodeError:
                logger.warning("Corrupted state file found. Starting fresh.", color='yellow')
    ...
    def _handle_error(self, error: Exception, context: str):
        ...
        self._save_state()  # Save state before potential crash
        ...
        if isinstance(error, ConnectionError):
            ...
            logger.error("Unrecoverable error, saving state and exiting...", color='red')
            self._save_state()
            raise error

    def run(self):
        """
        Execute the security scan with improved error handling and state management.
        """
        try:
            ...
            self.scan_state['pending_urls'] = set(urls_to_parse)
            ...
            while urls_to_parse:
                try:
                    ...
                    self._update_progress(current_url_index, total_urls)
                    
                    scan_results = self.scanner.scan(url)
                    self.scan_state['completed_urls'].add(url)
                    ...
                    if self.expand_scope:
                        ...
                        for _url in more_urls:
                            ...
                            if (_url not in urls_to_parse and 
                                _url not in self.scan_state['completed_urls'] and 
                ...
                try:
                    ...
                    if tool_output == "Completed":
                        ...
                        if successful_exploit:
                            self.scan_state['vulnerabilities_found'].append({
                                'url': self.scan_state['current_url'],
                                'plan': plan['title'],
                                'report': report
                            })
