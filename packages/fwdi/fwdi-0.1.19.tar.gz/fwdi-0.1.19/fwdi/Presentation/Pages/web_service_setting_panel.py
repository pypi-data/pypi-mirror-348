...
from ...Utilites.ext_gradio_request import ExtGradioRequest
...
class WebServiceSettingPanel(BaseWebTabItem):
    ...
    def __load_tab(self, session_state:SessionState, request:gr.Request):
        if not self.__user_session:
            self.__user_session = ExtGradioRequest.get_or_create_user_session(session_state, request)
        
        return session_state

    def __activate_tab(self, session_state:SessionState, request:gr.Request):
        if self.__user_session:
            test_text = gr.Label(value=f"User session load with session data.", elem_id='small')
        else:
            test_text = gr.Label(value=f"User session load without user data.", elem_id='small')

        return test_text, session_state
        ...
        with gr.Tab(label=self.Name, id=tab_id) as tab:
            ...
            parent_blocks.load(self.__load, inputs=None,
                                                outputs=[tb_name, 
                                                    ...
                                                    data_broker_conf,
                                                    data_zipkin_conf])
            #tab.select(self.__activate_tab, inputs=[user_state], outputs=[test_label, user_state])