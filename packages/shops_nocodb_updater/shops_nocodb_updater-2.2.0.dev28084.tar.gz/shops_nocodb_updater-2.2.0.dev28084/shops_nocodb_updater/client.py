...
class NocodbClient:
    ...
    async def synchronize_records(self, records_by_model: dict, upload_attachments: bool = False, set_checkout_mode : bool = False, remove_obsolete_records: bool = False, lang: str ="EN", **mapper_kwargs):
        ...
        if set_checkout_mode:
            object_model=list(records_by_model.values())[0][0]
            object_model.__metadata__ = await self.get_table_metadata(object_model.__tableid__)
            self.checkout_field_name=object_model.__nocodb_field_name__("checkout_mode", lang)
            existing_columns = {col["title"]: col for col in object_model.__metadata__["columns"]}
        ...
        for model, records in records_by_model.items():
            ...
            await self.cleaning_batches(model, remove_obsolete_records)

        return self._id_maps
    
    async def initialize_get_cache(self,table_id,external_id_field):
    ...
    async def synchronize_batch_records(self, records: list, upload_attachments: bool = False, default_checkout : Optional[str] = None, **mapper_kwargs):
        ...
        existing_lookup_map = await self.initialize_get_cache(table_id, external_id_field)
    ...
    async def cleaning_batches(
        ...
        await self.initialize_get_cache(table_id,external_id_field)
        existing_lookup_map = self._cached_existing_lookup_maps[table_id]
        all_existing_ids = set(existing_lookup_map.keys())

        if delete_all:
            id_map={}
            delete_objects_ids = list(all_existing_ids)
        else:
            id_map = self._id_maps[table_id]
            all_external_ids_synced = set(id_map.keys())
            delete_objects_ids = list(all_existing_ids - all_external_ids_synced)

        # We only know the obsolete external_ids if we fetched the existing_lookup_map initially
        if (remove_obsolete_records and existing_lookup_map) or delete_all:

            if delete_objects_ids:
                self.logger.info(
                    f"Removing {len(delete_objects_ids)} obsolete records from table {table_id}"
                )
                # Perform the deletions in suitable batch sizes
                for i in range(0, len(delete_objects_ids), batch_size):
                    chunk = delete_objects_ids[i : i + batch_size]
                    to_delete_ids = []
    ...
    async def sync_external_connector(self,
                                      external_data=None,
                                      clean_before_loading : bool=False , batch_size: int=50):
        ...
        if clean_before_loading:
            await self.cleaning_batches(nocodb_model=ShopProduct,batch_size=batch_size,delete_all=clean_before_loading)
            await self.cleaning_batches(nocodb_model=ShopCategory,batch_size=batch_size,delete_all=clean_before_loading)
