...
def install_dependencies(path: pathlib.Path):
    ...
    if len(redundant) > 0:
        print(f"Uninstalling {len(redundant)} dependencies: {', '.join(redundant)}")
...
def modify(path: pathlib.Path, packages: list[str], action: Action):
    ...
    match action:
        case Action.ADD:
            ...
            print(f"Package(s) {', '.join(packages)} added to qbproject.toml.") if added else ...
        ...
        case Action.REMOVE:
            ...
            print(f"Package(s) {', '.join(removed)} removed from qbproject.toml.") if removed else ...
