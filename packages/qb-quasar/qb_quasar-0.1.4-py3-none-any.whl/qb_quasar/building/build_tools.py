...
def build_project(path: pathlib.Path, clean: bool = False):
    ...
    qb_files = find_qb_files(path)

    bar_format = f"{{l_bar}}{{bar}}| [{{n_fmt}}/{{total_fmt}}]{{postfix}} ({cpu_count()}T)"

    with Pool() as pool, tqdm(total=len(qb_files), desc="Compiling", bar_format=bar_format) as pbar:
        for _ in pool.imap_unordered(__write_compiled_helper, [(path, qb_file) for qb_file in qb_files]):
            pbar.n += 1
            pbar.refresh()
...
def __write_compiled_helper(args):
    path = args[0]
    qb_file = args[1]

    return write_compiled(path, qb_file)
