...
class BaseRepository:
    ...
    async def get_by_attr(self, attr: Union[str, Dict[str, Any]], value: Any = None, collection_name: Optional[str] = None) -> Dict[str, Any]:
        """
        Get a document by one or multiple attributes.
        
        Args:
            attr: Either a single attribute name (str) or a dictionary of attribute-value pairs
            value: Value for the single attribute (only used if attr is a string)
            collection_name: Optional collection name
            
        Returns:
            Dict[str, Any]: The found document
        """
        try:
            ...
            if isinstance(attr, dict):
                ...
                for field, field_value in attr.items():
                    if field == "id":
                        query["_id"] = ObjectId(field_value)
                    else:
            ...
            if not result:
                raise NotFoundError(detail=f"Document not found with the given attributes")
    ...
    def _build_reference_stage(
        ...
        for ref in references:
            ...
            project_fields = ref.get("view_fields")
                                    ...
                                    "$map": {
                                        ...
                                        "in": {
                                            "$cond": {
                                                ...
                                                "then": { "$convert": { "input": "$$id", "to": "objectId", "onError": None, "onNull": None } },
                                                "else": "$$id"
                                            }
                                        }
                                    }
