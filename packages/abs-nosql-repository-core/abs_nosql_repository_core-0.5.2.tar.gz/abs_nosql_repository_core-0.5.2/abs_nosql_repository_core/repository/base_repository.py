...
class BaseRepository:
    ...
    def get_collection(self, collection_name: Optional[str] = None) -> Any:
        ...
        if collection_name and self.db:
            return self.db[collection_name]
        elif collection_name and self.db is None:
            raise BadRequestError(detail="Database is not initialized.")
        else:
            return self.document.get_motor_collection()
    ...
    async def get_all(self, find: ListFilter, collection_name: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        skip = (page - 1) * page_size
        pipeline = []
        if collection_name and self.db is None:
            raise BadRequestError(detail="Database is not initialized.")
