...
SIMILARITY_PHOTOS_NEW = "similarity/photos/v2/"
SIMILARITY_PRODUCTS = "similarity/products/v2/"
SIMILARITY_PRODUCTS_NEW = "similarity/products-new/v2/"
SIMILARITY_FASHION = "similarity/fashion-new/v2/"
SIMILARITY_FASHION_OLD = "similarity/fashion/v2/"
...
class SimilarityPhotosOldClient(SimilarityPhotosClient):
    def __init__(
...
class SimilarityProductsOldClient(SimilarityPhotosClient):
    def __init__(
...
class SimilarityProductsClient(SimilarityPhotosClient):
    def __init__(
        self, token, collection_id=None, endpoint=ENDPOINT + SIMILARITY_PRODUCTS_NEW, resource_name=PRODUCT_SIMILARITY
    ):
        super().__init__(token=token, collection_id=collection_id, endpoint=endpoint, resource_name=resource_name)
...
class SimilarityFashionClient(SimilarityPhotosClient):
    ...
    def insert(self, records, extract_colors=None, extract_tags=None):
        """
        Insert records into collection with all meta information.
        :param records: dictionary with your "_id" and with one of "_url", "_file" or "_base64" to extract descriptor.
        :param extract_colors: boolean, if True, colors will be extracted from the image
        :param extract_tags: boolean, if True, tags will be extracted from the image
        :return: json response
        """
        records = self.preprocess_records(records)
        data = {RECORDS: self.fill_data(records, None), "extract_colors": extract_colors, "extract_tags": extract_tags}
        return self.post(INSERT, data=data)
