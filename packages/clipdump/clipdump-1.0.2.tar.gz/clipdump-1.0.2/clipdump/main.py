...
def strip_text():
    ...
    print("Whitespace stripped from clipboard contents")

def help():
    print("Clipdump usage:")
    print("• --append: appends clipboard contents to a specified pre-existing file. Usage: clipdump --append <file>")
    print("• -a: (auto) automatically saves clipboard contents to an new timestamped text file in C:\\users\\<username>\\Documents\\Clipdump. Usage: clipdump --auto")
    print("• -s: (strip) removes trailing whitespace from clipboard contents. Usage: clipdump -s")
...
def cli():
    if len(sys.argv) < 2:
        print("Unknown command. Usage: clipdump -a | --append <file>| -s | --help.  Enter clipdump --help for usage")
        return
