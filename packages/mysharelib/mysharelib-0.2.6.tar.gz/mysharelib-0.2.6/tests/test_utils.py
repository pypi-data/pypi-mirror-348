import pytest
from mysharelib.utils import normalize_symbol, get_symbol_base, normalize_date
...
@pytest.mark.parametrize(
    "date_input, expected",
    [
        ("2023-01-01", "20230101"),  # Regular hyphenated date
        ("2023-12-31", "20231231"),  # End of year date
        ("2023-06-15", "20230615"),  # Mid-year date
        ("20230101", "20230101"),    # Already in correct format
        ("20231231", "20231231"),    # Already in correct format
        ("20230615", "20230615"),    # Already in correct format
    ]
)
def test_normalize_date(date_input, expected):
    """Test date normalization for both hyphenated and non-hyphenated formats"""
    assert normalize_date(date_input) == expected
