...
if sys.platform.startswith("darw") or sys.platform.startswith("win"):
    pytest.skip("skipping tests on mac and windows", allow_module_level=True)
    ...
    for algorithm in algorithms_knots.keys():
        ...
        for code in knot_codes:
            ...
            if platform.system() == 'Windows':
                polynomials[algorithm][code] = algorithms_knots[algorithm](data, chiral=True, max_cross=30, tries=1, run_parallel=False)
            else:
...
def prepare_knot_polynomials_ideal():
    ...
    for algorithm in algorithms_knots.keys():
        ...
        for curve in knot_ideal.keys():
            ...
            if platform.system() == 'Windows':
                polynomials[algorithm][curve] = algorithms_knots[algorithm](knot_ideal[curve], closure=Closure.CLOSED, chiral=True, max_cross=30, tries=1, run_parallel=False)
            else:
