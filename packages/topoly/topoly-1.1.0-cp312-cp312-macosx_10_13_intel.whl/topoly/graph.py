...
from topoly.params import Closure, ReduceMethod, TopolyException, Bridges, OutputType, Colors, TopolyRareGraphException
...
class Graph:
    ...
    def generate_abstract_graph(self):
        ...
        for arc in self.orientation:
            ...
            if beg != end:
                if end not in graph:
                    #TODO - tests often fail here 2025.04.11, topoly version: 1.0.4
                    #print('Error in graph.py:323: {}'.format(str(self)))
                    #print('beg!=end: {}!={} arc: {}, graph: {}'.format(beg, end, arc, graph))
                    raise TopolyRareGraphException('Rare exception in in graph.py:324: beg!=end: {}!={} arc: {}, graph: {}'.format(beg, end, arc, graph))
    ...
    def find_links(self, output_type=OutputType.PDcode, arc_bridges=1, components=2):
        ...
        for pot_link in combinations(self.find_loops(output_type=o_type, arc_bridges=arc_bridges, output='generator'),
            ...
            if self.find_commons(pot_link, abstract):
                continue
            if abstract:
                arcs = [list(set([int(_) for _ in re.sub(r'[VX\[\]]', '', component).replace(';', ',').split(',')]))
