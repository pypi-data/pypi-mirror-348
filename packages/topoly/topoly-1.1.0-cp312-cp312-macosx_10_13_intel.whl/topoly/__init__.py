...
from .tangle_drawing import draw_final
...
def draw_tangle(tangle_str, filename=""):
    """
    Makes an image corresponding to the topology of the structure
    and saves it as a .svg file in the api/download/SVGs/ directory.

    Please ensure that these directories exist in the location from
    which you execute your code.

    Args:
        tangle_str (str):
                The representation of the knot that is to be drawn, 
                the string should trivially convert to a python tuple.
        filename (str, optional):
                Specifies the base file name. 
                Only alphanumeric, hyphen and underscore 
                characters (A-Z, a-z, '_', '-') are allowed.
                Default: modified tangle_str (with numbers only) 
                

    Returns:
        0 if the program encountered no internal errors during
        execution, and an unspecified integer otherwise.
    """
    return draw_final(tangle_str, filename)
