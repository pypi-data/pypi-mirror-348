...
from snipkit.exceptions import UndefinedVariableInTemplate
from snipkit.utils import create_env_with_context, rmtree
...
def render_variable(
    env: Environment,
    raw: _Raw,
    snipkit_dict: dict[str, Any],
) -> str:
    """Render the next variable to be displayed in the user prompt.

    Inside the prompting taken from the snipkit.json file, this renders
    the next variable. For example, if a project_name is "Peanut Butter
    Cookie", the repo_name could be be rendered with:

        `{{ snipkit.project_name.replace(" ", "_") }}`.

    This is then presented to the user as the default.

    :param Environment env: A Jinja2 Environment object.
    :param raw: The next value to be prompted for by the user.
    :param dict snipkit_dict: The current context as it's gradually
    ...
    if isinstance(raw, dict):
        return {
            render_variable(env, k, snipkit_dict): render_variable(
                env, v, snipkit_dict
            )
            for k, v in raw.items()
        }
    if isinstance(raw, list):
        return [render_variable(env, v, snipkit_dict) for v in raw]
    if not isinstance(raw, str):
        raw = str(raw)

    template = env.from_string(raw)

    return template.render(snipkit=snipkit_dict)
...
def prompt_choice_for_config(
    ...
    :param no_input: Do not prompt for user input and return the first available option.
    """
    rendered_options = [render_variable(env, raw, snipkit_dict) for raw in options]
    if no_input:
...
def prompt_for_config(
    ...
    """
    snipkit_dict = OrderedDict([])
    env = create_env_with_context(context)
    prompts = context['snipkit'].pop('__prompts__', {})

    # First pass: Handle simple and raw variables, plus choices.
    # These must be done first because the dictionaries keys and
    # values might refer to them.
    count = 0
    all_prompts = context['snipkit'].items()
    ...
    for key, raw in all_prompts:
        if key.startswith('_') and not key.startswith('__'):
            snipkit_dict[key] = raw
            continue
        if key.startswith('__'):
            snipkit_dict[key] = render_variable(env, raw, snipkit_dict)
            continue
        ...
        try:
            if isinstance(raw, list):
                ...
                val = prompt_choice_for_config(
                    snipkit_dict, env, key, raw, no_input, prompts, prefix
                )
                snipkit_dict[key] = val
            elif isinstance(raw, bool):
                # We are dealing with a boolean variable
                if no_input:
                    snipkit_dict[key] = render_variable(
                        env, raw, snipkit_dict
                    )
                else:
                    snipkit_dict[key] = read_user_yes_no(key, raw, prompts, prefix)
            elif not isinstance(raw, dict):
                # We are dealing with a regular variable
                val = render_variable(env, raw, snipkit_dict)
    ...
    for key, raw in context['snipkit'].items():
...
def choose_nested_template(
    ...
    """
    snipkit_dict: OrderedDict[str, Any] = OrderedDict([])
    env = create_env_with_context(context)
    prefix = ""
    prompts = context['snipkit'].pop('__prompts__', {})
    key = "templates"
    config = context['snipkit'].get(key, {})
    if config:
        # Pass
        ...
        config = context['snipkit'].get(key, [])
        val = prompt_choice_for_config(
            snipkit_dict, env, key, config, no_input, prompts, prefix
        )
...
def prompt_and_delete(path: Path | str, no_input: bool = False) -> bool:
    ...
    reused. If yes, it's reused; otherwise, Snipkit exits.
