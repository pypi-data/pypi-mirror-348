...
from snipkit import __version__
from snipkit.config import get_user_config
from snipkit.exceptions import (
...
from snipkit.log import configure_logger
from snipkit.main import snipkit


def version_msg() -> str:
    """Return the Snipkit version, location and Python powering it."""
    python_version = sys.version
    location = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    return f"Snipkit {__version__} from {location} (Python {python_version})"
...
def list_installed_templates(
    ...
    """List installed (locally cloned) templates. Use snipkit --list-installed."""
    config = get_user_config(passed_config_file, default_config)
    snipkit_folder: str = config['snipkits_dir']
    if not os.path.exists(snipkit_folder):
        click.echo(
            f"Error: Cannot list installed templates. "
            f"Folder does not exist: {snipkit_folder}"
        )
        sys.exit(-1)

    template_names = [
        folder
        for folder in os.listdir(snipkit_folder)
        if os.path.exists(
            os.path.join(snipkit_folder, folder, 'snipkit.json')
        )
    ]
...
@click.option(
    '--no-input',
    is_flag=True,
    help='Do not prompt for parameters and only use snipkit.json file content. '
...
@click.option(
    '--directory',
    help='Directory within repo that holds snipkit.json file '
...
def main(
    ...
    """Create a project from a Snipkit project template (TEMPLATE).

    Snipkit is free and open source software, developed and managed by
    volunteers. If you would like to help out or fund the project, please get
    in touch at https://github.com/khulnasoft/snipkit.
    """
    ...
    try:
        snipkit(
            ...
            password=os.environ.get('SNIPKIT_REPO_PASSWORD'),
            directory=directory,
