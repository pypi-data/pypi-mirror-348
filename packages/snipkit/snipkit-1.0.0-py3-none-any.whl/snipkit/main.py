"""
Main entry point for the `snipkit` command.

The code in this module is also a good example of how to use Snipkit as a
...
from snipkit.config import get_user_config
from snipkit.exceptions import InvalidModeException
from snipkit.generate import generate_context, generate_files
from snipkit.hooks import run_pre_prompt_hook
from snipkit.prompt import choose_nested_template, prompt_for_config
from snipkit.replay import dump, load
from snipkit.repository import determine_repo_dir
from snipkit.utils import rmtree
...
def snipkit(
    ...
    """
    Run Snipkit just as if using it from the command line.

    :param template: A directory containing a project template directory,
        or a URL to a git repository.
    :param checkout: The branch, tag or commit ID to checkout after clone.
    :param no_input: Do not prompt for user input.
        Use default values for template parameters taken from `snipkit.json`, user
    ...
    :param directory: Relative path to a snipkit template in a repository.
    ...
    base_repo_dir, cleanup_base_repo_dir = determine_repo_dir(
        ...
        clone_to_dir=config_dict['snipkits_dir'],
        checkout=checkout,
        no_input=no_input,
        password=password,
        directory=directory,
    ...
    context_file = os.path.join(repo_dir, 'snipkit.json')
    ...
    if replay:
        ...
        items_for_prompting = {
            k: v
            for k, v in context['snipkit'].items()
            if k not in context_from_replayfile['snipkit']
        }
        context_for_prompting = {}
        context_for_prompting['snipkit'] = items_for_prompting
    ...
    # preserve the original snipkit options
    # print(context['snipkit'])
    context['_snipkit'] = {
        k: v for k, v in context['snipkit'].items() if not k.startswith("_")
    }
    ...
    with import_patch:
        if {"template", "templates"} & set(context["snipkit"].keys()):
        ...
        if context_for_prompting['snipkit']:
            context['snipkit'].update(
    ...
    context['snipkit']['_template'] = template

    # include output+dir in the context dict
    context['snipkit']['_output_dir'] = os.path.abspath(output_dir)

    # include repo dir or url in the context dict
    context['snipkit']['_repo_dir'] = f"{repo_dir}"

    # include checkout details in the context dict
    context['snipkit']['_checkout'] = checkout
