"""Tests for `snipkit.prompt` module."""
...
from snipkit import environment, exceptions, prompt
...
class TestRenderVariable:
    ...
    def test_convert_to_str(self, mocker, raw_var, rendered_var) -> None:
        ...
        from_string = mocker.patch(
            'snipkit.utils.StrictEnvironment.from_string', wraps=env.from_string
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                {'{{snipkit.project}}': ['foo', 1], 'bar': False},
                {'foobar': ['foo', '1'], 'bar': False},
            ),
            (['foo', '{{snipkit.project}}', None], ['foo', 'foobar', None]),
        ],
    )
...
class TestPrompt:
    ...
    @pytest.mark.parametrize(
        'context',
        [
            {'snipkit': {'full_name': 'Your Name'}},
            {'snipkit': {'full_name': 'Řekni či napiš své jméno'}},
        ],
    ...
    def test_prompt_for_config(self, monkeypatch, context) -> None:
        ...
        snipkit_dict = prompt.prompt_for_config(context)
        assert snipkit_dict == context['snipkit']
    ...
    def test_prompt_for_config_with_human_prompts(self, monkeypatch, context) -> None:
        ...
        monkeypatch.setattr(
            'snipkit.prompt.read_user_yes_no',
            lambda _var, default, _prompts, _prefix: default,
        )
        monkeypatch.setattr(
            'snipkit.prompt.read_user_choice',
            lambda _var, default, _prompts, _prefix: default,
        )
    ...
    def test_should_render_dict(self) -> None:
        ...
        context = {
            'snipkit': {
                ...
                'details': {
                    '{{snipkit.project_name}}': '{{snipkit.project_name}}'
                },
            }
        }

        snipkit_dict = prompt.prompt_for_config(context, no_input=True)
        assert snipkit_dict == {
            'project_name': 'Slartibartfast',
        ...
        context = {
            'snipkit': {
                ...
                'details': {
                    "key": "value",
                    "integer_key": 37,
                    "other_name": '{{snipkit.project_name}}',
                    "dict_key": {
                        "deep_key": "deep_value",
                        "deep_integer": 42,
                        "deep_other_name": '{{snipkit.project_name}}',
                        "deep_list": [
                            "deep value 1",
                            "{{snipkit.project_name}}",
                            "deep value 3",
                        ],
                    },
                    "list_key": [
    ...
    def test_prompt_for_templated_config(self, monkeypatch) -> None:
        ...
        context = {
            'snipkit': OrderedDict(
                [
                    ...
                    (
                        'pkg_name',
                        '{{ snipkit.project_name|lower|replace(" ", "") }}',
                    ),
                ]
            )
        }
        ...
        snipkit_dict = prompt.prompt_for_config(context)
        assert snipkit_dict == exp_snipkit_dict
    ...
    def test_dont_prompt_for_private_context_var(self, monkeypatch) -> None:
        ...
        context = {'snipkit': {'_copy_without_render': ['*.html']}}
        snipkit_dict = prompt.prompt_for_config(context)
        assert snipkit_dict == {'_copy_without_render': ['*.html']}
        ...
        context = {
            'snipkit': OrderedDict(
                [
                    ('foo', 'Hello world'),
                    ('bar', 123),
                    ('rendered_foo', '{{ snipkit.foo|lower }}'),
                    ('rendered_bar', 123),
                    ('_hidden_foo', '{{ snipkit.foo|lower }}'),
                    ('_hidden_bar', 123),
                    ('__rendered_hidden_foo', '{{ snipkit.foo|lower }}'),
                    ('__rendered_hidden_bar', 123),
                ]
            )
        }
        snipkit_dict = prompt.prompt_for_config(context, no_input=True)
        assert snipkit_dict == OrderedDict(
            [
                ('foo', 'Hello world'),
                ('bar', '123'),
    ...
    def test_should_not_render_private_variables(self) -> None:
        ...
        context = {
            'snipkit': {
                ...
                '_skip_jinja_template': '{{snipkit.project_name}}',
                '_skip_float': 123.25,
                '_skip_integer': 123,
                '_skip_boolean': True,
                '_skip_nested': True,
...
class TestReadUserChoice:
    ...
    def test_should_invoke_read_user_choice(self, mocker) -> None:
        ...
        prompt_choice = mocker.patch(
            'snipkit.prompt.prompt_choice_for_config',
            wraps=prompt.prompt_choice_for_config,
        )

        read_user_choice = mocker.patch('snipkit.prompt.read_user_choice')
        read_user_choice.return_value = 'all'

        read_user_variable = mocker.patch('snipkit.prompt.read_user_variable')

        choices = ['landscape', 'portrait', 'all']
        context = {'snipkit': {'orientation': choices}}
        ...
        assert snipkit_dict == {'orientation': 'all'}
    ...
    def test_should_invoke_read_user_variable(self, mocker) -> None:
        ...
        prompt_choice = mocker.patch('snipkit.prompt.prompt_choice_for_config')

        read_user_choice = mocker.patch('snipkit.prompt.read_user_choice')

        context = {'snipkit': {'full_name': 'Your Name'}}
        ...
        assert snipkit_dict == {'full_name': 'Audrey Roy'}
...
class TestPromptChoiceForConfig:
    ...
    def context(self, choices):
        ...
        return {'snipkit': {'orientation': choices}}
...
class TestReadUserYesNo:
    ...
    def test_should_invoke_read_user_yes_no(self, mocker, run_as_docker) -> None:
        ...
        read_user_yes_no = mocker.patch('snipkit.prompt.read_user_yes_no')
        read_user_yes_no.return_value = run_as_docker

        read_user_variable = mocker.patch('snipkit.prompt.read_user_variable')

        context = {'snipkit': {'run_as_docker': run_as_docker}}
        ...
        assert snipkit_dict == {'run_as_docker': run_as_docker}
...
@pytest.mark.parametrize(
    'context',
    (
        {'snipkit': {'foo': '{{snipkit.nope}}'}},
        {'snipkit': {'foo': ['123', '{{snipkit.nope}}', '456']}},
        {'snipkit': {'foo': {'{{snipkit.nope}}': 'value'}}},
        {'snipkit': {'foo': {'key': '{{snipkit.nope}}'}}},
    ),
    ids=[
        'Undefined variable in snipkit dict',
        'Undefined variable in snipkit dict with choices',
        'Undefined variable in snipkit dict with dict_key',
        'Undefined variable in snipkit dict with key_value',
    ],
)
...
def test_snipkit_nested_templates(template_dir: str, expected: Path | str) -> None:
    """Test nested_templates generation."""
    from snipkit import prompt

    main_dir = (Path("tests") / template_dir).resolve()
    cookiecuter_context = json.loads((main_dir / "snipkit.json").read_text())
    context = {"snipkit": cookiecuter_context}
...
def test_snipkit_nested_templates_invalid_paths(path: str) -> None:
    """Test nested_templates generation."""
    from snipkit import prompt
...
def test_snipkit_nested_templates_invalid_win_paths(path: str) -> None:
    """Test nested_templates generation."""
    from snipkit import prompt
...
def test_prompt_should_ask_and_rm_repo_dir(mocker, tmp_path) -> None:
    ...
    mock_read_user = mocker.patch(
        'snipkit.prompt.read_user_yes_no', return_value=True
    )
...
def test_prompt_should_ask_and_rm_repo_file(mocker, tmp_path) -> None:
    ...
    mock_read_user = mocker.patch(
        'snipkit.prompt.read_user_yes_no', return_value=True, autospec=True
    )
...
def test_prompt_should_ask_and_keep_repo_on_no_reuse(mocker, tmp_path) -> None:
    ...
    mock_read_user = mocker.patch(
        'snipkit.prompt.read_user_yes_no', return_value=False, autospec=True
    )
    repo_dir = Path(tmp_path, 'repo')
    repo_dir.mkdir()
...
def test_prompt_should_ask_and_keep_repo_on_reuse(mocker, tmp_path) -> None:
    ...
    mock_read_user = mocker.patch(
        'snipkit.prompt.read_user_yes_no', side_effect=answer, autospec=True
    )
    repo_dir = Path(tmp_path, 'repo')
    repo_dir.mkdir()
