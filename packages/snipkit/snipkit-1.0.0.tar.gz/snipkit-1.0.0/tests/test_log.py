...
def create_log_records() -> None:
    ...
    snipkit_logger = logging.getLogger('snipkit')
    foo_logger = logging.getLogger('snipkit.foo')
    foobar_logger = logging.getLogger('snipkit.foo.bar')

    snipkit_logger.info('Welcome to Snipkit')
    snipkit_logger.debug('Generating project from pytest-plugin')
    ...
    snipkit_logger.debug('Successfully generated project')
...
def debug_messages():
    ...
    return [
        "INFO snipkit: Welcome to Snipkit",
        "DEBUG snipkit: Generating project from pytest-plugin",
        "INFO snipkit.foo: Loading user config from home dir",
        "DEBUG snipkit.foo.bar: I don't know.",
        "DEBUG snipkit.foo.bar: I wanted to save the world.",
        "ERROR snipkit.foo: Aw, snap! Something went wrong",
        "DEBUG snipkit: Successfully generated project",
    ]


@pytest.fixture
def info_logger():
    """Fixture. Call snipkit logger setup with `info` debug level."""
    return configure_logger(stream_level='INFO')


@pytest.fixture
def debug_logger():
    """Fixture. Call snipkit logger setup with `debug` debug level."""
...
def info_logger_with_file(debug_file):
    """Fixture. Call snipkit logger setup with `info` debug level + `file`."""
