"""Test `process_json`, `read_user_dict` functions in `snipkit.prompt`."""

import click
import pytest
from rich.prompt import InvalidResponse

from snipkit.prompt import JsonPrompt, process_json, read_user_dict
...
def test_should_raise_type_error(mocker) -> None:
    ...
    prompt = mocker.patch('snipkit.prompt.JsonPrompt.ask')
...
def test_should_call_prompt_with_process_json(mocker) -> None:
    ...
    """
    mock_prompt = mocker.patch('snipkit.prompt.JsonPrompt.ask', autospec=True)
...
def test_should_not_load_json_from_sentinel(mocker) -> None:
    ...
    mock_json_loads = mocker.patch(
        'snipkit.prompt.json.loads', autospec=True, return_value={}
    )
