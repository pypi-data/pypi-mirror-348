...
def backup_configuration(cwd: Path) -> Path:
    """Create a backup of snipkit.json."""
    src_data = (cwd / 'snipkit.json').read_text()
    dst = cwd / '_snipkit.json'
...
def main():
    """Check if we can run the  snipkit."""
    if os.environ.get("SNIPKIT_FAIL_PRE_PROMPT", False):
        sys.exit(1)
    cwd = Path('.').resolve()
