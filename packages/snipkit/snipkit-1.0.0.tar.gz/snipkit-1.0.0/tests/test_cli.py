"""Collection of tests around snipkit's command-line interface."""
...
def cli_runner():
    """Fixture that returns a helper function to run the snipkit cli."""
    runner = CliRunner()

    def cli_main(*cli_args, **cli_kwargs):
        """Run snipkit cli main with the given args."""
...
def test_cli_version(cli_runner, version_cli_flag) -> None:
    """Verify Snipkit version output by `snipkit` on cli invocation."""
    result = cli_runner(version_cli_flag)
    assert result.exit_code == 0
    assert result.output.startswith('Snipkit')
...
def test_cli_replay(mocker, cli_runner) -> None:
    ...
    mock_snipkit = mocker.patch('snipkit.cli.snipkit')

    template_path = 'tests/fake-repo-pre/'
    result = cli_runner(template_path, '--replay', '-v')

    assert result.exit_code == 0
    mock_snipkit.assert_called_once_with(
        template_path,
        None,
        False,
        replay=True,
...
def test_cli_exit_on_noinput_and_replay(mocker, cli_runner) -> None:
    ...
    mock_snipkit = mocker.patch(
        'snipkit.cli.snipkit', side_effect=snipkit
    )
...
@pytest.mark.usefixtures('remove_fake_project_dir')
def test_run_snipkit_on_overwrite_if_exists_and_replay(
...
def test_echo_undefined_variable_error(output_dir, cli_runner) -> None:
    ...
    error = "Unable to create file '{{snipkit.foobar}}'"
...
def test_debug_file_non_verbose(cli_runner, debug_file) -> None:
    ...
    context_log = (
        "DEBUG snipkit.main: context_file is "
        "tests/fake-repo-pre/snipkit.json"
    )
...
def test_debug_list_installed_templates(
    ...
    Path(user_config_path).write_text(f"snipkits_dir: '{fake_template_dir}'")
    Path("fake-project", "snipkit.json").write_text('{}')
...
def test_debug_list_installed_templates_failure(
    ...
    os.makedirs(os.path.dirname(user_config_path))
    Path(user_config_path).write_text('snipkits_dir: "/notarealplace/"')
...
def test_cli_with_json_decoding_error(cli_runner) -> None:
    ...
    # this point.
    path = os.path.sep.join(['tests', 'fake-repo-bad-json', 'snipkit.json'])
...
def test_cli_with_pre_prompt_hook_fail(cli_runner, monkeypatch) -> None:
    ...
    with monkeypatch.context() as m:
        m.setenv('SNIPKIT_FAIL_PRE_PROMPT', '1')
        result = cli_runner(template_path, '--no-input')
    assert result.exit_code == 1
    dir_name = 'inputfake-project'
