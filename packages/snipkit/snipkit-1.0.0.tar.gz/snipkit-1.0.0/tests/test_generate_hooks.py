...
from snipkit import generate, utils
from snipkit.exceptions import FailedHookException
...
def test_ignore_hooks_dirs() -> None:
    ...
    generate.generate_files(
        context={'snipkit': {'pyhooks': 'pyhooks'}},
...
def test_run_python_hooks_cwd() -> None:
    ...
    """
    generate.generate_files(
        context={'snipkit': {'pyhooks': 'pyhooks'}}, repo_dir='tests/test-pyhooks/'
    )
...
def test_empty_hooks() -> None:
    ...
    with pytest.raises(FailedHookException) as excinfo:
        generate.generate_files(
            context={'snipkit': {'shellhooks': 'shellhooks'}},
...
def test_oserror_hooks(mocker) -> None:
    """Verify script error passed correctly to snipkit error.
...
def test_run_failing_hook_removes_output_directory() -> None:
    ...
    template = os.path.join(repo_path, 'input{{snipkit.hooks}}')
    os.mkdir(repo_path)
    os.mkdir(hook_dir)
    os.mkdir(template)
