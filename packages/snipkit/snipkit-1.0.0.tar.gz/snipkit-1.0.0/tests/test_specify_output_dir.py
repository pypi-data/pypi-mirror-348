"""Tests for snipkit's output directory customization feature."""
...
def template(tmp_path):
    ...
    template_dir.mkdir()
    template_dir.joinpath('snipkit.json').touch()
    return str(template_dir)


@pytest.fixture(autouse=True)
def mock_gen_context(mocker, context) -> None:
    """Fixture. Automatically mock snipkit's function with expected output."""
    mocker.patch('snipkit.main.generate_context', return_value=context)


@pytest.fixture(autouse=True)
def mock_prompt(mocker) -> None:
    """Fixture. Automatically mock snipkit's function with expected output."""
    mocker.patch('snipkit.main.prompt_for_config')
...
def test_api_invocation(mocker, template, output_dir, context) -> None:
    ...
    main.snipkit(template, output_dir=output_dir)
