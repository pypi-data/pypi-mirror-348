"""Test snipkit for work without any input.

Tests in this file execute `snipkit()` with `no_input=True` flag and
verify result with different settings in `snipkit.json`.
"""
...
def test_snipkit_no_input_return_project_dir(path) -> None:
    """Verify `snipkit` create project dir on input with or without slash."""
    project_dir = main.snipkit(path, no_input=True)
...
@pytest.mark.usefixtures('clean_system', 'remove_additional_dirs')
def test_snipkit_no_input_extra_context() -> None:
    """Verify `snipkit` accept `extra_context` argument."""
    main.snipkit(
        'tests/fake-repo-pre',
        no_input=True,
...
@pytest.mark.usefixtures('clean_system', 'remove_additional_dirs')
def test_snipkit_templated_context() -> None:
    """Verify Jinja2 templating correctly works in `snipkit.json` file."""
    main.snipkit('tests/fake-repo-tmpl', no_input=True)
    assert os.path.isdir('fake-project-templated')


@pytest.mark.usefixtures('clean_system', 'remove_additional_dirs')
def test_snipkit_no_input_return_rendered_file() -> None:
    """Verify Jinja2 templating correctly works in `snipkit.json` file."""
    project_dir = main.snipkit('tests/fake-repo-pre', no_input=True)
...
@pytest.mark.usefixtures('clean_system', 'remove_additional_dirs')
def test_snipkit_dict_values_in_context() -> None:
    """Verify configured dictionary from `snipkit.json` correctly unpacked."""
    project_dir = main.snipkit('tests/fake-repo-dict', no_input=True)
...
@pytest.mark.usefixtures('clean_system', 'remove_additional_dirs')
def test_snipkit_template_cleanup(mocker) -> None:
    """Verify temporary folder for zip unpacking dropped."""
    mocker.patch('tempfile.mkdtemp', return_value='fake-tmp', autospec=True)

    mocker.patch(
        'snipkit.prompt.prompt_and_delete', return_value=True, autospec=True
    )

    main.snipkit('tests/files/fake-repo-tmpl.zip', no_input=True)
    assert os.path.isdir('fake-project-templated')
