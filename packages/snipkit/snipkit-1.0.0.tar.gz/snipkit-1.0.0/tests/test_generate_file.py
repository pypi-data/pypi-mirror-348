...
def tear_down():
    ...
    if os.path.exists('tests/files/{{snipkit.generate_file}}_mixed_newlines.txt'):
        os.remove('tests/files/{{snipkit.generate_file}}_mixed_newlines.txt')
...
def test_generate_file(env) -> None:
    ...
    infile = 'tests/files/{{snipkit.generate_file}}.txt'
    generate.generate_file(
        project_dir=".",
        infile=infile,
        context={'snipkit': {'generate_file': 'cheese'}},
        env=env,
    )
...
def test_generate_file_jsonify_filter(env) -> None:
    ...
    infile = 'tests/files/{{snipkit.jsonify_file}}.txt'
    data = {'jsonify_file': 'cheese', 'type': 'roquefort'}
    generate.generate_file(
        project_dir=".", infile=infile, context={'snipkit': data}, env=env
    )
    assert os.path.isfile('tests/files/cheese.txt')
    generated_text = Path('tests/files/cheese.txt').read_text()
...
def test_generate_file_random_ascii_string(env, length, punctuation) -> None:
    ...
    infile = 'tests/files/{{snipkit.random_string_file}}.txt'
    data = {'random_string_file': 'cheese'}
    context = {"snipkit": data, "length": length, "punctuation": punctuation}
...
def test_generate_file_with_true_condition(env) -> None:
    ...
    """
    infile = (
        'tests/files/{% if snipkit.generate_file == \'y\' %}cheese.txt{% endif %}'
    )
    generate.generate_file(
        project_dir=".",
        infile=infile,
        context={'snipkit': {'generate_file': 'y'}},
        env=env,
    )
    assert os.path.isfile('tests/files/cheese.txt')
    generated_text = Path('tests/files/cheese.txt').read_text()
...
def test_generate_file_with_false_condition(env) -> None:
    ...
    generate.generate_file(
        project_dir=".",
        infile=infile,
        context={'snipkit': {'generate_file': 'n'}},
        env=env,
    )
...
def test_generate_file_does_not_translate_lf_newlines_to_crlf(env) -> None:
    ...
    infile = 'tests/files/{{snipkit.generate_file}}_lf_newlines.txt'
    generate.generate_file(
        project_dir=".",
        infile=infile,
        context={'snipkit': {'generate_file': 'cheese'}},
...
def test_generate_file_does_not_translate_crlf_newlines_to_lf(env) -> None:
    """Verify that file generation use same line ending, as in source file."""
    infile = 'tests/files/{{snipkit.generate_file}}_crlf_newlines.txt'
    generate.generate_file(
        project_dir=".",
        infile=infile,
        context={'snipkit': {'generate_file': 'cheese'}},
...
def test_generate_file_handles_mixed_line_endings(env) -> None:
    ...
    infile = 'tests/files/{{snipkit.generate_file}}_mixed_newlines.txt'
