...
from snipkit import exceptions, main, replay
...
def test_raise_on_invalid_mode(invalid_kwargs) -> None:
    """Test `snipkit` raise exception on unacceptable `replay` request."""
    with pytest.raises(exceptions.InvalidModeException):
        main.snipkit('foo', replay=True, **invalid_kwargs)


def test_main_does_not_invoke_dump_but_load(mocker) -> None:
    """Test `snipkit` calling correct functions on `replay`."""
    mock_prompt = mocker.patch('snipkit.main.prompt_for_config')
    mock_gen_context = mocker.patch('snipkit.main.generate_context')
    mock_gen_files = mocker.patch('snipkit.main.generate_files')
    mock_replay_dump = mocker.patch('snipkit.main.dump')
    mock_replay_load = mocker.patch('snipkit.main.load')

    main.snipkit('tests/fake-repo-tmpl/', replay=True)
...
def test_main_does_not_invoke_load_but_dump(mocker) -> None:
    """Test `snipkit` calling correct functions on non-replay launch."""
    mock_prompt = mocker.patch('snipkit.main.prompt_for_config')
    mock_gen_context = mocker.patch('snipkit.main.generate_context')
    mock_gen_files = mocker.patch('snipkit.main.generate_files')
    mock_replay_dump = mocker.patch('snipkit.main.dump')
    ...
    main.snipkit('tests/fake-repo-tmpl/', replay=False)
