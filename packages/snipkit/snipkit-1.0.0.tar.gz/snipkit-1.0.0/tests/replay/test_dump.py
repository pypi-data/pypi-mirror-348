...
def mock_ensure_failure(mocker):
    """Replace snipkit.replay.make_sure_path_exists function.

    Used to mock internal function and limit test scope.
    Always return expected value: False
    """
    return mocker.patch(
        'snipkit.replay.make_sure_path_exists', side_effect=OSError
    )
...
def mock_ensure_success(mocker):
    ...
    """
    return mocker.patch('snipkit.replay.make_sure_path_exists', return_value=True)
...
def test_ioerror_if_replay_dir_creation_fails(
    ...
    with pytest.raises(OSError):
        replay.dump(replay_test_dir, 'foo', {'snipkit': {'hello': 'world'}})
