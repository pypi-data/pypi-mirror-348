...
def test_clone_should_raise_if_vcs_not_installed(mocker, clone_dir) -> None:
    ...
    is installed."""
    mocker.patch('snipkit.vcs.is_vcs_installed', autospec=True, return_value=False)

    repo_url = 'https://github.com/pytest-dev/snipkit-pytest-plugin.git'
...
def test_clone_should_rstrip_trailing_slash_in_repo_url(mocker, clone_dir) -> None:
    ...
    present."""
    mocker.patch('snipkit.vcs.is_vcs_installed', autospec=True, return_value=True)

    mock_subprocess = mocker.patch(
        'snipkit.vcs.subprocess.check_output',
        autospec=True,
    )
...
def test_clone_should_abort_if_user_does_not_want_to_reclone(mocker, clone_dir) -> None:
    """In `clone()`, if user doesn't want to reclone, Snipkit should exit \
    without cloning anything."""
    mocker.patch('snipkit.vcs.is_vcs_installed', autospec=True, return_value=True)
    mocker.patch(
        'snipkit.vcs.prompt_and_delete', side_effect=SystemExit, autospec=True
    )
    mock_subprocess = mocker.patch(
        'snipkit.vcs.subprocess.check_output',
        autospec=True,
    ...
    repo_dir = clone_dir.joinpath('snipkit-pytest-plugin')
    repo_dir.mkdir()
...
def test_clone_should_silent_exit_if_ok_to_reuse(mocker, tmpdir) -> None:
    ...
    mocker.patch('snipkit.vcs.is_vcs_installed', autospec=True, return_value=True)
    mocker.patch(
        'snipkit.vcs.prompt_and_delete', return_value=False, autospec=True
    )
    mock_subprocess = mocker.patch(
        'snipkit.vcs.subprocess.check_output',
        autospec=True,
    ...
    # Create repo_dir to trigger prompt_and_delete
    clone_to_dir.mkdir('snipkit-pytest-plugin')
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('git', 'git@gitlab.com:snipkit/snipkit.git', 'snipkit'),
        ('git', 'git@github.com:snipkit/snipkit.git', 'snipkit'),
    ],
)
...
def test_clone_handles_branch_typo(mocker, clone_dir, error_message) -> None:
    ...
    repository_url = 'https://github.com/pytest-dev/snipkit-pytest-plugin'
...
def test_clone_unknown_subprocess_error(mocker, clone_dir) -> None:
    ...
    with pytest.raises(subprocess.CalledProcessError):
        vcs.clone(
            'https://github.com/pytest-dev/snipkit-pytest-plugin',
            clone_to_dir=str(clone_dir),
            no_input=True,
        )
