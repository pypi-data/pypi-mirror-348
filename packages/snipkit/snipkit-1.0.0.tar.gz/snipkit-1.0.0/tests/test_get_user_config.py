...
from snipkit import config
from snipkit.exceptions import InvalidConfiguration


@pytest.fixture(scope='module')
def user_config_path():
    """Fixture. Return user config path for current user."""
    return os.path.expanduser('~/.snipkitrc')


@pytest.fixture(scope='function')
def back_up_rc(user_config_path):
    """
    Back up an existing snipkit rc and restore it after the test.
    ...
    yield
    # Remove the ~/.snipkitrc that has been created in the test.
    if os.path.exists(user_config_path):
        os.remove(user_config_path)

    # If it existed, restore the original ~/.snipkitrc.
...
def custom_config():
    ...
    return {
        ...
        'snipkits_dir': '/home/example/some-path-to-templates',
...
def test_default_config_from_env_variable(
    ...
    monkeypatch.setenv('SNIPKIT_CONFIG', custom_config_path)
...
def test_expand_user_for_directories_in_config(monkeypatch) -> None:
    ...
    assert user_config['snipkits_dir'] == 'Users/bob/templates'
...
def test_expand_vars_for_directories_in_config(monkeypatch) -> None:
    ...
    assert user_config['snipkits_dir'] == 'Users/bob/cookies/templates'
