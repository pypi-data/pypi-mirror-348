...
from snipkit.find import find_template
from snipkit.hooks import run_hook_from_repo_dir
from snipkit.prompt import YesNoPrompt
from snipkit.utils import (
...
def is_copy_only_path(path: str, context: dict[str, Any]) -> bool:
    ...
    :param context: snipkit context.
    """
    try:
        for dont_render in context['snipkit']['_copy_without_render']:
        ...
        if isinstance(context_value, list):
            ...
            if isinstance(overwrite, list):
                ...
                if overwrite in context_value:
                    ...
                    # see ``snipkit.prompt.prompt_choice_for_config``
...
def generate_context(
    context_file: str = 'snipkit.json',
    default_context: dict[str, Any] | None = None,
    extra_context: dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Generate the context for a Snipkit project template.

    Loads the JSON file as a Python object, with key being the JSON filename.

    :param context_file: JSON file containing key/value pairs for populating
        the snipkit's variables.
...
def generate_file(
    ...
    :param infile: Input file to generate the file from. Relative to the root
        template dir.
    :param context: Dict for populating the snipkit's variables.
    ...
    if context['snipkit'].get('_new_lines', False):
        # Use `_new_lines` from context, if configured.
        newline = context['snipkit']['_new_lines']
...
def _run_hook_from_repo_dir(
    ...
    :param context: Snipkit project context.
    :param delete_project_on_failure: Delete the project directory on hook
        failure?
    """
    warnings.warn(
        "The '_run_hook_from_repo_dir' function is deprecated, "
        "use 'snipkit.hooks.run_hook_from_repo_dir' instead",
        DeprecationWarning,
        2,
    )
