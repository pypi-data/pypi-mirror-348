...
from feng_tools.web.fastapi.admin.core import schema_tools
from feng_tools.web.fastapi.admin.core.model_admin_settings import ModelAdminSettings
from feng_tools.web.fastapi.admin.model_admin_api.model_admin_api import ModelAdminApi
from feng_tools.web.fastapi.admin.model_admin_api.model_admin_api_post_processor import ModelAdminApiPostProcessor
from feng_tools.web.fastapi.admin.model_admin_api.model_admin_api_pre_processor import ModelAdminApiPreProcessor
from feng_tools.web.fastapi.admin.model_admin_api.model_value_transformer import ModelValueTransformer
from feng_tools.web.fastapi.admin.model_admin_api.model_value_validator import ModelValueValidator
from feng_tools.web.fastapi.api.api_response import ApiResponse, ListData, PageListData
from feng_tools.web.fastapi.api.api_tools import ApiTools
...
class ModelAdminApiProcessor(ModelAdminApi):
    ...
    def delete_batch_api_process(self, return_schema_class: Type[BaseModel],
                           depends: Optional[Sequence[params.Depends]] = None):
        def handle_delete_batch_api(request: Request, id_list: list[int]) -> ApiResponse[return_schema_class]:
            self.pre_processor.delete_batch_api_process(request=request, id_list=id_list)
            with Session(self.db_engine) as session:
                try:
                    db_data = session.get(self.model_class, id_list)
                    if not db_data:
                        return ApiResponse(success=False, message="该条数据不存在，无法完成删除！")
                    session.delete(db_data)
                    session.commit()
                    ...
                    schema_data = return_schema_class.model_validate(db_data)
                    return_data = jsonable_encoder(schema_data)
                    self.post_processor.delete_batch_api_process(request=request, data=return_data)
                    return ApiResponse(data=return_data)
                except HTTPException as e:
                    session.rollback()
                    print(f'删除接口出现异常：{str(e)}')
