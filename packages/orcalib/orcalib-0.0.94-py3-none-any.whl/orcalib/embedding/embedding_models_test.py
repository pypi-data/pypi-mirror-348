...
def test_e5_model_prompts_from_config():
    # Given an E5 model
    model_name = "OrcaDB/e5-large"
    text = "This is a test document"

    # Test default model config
    model = EmbeddingModel(model_name)
    assert model.config.document_prompt is not None  # E5 model has a default document prompt
    assert model.config.query_prompt is None  # E5 model doesn't have a default query prompt

    # Test model with custom document prompt override
    custom_prompt = "Custom prompt: "
    model_with_override = EmbeddingModel(model_name, document_prompt_override=custom_prompt)
    assert model_with_override.config.document_prompt == custom_prompt

    # Verify embeddings use the configured prompts
    # Default model uses its default document prompt
    embedding_default = model.embed(text)
    # Model with override uses the custom prompt
    embedding_with_override = model_with_override.embed(text, prompt=custom_prompt)
