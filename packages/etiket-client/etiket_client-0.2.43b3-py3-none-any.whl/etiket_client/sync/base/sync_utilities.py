    ...
    def upload_file(file_path, s_item : sync_item, f_info : file_info):
        ...
        if len(r_files) == 0:
            ...
            if version_id in f_dict.keys():
                if f_dict[version_id].md5_checksum == md5_checksum.hexdigest() or f_dict[version_id].md5_checksum == md5_checksum_netcdf4.hexdigest():
                    ...
                    if f_dict[max(f_dict.keys())].md5_checksum == md5_checksum.hexdigest() or f_dict[max(f_dict.keys())].md5_checksum == md5_checksum_netcdf4.hexdigest():
