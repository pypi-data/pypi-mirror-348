...
# Path where you want to save editable data
writable_file_path = 'data.json'

# Step 1: Check if writable file exists
if not os.path.exists(writable_file_path):
  # Read default data from package resource (read-only)
  with importlib.resources.open_text('termnotes', 'data.json') as data_file:
    content = data_file.read()
  
  # If resource file empty, start with empty dict
  if len(content.strip()) == 0:
    calendar_items = {}
  else:
    calendar_items = json.loads(content)
  
  # Save this data to writable file
  with open(writable_file_path, 'w') as f:
    json.dump(calendar_items, f)
else:
  # Step 2: If writable file exists, load from it
  with open(writable_file_path, 'r') as f:
