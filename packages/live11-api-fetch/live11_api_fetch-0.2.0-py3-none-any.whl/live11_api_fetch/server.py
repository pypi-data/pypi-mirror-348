...
async def serve(
    ...
    async def call_tool(name: str, arguments: dict) -> list[TextContent]:
        ...
        try:
            ...
            # Clean up the results by removing chunk_id field
            if "results" in api_response_json:
            ...
            return [
                TextContent(type="text", text=json.dumps(api_response_json, indent=2))
            ]
        except json.JSONDecodeError:
            # Fallback to returning raw text if JSON parsing fails
            return [TextContent(type="text", text=api_response_text)]
