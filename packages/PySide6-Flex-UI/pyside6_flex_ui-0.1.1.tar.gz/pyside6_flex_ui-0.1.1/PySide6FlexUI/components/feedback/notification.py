...
class Notification(QWidget):
    ...
    def _init_ui(self):
        ...
        main_layout.setContentsMargins(self._hmargin, self._vmargin, self._hmargin, self._vmargin)
    ...
    def _calculate_height(self) -> int:
        ...
        height = self.title_label.height() + self._vmargin * 2

        #  If message label exists
        if self.message_label:
            fm = QFontMetrics(self.message_label.font())
            text_width = self.width - self._hmargin * 2  # Account for margins
