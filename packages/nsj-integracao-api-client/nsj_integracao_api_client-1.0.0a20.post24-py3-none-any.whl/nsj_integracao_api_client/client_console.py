...
#color=94 if _is_console else 0
    ...
    def executar_integracao(self, entidades: List[str], env: integrador.Environment, forca_tenant: int = None):
        with InjectorFactory() as injector:
            if entidades:
                ...
                _, tempo = self.get_integrador(injector, env, forca_tenant).executar_integracao()
                self.mensagem(tempo)
            else:
    ...
    def executar_carga_inicial(self, entidades: list, env: integrador.Environment, forca_tenant: int = None):
        ...
        with InjectorFactory() as injector:
            _, tempo = self.get_integrador(injector, env, forca_tenant).executar_carga_inicial(entidades)
            self.mensagem(tempo)
    ...
    def executar_verificacao_integridade(self, args):
        ...
        with InjectorFactory() as injector:
            _, tempo = self.get_integrador(injector, args.env, args.forca_tenant).executar_verificacao_integridade(
