...
class ModalBladeModelling(BladeModelling):
    r"""
    The properties for the structural modelling of the blade using modal reduction.
    
    Attributes
    ----------
    BladeModellingType : Literal['ModalBladeModelling'], default='ModalBladeModelling'
    ...
    BladeModellingType: Literal['ModalBladeModelling'] = Field(alias="BladeModellingType", default='ModalBladeModelling', allow_mutation=False, const=True)
    WholeBladeModeDampingRatios: List[StructuralMode] = Field(alias="WholeBladeModeDampingRatios", default=list())
    NumberOfModesPerBladePart: int = Field(alias="NumberOfModesPerBladePart", default=None)
    ...
    def _factory(cls, data):
        if isinstance(data, ModalBladeModelling):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define ModalBladeModelling models')
        return ModalBladeModelling.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.WholeBladeModeDampingRatios is not None and len(self.WholeBladeModeDampingRatios) == 0: #type: ignore
            unused_containers.add('WholeBladeModeDampingRatios')
    ...
    def _is_unused(self) -> bool:
        ...
        """
        candidate_fields = [x[1] for x in self.__dict__.items() if x[0] != 'BladeModellingType']
