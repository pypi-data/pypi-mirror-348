...
class CurrentVerticalShearTimeHistory(CurrentVerticalShearVariation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/SeaState/Current/CurrentVerticalShearVariation/CurrentVerticalShearTimeHistory.json')

    VerticalShearVariationType: Literal['TimeHistory'] = Field(alias="VerticalShearVariationType", default='TimeHistory', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, CurrentVerticalShearTimeHistory):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define CurrentVerticalShearTimeHistory models')
        return CurrentVerticalShearTimeHistory.parse_obj(data)
