...
class YawSystem(Component):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/YawSystem/YawSystem.json')

    ComponentType: Literal['YawSystem'] = Field(alias="ComponentType", default='YawSystem', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, YawSystem):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define YawSystem models')
        return YawSystem.parse_obj(data)
