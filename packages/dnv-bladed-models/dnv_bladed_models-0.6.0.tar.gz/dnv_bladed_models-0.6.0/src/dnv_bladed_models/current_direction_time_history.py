...
class CurrentDirectionTimeHistory(CurrentDirectionVariation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/SeaState/Current/CurrentDirectionVariation/CurrentDirectionTimeHistory.json')

    DirectionVariationType: Literal['TimeHistory'] = Field(alias="DirectionVariationType", default='TimeHistory', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, CurrentDirectionTimeHistory):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define CurrentDirectionTimeHistory models')
        return CurrentDirectionTimeHistory.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.TimeVsDirection is not None and len(self.TimeVsDirection) == 0: #type: ignore
            unused_containers.add('TimeVsDirection')
