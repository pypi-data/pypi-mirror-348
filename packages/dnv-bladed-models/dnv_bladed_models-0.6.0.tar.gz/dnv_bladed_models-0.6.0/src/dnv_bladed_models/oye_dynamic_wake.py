...
class OyeDynamicWake(DynamicWake):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Settings/AerodynamicSettings/AerodynamicModel/MomentumTheoryCorrections/DynamicWake/OyeDynamicWake.json')

    DynamicWakeType: Literal['OyeDynamicWake'] = Field(alias="DynamicWakeType", default='OyeDynamicWake', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, OyeDynamicWake):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define OyeDynamicWake models')
        return OyeDynamicWake.parse_obj(data)
