...
class StreamFunction(RegularWaves):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/SeaState/Waves/StreamFunction.json')

    WavesType: Literal['StreamFunction'] = Field(alias="WavesType", default='StreamFunction', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, StreamFunction):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define StreamFunction models')
        return StreamFunction.parse_obj(data)
