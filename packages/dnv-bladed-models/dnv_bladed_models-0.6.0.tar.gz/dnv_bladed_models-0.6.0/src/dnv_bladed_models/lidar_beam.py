...
class LidarBeam(BladedModel):
    ...
    MountingPosition: LidarMountingPosition = Field(alias="MountingPosition", default=None) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, LidarBeam):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define LidarBeam models')
        return LidarBeam.parse_obj(data)
