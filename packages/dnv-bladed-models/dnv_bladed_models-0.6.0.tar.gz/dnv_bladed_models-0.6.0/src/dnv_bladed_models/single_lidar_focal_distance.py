...
class SingleLidarFocalDistance(LidarFocalDistanceControl):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Lidar/LidarFocalDistanceControl/SingleLidarFocalDistance.json')

    FocalDistanceControlType: Literal['SingleFocalDistance'] = Field(alias="FocalDistanceControlType", default='SingleFocalDistance', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, SingleLidarFocalDistance):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define SingleLidarFocalDistance models')
        return SingleLidarFocalDistance.parse_obj(data)
