...
class PitchFaultSeizure(PitchFault):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Event/PitchFaultSeizure.json')

    EventType: Literal['PitchFaultSeizure'] = Field(alias="EventType", default='PitchFaultSeizure', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, PitchFaultSeizure):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PitchFaultSeizure models')
        return PitchFaultSeizure.parse_obj(data)
