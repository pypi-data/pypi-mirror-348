...
class YawFaultLimp(YawFault):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Event/YawFaultLimp.json')

    EventType: Literal['YawFaultLimp'] = Field(alias="EventType", default='YawFaultLimp', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, YawFaultLimp):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define YawFaultLimp models')
        return YawFaultLimp.parse_obj(data)
