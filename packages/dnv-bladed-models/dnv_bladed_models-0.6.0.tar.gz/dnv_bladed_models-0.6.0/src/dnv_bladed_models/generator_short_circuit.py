...
class GeneratorShortCircuit(BladedModel):
    r"""
    The definition of the safety system behaviour when there is a short circuit.
    ...
    """
    _relative_schema_path: str = PrivateAttr('Turbine/BladedControl/SafetySystem/GeneratorShortCircuit/GeneratorShortCircuit.json')
    ...
    def _factory(cls, data):
        if isinstance(data, GeneratorShortCircuit):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define GeneratorShortCircuit models')
        return GeneratorShortCircuit.parse_obj(data)
