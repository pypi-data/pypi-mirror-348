...
class ExternalFlowSourceForWind(Wind):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/Wind/ExternalFlowSourceForWind.json')

    WindType: Literal['ExternalFlowSourceForWind'] = Field(alias="WindType", default='ExternalFlowSourceForWind', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, ExternalFlowSourceForWind):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define ExternalFlowSourceForWind models')
        return ExternalFlowSourceForWind.parse_obj(data)
