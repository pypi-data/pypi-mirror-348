...
class NonLinearShaftBrake(Brake):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/DrivetrainAndNacelle/Brake/NonLinearShaftBrake.json')

    BrakeType: Literal['NonLinearShaftBrake'] = Field(alias="BrakeType", default='NonLinearShaftBrake', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, NonLinearShaftBrake):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define NonLinearShaftBrake models')
        return NonLinearShaftBrake.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.TimeVsTorque is not None and len(self.TimeVsTorque) == 0: #type: ignore
            unused_containers.add('TimeVsTorque')
    ...
    def _is_unused(self) -> bool:
        ...
        """
        candidate_fields = [x[1] for x in self.__dict__.items() if x[0] != 'BrakeType']
