...
class CircularLidarScan(PresetLidarScan):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Lidar/LidarScanningPattern/CircularLidarScan.json')

    ScanningPatternType: Literal['CircularScan'] = Field(alias="ScanningPatternType", default='CircularScan', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, CircularLidarScan):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define CircularLidarScan models')
        return CircularLidarScan.parse_obj(data)
