...
class VariableSpeedGenerator(Generator):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Generator/VariableSpeedGenerator.json')

    ComponentType: Literal['VariableSpeedGenerator'] = Field(alias="ComponentType", default='VariableSpeedGenerator', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, VariableSpeedGenerator):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define VariableSpeedGenerator models')
        return VariableSpeedGenerator.parse_obj(data)
