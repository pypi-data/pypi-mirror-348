...
class GL2010Icing(IcedCondition):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/InitialCondition/GL2010Icing.json')

    InitialConditionType: Literal['GL2010Icing'] = Field(alias="InitialConditionType", default='GL2010Icing', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, GL2010Icing):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define GL2010Icing models')
        return GL2010Icing.parse_obj(data)
