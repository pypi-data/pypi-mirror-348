...
class TowerHydrodynamicProperties(BladedModel):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Tower/TowerHydrodynamicProperties/TowerHydrodynamicProperties.json')
    ...
    def _factory(cls, data):
        if isinstance(data, TowerHydrodynamicProperties):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define TowerHydrodynamicProperties models')
        return TowerHydrodynamicProperties.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.HeightUpTowerVsCdCm is not None and len(self.HeightUpTowerVsCdCm) == 0: #type: ignore
            unused_containers.add('HeightUpTowerVsCdCm')
