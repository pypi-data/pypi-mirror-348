...
class UserDefinedWakeDeficit(SteadyWakeDeficit):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/Wind/SteadyWakeDeficit/UserDefinedWakeDeficit.json')

    SteadyWakeDeficitType: Literal['UserDefined'] = Field(alias="SteadyWakeDeficitType", default='UserDefined', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, UserDefinedWakeDeficit):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define UserDefinedWakeDeficit models')
        return UserDefinedWakeDeficit.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.RadiusVsDeficit is not None and len(self.RadiusVsDeficit) == 0: #type: ignore
            unused_containers.add('RadiusVsDeficit')
    ...
    def _is_unused(self) -> bool:
        ...
        """
        candidate_fields = [x[1] for x in self.__dict__.items() if x[0] != 'SteadyWakeDeficitType']
