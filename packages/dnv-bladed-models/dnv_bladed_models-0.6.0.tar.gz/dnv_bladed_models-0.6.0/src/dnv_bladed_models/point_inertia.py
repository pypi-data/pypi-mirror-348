...
class PointInertia(AddedInertia):
    ...
    AddedInertiaType: Literal['PointInertia'] = Field(alias="AddedInertiaType", default='PointInertia', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, PointInertia):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PointInertia models')
        return PointInertia.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.MomentOfInertiaTensor is not None and len(self.MomentOfInertiaTensor) == 0: #type: ignore
            unused_containers.add('MomentOfInertiaTensor')
