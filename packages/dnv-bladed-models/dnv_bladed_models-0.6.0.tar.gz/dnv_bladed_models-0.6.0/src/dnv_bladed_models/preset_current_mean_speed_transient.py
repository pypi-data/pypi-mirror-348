...
class PresetCurrentMeanSpeedTransient(CurrentMeanSpeedVariation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/SeaState/Current/CurrentMeanSpeedVariation/PresetCurrentMeanSpeedTransient.json')

    MeanSpeedVariationType: Literal['PresetTransient'] = Field(alias="MeanSpeedVariationType", default='PresetTransient', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, PresetCurrentMeanSpeedTransient):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PresetCurrentMeanSpeedTransient models')
        return PresetCurrentMeanSpeedTransient.parse_obj(data)
