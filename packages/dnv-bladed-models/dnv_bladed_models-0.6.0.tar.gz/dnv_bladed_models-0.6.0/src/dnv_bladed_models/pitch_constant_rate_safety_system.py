...
class PitchConstantRateSafetySystem(PitchControllerRateSafetySystem):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/PitchSystem/PitchController/PitchSafetySystem/PitchConstantRateSafetySystem.json')

    PitchSafetySystemType: Literal['ConstantRate'] = Field(alias="PitchSafetySystemType", default='ConstantRate', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, PitchConstantRateSafetySystem):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PitchConstantRateSafetySystem models')
        return PitchConstantRateSafetySystem.parse_obj(data)
