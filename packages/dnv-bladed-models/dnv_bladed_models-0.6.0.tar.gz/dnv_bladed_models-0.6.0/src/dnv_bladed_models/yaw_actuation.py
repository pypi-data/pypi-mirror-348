...
class YawActuation(BladedModel):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/YawSystem/YawActuation/YawActuation.json')
    ...
    def _factory(cls, data):
        if isinstance(data, YawActuation):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define YawActuation models')
        return YawActuation.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.LoadBanks is not None and len(self.LoadBanks) == 0: #type: ignore
            unused_containers.add('LoadBanks')
    ...
    def from_file(cls: Type['Model'], path: Union[str, Path]) -> 'Model':
        ...
        Examples
        --------
        >>> model = YawActuation.from_file('/path/to/file')
        """
