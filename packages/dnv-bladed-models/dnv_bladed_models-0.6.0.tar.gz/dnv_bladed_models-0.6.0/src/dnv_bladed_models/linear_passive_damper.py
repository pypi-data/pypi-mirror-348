...
class LinearPassiveDamper(Damper):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Damper/LinearPassiveDamper.json')

    ComponentType: Literal['LinearPassiveDamper'] = Field(alias="ComponentType", default='LinearPassiveDamper', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, LinearPassiveDamper):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define LinearPassiveDamper models')
        return LinearPassiveDamper.parse_obj(data)
