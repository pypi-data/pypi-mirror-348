...
class YawFaultConstantTorque(YawFault):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Event/YawFaultConstantTorque.json')

    EventType: Literal['YawFaultConstantTorque'] = Field(alias="EventType", default='YawFaultConstantTorque', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, YawFaultConstantTorque):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define YawFaultConstantTorque models')
        return YawFaultConstantTorque.parse_obj(data)
