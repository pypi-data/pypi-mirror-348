...
class PitchRateVariesWithTime(PitchControllerRateSafetySystem):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/PitchSystem/PitchController/PitchSafetySystem/PitchRateVariesWithTime.json')

    PitchSafetySystemType: Literal['RateVariesWithTime'] = Field(alias="PitchSafetySystemType", default='RateVariesWithTime', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, PitchRateVariesWithTime):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PitchRateVariesWithTime models')
        return PitchRateVariesWithTime.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.TimeVsRate is not None and len(self.TimeVsRate) == 0: #type: ignore
            unused_containers.add('TimeVsRate')
