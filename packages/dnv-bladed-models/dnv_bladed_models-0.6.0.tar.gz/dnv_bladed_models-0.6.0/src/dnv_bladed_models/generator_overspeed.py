...
class GeneratorOverspeed(BladedModel):
    r"""
    The definition of the safety system behaviour when the generator is running too fast
    ...
    TripValue : float, Not supported yet
        The rotation speed above which the circuit should be activated.
    ...
    """
    _relative_schema_path: str = PrivateAttr('Turbine/BladedControl/SafetySystem/GeneratorOverspeed/GeneratorOverspeed.json')
    ...
    def _factory(cls, data):
        if isinstance(data, GeneratorOverspeed):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define GeneratorOverspeed models')
        return GeneratorOverspeed.parse_obj(data)
