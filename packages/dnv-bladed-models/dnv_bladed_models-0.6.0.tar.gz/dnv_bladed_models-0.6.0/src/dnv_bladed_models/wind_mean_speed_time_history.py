...
class WindMeanSpeedTimeHistory(WindMeanSpeedVariation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/Wind/WindMeanSpeedVariation/WindMeanSpeedTimeHistory.json')

    MeanSpeedVariationType: Literal['TimeHistory'] = Field(alias="MeanSpeedVariationType", default='TimeHistory', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, WindMeanSpeedTimeHistory):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define WindMeanSpeedTimeHistory models')
        return WindMeanSpeedTimeHistory.parse_obj(data)
