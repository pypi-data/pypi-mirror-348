...
class TowerConnectableNodes(BladedModel):
    r"""
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Tower/TowerConnectableNodes/TowerConnectableNodes.json')
    ...
    def items(self) -> list[tuple[str, TowerConnectableNode]]:
        """
    ...
    def values(self) -> list[TowerConnectableNode]:
        """
    ...
    def __getitem__(self, key: Union[str, int]) -> TowerConnectableNode:
    ...
    def __setitem__(self, key: str, value: TowerConnectableNode):
        if not isinstance(key, str):
            raise KeyError(f"Custom entries can only be added with string keys")
        if key in self.__fields__:
            raise KeyError(f"Item accessors can only be used for custom entries. '{key}' is a pre-defined model property.")
        if not isinstance(value, TowerConnectableNode):
            raise TypeError(f"Entries must be of type 'TowerConnectableNode'; received '{type(value).__name__}'")
    ...
    def __setattr__(self, name: str, value: TowerConnectableNode):
        if not name in self.__fields__ and not isinstance(value, TowerConnectableNode):
            raise TypeError(f"Entries must be of type 'TowerConnectableNode'; received '{type(value).__name__}'")
    ...
    class __CustomEntries__(BaseModel):
        entries: Dict[str, TowerConnectableNode]
    ...
    def _factory(cls, data):
        if isinstance(data, TowerConnectableNodes):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define TowerConnectableNodes models')
        return TowerConnectableNodes.parse_obj(data)


    @classmethod
    def parse_obj(cls: Type['Model'], obj: Any) -> 'Model':
        return custom_entries_parser(cls, obj, dict, TowerConnectableNodes.__CustomEntries__)
    ...
    def from_file(cls: Type['Model'], path: Union[str, Path]) -> 'Model':
        ...
        Examples
        --------
        >>> model = TowerConnectableNodes.from_file('/path/to/file')
        """
    ...
    def from_json(cls: Type['Model'], b: StrBytes) -> 'Model':
        ...
        Examples
        --------
        >>> model = TowerConnectableNodes.from_json('{ ... }')
        """
...
TowerConnectableNodes.update_forward_refs()
TowerConnectableNodes.__CustomEntries__.update_forward_refs()
