...
class CurrentMeanSpeedTimeHistory(CurrentMeanSpeedVariation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/SeaState/Current/CurrentMeanSpeedVariation/CurrentMeanSpeedTimeHistory.json')

    MeanSpeedVariationType: Literal['TimeHistory'] = Field(alias="MeanSpeedVariationType", default='TimeHistory', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, CurrentMeanSpeedTimeHistory):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define CurrentMeanSpeedTimeHistory models')
        return CurrentMeanSpeedTimeHistory.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.TimeVsMeanSpeed is not None and len(self.TimeVsMeanSpeed) == 0: #type: ignore
            unused_containers.add('TimeVsMeanSpeed')
    ...
    def _is_unused(self) -> bool:
        ...
        """
        candidate_fields = [x[1] for x in self.__dict__.items() if x[0] != 'MeanSpeedVariationType']
