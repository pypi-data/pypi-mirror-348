...
class IndependentPitchHub(StandardHub):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Hub/IndependentPitchHub.json')

    ComponentType: Literal['IndependentPitchHub'] = Field(alias="ComponentType", default='IndependentPitchHub', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, IndependentPitchHub):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define IndependentPitchHub models')
        return IndependentPitchHub.parse_obj(data)
