...
class Aerofoil(BladedModel):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Blade/AerofoilLibrary/Aerofoil/Aerofoil.json')
    ...
    def _factory(cls, data):
        if isinstance(data, Aerofoil):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define Aerofoil models')
        return Aerofoil.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.AerodynamicPerformance is not None and len(self.AerodynamicPerformance) == 0: #type: ignore
            unused_containers.add('AerodynamicPerformance')
