...
class CampbellPowerProduction(CampbellDiagramModeOfOperation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('SteadyCalculation/CampbellDiagramModeOfOperation/CampbellPowerProduction.json')

    CampbellDiagramModeOfOperationType: Literal['PowerProduction'] = Field(alias="CampbellDiagramModeOfOperationType", default='PowerProduction', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, CampbellPowerProduction):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define CampbellPowerProduction models')
        return CampbellPowerProduction.parse_obj(data)
