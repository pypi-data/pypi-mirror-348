...
class PassiveTransducerResponse(TransducerBehaviour):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Turbine/BladedControl/MeasuredSignalProperties/common/PassiveTransducerResponse.json')

    TransducerBehaviourType: Literal['PassiveTransducerResponse'] = Field(alias="TransducerBehaviourType", default='PassiveTransducerResponse', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, PassiveTransducerResponse):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PassiveTransducerResponse models')
        return PassiveTransducerResponse.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.Numerators is not None and len(self.Numerators) == 0: #type: ignore
            unused_containers.add('Numerators')
        if self.Denominators is not None and len(self.Denominators) == 0: #type: ignore
            unused_containers.add('Denominators')
    ...
    def from_file(cls: Type['Model'], path: Union[str, Path]) -> 'Model':
        ...
        Examples
        --------
        >>> model = PassiveTransducerResponse.from_file('/path/to/file')
        """
    ...
    def from_json(cls: Type['Model'], b: StrBytes) -> 'Model':
        ...
        Examples
        --------
        >>> model = PassiveTransducerResponse.from_json('{ ... }')
        """
