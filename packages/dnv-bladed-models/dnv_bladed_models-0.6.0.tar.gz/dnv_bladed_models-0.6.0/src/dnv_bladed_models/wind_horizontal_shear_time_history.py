...
class WindHorizontalShearTimeHistory(WindHorizontalShearVariation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/Wind/WindHorizontalShearVariation/WindHorizontalShearTimeHistory.json')

    HorizontalShearVariationType: Literal['TimeHistory'] = Field(alias="HorizontalShearVariationType", default='TimeHistory', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, WindHorizontalShearTimeHistory):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define WindHorizontalShearTimeHistory models')
        return WindHorizontalShearTimeHistory.parse_obj(data)
