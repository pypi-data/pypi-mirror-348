...
class LookUpShearModel(WindShear):
    ...
    """
    _relative_schema_path: str = PrivateAttr('common/LookUpShearModel.json')

    WindShearType: Literal['LookUpShearModel'] = Field(alias="WindShearType", default='LookUpShearModel', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, LookUpShearModel):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define LookUpShearModel models')
        return LookUpShearModel.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.HeightVsShearFactor is not None and len(self.HeightVsShearFactor) == 0: #type: ignore
            unused_containers.add('HeightVsShearFactor')
