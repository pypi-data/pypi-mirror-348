...
class ExplicitNewmarkBetaFixedStep(FixedStepIntegrator):
    ...
    IntegratorType: Literal['ExplicitNewmarkBetaFixedStep'] = Field(alias="IntegratorType", default='ExplicitNewmarkBetaFixedStep', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, ExplicitNewmarkBetaFixedStep):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define ExplicitNewmarkBetaFixedStep models')
        return ExplicitNewmarkBetaFixedStep.parse_obj(data)
