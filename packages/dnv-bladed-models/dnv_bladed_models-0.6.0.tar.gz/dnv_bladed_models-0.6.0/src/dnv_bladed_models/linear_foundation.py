...
class LinearFoundation(Foundation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Tower/Foundation/LinearFoundation.json')

    FoundationType: Literal['LinearFoundation'] = Field(alias="FoundationType", default='LinearFoundation', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, LinearFoundation):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define LinearFoundation models')
        return LinearFoundation.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.StiffnessMatrix is not None and len(self.StiffnessMatrix) == 0: #type: ignore
            unused_containers.add('StiffnessMatrix')
        if self.MassMatrix is not None and len(self.MassMatrix) == 0: #type: ignore
            unused_containers.add('MassMatrix')
    ...
    def _is_unused(self) -> bool:
        ...
        """
        candidate_fields = [x[1] for x in self.__dict__.items() if x[0] != 'FoundationType']
