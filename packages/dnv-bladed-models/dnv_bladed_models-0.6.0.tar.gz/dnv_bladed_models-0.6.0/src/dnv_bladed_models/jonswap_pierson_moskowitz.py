...
class JonswapPiersonMoskowitz(IrregularWaves):
    ...
    WavesType: Literal['JonswapPiersonMoskowitz'] = Field(alias="WavesType", default='JonswapPiersonMoskowitz', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, JonswapPiersonMoskowitz):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define JonswapPiersonMoskowitz models')
        return JonswapPiersonMoskowitz.parse_obj(data)
