...
class TowerOutputGroupLibrary(BladedModel):
    r"""
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Tower/TowerOutputGroupLibrary/TowerOutputGroupLibrary.json')
    ...
    def items(self) -> list[tuple[str, TowerOutputGroup]]:
        """
    ...
    def values(self) -> list[TowerOutputGroup]:
        """
    ...
    def __getitem__(self, key: Union[str, int]) -> TowerOutputGroup:
    ...
    def __setitem__(self, key: str, value: TowerOutputGroup):
        if not isinstance(key, str):
            raise KeyError(f"Custom entries can only be added with string keys")
        if key in self.__fields__:
            raise KeyError(f"Item accessors can only be used for custom entries. '{key}' is a pre-defined model property.")
        if not isinstance(value, TowerOutputGroup):
            raise TypeError(f"Entries must be of type 'TowerOutputGroup'; received '{type(value).__name__}'")
    ...
    def __setattr__(self, name: str, value: TowerOutputGroup):
        if not name in self.__fields__ and not isinstance(value, TowerOutputGroup):
            raise TypeError(f"Entries must be of type 'TowerOutputGroup'; received '{type(value).__name__}'")
    ...
    class __CustomEntries__(BaseModel):
        entries: Dict[str, TowerOutputGroup]
    ...
    def _factory(cls, data):
        if isinstance(data, TowerOutputGroupLibrary):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define TowerOutputGroupLibrary models')
        return TowerOutputGroupLibrary.parse_obj(data)


    @classmethod
    def parse_obj(cls: Type['Model'], obj: Any) -> 'Model':
        return custom_entries_parser(cls, obj, dict, TowerOutputGroupLibrary.__CustomEntries__)
    ...
    def from_file(cls: Type['Model'], path: Union[str, Path]) -> 'Model':
        ...
        Examples
        --------
        >>> model = TowerOutputGroupLibrary.from_file('/path/to/file')
        """
    ...
    def from_json(cls: Type['Model'], b: StrBytes) -> 'Model':
        ...
        Examples
        --------
        >>> model = TowerOutputGroupLibrary.from_json('{ ... }')
        """
...
TowerOutputGroupLibrary.update_forward_refs()
TowerOutputGroupLibrary.__CustomEntries__.update_forward_refs()
