...
class RotorIdlingControlState(InitialControlState):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/InitialCondition/RotorIdlingControlState.json')

    InitialConditionType: Literal['RotorIdling'] = Field(alias="InitialConditionType", default='RotorIdling', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, RotorIdlingControlState):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define RotorIdlingControlState models')
        return RotorIdlingControlState.parse_obj(data)
