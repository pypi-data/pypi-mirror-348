...
class YawFaultConstantRate(YawFault):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Event/YawFaultConstantRate.json')

    EventType: Literal['YawFaultConstantRate'] = Field(alias="EventType", default='YawFaultConstantRate', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, YawFaultConstantRate):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define YawFaultConstantRate models')
        return YawFaultConstantRate.parse_obj(data)
