...
class BladeElementMomentum(AerodynamicModel):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Settings/AerodynamicSettings/AerodynamicModel/BladeElementMomentum.json')

    AerodynamicModelType: Literal['BladeElementMomentum'] = Field(alias="AerodynamicModelType", default='BladeElementMomentum', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, BladeElementMomentum):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define BladeElementMomentum models')
        return BladeElementMomentum.parse_obj(data)
