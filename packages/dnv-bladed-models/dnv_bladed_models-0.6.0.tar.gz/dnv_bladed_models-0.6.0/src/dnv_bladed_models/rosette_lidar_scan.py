...
class RosetteLidarScan(PresetLidarScan):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Lidar/LidarScanningPattern/RosetteLidarScan.json')

    ScanningPatternType: Literal['RosetteScan'] = Field(alias="ScanningPatternType", default='RosetteScan', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, RosetteLidarScan):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define RosetteLidarScan models')
        return RosetteLidarScan.parse_obj(data)
