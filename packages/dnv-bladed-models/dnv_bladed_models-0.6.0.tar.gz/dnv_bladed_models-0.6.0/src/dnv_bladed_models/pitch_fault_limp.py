...
class PitchFaultLimp(PitchFault):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Event/PitchFaultLimp.json')

    EventType: Literal['PitchFaultLimp'] = Field(alias="EventType", default='PitchFaultLimp', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, PitchFaultLimp):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PitchFaultLimp models')
        return PitchFaultLimp.parse_obj(data)
