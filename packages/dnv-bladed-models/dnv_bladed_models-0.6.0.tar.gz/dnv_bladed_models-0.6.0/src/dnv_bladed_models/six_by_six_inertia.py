...
class SixBySixInertia(AddedInertia):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Tower/AddedInertia/SixBySixInertia.json')

    AddedInertiaType: Literal['SixBySixInertia'] = Field(alias="AddedInertiaType", default='SixBySixInertia', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, SixBySixInertia):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define SixBySixInertia models')
        return SixBySixInertia.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.Inertia is not None and len(self.Inertia) == 0: #type: ignore
            unused_containers.add('Inertia')
    ...
    def _is_unused(self) -> bool:
        ...
        """
        candidate_fields = [x[1] for x in self.__dict__.items() if x[0] != 'AddedInertiaType']
