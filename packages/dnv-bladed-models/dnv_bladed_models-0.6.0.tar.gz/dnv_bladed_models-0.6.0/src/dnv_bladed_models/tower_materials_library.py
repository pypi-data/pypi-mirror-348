...
class TowerMaterialsLibrary(BladedModel):
    r"""
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Tower/TowerMaterialsLibrary/TowerMaterialsLibrary.json')
    ...
    def items(self) -> list[tuple[str, TowerMaterial]]:
        """
    ...
    def values(self) -> list[TowerMaterial]:
        """
    ...
    def __getitem__(self, key: Union[str, int]) -> TowerMaterial:
    ...
    def __setitem__(self, key: str, value: TowerMaterial):
        if not isinstance(key, str):
            raise KeyError(f"Custom entries can only be added with string keys")
        if key in self.__fields__:
            raise KeyError(f"Item accessors can only be used for custom entries. '{key}' is a pre-defined model property.")
        if not isinstance(value, TowerMaterial):
            raise TypeError(f"Entries must be of type 'TowerMaterial'; received '{type(value).__name__}'")
    ...
    def __setattr__(self, name: str, value: TowerMaterial):
        if not name in self.__fields__ and not isinstance(value, TowerMaterial):
            raise TypeError(f"Entries must be of type 'TowerMaterial'; received '{type(value).__name__}'")
    ...
    class __CustomEntries__(BaseModel):
        entries: Dict[str, TowerMaterial]
    ...
    def _factory(cls, data):
        if isinstance(data, TowerMaterialsLibrary):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define TowerMaterialsLibrary models')
        return TowerMaterialsLibrary.parse_obj(data)


    @classmethod
    def parse_obj(cls: Type['Model'], obj: Any) -> 'Model':
        return custom_entries_parser(cls, obj, dict, TowerMaterialsLibrary.__CustomEntries__)
    ...
    def from_file(cls: Type['Model'], path: Union[str, Path]) -> 'Model':
        ...
        Examples
        --------
        >>> model = TowerMaterialsLibrary.from_file('/path/to/file')
        """
    ...
    def from_json(cls: Type['Model'], b: StrBytes) -> 'Model':
        ...
        Examples
        --------
        >>> model = TowerMaterialsLibrary.from_json('{ ... }')
        """
...
TowerMaterialsLibrary.update_forward_refs()
TowerMaterialsLibrary.__CustomEntries__.update_forward_refs()
