...
class PresetWindDirectionShearTransient(WindDirectionShearVariation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/Wind/WindDirectionShearVariation/PresetWindDirectionShearTransient.json')

    DirectionShearVariationType: Literal['PresetTransient'] = Field(alias="DirectionShearVariationType", default='PresetTransient', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, PresetWindDirectionShearTransient):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PresetWindDirectionShearTransient models')
        return PresetWindDirectionShearTransient.parse_obj(data)
