...
class PitchBearingStiction(BladedModel):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/PitchSystem/Friction/PitchBearingStiction/PitchBearingStiction.json')
    ...
    def _factory(cls, data):
        if isinstance(data, PitchBearingStiction):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PitchBearingStiction models')
        return PitchBearingStiction.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.FrictionTorqueVsStictionTorque is not None and len(self.FrictionTorqueVsStictionTorque) == 0: #type: ignore
            unused_containers.add('FrictionTorqueVsStictionTorque')
