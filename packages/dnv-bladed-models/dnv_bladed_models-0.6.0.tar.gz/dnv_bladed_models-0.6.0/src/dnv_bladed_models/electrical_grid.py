...
class ElectricalGrid(BladedModel):
    ...
    def _factory(cls, data):
        if isinstance(data, ElectricalGrid):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define ElectricalGrid models')
        return ElectricalGrid.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.GridLossEnergySinks is not None and len(self.GridLossEnergySinks) == 0: #type: ignore
            unused_containers.add('GridLossEnergySinks')
