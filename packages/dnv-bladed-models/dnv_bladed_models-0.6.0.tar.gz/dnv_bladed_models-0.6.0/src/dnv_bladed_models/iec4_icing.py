...
class IEC4Icing(IcedCondition):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/InitialCondition/IEC4Icing.json')

    InitialConditionType: Literal['IEC4Icing'] = Field(alias="InitialConditionType", default='IEC4Icing', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, IEC4Icing):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define IEC4Icing models')
        return IEC4Icing.parse_obj(data)
