...
class NonLinearElectricalLosses(ElectricalLosses):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Generator/ElectricalLosses/NonLinearElectricalLosses.json')

    ElectricalLossesType: Literal['NonLinearElectricalLosses'] = Field(alias="ElectricalLossesType", default='NonLinearElectricalLosses', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, NonLinearElectricalLosses):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define NonLinearElectricalLosses models')
        return NonLinearElectricalLosses.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.InputPowerVsLoss is not None and len(self.InputPowerVsLoss) == 0: #type: ignore
            unused_containers.add('InputPowerVsLoss')
    ...
    def _is_unused(self) -> bool:
        ...
        """
        candidate_fields = [x[1] for x in self.__dict__.items() if x[0] != 'ElectricalLossesType']
