...
class Aileron(BladedModel):
    r"""
    An aerofoil whose properties depends on the aileron deployment angle.  For angles between the specified sections, the aerofoil properties will be interpolated.
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Blade/AileronAerofoilLibrary/Aileron/Aileron.json')
    ...
    def __setitem__(self, key: str, value: str):
        ...
        if not isinstance(value, str):
            raise TypeError(f"Entries must be of type 'str'; received '{type(value).__name__}'")
    ...
    def __setattr__(self, name: str, value: str):
        if not name in self.__fields__ and not isinstance(value, str):
            raise TypeError(f"Entries must be of type 'str'; received '{type(value).__name__}'")
    ...
    def _factory(cls, data):
        if isinstance(data, Aileron):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define Aileron models')
        return Aileron.parse_obj(data)


    @classmethod
    def parse_obj(cls: Type['Model'], obj: Any) -> 'Model':
        return custom_entries_parser(cls, obj, str, Aileron.__CustomEntries__)
    ...
    def from_file(cls: Type['Model'], path: Union[str, Path]) -> 'Model':
        ...
        Examples
        --------
        >>> model = Aileron.from_file('/path/to/file')
        """
...
Aileron.update_forward_refs()
Aileron.__CustomEntries__.update_forward_refs()
