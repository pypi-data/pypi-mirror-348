...
class ExternalModuleComponent(Component):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/ExternalModuleComponent/ExternalModuleComponent.json')

    ComponentType: Literal['ExternalModuleComponent'] = Field(alias="ComponentType", default='ExternalModuleComponent', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, ExternalModuleComponent):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define ExternalModuleComponent models')
        return ExternalModuleComponent.parse_obj(data)
