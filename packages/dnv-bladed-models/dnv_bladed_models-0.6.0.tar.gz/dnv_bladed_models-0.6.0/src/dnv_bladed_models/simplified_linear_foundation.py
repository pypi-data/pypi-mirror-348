...
class SimplifiedLinearFoundation(Foundation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Tower/Foundation/SimplifiedLinearFoundation.json')

    FoundationType: Literal['SimplifiedLinearFoundation'] = Field(alias="FoundationType", default='SimplifiedLinearFoundation', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, SimplifiedLinearFoundation):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define SimplifiedLinearFoundation models')
        return SimplifiedLinearFoundation.parse_obj(data)
