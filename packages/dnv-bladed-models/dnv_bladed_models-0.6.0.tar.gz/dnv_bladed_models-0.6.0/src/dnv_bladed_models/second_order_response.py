...
class SecondOrderResponse(ResponseToDemand):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/PitchSystem/PitchController/PitchSystemDemand/ResponseToDemand/SecondOrderResponse.json')

    ResponseToDemandType: Literal['SecondOrderResponse'] = Field(alias="ResponseToDemandType", default='SecondOrderResponse', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, SecondOrderResponse):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define SecondOrderResponse models')
        return SecondOrderResponse.parse_obj(data)
