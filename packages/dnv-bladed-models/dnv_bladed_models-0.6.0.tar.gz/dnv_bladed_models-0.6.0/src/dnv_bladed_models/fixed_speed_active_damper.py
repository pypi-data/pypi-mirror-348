...
class FixedSpeedActiveDamper(ActiveDamper):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Damper/FixedSpeedActiveDamper.json')

    ComponentType: Literal['FixedSpeedActiveDamper'] = Field(alias="ComponentType", default='FixedSpeedActiveDamper', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, FixedSpeedActiveDamper):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define FixedSpeedActiveDamper models')
        return FixedSpeedActiveDamper.parse_obj(data)
