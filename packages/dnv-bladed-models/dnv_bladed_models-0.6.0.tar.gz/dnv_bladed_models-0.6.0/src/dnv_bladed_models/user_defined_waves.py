...
class UserDefinedWaves(IrregularWaves):
    ...
    WavesType: Literal['UserDefined'] = Field(alias="WavesType", default='UserDefined', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, UserDefinedWaves):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define UserDefinedWaves models')
        return UserDefinedWaves.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.FrequencyVsSpectralDensity is not None and len(self.FrequencyVsSpectralDensity) == 0: #type: ignore
            unused_containers.add('FrequencyVsSpectralDensity')
    ...
    def _is_unused(self) -> bool:
        ...
        """
        candidate_fields = [x[1] for x in self.__dict__.items() if x[0] != 'WavesType']
