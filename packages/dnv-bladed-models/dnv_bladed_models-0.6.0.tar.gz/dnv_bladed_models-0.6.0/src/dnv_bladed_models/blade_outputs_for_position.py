...
class BladeOutputsForPosition(BladeOutputsForLocation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Blade/BladeOutputGroupLibrary/BladeOutputGroup/BladeOutputsForLocation/BladeOutputsForPosition.json')

    BladeOutputsForLocationType: Literal['OutputsForPosition'] = Field(alias="BladeOutputsForLocationType", default='OutputsForPosition', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, BladeOutputsForPosition):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define BladeOutputsForPosition models')
        return BladeOutputsForPosition.parse_obj(data)
