...
class HubOutputGroupLibrary(BladedModel):
    r"""
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Hub/HubOutputGroupLibrary/HubOutputGroupLibrary.json')
    ...
    def items(self) -> list[tuple[str, HubOutputGroup]]:
        """
    ...
    def values(self) -> list[HubOutputGroup]:
        """
    ...
    def __getitem__(self, key: Union[str, int]) -> HubOutputGroup:
    ...
    def __setitem__(self, key: str, value: HubOutputGroup):
        if not isinstance(key, str):
            raise KeyError(f"Custom entries can only be added with string keys")
        if key in self.__fields__:
            raise KeyError(f"Item accessors can only be used for custom entries. '{key}' is a pre-defined model property.")
        if not isinstance(value, HubOutputGroup):
            raise TypeError(f"Entries must be of type 'HubOutputGroup'; received '{type(value).__name__}'")
    ...
    def __setattr__(self, name: str, value: HubOutputGroup):
        if not name in self.__fields__ and not isinstance(value, HubOutputGroup):
            raise TypeError(f"Entries must be of type 'HubOutputGroup'; received '{type(value).__name__}'")
    ...
    class __CustomEntries__(BaseModel):
        entries: Dict[str, HubOutputGroup]
    ...
    def _factory(cls, data):
        if isinstance(data, HubOutputGroupLibrary):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define HubOutputGroupLibrary models')
        return HubOutputGroupLibrary.parse_obj(data)


    @classmethod
    def parse_obj(cls: Type['Model'], obj: Any) -> 'Model':
        return custom_entries_parser(cls, obj, dict, HubOutputGroupLibrary.__CustomEntries__)
    ...
    def from_file(cls: Type['Model'], path: Union[str, Path]) -> 'Model':
        ...
        Examples
        --------
        >>> model = HubOutputGroupLibrary.from_file('/path/to/file')
        """
    ...
    def from_json(cls: Type['Model'], b: StrBytes) -> 'Model':
        ...
        Examples
        --------
        >>> model = HubOutputGroupLibrary.from_json('{ ... }')
        """
...
HubOutputGroupLibrary.update_forward_refs()
HubOutputGroupLibrary.__CustomEntries__.update_forward_refs()
