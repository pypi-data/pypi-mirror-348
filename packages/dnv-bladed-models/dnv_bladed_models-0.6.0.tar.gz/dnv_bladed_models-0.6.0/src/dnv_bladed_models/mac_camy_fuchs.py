...
class MacCamyFuchs(WaveDiffractionApproximation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/SeaState/Waves/WaveDiffractionApproximation/MacCamyFuchs.json')

    WaveDiffractionApproximationType: Literal['MacCamyFuchs'] = Field(alias="WaveDiffractionApproximationType", default='MacCamyFuchs', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, MacCamyFuchs):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define MacCamyFuchs models')
        return MacCamyFuchs.parse_obj(data)
