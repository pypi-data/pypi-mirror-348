...
class TowerNodeAtHeightOutputs(BladedModel):
    r"""
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Tower/TowerOutputGroupLibrary/TowerOutputGroup/NodesAtHeights/TowerNodeAtHeightOutputs/TowerNodeAtHeightOutputs.json')
    ...
    SnapToNearestNode: bool = Field(alias="SnapToNearestNode", default=None) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, TowerNodeAtHeightOutputs):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define TowerNodeAtHeightOutputs models')
        return TowerNodeAtHeightOutputs.parse_obj(data)
