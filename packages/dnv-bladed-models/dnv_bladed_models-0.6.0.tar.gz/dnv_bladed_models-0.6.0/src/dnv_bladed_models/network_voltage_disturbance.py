...
class NetworkVoltageDisturbance(Fault):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Event/NetworkVoltageDisturbance.json')

    EventType: Literal['NetworkVoltageDisturbance'] = Field(alias="EventType", default='NetworkVoltageDisturbance', allow_mutation=False, const=True) # Not supported yet
    Filepath: str = Field(alias="Filepath", default=None) # Not supported yet
    TimeVsVoltageChange: List[TimeVsVoltageChange] = Field(alias="TimeVsVoltageChange", default=list()) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, NetworkVoltageDisturbance):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define NetworkVoltageDisturbance models')
        return NetworkVoltageDisturbance.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.TimeVsVoltageChange is not None and len(self.TimeVsVoltageChange) == 0: #type: ignore
            unused_containers.add('TimeVsVoltageChange')
