...
class LoadBank(BladedModel):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/YawSystem/YawActuation/LoadBank/LoadBank.json')
    ...
    def _factory(cls, data):
        if isinstance(data, LoadBank):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define LoadBank models')
        return LoadBank.parse_obj(data)
    ...
    def from_file(cls: Type['Model'], path: Union[str, Path]) -> 'Model':
        ...
        Examples
        --------
        >>> model = LoadBank.from_file('/path/to/file')
        """
