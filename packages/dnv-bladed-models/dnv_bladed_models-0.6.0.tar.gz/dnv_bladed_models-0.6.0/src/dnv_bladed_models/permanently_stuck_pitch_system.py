...
class PermanentlyStuckPitchSystem(Event):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Event/PermanentlyStuckPitchSystem.json')

    EventType: Literal['PermanentlyStuckPitchSystem'] = Field(alias="EventType", default='PermanentlyStuckPitchSystem', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, PermanentlyStuckPitchSystem):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PermanentlyStuckPitchSystem models')
        return PermanentlyStuckPitchSystem.parse_obj(data)
