...
class SimpleCutoffFrequency(WaveDiffractionApproximation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/SeaState/Waves/WaveDiffractionApproximation/SimpleCutoffFrequency.json')

    WaveDiffractionApproximationType: Literal['SimpleCutoffFrequency'] = Field(alias="WaveDiffractionApproximationType", default='SimpleCutoffFrequency', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, SimpleCutoffFrequency):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define SimpleCutoffFrequency models')
        return SimpleCutoffFrequency.parse_obj(data)
