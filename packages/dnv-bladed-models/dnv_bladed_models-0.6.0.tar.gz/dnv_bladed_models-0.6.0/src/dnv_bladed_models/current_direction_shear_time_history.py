...
class CurrentDirectionShearTimeHistory(CurrentDirectionShearVariation):
    ...
    DirectionShearVariationType: Literal['TimeHistory'] = Field(alias="DirectionShearVariationType", default='TimeHistory', allow_mutation=False, const=True) # Not supported yet
    TimeVsDirectionShear: List[TimeVsCurrentDirectionShear] = Field(alias="TimeVsDirectionShear", default=list()) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, CurrentDirectionShearTimeHistory):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define CurrentDirectionShearTimeHistory models')
        return CurrentDirectionShearTimeHistory.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.TimeVsDirectionShear is not None and len(self.TimeVsDirectionShear) == 0: #type: ignore
            unused_containers.add('TimeVsDirectionShear')
