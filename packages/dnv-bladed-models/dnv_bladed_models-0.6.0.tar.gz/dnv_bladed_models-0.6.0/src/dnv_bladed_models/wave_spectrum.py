...
class WaveSpectrum(Waves):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/SeaState/Waves/WaveSpectrum.json')

    WavesType: Literal['WaveSpectrum'] = Field(alias="WavesType", default='WaveSpectrum', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, WaveSpectrum):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define WaveSpectrum models')
        return WaveSpectrum.parse_obj(data)
