...
class CampbellIdling(CampbellDiagramModeOfOperation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('SteadyCalculation/CampbellDiagramModeOfOperation/CampbellIdling.json')

    CampbellDiagramModeOfOperationType: Literal['Idling'] = Field(alias="CampbellDiagramModeOfOperationType", default='Idling', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, CampbellIdling):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define CampbellIdling models')
        return CampbellIdling.parse_obj(data)
