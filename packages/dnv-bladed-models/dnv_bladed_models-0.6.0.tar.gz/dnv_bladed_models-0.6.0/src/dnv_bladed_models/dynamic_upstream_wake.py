...
class DynamicUpstreamWake(BladedModel):
    ...
    def _factory(cls, data):
        if isinstance(data, DynamicUpstreamWake):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define DynamicUpstreamWake models')
        return DynamicUpstreamWake.parse_obj(data)
