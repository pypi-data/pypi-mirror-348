...
class FirstOrderResponse(ResponseToDemand):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/PitchSystem/PitchController/PitchSystemDemand/ResponseToDemand/FirstOrderResponse.json')

    ResponseToDemandType: Literal['FirstOrderResponse'] = Field(alias="ResponseToDemandType", default='FirstOrderResponse', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, FirstOrderResponse):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define FirstOrderResponse models')
        return FirstOrderResponse.parse_obj(data)
