...
class PitchFaultConstantTorque(PitchFault):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Event/PitchFaultConstantTorque.json')

    EventType: Literal['PitchFaultConstantTorque'] = Field(alias="EventType", default='PitchFaultConstantTorque', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, PitchFaultConstantTorque):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PitchFaultConstantTorque models')
        return PitchFaultConstantTorque.parse_obj(data)
