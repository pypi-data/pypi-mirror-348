...
class PresetCurrentDirectionTransient(CurrentDirectionVariation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/SeaState/Current/CurrentDirectionVariation/PresetCurrentDirectionTransient.json')

    DirectionVariationType: Literal['PresetTransient'] = Field(alias="DirectionVariationType", default='PresetTransient', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, PresetCurrentDirectionTransient):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PresetCurrentDirectionTransient models')
        return PresetCurrentDirectionTransient.parse_obj(data)
