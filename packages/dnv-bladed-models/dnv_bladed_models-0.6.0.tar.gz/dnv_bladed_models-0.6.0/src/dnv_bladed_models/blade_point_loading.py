...
class BladePointLoading(AppliedLoad):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/AppliedLoad/BladePointLoading.json')

    AppliedLoadType: Literal['BladePointLoading'] = Field(alias="AppliedLoadType", default='BladePointLoading', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, BladePointLoading):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define BladePointLoading models')
        return BladePointLoading.parse_obj(data)
    ...
    def _is_unused(self) -> bool:
        ...
        """
        candidate_fields = [x[1] for x in self.__dict__.items() if x[0] != 'AppliedLoadType']
