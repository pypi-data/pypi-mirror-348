...
class RotorImbalances(BladedModel):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Hub/RotorImbalances/RotorImbalances.json')
    ...
    def _factory(cls, data):
        if isinstance(data, RotorImbalances):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define RotorImbalances models')
        return RotorImbalances.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.ErrorsInSetAngle is not None and len(self.ErrorsInSetAngle) == 0: #type: ignore
            unused_containers.add('ErrorsInSetAngle')
        if self.ErrorsInAzimuthAngle is not None and len(self.ErrorsInAzimuthAngle) == 0: #type: ignore
            unused_containers.add('ErrorsInAzimuthAngle')
