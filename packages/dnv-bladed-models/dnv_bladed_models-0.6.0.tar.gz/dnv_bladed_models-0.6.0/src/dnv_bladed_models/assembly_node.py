...
class AssemblyNode(BladedModel):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Turbine/Assembly/AssemblyNode/AssemblyNode.json')
    ...
    def _factory(cls, data):
        if isinstance(data, AssemblyNode):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define AssemblyNode models')
        return AssemblyNode.parse_obj(data)
