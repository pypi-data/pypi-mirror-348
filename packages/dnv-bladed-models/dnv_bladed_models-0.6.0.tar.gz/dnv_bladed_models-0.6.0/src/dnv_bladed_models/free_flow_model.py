...
class FreeFlowModel(DynamicWake):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Settings/AerodynamicSettings/AerodynamicModel/MomentumTheoryCorrections/DynamicWake/FreeFlowModel.json')

    DynamicWakeType: Literal['FreeFlowModel'] = Field(alias="DynamicWakeType", default='FreeFlowModel', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, FreeFlowModel):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define FreeFlowModel models')
        return FreeFlowModel.parse_obj(data)
