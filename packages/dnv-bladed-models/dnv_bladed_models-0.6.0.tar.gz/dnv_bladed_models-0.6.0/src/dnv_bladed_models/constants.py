...
class Constants(BladedModel):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Constants/Constants.json')
    ...
    def _factory(cls, data):
        if isinstance(data, Constants):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define Constants models')
        return Constants.parse_obj(data)
