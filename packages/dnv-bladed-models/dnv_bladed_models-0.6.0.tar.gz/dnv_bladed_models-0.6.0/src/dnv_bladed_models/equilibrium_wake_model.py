...
class EquilibriumWakeModel(DynamicWake):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Settings/AerodynamicSettings/AerodynamicModel/MomentumTheoryCorrections/DynamicWake/EquilibriumWakeModel.json')

    DynamicWakeType: Literal['EquilibriumWakeModel'] = Field(alias="DynamicWakeType", default='EquilibriumWakeModel', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, EquilibriumWakeModel):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define EquilibriumWakeModel models')
        return EquilibriumWakeModel.parse_obj(data)
    ...
    def _is_unused(self) -> bool:
        ...
        """
        candidate_fields = [x[1] for x in self.__dict__.items() if x[0] != 'DynamicWakeType']
