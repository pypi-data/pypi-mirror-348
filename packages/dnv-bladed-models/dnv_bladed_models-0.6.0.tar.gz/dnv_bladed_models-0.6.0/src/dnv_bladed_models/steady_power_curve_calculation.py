...
class SteadyPowerCurveCalculation(SteadyCalculation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('SteadyCalculation/SteadyPowerCurveCalculation.json')

    SteadyCalculationType: Literal['SteadyPowerCurve'] = Field(alias="SteadyCalculationType", default='SteadyPowerCurve', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, SteadyPowerCurveCalculation):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define SteadyPowerCurveCalculation models')
        return SteadyPowerCurveCalculation.parse_obj(data)
