...
class DrivetrainOutputGroupLibrary(BladedModel):
    r"""
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/DrivetrainAndNacelle/DrivetrainOutputGroupLibrary/DrivetrainOutputGroupLibrary.json')
    ...
    def items(self) -> list[tuple[str, DrivetrainOutputGroup]]:
        """
    ...
    def values(self) -> list[DrivetrainOutputGroup]:
        """
    ...
    def __getitem__(self, key: Union[str, int]) -> DrivetrainOutputGroup:
    ...
    def __setitem__(self, key: str, value: DrivetrainOutputGroup):
        if not isinstance(key, str):
            raise KeyError(f"Custom entries can only be added with string keys")
        if key in self.__fields__:
            raise KeyError(f"Item accessors can only be used for custom entries. '{key}' is a pre-defined model property.")
        if not isinstance(value, DrivetrainOutputGroup):
            raise TypeError(f"Entries must be of type 'DrivetrainOutputGroup'; received '{type(value).__name__}'")
    ...
    def __setattr__(self, name: str, value: DrivetrainOutputGroup):
        if not name in self.__fields__ and not isinstance(value, DrivetrainOutputGroup):
            raise TypeError(f"Entries must be of type 'DrivetrainOutputGroup'; received '{type(value).__name__}'")
    ...
    class __CustomEntries__(BaseModel):
        entries: Dict[str, DrivetrainOutputGroup]
    ...
    def _factory(cls, data):
        if isinstance(data, DrivetrainOutputGroupLibrary):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define DrivetrainOutputGroupLibrary models')
        return DrivetrainOutputGroupLibrary.parse_obj(data)


    @classmethod
    def parse_obj(cls: Type['Model'], obj: Any) -> 'Model':
        return custom_entries_parser(cls, obj, dict, DrivetrainOutputGroupLibrary.__CustomEntries__)
    ...
    def from_file(cls: Type['Model'], path: Union[str, Path]) -> 'Model':
        ...
        Examples
        --------
        >>> model = DrivetrainOutputGroupLibrary.from_file('/path/to/file')
        """
    ...
    def from_json(cls: Type['Model'], b: StrBytes) -> 'Model':
        ...
        Examples
        --------
        >>> model = DrivetrainOutputGroupLibrary.from_json('{ ... }')
        """
...
DrivetrainOutputGroupLibrary.update_forward_refs()
DrivetrainOutputGroupLibrary.__CustomEntries__.update_forward_refs()
