...
class Assembly(BladedModel):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Turbine/Assembly/Assembly.json')
    ...
    def _factory(cls, data):
        ...
        if not isinstance(data, dict):
            raise TypeError('An object is required to define Assembly models')
        return Assembly.parse_obj(data)
    ...
    def from_file(cls: Type['Model'], path: Union[str, Path]) -> 'Model':
        ...
        Examples
        --------
        >>> model = Assembly.from_file('/path/to/file')
        """
