...
class RotorOverspeed(BladedModel):
    r"""
    The definition of the safety system behaviour when the rotor is rotating too fast.
    ...
    """
    _relative_schema_path: str = PrivateAttr('Turbine/BladedControl/SafetySystem/RotorOverspeed/RotorOverspeed.json')
    ...
    def _factory(cls, data):
        if isinstance(data, RotorOverspeed):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define RotorOverspeed models')
        return RotorOverspeed.parse_obj(data)
