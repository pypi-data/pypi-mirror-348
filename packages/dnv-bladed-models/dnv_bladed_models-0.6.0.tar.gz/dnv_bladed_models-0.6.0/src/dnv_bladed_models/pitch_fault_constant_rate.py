...
class PitchFaultConstantRate(PitchFault):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Event/PitchFaultConstantRate.json')

    EventType: Literal['PitchFaultConstantRate'] = Field(alias="EventType", default='PitchFaultConstantRate', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, PitchFaultConstantRate):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PitchFaultConstantRate models')
        return PitchFaultConstantRate.parse_obj(data)
