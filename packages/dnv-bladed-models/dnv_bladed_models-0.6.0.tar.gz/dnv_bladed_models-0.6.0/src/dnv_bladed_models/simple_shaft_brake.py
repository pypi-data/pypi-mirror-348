...
class SimpleShaftBrake(Brake):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/DrivetrainAndNacelle/Brake/SimpleShaftBrake.json')

    BrakeType: Literal['SimpleShaftBrake'] = Field(alias="BrakeType", default='SimpleShaftBrake', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, SimpleShaftBrake):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define SimpleShaftBrake models')
        return SimpleShaftBrake.parse_obj(data)
