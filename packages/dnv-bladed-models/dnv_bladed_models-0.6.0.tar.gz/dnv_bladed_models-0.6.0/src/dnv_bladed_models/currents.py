...
class Currents(TimeDomainCurrent):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/SeaState/Current/Currents.json')

    CurrentType: Literal['Currents'] = Field(alias="CurrentType", default='Currents', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, Currents):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define Currents models')
        return Currents.parse_obj(data)
