...
class PitchRateVariesWithPosition(PitchControllerRateSafetySystem):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/PitchSystem/PitchController/PitchSafetySystem/PitchRateVariesWithPosition.json')

    PitchSafetySystemType: Literal['RateVariesWithPosition'] = Field(alias="PitchSafetySystemType", default='RateVariesWithPosition', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, PitchRateVariesWithPosition):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PitchRateVariesWithPosition models')
        return PitchRateVariesWithPosition.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.PositionVsRate is not None and len(self.PositionVsRate) == 0: #type: ignore
            unused_containers.add('PositionVsRate')
