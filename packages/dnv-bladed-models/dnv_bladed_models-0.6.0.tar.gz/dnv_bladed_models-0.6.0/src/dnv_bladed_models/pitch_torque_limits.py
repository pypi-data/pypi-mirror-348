...
class PitchTorqueLimits(BladedModel):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/PitchSystem/PitchActuator/PitchTorqueLimits/PitchTorqueLimits.json')
    ...
    def _factory(cls, data):
        if isinstance(data, PitchTorqueLimits):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PitchTorqueLimits models')
        return PitchTorqueLimits.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.PositionVsLimits is not None and len(self.PositionVsLimits) == 0: #type: ignore
            unused_containers.add('PositionVsLimits')
        if self.RateVsLimits is not None and len(self.RateVsLimits) == 0: #type: ignore
            unused_containers.add('RateVsLimits')
