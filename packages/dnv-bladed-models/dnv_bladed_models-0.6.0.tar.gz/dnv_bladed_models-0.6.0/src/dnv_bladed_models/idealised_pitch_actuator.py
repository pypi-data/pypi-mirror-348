...
class IdealisedPitchActuator(PitchActuator):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/PitchSystem/PitchActuator/IdealisedPitchActuator.json')

    ActuatorDriveType: Literal['IdealisedActuator'] = Field(alias="ActuatorDriveType", default='IdealisedActuator', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, IdealisedPitchActuator):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define IdealisedPitchActuator models')
        return IdealisedPitchActuator.parse_obj(data)
