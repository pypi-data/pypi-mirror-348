...
class FrozenWakeModel(DynamicWake):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Settings/AerodynamicSettings/AerodynamicModel/MomentumTheoryCorrections/DynamicWake/FrozenWakeModel.json')

    DynamicWakeType: Literal['FrozenWakeModel'] = Field(alias="DynamicWakeType", default='FrozenWakeModel', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, FrozenWakeModel):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define FrozenWakeModel models')
        return FrozenWakeModel.parse_obj(data)
