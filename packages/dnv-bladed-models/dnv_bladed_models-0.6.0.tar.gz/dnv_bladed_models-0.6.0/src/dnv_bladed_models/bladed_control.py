...
class BladedControl(BladedModel):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Turbine/BladedControl/BladedControl.json')
    ...
    def _factory(cls, data):
        if isinstance(data, BladedControl):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define BladedControl models')
        return BladedControl.parse_obj(data)
    ...
    def _find_unused_containers(self) -> Set[str]:
        ...
        if self.Controllers is not None and len(self.Controllers) == 0: #type: ignore
            unused_containers.add('Controllers')
