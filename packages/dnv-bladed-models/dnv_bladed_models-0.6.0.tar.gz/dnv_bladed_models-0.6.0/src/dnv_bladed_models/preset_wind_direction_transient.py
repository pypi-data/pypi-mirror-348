...
class PresetWindDirectionTransient(WindDirectionVariation):
    r"""
    A preset transient in the wind direction.
    
    Attributes
    ----------
    DirectionVariationType : Literal['PresetTransient'], default='PresetTransient'
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/Wind/WindDirectionVariation/PresetWindDirectionTransient.json')

    DirectionVariationType: Literal['PresetTransient'] = Field(alias="DirectionVariationType", default='PresetTransient', allow_mutation=False, const=True)
    DirectionAtStart: float = Field(alias="DirectionAtStart", default=None)
    ...
    VariationShape: PresetWindDirectionTransient_VariationShapeEnum = Field(alias="VariationShape", default=None)
    ...
    def _factory(cls, data):
        if isinstance(data, PresetWindDirectionTransient):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PresetWindDirectionTransient models')
        return PresetWindDirectionTransient.parse_obj(data)
    ...
    def _is_unused(self) -> bool:
        ...
        """
        candidate_fields = [x[1] for x in self.__dict__.items() if x[0] != 'DirectionVariationType']
