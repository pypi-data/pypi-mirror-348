...
class WindDirectionTimeHistory(WindDirectionVariation):
    ...
    DirectionVariationType: Literal['TimeHistory'] = Field(alias="DirectionVariationType", default='TimeHistory', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, WindDirectionTimeHistory):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define WindDirectionTimeHistory models')
        return WindDirectionTimeHistory.parse_obj(data)
