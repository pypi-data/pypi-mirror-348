...
class PresetWindMeanSpeedTransient(WindMeanSpeedVariation):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/Environment/Wind/WindMeanSpeedVariation/PresetWindMeanSpeedTransient.json')

    MeanSpeedVariationType: Literal['PresetTransient'] = Field(alias="MeanSpeedVariationType", default='PresetTransient', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, PresetWindMeanSpeedTransient):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define PresetWindMeanSpeedTransient models')
        return PresetWindMeanSpeedTransient.parse_obj(data)
