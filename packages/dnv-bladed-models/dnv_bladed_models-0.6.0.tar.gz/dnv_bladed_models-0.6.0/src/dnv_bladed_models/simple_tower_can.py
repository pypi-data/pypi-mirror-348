...
class SimpleTowerCan(TowerCan):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Tower/TowerCan/SimpleTowerCan.json')

    TowerCanType: Literal['SimpleTowerCan'] = Field(alias="TowerCanType", default='SimpleTowerCan', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, SimpleTowerCan):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define SimpleTowerCan models')
        return SimpleTowerCan.parse_obj(data)
