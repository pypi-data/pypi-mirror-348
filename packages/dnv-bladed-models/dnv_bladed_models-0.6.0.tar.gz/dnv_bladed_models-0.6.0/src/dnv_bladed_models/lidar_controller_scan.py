...
class LidarControllerScan(LidarScanningPattern):
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Lidar/LidarScanningPattern/LidarControllerScan.json')

    ScanningPatternType: Literal['ControllerScan'] = Field(alias="ScanningPatternType", default='ControllerScan', allow_mutation=False, const=True) # Not supported yet
    ...
    def _factory(cls, data):
        if isinstance(data, LidarControllerScan):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define LidarControllerScan models')
        return LidarControllerScan.parse_obj(data)
    ...
    def _is_unused(self) -> bool:
        ...
        """
        candidate_fields = [x[1] for x in self.__dict__.items() if x[0] != 'ScanningPatternType']
