...
class RotorParkedControlState(InitialControlState):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/InitialCondition/RotorParkedControlState.json')

    InitialConditionType: Literal['RotorParked'] = Field(alias="InitialConditionType", default='RotorParked', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, RotorParkedControlState):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define RotorParkedControlState models')
        return RotorParkedControlState.parse_obj(data)
