...
class InitialAzimuthPosition(InitialPosition):
    ...
    """
    _relative_schema_path: str = PrivateAttr('TimeDomainSimulation/InitialCondition/InitialAzimuthPosition.json')

    InitialConditionType: Literal['InitialAzimuthPosition'] = Field(alias="InitialConditionType", default='InitialAzimuthPosition', allow_mutation=False, const=True)
    ...
    def _factory(cls, data):
        if isinstance(data, InitialAzimuthPosition):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define InitialAzimuthPosition models')
        return InitialAzimuthPosition.parse_obj(data)
