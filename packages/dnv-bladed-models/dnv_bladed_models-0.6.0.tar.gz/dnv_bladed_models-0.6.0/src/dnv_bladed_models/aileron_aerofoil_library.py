...
class AileronAerofoilLibrary(BladedModel):
    r"""
    ...
    """
    _relative_schema_path: str = PrivateAttr('Components/Blade/AileronAerofoilLibrary/AileronAerofoilLibrary.json')
    ...
    def items(self) -> list[tuple[str, Aileron]]:
        """
    ...
    def __getitem__(self, key: Union[str, int]) -> Aileron:
    ...
    def __setitem__(self, key: str, value: Aileron):
        if not isinstance(key, str):
            raise KeyError(f"Custom entries can only be added with string keys")
        if key in self.__fields__:
            raise KeyError(f"Item accessors can only be used for custom entries. '{key}' is a pre-defined model property.")
        if not isinstance(value, Aileron):
            raise TypeError(f"Entries must be of type 'Aileron'; received '{type(value).__name__}'")
    ...
    def __setattr__(self, name: str, value: Aileron):
        if not name in self.__fields__ and not isinstance(value, Aileron):
            raise TypeError(f"Entries must be of type 'Aileron'; received '{type(value).__name__}'")
    ...
    def _factory(cls, data):
        if isinstance(data, AileronAerofoilLibrary):
            return data
        if not isinstance(data, dict):
            raise TypeError('An object is required to define AileronAerofoilLibrary models')
        return AileronAerofoilLibrary.parse_obj(data)


    @classmethod
    def parse_obj(cls: Type['Model'], obj: Any) -> 'Model':
        return custom_entries_parser(cls, obj, dict, AileronAerofoilLibrary.__CustomEntries__)
    ...
    def from_file(cls: Type['Model'], path: Union[str, Path]) -> 'Model':
        ...
        Examples
        --------
        >>> model = AileronAerofoilLibrary.from_file('/path/to/file')
        """
    ...
    def from_json(cls: Type['Model'], b: StrBytes) -> 'Model':
        ...
        Examples
        --------
        >>> model = AileronAerofoilLibrary.from_json('{ ... }')
        """
...
AileronAerofoilLibrary.update_forward_refs()
AileronAerofoilLibrary.__CustomEntries__.update_forward_refs()
