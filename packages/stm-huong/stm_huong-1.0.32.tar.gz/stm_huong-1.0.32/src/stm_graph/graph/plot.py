...
def plot_node_time_series(
    ...
    if plot_type.lower() == "2d":
        ...
        for node_id, values in node_data.items():
            ax.plot(time_array, values, label=f"Node {node_id}", linewidth=2)
...
def plot_temporal_heatmap(
    ...
    Args:
        temporal_dataset: PyTorch Geometric Temporal dataset
        node_ids: List of node IDs to plot (if None, first max_nodes used)
        feature_idx: Index of feature to plot (default: 0, event count)
        max_nodes: Maximum number of nodes to include (for readability)
        max_timesteps: Maximum number of timesteps to include (for readability)
        title: Plot title
    ...
    if node_ids is None:
        n_avail = temporal_dataset.features[0].shape[0]
        node_ids = list(range(min(max_nodes, n_avail)))
    elif len(node_ids) > max_nodes:
        node_ids = node_ids[:max_nodes]
        print(f"Warning: Limited to {max_nodes} nodes for readability")

    n_timesteps = min(len(temporal_dataset.features), max_timesteps)
    ...
    ax.set_title(
        title or f"Temporal Heatmap for Selected Nodes (Feature {feature_idx})"
    )
