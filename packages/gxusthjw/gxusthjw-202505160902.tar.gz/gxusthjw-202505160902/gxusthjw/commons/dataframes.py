...
# 导包 ==============================================================
from typing import (
    Optional, Mapping, Set, FrozenSet, Any, Union
)
...
def create_df_from_item(data: Any, name: Optional[str] = None) -> pd.DataFrame:
    ...
    基于指定的数据与数据项名创建pd.DataFrame。
        ...
        说明：
            ...
            1. 如果参数name不为类型str或None时，抛出异常，如果参数name为None，则用“item”代替。
            ...
            4. 如果参数data为标量数据或字符串，则将其封装为列表（[data]），标量类型包括：int、float、str、np.generic等。
    ...
    if not isinstance(name, Union[str, None]):
        raise ValueError("Parameter name must be a str.")
...
def merge_dfs(df1: pd.DataFrame, df2: pd.DataFrame, *dfs):
    """
    合并多个DataFrame对象。

    该函数首先合并前两个DataFrame对象，然后将剩余的DataFrame对象依次与合并结果合并。

    如果出现列明重复，则后面相同列名的数据覆盖前面。

    :param df1: 第一个DataFrame。
    :param df2: 第二个DataFrame。
    :param dfs: 其余DataFrame。
    :return: 合并后的DataFrame。
    :rtype: pandas.DataFrame
    """
    # 首先合并前两个DataFrame
