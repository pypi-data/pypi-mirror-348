...
def read_xrdml(xrdml_file: Union[str, FileInfo],
    ...
    # 解析 XML
    try:
        dom = xml.dom.minidom.parse(xrdml_file_info.full_path)
        ...
        if out_file is None:
            if separator.__eq__("\t"):
                out_file_info = info_of(
                    os.path.join(xrdml_file_info.directory_path, xrdml_file_info.base_name + ".raw"))
            else:
                out_file_info = info_of(
                    os.path.join(xrdml_file_info.directory_path, xrdml_file_info.base_name + ".txt"))
        else:
...
class XrdmlFile(XrdFile):
    ...
    def read(self, reset_data: bool = False) -> Tuple[
        ...
        :rtype:Tuple[npt.NDArray[np.float64], npt.NDArray[np.float64]
        """
        dom = xml.dom.minidom.parse(self.file_full_path)
        root = dom.documentElement
