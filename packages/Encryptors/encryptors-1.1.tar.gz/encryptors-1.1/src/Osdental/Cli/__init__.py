...
def start_app(app: str):
    ...
    directories = [
        ...
        os.path.join(DOMAIN_PATH, 'Ports', 'Input'),
        os.path.join(DOMAIN_PATH, 'Ports', 'Output'),
    ...
    files = {
        ...
        os.path.join(DOMAIN_PATH, 'Ports', 'Input', f'{app}UseCaseInput.py'): f'class {app}UseCaseInput:\n    pass\n',
        os.path.join(DOMAIN_PATH, 'Ports', 'Output', f'{app}RepositoryOutput.py'): f'class {app}RepositoryOutput:\n    pass\n',
...
def start(port:int):
    ...
    try:
        subprocess.run(['uvicorn', 'app:app', 'port', port, '--reload'], check=True)
...
def serve(port:int):
    ...
    try:
        ...
        subprocess.run(['uvicorn', 'app:app', '--host', '0.0.0.0', '--port', port, '--reload'], check=True)
    except subprocess.CalledProcessError as e:
        logger.error(f'{Message.SERVER_NETWORK_ACCESS_ERROR_MSG}: {e}')
