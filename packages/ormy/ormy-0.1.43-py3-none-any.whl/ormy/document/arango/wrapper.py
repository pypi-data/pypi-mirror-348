...
class ArangoBaseGraph(AbstractABC):
    ...
    @staticmethod
    def _build_and_or_expression(exs: list[str | list[str]]) -> str:
        """
        Build an AND/OR expression

        Args:
            exs (list[str | list[str]]): List of expressions

        Returns:
            expr (str): AND/OR expression
    ...
    def _build_structured_query(
        ...
        prunes: Optional[list[str | list[str]]] = None,
        ...
        Args:
            ...
            query (Optional[str], optional): AQL query. Defaults to None
            direction (Literal["in", "out", "any"], optional): Direction of the query. Defaults to "any"
            filters (Optional[list[str | list[str]]], optional): Filter expressions to apply to the query. Defaults to None
            prunes (Optional[list[str | list[str]]], optional): Prune expressions to apply to the query. Defaults to None
            ...
            sort (Optional[list[str]], optional): Sort the results. Defaults to None
            return_clause (Optional[str], optional): Return clause. Defaults to None
            ...
            options (Optional[dict[str, Any]], optional): Options to pass to the query. Defaults to None

        Returns:
            q (str): Structured query
        """
        ...
        if (limit is None) ^ (offset is None):
            raise InternalError("Limit and offset must be provided together")

        if filters is not None:
            _filter = "FILTER " + cls._build_and_or_expression(filters)

        else:
            _filter = ""

        if prunes is not None:
            _prune = "PRUNE " + cls._build_and_or_expression(prunes)

        else:
            _prune = ""

        if return_clause is not None:
            _return = f"RETURN {return_clause}"

        else:
            _return = ""

        if options is not None:
            _opts = "OPTIONS " + json.dumps(options)

        else:
            _opts = ""

        if sort is not None:
            _sort = "SORT " + ", ".join(sort)
        ...
        _limit = f"LIMIT {offset}, {limit}" if limit is not None else ""
    ...
    def count(
        ...
        """
        Count documents based on a structured query
        ...
        Returns:
            cnt (int): Number of documents
        """

        original_q = cls._build_structured_query(
            query=query,
            start_vertex=start_vertex,
            direction=direction,
            min_depth=min_depth,
