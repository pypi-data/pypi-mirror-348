...
class Video_material:
    ...
    def __init__(self, path: str, material_name: Optional[str] = None, crop_settings: Crop_settings = Crop_settings(), use_placeholder: bool = False):
        """从指定位置加载视频（或图片）素材
        ...
        Args:
            ...
            use_placeholder (`bool`, optional): 是否使用占位符路径, 默认为False.
        ...
        if not os.path.exists(abs_path):
            raise FileNotFoundError(f"找不到 {abs_path}")

        self.material_name = material_name if material_name else os.path.basename(abs_path)
        ...
        if use_placeholder:
            ...
            self.path = f"{PLACEHOLDER}\\{rel_path}"
        else:
            self.path = abs_path
        ...
        info: pymediainfo.MediaInfo = \
            pymediainfo.MediaInfo.parse(abs_path, mediainfo_options={"File_TestContinuousFileNames": "0"})  # type: ignore
        # 有视频轨道的视为视频素材
        ...
        # gif文件使用imageio库获取长度
            ...
            import imageio
            gif = imageio.get_reader(abs_path)
            ...
            raise ValueError(f"输入的素材文件 {abs_path} 没有视频轨道或图片轨道")
...
class Audio_material:
    ...
    def __init__(self, path: str, material_name: Optional[str] = None, use_placeholder: bool = False):
        ...
        if not pymediainfo.MediaInfo.can_parse():
            raise ValueError("不支持的音频素材类型 %s" % os.path.splitext(abs_path)[1])
        info: pymediainfo.MediaInfo = pymediainfo.MediaInfo.parse(abs_path)  # type: ignore
        if len(info.video_tracks):
            raise ValueError("音频素材不应包含视频轨道")
        if not len(info.audio_tracks):
            raise ValueError(f"给定的素材文件 {abs_path} 没有音频轨道")
