...
class MoviepyArgument:
    ...
    """
    def __init__(self, start: Union[int, tuple, float], end: Union[int, tuple, float], t_function: type, rate_function: RateFunctionArgument = RateFunctionArgument.default()):
...
class WithPositionArgument:
    ...
    def __init__(self, argument: Union[tuple[int, int], 'Map'], extra_argument: Union[any, None] = None):
        ...
        if (
            (not PythonValidator.is_tuple(argument) or len(argument) != 2) and
    ...
    def get_values(self, n: int):
        ...
        if PythonValidator.is_tuple(self.argument): return [self.argument] * n
        if PythonValidator.is_instance(self.argument, 'Map'): return self.argument.get_n_values(n)

class RotatedArgument:
    """
    A wrapper to hold a 'rotated' value generator.
    """
    def __init__(self, argument: Union[int, 'SubClipSetting', 'Graphic']):
        from yta_general_utils.programming.parameter_validator import PythonValidator
    ...
    def get_values(self, n: int):
        ...
        if PythonValidator.is_number(self.argument): return [int(self.argument)] * n
        if PythonValidator.is_instance(self.argument, 'SubClipSetting'): return [int(value) for value in self.argument.get_values(n)]
        if PythonValidator.is_instance(self.argument, 'Graphic'): return [int(value[1]) for value in self.argument.get_n_values(n)]
...
class ResizedArgument:
    ...
    def __init__(self, argument: Union[tuple[float, float], 'SubClipSetting', 'Graphic']):
        from yta_general_utils.programming.parameter_validator import PythonValidator
    ...
    def get_values(self, n: int):
        ...
        if PythonValidator.is_tuple(self.argument): return [self.argument] * n
        if PythonValidator.is_instance(self.argument, 'SubClipSetting'):
            return [[value, value] for value in self.argument.get_values(n)]
        if PythonValidator.is_instance(self.argument, 'Graphic'):
            return [[value[1], value[1]] for value in self.argument.get_n_values(n)]
...
class MoviepyWith:
    ...
    def apply_over_video(video: Clip, background_video: Clip, with_position: Union[MoviepyArgument, None] = None, resized: Union[MoviepyArgument, None] = None, rotated: Union[MoviepyArgument, None] = None):
        ...
        if resized:
            resizes = [resized.t_function(t, video_handler.duration, resized.start, resized.end, resized.rate_function) for t in video_handler.frames_time_moments]
        else:
        ...
        rotations = None
        if rotated:
            rotations = [rotated.t_function(t, video_handler.duration, rotated.start, rotated.end, rotated.rate_function) for t in video_handler.frames_time_moments]
    ...
    def get_arrays(
        ...
        resized = resized.get_values(number_of_frames) if resized is not None else None
        ...
        with_position = with_position.get_values(number_of_frames) if with_position is not None else None
        rotated = rotated.get_values(number_of_frames) if rotated is not None else None
