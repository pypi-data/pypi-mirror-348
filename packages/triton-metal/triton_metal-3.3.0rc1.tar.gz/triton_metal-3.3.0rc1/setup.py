...
def get_llvm_package_info():
    # Skip LLVM download for source distribution
    print("Using dummy LLVM package for source distribution")
    llvm_dir = os.getenv("LLVM_DIR")
    if llvm_dir:
        return llvm_dir
    
    # Return a dummy package that doesn't require downloading
    return Package("llvm", "dummy-llvm", "", "LLVM_INCLUDE_DIRS", "LLVM_LIBRARY_DIR", "LLVM_SYSPATH")


def open_url(url):
    print(f"WARNING: Skipping download of {url}")
    print("This is a source distribution that doesn't require downloading dependencies.")
    print("If you need LLVM, please set the LLVM_DIR environment variable.")
...
TRITON_VERSION = "3.3.0rc1"  # Fixed version without git hash for stable release
