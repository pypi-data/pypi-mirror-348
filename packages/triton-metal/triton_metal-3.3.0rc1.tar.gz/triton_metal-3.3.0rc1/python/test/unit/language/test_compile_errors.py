...
from triton_metal.compiler.errors import CompilationError, CompileTimeAssertionFailure
import traceback
...
def test_err_undefined_variable():
    ...
    with pytest.raises(CompilationError) as e:
        triton_metal.compile(triton_metal.compiler.ASTSource(fn=kernel, signature={}, constexprs={}))
...
def test_not_const_annotate_no_err():
    ...
    triton_metal.compile(triton_metal.compiler.ASTSource(fn=kernel, signature={'N': 'i32'}, constexprs={}))
...
def test_returns_branched_on_constexpr():
    ...
    triton_metal.compile(triton_metal.compiler.ASTSource(fn=kernel1, signature={"N": "constexpr"}, constexprs={"N": 0}))
    ...
    triton_metal.compile(triton_metal.compiler.ASTSource(fn=kernel2, signature={"N": "constexpr"}, constexprs={"N": 1}))
...
def test_global_access_in_fn_default_arg():
    ...
    # No error.
    triton_metal.compile(triton_metal.compiler.ASTSource(fn=kernel, signature={'a': "i32"}, constexprs={}))
...
def test_defaults_assign_no_err():
    ...
    triton_metal.compile(triton_metal.compiler.ASTSource(fn=kernel, signature={'a': 'i32', 'B': 'constexpr'}, constexprs={'B': ""}))
...
def test_fp8_support(fresh_triton_cache, dtype):
    ...
    with ctx as e:
        triton_metal.compile(
            triton_metal.compiler.ASTSource(fn=dtype_kernel, signature={"dtype": "constexpr"}, constexprs={"dtype": dtype}))
...
def test_min_dot_size(dtype):
    ...
    if error_msg is None:
        triton_metal.compile(
            triton_metal.compiler.ASTSource(fn=dot_kernel, signature={"dtype": "constexpr"}, constexprs={"dtype": dtype}))
    else:
        with pytest.raises(CompilationError) as e:
            triton_metal.compile(
                triton_metal.compiler.ASTSource(fn=dot_kernel, signature={"dtype": "constexpr"}, constexprs={"dtype": dtype}))
...
def test_max_num_imprecise_acc_limit():
    ...
    with pytest.raises(CompilationError) as e:
        triton_metal.compile(triton_metal.compiler.ASTSource(fn=dot_kernel, signature={}, constexprs={}))
    try:
...
@triton_metal.must_use_result(extra_words)
@triton_metal.jit
def cube(x):
    return x * x * x
...
def test_unused_result():
    ...
    triton_metal.compile(triton_metal.compiler.ASTSource(fn=good_cube_kernel, signature={}, constexprs={}))

    with pytest.raises(CompilationError) as e:
        triton_metal.compile(triton_metal.compiler.ASTSource(fn=evil_cube_kernel, signature={}, constexprs={}))
