...
def test_triton_heuristic(device):
    ...
    do_bench = lambda kernel, quantiles: triton_metal.testing.do_bench(kernel, quantiles=quantiles, warmup=1, rep=1)

    @triton_metal.autotune(configs=[triton_metal.Config(kwargs={'BLOCK_SIZE': 32})], key=['N'], do_bench=do_bench)
    @triton_metal.heuristics({'EVEN_N': lambda nargs: nargs['N'] % 2 == 0})  # test kwargs
    @triton_metal.heuristics({'EVEN_src': lambda nargs: nargs['src'].data_ptr() % 2 == 0})  # test args
    @triton_metal.jit
