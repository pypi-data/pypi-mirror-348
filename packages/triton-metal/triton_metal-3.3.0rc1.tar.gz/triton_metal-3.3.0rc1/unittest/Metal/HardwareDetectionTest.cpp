...
public:
  ...
  void detectHardware() {
    ...
    const char* genStr = std::getenv("TRITON_METAL_GENERATION");
    if (genStr != nullptr) {
...
TEST_F(HardwareDetectionTest, OverrideGeneration) {
  ...
  setenv("TRITON_METAL_GENERATION", "M3", 1);
  ...
  unsetenv("TRITON_METAL_GENERATION");
#else
  GTEST_SKIP() << "Test only runs on Apple hardware";
#endif
}
...
TEST_F(HardwareDetectionTest, CapabilitiesForM1) {
  ...
  setenv("TRITON_METAL_GENERATION", "M1", 1);
...
TEST_F(HardwareDetectionTest, CapabilitiesForM2) {
  ...
  setenv("TRITON_METAL_GENERATION", "M2", 1);
