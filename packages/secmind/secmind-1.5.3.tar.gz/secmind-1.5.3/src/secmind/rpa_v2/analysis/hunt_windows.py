    ...
    try:
        ...
        for path in paths:
            ...
            if exist.get('std_out'):
                ...
                get_file_name = f'''
                Get-ChildItem -Path "{path}" -Recurse -Include *.txt, *.md, *.conf, *.json, *.cfg, *.ini, *.properties, *.config, *.xml, *.env, *.sql, *.yaml, *.yml | 
...
def hunt_window_EnbededAccount(link, result_queue):
    try:
        ...
        search_c_path = 'Get-ChildItem -Path "C:\Program Files" -Recurse | Select-Object -ExpandProperty FullName'
...
def do_analysis(data_map):
    ...
    try:
        ...
        #账号安全检查-弱口令扫描
        # if str(data_map['discoverEnable']) == "1":
        #     analyzeWeakPassword(user_hunter_List, l, data_map)
        #将用户名转小写，与账号发现统一
        for user in user_hunter_List:
...
def analyzeWeakPassword(user_hunter_List, link, data_map):
    #获取密码本
    job_id = data_map['taskId'].split('-')[0]
    try:
        # 判断是否用自定义密码本
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        for user in user_hunter_List:
            try:
                executor.submit(weakPasswordCrack, user, passWordList, link)
            except Exception as ex:
                common.logger.error(f"弱口令检测错误 : {repr(ex)}")
                raise ex

def weakPasswordCrack(user, passWordList, link):
    common.logger.info("开始对用户" + user.userName + "进行检测")
    for passwd in passWordList:
        try:
            winRM_wrapper = WinRMWrapper(link.host, user.userName, passwd)
            winRM_wrapper.connect()
            user.xxx = passwd
            common.logger.info("用户" + user.userName + "弱口令检测成功，密码为:" + passwd)
            return
        except Exception as e:
            pass
    common.logger.info("用户" + user.userName + "弱口令检测完成,未发现密码")

def codebook(id):
    import requests
    print("请求密码本")
    response = requests.post(f'http://pam-runner-core:33100/account/device/codebook?jobId={id}')
