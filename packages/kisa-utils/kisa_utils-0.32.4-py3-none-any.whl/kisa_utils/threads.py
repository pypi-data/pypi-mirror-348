...
class Group:
    ...
    def add(self, function:callable, *args, **kwargs) -> dict:
        '''
        ...
        for functionId in self._functions:
            ...
            if (functionName == functionDetails['functionName']) and functionDetails['args']==args and functionDetails['kargs']==kwargs:
                reply['log'] = f'function `{functionName}` already registered in the group with exact `args` and `kwargs` given'
                return reply
    ...
    def waitForAll(self) -> list:
        ...
        )['future'] for functionId in self._functions]
...
def runOnce(function:callable, *args, **kwargs) -> dict[str,str|bool]:
    '''
    ...
    reply = {'status':False, 'log':'', 'future':None}
    ...
    try:
        future = __executor.run(function, *args, **kwargs)
        reply['future'] = future
    except Exception as e:
...
def runAt(function:callable, timesOfDay:list[tuple], weekDays:list[int], *args, tzHoursFromUTC:float = +3.00, **kwargs) -> dict[str,str|bool]:
    '''
    ...
    if (function.__name__ in __DATA['routineLoops']) and __DATA['routineLoops'][function.__name__]['args']==args and __DATA['routineLoops'][function.__name__]['kargs']==kwargs:
        reply['log'] = f'function `{function.__name__}` already registered with `runAt`'
        return reply
    ...
    for index, (hour, minute) in enumerate(timesOfDay):
        if not(isinstance(hour,int) and 0<=hour<=23 and isinstance(minute,int) and 0<=minute<=59):
            reply['log'] = f'timesOfDay[{index}]: invalid time given `{(hour,minute)}`'
            return reply
