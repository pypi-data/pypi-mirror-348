...
from liteauto.parselite import aparse,parse
...
class LLMSearch:
    ...
    async def __call__(self, user_query: str) -> Generator[Union[StreamMessage, FinalReport], None, None]:
        """
        ...
        parameters = await loop.run_in_executor(
            self.llm_executor,
            self._get_optimal_parameters,
            user_query
        )

        # Add current time to the query
        import datetime
        now = datetime.datetime.now()
        formatted_time = now.strftime("%Y-%m-%d %H:%M:%S")
        user_query_with_time = f"{user_query} \nFor your information current time is: {formatted_time}"

        # Yield parameter information if desired
        if self.enable_think_tag:
            yield StreamMessage(type="parameters", content=f"<think>Search parameters: {parameters.dict()}</think>")
    ...
    def _get_optimal_parameters(self, user_query: str) -> SearchParameters:
        ...
        2. max_iterations: Maximum number of search iterations per research step (1-10)
           - For straightforward queries: 1 iteration is usually enough
           - For questions requiring verification: 2-3 iterations helps confirm information
           - For little higher topics: 4-5 iterations allows for thorough investigation
           - For other than and if you feel like answer can't be find easity go beyond 5 and upto 10
    ...
    def _perform_web_search_sync_backup(self, query: str, max_urls) -> List[SearchResult]:
    ...
    def _perform_web_search_sync(self, query: str, max_urls) -> List[SearchResult]:
        """Synchronous version of web search for thread pool execution with retry logic."""
    ...
    async def _execute_step(self, step: PlanStep, max_urls, max_iterations,num_gen_search_queries,delay_between_searches) -> Generator[StreamMessage, None, None]:
        ...
        for iteration in range(max_iterations):
            ...
            # Option 1: Sequential execution (safer but slower)
