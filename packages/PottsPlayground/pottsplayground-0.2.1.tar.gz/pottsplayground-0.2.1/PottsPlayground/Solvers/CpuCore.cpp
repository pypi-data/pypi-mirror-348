...
template <typename Annealable> void OptionsActionsCpu(
	...
	for (long iter = DA.MinIter; iter < DA.MaxIter; iter+=DA.nActions){
		...
		for (int act = 0; act<DA.nActions; act++)
			DA.ActArb(replicate_index, act, 0) = -log(uniform_distribution(RngGenerator));
...
template <typename Annealable> void BirdsEyeCpu(
	//try to make the most efficient possible BirdsEye.
...
template <typename Annealable> void SimpleUpdatesCpu(
	Annealable &&task,
	DispatchArgs &&DA,
	int replicate_index,
	volatile int *TaskDone){
...
template <typename Annealable> void CpuDispatch(void* void_task, DispatchArgs &DA){
	...
	while (thread_num < nReplicates){
		if (workers.size() < nWorkers){
			...
			else if (!strcmp(DA.algo, "BirdsEye"))
				workers.push_back(std::thread(BirdsEyeCpu<Annealable>, task, DA, thread_num, DA.GlobalHalt.hd));
			
			else
				workers.push_back(std::thread(SimpleUpdatesCpu<Annealable>, task, DA, thread_num, DA.GlobalHalt.hd));
