...
__h__ __d__ void PottsPrecomputeBase::TakeAction_toc(int action_num){
	...
	nRecentUpdates = 0;
	for (int uu = 0; uu < qSizes(j); uu++){
		recentUpdates[nRecentUpdates] = qCumulative(j)-qSizes(j)+uu;
		nRecentUpdates++;
	}
    ...
    for (int c = 1; c < kmap(j, 0, 0); c++){
		...
		if (qSizes(i)*2+nRecentUpdates > NumActions || nRecentUpdates < 0) {
		...
		for (int d = 1; d < sparse_kernels(k, old_Mi, 0, 0); d++){
			...
			recentUpdates[sparse_incriment_cnt] = NHPP+q;
		...
		if (dense_effect && nRecentUpdates > 0){
			for (int uu = 0; uu < qSizes(i); uu++){
				recentUpdates[nRecentUpdates] = qCumulative(i)-qSizes(i)+uu;
				nRecentUpdates++;
			}
		}
