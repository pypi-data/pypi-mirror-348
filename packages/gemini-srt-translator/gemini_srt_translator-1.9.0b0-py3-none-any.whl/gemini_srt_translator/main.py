        ...
        for line in translated_lines:
            if "content" not in line or "index" not in line:
                if line != last_translated_line or finished:
                    ...
                    raise Exception(f"Gemini has returned an incorrect translation at line {int(indexes[i]) + 1}.")
                else:
                    continue
            ...
            if line != last_translated_line and line["content"] == "":
                warning_with_progress("", chunk_size=i)
                raise Exception(f"Gemini has returned an empty translation for line {int(line['index']) + 1}.")
