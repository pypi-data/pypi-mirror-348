...
from django_post_api.fields import ModelField, ForeignKeyField, ManyToManyField, AnnotateField, BaseReturnField, ModelFuncField
from django_post_api.helpers import build_tree_data, merge_two_field_list
from django_post_api.params import IntParam, BaseParam, ListParam, StrParam
...
class APIGetPageView(BaseAPIGetView):
    ...
    def get_page_api(self):
        ...
        res = ModelToJson(model=self.model).get_data_list(
            self.get_queryset(), self.page_field_list, offset=pagination_params.get("offset"),
...
class APIGetDetailView(BaseAPIGetView):
    ...
    def get_detail_api(self):
        ...
        res = ModelToJson(model=self.model).get_detail(
            queryset=self.get_queryset(), field_list=self.page_field_list, detail_field_list=self.detail_field_list,
            query_params=self.detail_query_params)
        return success_return(res)
...
class APIGetTreeView(APIGetAllView):
    ...
    def get_tree_api(self):
        ...
        return success_return(data_list, count=res.get("count", None))
...
class APIPostView(BaseAPIView):
    ...
    def post_api(self):
        ...
        return success_return({"id": obj.id} if obj else None)
...
class APIPutView(BaseAPIView):
    ...
    def put_api(self):
        ...
        return success_return({"count": count})
...
class APITableView(APIGetPageView, APIGetAllView, APIPostView, APIPutView, APIDeleteView):
    pass
...
class ModelToJson:
    ...
    def get_data_list(self, queryset, field_list: List[BaseReturnField],
                      ...
                      order_by_params: Optional[list] = None) -> dict:
    ...
    def get_detail(self, queryset, field_list, detail_field_list=None, query_params=None, exclude_params=None):
        all_field_list = merge_two_field_list(field_list, detail_field_list)
        res = self.get_data_list(
            queryset, field_list=all_field_list, query_params=query_params,
