...
from temp_mails import Tmail_io as Mail
from send_email import send_email_sync

name, domain = "".join(choices(ascii_lowercase, k=6)), "gmail.com"#, choice(Mail.get_valid_domains())

mail = Mail(name=name, domain=domain)
print(mail.email)
assert mail.email != "", "Mail name empty"

assert f"{name}@{domain}" in mail.email, "Mail does not match the used mail"
