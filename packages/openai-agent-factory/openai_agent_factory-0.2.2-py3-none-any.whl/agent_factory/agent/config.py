...
class AgentConfig(BaseModel):
    ...
    instructions: str = Field(description="Instructions for the agent that define its behavior")
    mcp_servers: List[str] = Field(default_factory=list)
    model: Optional[str] = Field(default=None)
    model_settings: Optional[ModelSettings] = Field(
        default=None, description="Advanced model configuration settings"
    )
...
class FactoryConfigBase(BaseSettings):
    """Base configuration for factory classes."""
    ...
    def check_mcp_servers(self, configs: List, get_server_names: Callable) -> None:
        """Validate that all required MCP servers are defined."""
