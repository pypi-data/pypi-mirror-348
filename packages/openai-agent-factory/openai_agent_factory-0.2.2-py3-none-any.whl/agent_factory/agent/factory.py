...
class AgentFactory:
    """Factory for creating OpenAI agents based on configuration.

    This factory manages the creation of agent instances based on the configurations
    provided in AgentFactoryConfig. It handles the dependency resolution between
    agents and MCP servers, and manages the lifecycle of the MCPServerManager.

    All agents defined in the AgentFactoryConfig are created during initialization
    and can be accessed via get_agent() or get_all_agents() methods.

    It can be used both with async context manager pattern and explicit initialize/shutdown:
