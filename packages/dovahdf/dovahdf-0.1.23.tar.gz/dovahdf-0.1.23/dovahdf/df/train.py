...
def run_df_train(
    ...
    *,
    ...
    """
    Direct-call training entry for DeepFilterNet.

    This version
    1. guarantees ``base_dir/config.ini`` is always present and complete;
    2. allows overriding epochs / batch_size / lr / gpu from function args.
    """
    # -------------------------------------------------------- 0. globals
    global should_stop, debug, state, log_timings
    ...
    if os.path.isfile(config_file):                      # 已有文件 → 加载
        try:
            config.load(config_file)
            logger.info(f"Loaded existing config: {config_file}")
        except ValueError:                               # 之前 load 过别的文件
        ...
        config.parser = configparser.ConfigParser()      # 空 parser
