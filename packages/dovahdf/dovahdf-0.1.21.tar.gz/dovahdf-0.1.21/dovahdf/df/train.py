...
def run_df_train(
    ...
    *,
    ...
    """Train DeepFilterNet through a direct Python call.

    * 在 ``base_dir`` 写入 / 维护一份 **完整** 的 ``config.ini``；
    * 通过函数参数即可覆盖常用超参，不必手改配置文件。
    """
    # ------------------------------------------------------------------
    # 0. 让这些符号指向全局，供 setup_losses() 等函数使用
    global should_stop, debug, state, log_timings
    ...
    # ------------------------------------------------------------------
    # 4. 载入或生成 base_dir/config.ini
    config_file = os.path.join(base_dir, "config.ini")
    if os.path.isfile(config_file):
        try:
            config.load(config_file)
            logger.info(f"Loaded existing config: {config_file}")
        except ValueError:            # 已加载过别的文件
            config._loaded = False    # type: ignore[attr-defined]
            config.load(config_file)
            logger.info(f"Reloaded config: {config_file}")
    else:
        logger.info("No config.ini found for this run; will create one after overrides.")
    ...
    # scheduler 依赖 max_epochs — 再次确保它存在
    max_epochs_default = config("MAX_EPOCHS", 10, int, section="train")
    config.overwrite("train", "max_epochs", max_epochs_default)

    # ------------------------------------------------------------------
    # 6. **保存完整配置**（包含所有键，而非仅 overwrite 的部分）
    with open(config_file, "w", encoding="utf-8") as fp:
        for section in config.sections():
            ...
            for key, val in config.items(section):
                fp.write(f"{key} = {val}\n")
            fp.write("\n")
    logger.info(f"Written full config to {config_file}")
    ...
    for epoch_idx in range(start_epoch, max_epochs):
        # ---------- Train ----------
        train_loss = run_epoch(
            model, epoch_idx, dataloader, "train", opt, losses,
            summary_dir, lrs, wds
        )
        ...
        # ---------- Validation ----------
        val_loss = run_epoch(
            model, epoch_idx, dataloader, "valid", opt, losses, summary_dir
        )
        ...
        # 早停检查
        if not check_patience(
            ckpt_dir, patience, val_loss, val_rule, raise_=False
        ):
            break
