...
def run_df_train(
    ...
    *,
    ...
    from df.checkpoint import load_model, write_cp, check_patience
    from df.utils import check_manual_seed, get_sigusr1_handler
    from df.model import ModelParams
    from df.modules import get_device
    ...
    config.load(config_file)
    # 3.1 定义 summary_dir，用于 TensorBoard 或其它可视化输出
    summary_dir = os.path.join(base_dir, "summaries")
    os.makedirs(summary_dir, exist_ok=True)

    # 4. 覆盖超参数到 config（仅覆盖非 None 的值）
    if pretrained:
    ...
    debug = False
    log_timings = config("LOG_TIMINGS", False, bool, section="train", save=False)
    logger = init_logger  # 已在 init_logger 内部绑定
    logger(f"Running on device {get_device()}")
    ...
    for epoch_idx in range(start_epoch, max_epochs):
        train_loss = run_epoch(model, epoch_idx, dataloader, "train", opt, losses, summary_dir, lr_scheduler_values=lrs, wd_scheduler_values=wds)
        ...
        val_loss = run_epoch(model, epoch_idx, dataloader, "valid", opt, losses, summary_dir)
