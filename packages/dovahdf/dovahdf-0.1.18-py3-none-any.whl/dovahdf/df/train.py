...
def run_df_train(
    ...
    *,
    ...
    """Train DeepFilterNet via a direct function call.

    A complete config file is written to ``{base_dir}/config.ini`` on every run,
    containing all default parameters plus any overrides supplied via function
    arguments. Subsequent runs load and update the same file so you always have
    an auditable record of hyper‑parameters.
    """
    global should_stop, debug, state, log_timings
    ...
    # 4. Ensure base_dir/config.ini exists & load -----------------------
    config_file = Path(base_dir) / "config.ini"
    if config_file.is_file():
        try:
            config.load(config_file)
            logger.info(f"Loaded run‑specific config from {config_file}")
        except ValueError:
            config._loaded = False  # type: ignore[attr-defined]
            config.load(config_file)
            logger.info(f"Reloaded config from {config_file}")
    else:
        logger.info("No run‑specific config found; will generate a new one after overrides.")

    # 5. Apply override hyper‑parameters --------------------------------
    if pretrained:
    ...
    # Ensure schedulers can always read max_epochs
    _max_epochs = config("MAX_EPOCHS", 10, int, section="train")
    config.overwrite("train", "max_epochs", _max_epochs)

    # Persist full configuration ---------------------------------------
    config.save(config_file)

    # 6. Reproducibility & misc flags -----------------------------------
    seed = config("SEED", 42, int, section="train")
    check_manual_seed(seed)
    debug = False
    log_timings = config("LOG_TIMINGS", False, bool, section="train", save=False)
    ...
    model, start_epoch = load_model(ckpt_dir, state, jit=jit,
                                    mask_only=mask_only, train_df_only=train_df_only)

    # 9. DataLoader ------------------------------------------------------
    bs = config("BATCH_SIZE", 1, int, section="train")
    bs_eval_cfg = config("BATCH_SIZE_EVAL", 0, int, section="train")
    bs_eval = bs if bs_eval_cfg <= 0 else bs_eval_cfg
    ...
    max_epochs = config("MAX_EPOCHS", 10, int, section="train")
    patience   = config("EARLY_STOPPING_PATIENCE", 5, int, section="train")
    val_rule   = config("VALIDATION_CRITERIA_RULE", "min", section="train").replace("less","min").replace("more","max")
    ...
    for epoch_idx in range(start_epoch, max_epochs):
        # ---- Train ----
        train_loss = run_epoch(model, epoch_idx, dataloader, "train", opt, losses,
                               str(summary_dir), lrs, wds)
        ...
        # ---- Validate ----
        val_loss = run_epoch(model, epoch_idx, dataloader, "valid", opt, losses,
                             str(summary_dir))
        write_cp(model, "model", ckpt_dir, epoch_idx+1, metric=val_loss, cmp=val_rule)
        ...
        if globals().get("should_stop", False):
            logger.info("Received external stop signal; terminating training.")
            break
