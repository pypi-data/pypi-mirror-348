...
def run_df_train(
    ...
    *,
    ...
    # --------------------------------------------------- 4. config handling  (方案 B)
    config_file = os.path.join(base_dir, "config.ini")

    # 4-a. 先加载 DeepFilterNet2 默认完整配置
    default_cfg = os.path.join(os.path.dirname(base_dir), "DeepFilterNet2", "config.ini")
    config._loaded = False                      # type: ignore[attr-defined]
    config.load(default_cfg)
    logger.debug(f"Loaded default config: {default_cfg}")

    # 4-b. 若训练目录已有 config.ini，则叠加（会覆盖默认键）
