...
import asyncio
import nats
from src.ssl_utils import NatsSSLContextBuilder
...
async def setup_version_endpoint(nc):
    ...
    queue_group = os.getenv("VERSION_QUEUE_GROUP")
...
async def setup_health_check_endpoint(nc, get_health_check_subject_func):
    """
    Sets up NATS endpoints that respond to health check requests.
    """
    hostname = socket.gethostname()
    instance_subject = f"{get_health_check_subject_func()}.{hostname}"
    broadcast_subject = f"{get_health_check_subject_func()}.>"
