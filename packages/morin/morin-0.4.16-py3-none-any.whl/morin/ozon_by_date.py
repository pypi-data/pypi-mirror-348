...
class OZONbyDate:
    ...
    def get_postings_report_info(self, report_code):
        try:
            ...
            if code == 200:
                result = response.json()
                message = f'Платформа: OZON. Имя: {self.add_name}. Функция: get_postings_report_info. Результат: ОК'
                self.common.log_func(self.bot_token, self.chat_list, message, 1)
                return result
            else:
                response.raise_for_status()
        except Exception as e:
            message = f'Платформа: OZON. Имя: {self.add_name}. Функция: get_postings_report_info. Ошибка: {e}.'
            self.common.log_func(self.bot_token, self.chat_list, message, 3)
            return message
    ...
    def get_postings_fbs_report(self, date):
        try:
            ...
            for k in range(10):
                time.sleep(10)
                get_link = self.get_postings_report_info(new_report)
