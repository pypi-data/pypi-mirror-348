...
class UpdateDataBase:
    def __init__(self, kind='dataset', ip=None, port=None, user=None, passwd=None, db=None):
        ...
        res = self.ds.get_daily_all(need_json=True)  # 一次性获取所有股票当天的数据，一天多次获取可能返回None，是服务器限制了请求次数
        if res is not None:
            d1, _ = res
    ...
    def update(self):
        ...
        if (isinstance(self.storage.daily_all_data, list)
            ...
            daily_all = pd.DataFrame(self.storage.daily_all_data) # 如果daily_all_data已经是当天的数据，则不在更新
