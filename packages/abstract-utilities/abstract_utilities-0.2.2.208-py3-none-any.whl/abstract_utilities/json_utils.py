...
def safe_json_loads(
    ...
    Args:
        ...
        raise_on_error: If True, raise JSONDecodeError on failure instead of returning default.

    Returns:
        The parsed JSON object, or default if parsing fails and raise_on_error is False.

    Raises:
        json.JSONDecodeError: If parsing fails and raise_on_error is True.
...
def safe_json_load(
    ...
    """
    Safely load JSON from a file or file-like object.

    Args:
        file: A file path (str) or file-like object (TextIO).
        default: Value to return if loading fails (default: None).
        log_errors: Whether to log loading errors (default: True).
        raise_on_error: If True, raise JSONDecodeError on failure instead of returning default.

    Returns:
        The parsed JSON object, or default if loading fails and raise_on_error is False.

    Raises:
        json.JSONDecodeError: If parsing fails and raise_on_error is True.
        TypeError: If file is not a string or file-like object.
        OSError: If file cannot be opened (when file is a path).
    """
    file_obj = None
    try:
        if isinstance(file, str):
            ...
            if not hasattr(file, 'read'):
                raise TypeError(f"Expected file-like object or str, got {type(file).__name__}")
            file_to_process = file
        
        return json.load(file_to_process)
...
def safe_json_dumps(
    ...
    Args:
        ...
        log_errors: Whether to log serialization errors (default: True).
        raise_on_error: If True, raise error on failure instead of returning default.

    Returns:
        The JSON string, or default if serialization fails and raise_on_error is False.

    Raises:
        TypeError: If data is not JSON-serializable and raise_on_error is True.
        json.JSONDecodeError: If serialization fails and raise_on_error is True.
    """
    try:
...
def safe_json_dump(
    ...
    """
    Safely serialize an object to a file or file-like object.
    ...
    Returns:
        None on success, or default if serialization fails and raise_on_error is False.

    Raises:
        TypeError: If data is not JSON-serializable or file is invalid, and raise_on_error is True.
        json.JSONDecodeError: If serialization fails and raise_on_error is True.
        OSError: If file cannot be opened (when file is a path) and raise_on_error is True.
    """
    if isinstance(file, str):
        try:
            with open(file, 'w', encoding='utf-8') as f:
                return safe_json_dump(data, f, default, log_errors, raise_on_error)
        except OSError as e:
            if log_errors:
                logger.error(f"Failed to open file {file}: {e}")
            if raise_on_error:
    ...
    try:
        ...
        if log_errors:
            logger.error(f"Failed to serialize object to JSON file: {e}")
        if raise_on_error:
            raise
        return default
