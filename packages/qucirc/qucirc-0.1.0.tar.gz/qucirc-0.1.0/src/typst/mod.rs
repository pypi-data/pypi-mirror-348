...
/// 
pub(crate) fn draw_circuit(circuit: &Circuit) -> String {
...
pub(crate) fn to_svg(circuit: &Circuit) -> Result<String, typst_as_lib::TypstAsLibError> {
    use typst_as_lib::TypstEngine;
    use typst_library::layout::Abs;

    let quill_table = draw_circuit(circuit);
    let engine = TypstEngine::builder()
        .main_file(quill_table)
