...
class GeminiLLM(LLMInterface):
    ...
    def _extract_and_parse_json(self, response: str):
        """Extract a JSON object from an LLM response in a ```json ... ``` block."""
        try:
            ...
            if match:
                ...
                # Step 2: Try to extract a valid JSON object (first { ... })
                json_obj_match = re.search(r"\{.*\}", json_candidate, re.DOTALL)
                if json_obj_match:
                    json_str = json_obj_match.group(0)
                    return json.loads(json_str)

            # Fallback: match first JSON object in the response
            ...
            logger.error(f"[JSONDecodeError] Failed to parse JSON: {e}")
