...
class MessageWorker:
    def __init__(
        ...
        self.routing_key_prefix = routing_key_prefix.rstrip(".")
    ...
    def reconnect(self, consume: bool = False):
        while True:
            try:
                ...
                routing_key = f"{self.routing_key_prefix}.*" if self.routing_key_prefix else "#"
                self.channel.queue_bind(exchange=self.exchange, queue=self.input_queue, routing_key=routing_key)
    ...
    def validate_response(self, response):
        ...
        if not isinstance(response["queue"], str) or "." not in response["queue"]:
            raise ValueError("'queue' (used as routing_key) must be a non-empty string containing at least one dot")
        try:
    ...
    def callback(self, ch, method, properties, body):
        try:
            ...
            self.safe_publish(routing_key, response_data)
            logger.info(f"[x] Sent to '{routing_key}': {response_data}")
    ...
    def safe_publish(self, routing_key, message, headers=None):
        while True:
            try:
