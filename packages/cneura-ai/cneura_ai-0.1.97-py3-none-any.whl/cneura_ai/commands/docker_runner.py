...
class DockerManager:
    ...
    def build_image(self, dockerfile_path: str, tag: str) -> None:
        with click.progressbar(length=1, label=f"🔧 Building image: {tag}") as bar:
    ...
    def run_container(self, image: str, name: Optional[str] = None,
        ...
        try:
            ...
            with click.progressbar(length=1, label=f"🚀 Starting container: {name}") as bar:
    ...
    def bulk_run_containers(self, image: str, count: int,
        ...
        for i in range(count):
            ...
            if ports:
                for idx, (container_port, _) in enumerate(ports.items()):
