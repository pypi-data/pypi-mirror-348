...
from yaspin.extensions import rainbow
...
class DockerManager:
    ...
    def build_image(self, dockerfile_path: str, tag: str) -> None:
        with yaspin(Spinners.earth, text=f"🔧 Building image: {tag}", color="cyan") as spinner:
            try:
    ...
    def run_container(self, image: str, name: Optional[str] = None,
        ...
        try:
            ...
            try:
                with yaspin(Spinners.earth, text=f"🚀 Starting container: {name}", color="cyan") as spinner:
    ...
    def stop_all(self):
        ...
        for container in containers:
            try:
                container.stop()
                click.echo(f"🛑 Stopped container: {container.name}")
            except Exception as e:
