...
class BlePeriph_WriteCharacteristic(Command):
    CommandGroupId = 0x4B
    CommandId = 0x04
    def build_frame(self, CharacteristicNdx: int, WriteOffset: int, WriteValue: bytes, BrpTimeout: int = 100) -> bytes:
        _send_buffer = BytesIO()
        _send_buffer.write(b"\x00\x4B\x04")
        _send_buffer.write(int(BrpTimeout).to_bytes(4, byteorder='big'))
        _send_buffer.write(CharacteristicNdx.to_bytes(length=1, byteorder='big'))
        _send_buffer.write(WriteOffset.to_bytes(length=2, byteorder='big'))
        _send_buffer.write(int(len(WriteValue)).to_bytes(2, byteorder='big'))
        _send_buffer.write(WriteValue)
        return _send_buffer.getvalue()
    def __call__(self, CharacteristicNdx: int, WriteOffset: int, WriteValue: bytes, BrpTimeout: int = 100) -> None:
        request_frame = self.build_frame(CharacteristicNdx=CharacteristicNdx, WriteOffset=WriteOffset, WriteValue=WriteValue, BrpTimeout=BrpTimeout)
        _recv_buffer = BytesIO(self.execute(request_frame))
        _additional_data = _recv_buffer.read()
        if _additional_data:
            raise PayloadTooLongError(_additional_data)
...
class BlePeriph_ReadCharacteristic(Command):
    CommandGroupId = 0x4B
    CommandId = 0x05
    def build_frame(self, CharacteristicNdx: int, ReadOffset: int, ReadLength: int, BrpTimeout: int = 100) -> bytes:
        _send_buffer = BytesIO()
        _send_buffer.write(b"\x00\x4B\x05")
        _send_buffer.write(int(BrpTimeout).to_bytes(4, byteorder='big'))
        _send_buffer.write(CharacteristicNdx.to_bytes(length=1, byteorder='big'))
        _send_buffer.write(ReadOffset.to_bytes(length=2, byteorder='big'))
        _send_buffer.write(ReadLength.to_bytes(length=2, byteorder='big'))
        return _send_buffer.getvalue()
    def __call__(self, CharacteristicNdx: int, ReadOffset: int, ReadLength: int, BrpTimeout: int = 100) -> bytes:
        request_frame = self.build_frame(CharacteristicNdx=CharacteristicNdx, ReadOffset=ReadOffset, ReadLength=ReadLength, BrpTimeout=BrpTimeout)
        _recv_buffer = BytesIO(self.execute(request_frame))
