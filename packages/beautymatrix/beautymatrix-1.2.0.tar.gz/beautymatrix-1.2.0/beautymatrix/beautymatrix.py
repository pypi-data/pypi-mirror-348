...
def beautymatrix(matrix: np.ndarray, content: str = "", precision: int = 3, gap: int = 1) -> str:
    """
    ...
    `gap`: `int`, *mặc định = 0*, tăng khoảng cách giữa các hàng
    ...
    matrix = np.array2string(matrix, formatter={'all':alignelement}, max_line_width=np.inf, separator=' '*gap)
    ...
    for i,row in enumerate(matrix):
        ...
        if row.strip() == "...":
            ...
            if len(row)+4 < len(wrap[0]): row = "."*len(wrap[0])-4
