...
class FakeEvrmoreProxyTestCase(unittest.TestCase):
    ...
    def test_fundrawtransaction(self):
        ...
        self.assertTrue(len(funded_transaction.vin) >
                        len(unfunded_transaction.vin))
    ...
    def test__batch_result_keys(self):
        ...
        results = proxy._batch(requests)
        result = results[0]
        self.assertTrue(all([expected_key in result.keys()
    ...
    def test_make_blocks_from_blockhashes(self):
        ...
        self.assertTrue(all(["previousblockhash" in block.keys()
                        for block in blocks[1:]]))
        self.assertEqual(blocks[-1]["previousblockhash"], blocks[-2]["hash"])
        self.assertEqual(sorted([block["hash"]
                         for block in blocks]), sorted(blockhashes))
