...
if sys.version > '3':
    ...
    def _bytes(x): return bytes(bytearray(x))

# Core definitions
COIN = 100000000
MAX_BLOCK_SIZE = 1000000*2
MAX_BLOCK_WEIGHT = 4000000
MAX_BLOCK_SIGOPS = MAX_BLOCK_SIZE/50
WITNESS_COINBASE_SCRIPTPUBKEY_MAGIC = _bytes(
...
class CTransaction(ImmutableSerializable):
    ...
    def __init__(self, vin=(), vout=(), nLockTime=0, nVersion=1, witness=CTxWitness()):
        ...
        object.__setattr__(self, 'vin', tuple(
            CTxIn.from_txin(txin) for txin in vin))
        object.__setattr__(self, 'vout', tuple(
...
class CBlockHeader(ImmutableSerializable):
    """A block header"""
    __slots__ = ['nVersion', 'hashPrevBlock', 'hashMerkleRoot',
                 'nTime', 'nBits', 'nNonce', 'nHeight', 'nonce64', 'mix_hash']
    ...
    def calc_difficulty(nBits):
        ...
        return dDiff
    difficulty = property(
        lambda self: CBlockHeader.calc_difficulty(self.nBits))
...
class CBlock(CBlockHeader):
    ...
    def __init__(self, nVersion=2, hashPrevBlock=b'\x00'*32, hashMerkleRoot=b'\x00'*32, nTime=0, nBits=0, nNonce=0, vtx=(), nHeight=0, nonce64=0, mix_hash=b''):
        ...
        if vtx:
            ...
            vMerkleTree = ()
        super(CBlock, self).__init__(nVersion, hashPrevBlock,
                                     hashMerkleRoot, nTime, nBits, nNonce, nHeight, nonce64, mix_hash)

        object.__setattr__(self, 'vMerkleTree', vMerkleTree)
        try:
            vWitnessMerkleTree = tuple(
                CBlock.build_witness_merkle_tree_from_txs(vtx))
        except NoWitnessData:
            vWitnessMerkleTree = ()
        object.__setattr__(self, 'vWitnessMerkleTree', vWitnessMerkleTree)
        object.__setattr__(self, 'vtx', tuple(
            CTransaction.from_tx(tx) for tx in vtx))
...
class CoreMainParams(CoreChainParams):
    MAX_MONEY = 21000000000 * 1000 * COIN
    NAME = 'mainnet'
    ...
    SUBSIDY_HALVING_INTERVAL = 210000000 * 10
    PROOF_OF_WORK_LIMIT = 0x00000fffffffffffffffffffffffffffffffffffffffffffffffffffffffffff
...
def _SelectCoreParams(name):
    ...
    Don't use this directly, use evrmore.SelectParams() instead so both
...
def CheckBlock(block, fCheckPoW=True, fCheckMerkleRoot=True, cur_time=None):
    ...
    # Block header checks
    CheckBlockHeader(block.get_header(),
                     fCheckPoW=fCheckPoW, cur_time=cur_time)
    ...
    if len(block.serialize(dict(include_witness=False))) > MAX_BLOCK_SIZE:
        raise CheckBlockError(
            "CheckBlock() : block larger than MAX_BLOCK_SIZE")

    if block.GetWeight() > MAX_BLOCK_WEIGHT:
        raise CheckBlockError(
            "CheckBlock() : block larger than MAX_BLOCK_WEIGHT")
    ...
    if fCheckMerkleRoot:
        ...
        if len(block.vWitnessMerkleTree):
            ...
            if len(nonce_script.stack) != 1 or len(nonce) != 32:
                raise CheckBlockError(
                    "CheckBlock() : invalid coinbase witnessScript")
            try:
