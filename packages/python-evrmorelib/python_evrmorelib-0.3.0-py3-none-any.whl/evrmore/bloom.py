...
class CBloomFilter(evrmore.core.serialize.Serializable):
    ...
    def __init__(self, nElements, nFPRate, nTweak, nFlags):
        ...
        self.vData = bytearray(int(
            min(-1 / LN2SQUARED * nElements * math.log(nFPRate), self.MAX_BLOOM_FILTER_SIZE * 8) / 8))
        self.nHashFuncs = int(
            min(len(self.vData) * 8 / nElements * LN2, self.MAX_HASH_FUNCS))
        self.nTweak = nTweak
        self.nFlags = nFlags
    ...
    def insert(self, elem):
        ...
        """
        if isinstance(elem, evrmore.core.COutPoint):
            elem = elem.serialize()
    ...
    def stream_deserialize(cls, f):
        vData = bytearray(
            evrmore.core.serialize.BytesSerializer.stream_deserialize(f))
        (nHashFuncs,
         nTweak,
         nFlags) = CBloomFilter.__struct.unpack(evrmore.core.ser_read(f, CBloomFilter.__struct.size))
