...
from evrmore.core.serialize import *
from evrmore.net import *
import evrmore
...
class MsgSerializable(Serializable):
    ...
    def to_bytes(self):
        ...
        self.msg_ser(f)
        body = f.getvalue()
        res = evrmore.params.MESSAGE_START
        res += self.command
    ...
    def stream_deserialize(cls, f, protover=PROTO_VERSION):
        ...
        # check magic
        if recvbuf[:4] != evrmore.params.MESSAGE_START:
            raise ValueError("Invalid message start '%s', expected '%s'" %
                             (b2x(recvbuf[:4]), b2x(evrmore.params.MESSAGE_START)))
