...
class Test_Segwit(unittest.TestCase):
    ...
    def test_p2wpkh_signaturehash(self):
        ...
        address = CBech32EvrmoreAddress.from_scriptPubKey(scriptpubkey)
    ...
    def test_p2sh_p2wpkh_signaturehash(self):
        ...
        scriptpubkey = CScript(
            x('a9144733f37cf4db86fbc2efed2500b4f4e49f31202387'))
        redeemscript = CScript(
            x('001479091972186c449eb1ded22b78e40d009bdf0089'))
        value = int(10*COIN)

        address = CBase58EvrmoreAddress.from_scriptPubKey(redeemscript)
        self.assertEqual(SignatureHash(address.to_redeemScript(), CTransaction.deserialize(unsigned_tx),
    ...
    def test_p2wsh_signaturehash1(self):
        ...
        scriptpubkey1 = CScript(
            x('21036d5c20fa14fb2f635474c1dc4ef5909d4568e5569b79fc94d3448486e14685f8ac'))
        value1 = int(1.5625*COIN)
        scriptpubkey2 = CScript(
            x('00205d1b56b63d714eebe542309525f484b7e9d6f686b3781b6f61ef925d66d6f6a0'))
        value2 = int(49*COIN)
        scriptcode1 = CScript(
    ...
    def test_p2wsh_signaturehash2(self):
        ...
        scriptpubkey1 = CScript(
            x('0020ba468eea561b26301e4cf69fa34bde4ad60c81e70f059f045ca9a79931004a4d'))
        value1 = int(0.16777215*COIN)
        witnessscript1 = CScript(
            x('0063ab68210392972e2eb617b2388771abe27235fd5ac44af8e61693261550447a4c3e39da98ac'))
        scriptcode1 = CScript(
            x('0063ab68210392972e2eb617b2388771abe27235fd5ac44af8e61693261550447a4c3e39da98ac'))
        scriptpubkey2 = CScript(
            x('0020d9bbfbe56af7c4b7f960a70d7ea107156913d9e5a26b0a71429df5e097ca6537'))
        value2 = int(0.16777215*COIN)
        witnessscript2 = CScript(
            x('5163ab68210392972e2eb617b2388771abe27235fd5ac44af8e61693261550447a4c3e39da98ac'))
        scriptcode2 = CScript(
    ...
    def test_p2sh_p2wsh_signaturehash(self):
        ...
        scriptPubKey = CScript(
            x('a9149993a429037b5d912407a71c252019287b8d27a587'))
        value = int(9.87654321*COIN)
        redeemScript = CScript(
            x('0020a16b5755f7f6f96dbd65f5f0d6ab9418b89af4b1f14a1bb8a09062c35f0dcb54'))
    ...
    def test_checkblock(self):
        # (No witness) coinbase generated by Evrmore Core
        ...
        coinbase.wit = CTxWitness(
            [CTxInWitness(CScriptWitness([witness_nonce]))])

        tx_legacy = CTransaction.deserialize(x(str_tx))
        tx_segwit = CTransaction.deserialize(x(str_w_tx))

        witness_merkle_root = CBlock.build_witness_merkle_tree_from_txs(
            (coinbase, tx_legacy, tx_segwit))[-1]

        commitment = Hash(witness_merkle_root + witness_nonce)
        commitment_script = evrmore.core.WITNESS_COINBASE_SCRIPTPUBKEY_MAGIC + commitment
        coinbase.vout.append(CTxOut(0, CScript(commitment_script)))

        block = CBlock(2, b'\x00'*32, b'\x00'*32, 0, 0, 0,
