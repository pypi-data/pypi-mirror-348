# Source: cpython/Lib/test/test_string/_support.py
...
class TStringTestCase(unittest.TestCase):
    ...
    def assertEqual(self, first, second, msg=None):
        match type(first), type(second):
            case lib.Template, compat.Template | compat.Template, lib.Template:
                self.assertTStringEqual(first, second.strings, second.interpolations)
            case _:
    ...
    for item in template:
        match item:
            case str() as s:
                parts.append(s)
            case (
                compat.Interpolation(value, _, conversion, format_spec)
                | lib.Interpolation(value, _, conversion, format_spec)  # type: ignore
            ):
