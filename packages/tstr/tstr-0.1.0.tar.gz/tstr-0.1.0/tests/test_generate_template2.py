...
class TestTString(TStringTestCase):
    ...
    def test_generate_template_flags(self):
        ...
        with raises(TemplateGenerationError):
            generate_template("hello, {name} {world}", use_eval=True)

        with raises(TemplateGenerationError):
            generate_template("hello, {name} {1 + 2}", use_eval=True)

        with raises(TemplateGenerationError):
            generate_template("hello, {name + 1}", use_eval=True)
        ...
        def func():
            return generate_template("hello, {name}", include_nonlocals=False, use_eval=True)
        with raises(TemplateGenerationError):
            func()

        def func():
            world = "world"
            return generate_template("hello, {name} {world}", use_eval=True)
        t_string_eq(func(), ("hello, ", " ", ""), [(name, "name"), ("world", "world")])

        t = generate_template("hello, {hello}", dict(hello="world"), use_eval=True)
