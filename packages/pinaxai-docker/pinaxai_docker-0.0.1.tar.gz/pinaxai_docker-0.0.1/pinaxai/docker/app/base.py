...
from pinaxai.docker.context import DockerBuildContext
from pinaxai.infra.app import InfraApp
from pinaxai.infra.context import ContainerContext
from pinaxai.utils.log import logger

if TYPE_CHECKING:
    from pinaxai.docker.resource.base import DockerResource
...
class DockerApp(InfraApp):
    ...
    mount_pinaxai_config: bool = True
        ...
        container_env.update(
            {
                ...
                PINAXAI_RUNTIME_ENV_VAR: "docker",
    ...
    def get_container_volumes(self, container_context: ContainerContext) -> Dict[str, dict]:
        from pinaxai.utils.defaults import get_default_volume_name
        ...
        if self.mount_pinaxai_config:
            pinaxai_config_host_path = str(Path.home().joinpath(".config/ag"))
            pinaxai_config_container_path = f"{self.workspace_dir_container_path}/.config/ag"
            logger.debug(f"Mounting: {pinaxai_config_host_path}")
            logger.debug(f"      to: {pinaxai_config_container_path}")
            container_volumes[pinaxai_config_host_path] = {
                "bind": pinaxai_config_container_path,
                "mode": "ro",
            }
    ...
    def build_resources(self, build_context: DockerBuildContext) -> List["DockerResource"]:
        ...
        from pinaxai.docker.resource.network import DockerNetwork
