...
from ..gptj.kv_caching import kv_concat_update
...
class LlamaRotaryEmbedding(torch.nn.Module):
    def __init__(
        ...
        if config is None:
            ...
            if config.inv_freq_config is not None:
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids):
    def apply(tensor, cos, sin):
        # sincos always have dtype float32. We need to cast tensor to float32 to avoid accuracy
        # degradation.
        embed = (tensor.to(cos.dtype) * cos) + (rotate_half(tensor.to(sin.dtype)) * sin)
        # get back to the original dtype
        return embed.to(tensor.dtype)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        kv_update_method: callable = kv_concat_update,
        **kwargs,
    ) -> Union[
        ...
        next_key_value = (key_states, value_states)
        if past_key_value is not None:
            key_states, value_states, next_key_value = kv_update_method(
        ...
        if use_cache:
            present_key_value = next_key_value
    ...
    def proj_qkv(
        self, hidden_states: torch.FloatTensor, **kwargs
    ...
    def compute_attn(
        ...
        **kwargs,
        ...
        # Comment out attention weight shape check
        # if attn_weights.size() != (bsz, self.num_heads, q_len, kv_seq_len):
        #     raise ValueError(
        #         "Attention weights should be of size {}, but is {}".format(
        #             (bsz, self.num_heads, q_len, kv_seq_len), attn_weights.size()
        #         )
        #     )

        if attention_mask is not None:
            # Comment out attention mask shape check
            # if attention_mask.size() != (bsz, 1, q_len, kv_seq_len):
            #     raise ValueError(
            #         "Attention mask should be of size {}, but is {}".format(
            #             (bsz, 1, q_len, kv_seq_len), attention_mask.size()
            #         )
            #     )
        ...
        # Comment out attn_output shape check
        # if attn_output.size() != (bsz, self.num_heads, q_len, self.head_dim):
        #     raise ValueError(
        #         "`attn_output` should be of size {}, but is {}".format(
        #             (bsz, self.num_heads, q_len, self.head_dim), attn_output.size()
        #         )
        #     )
