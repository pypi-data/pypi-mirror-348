...
from .utils import LlamaBasedModelConverter
...
class NewLlamaModel(LlamaModel):
    ...
    def forward(
        ...
        if causal_mask is not None:
            causal_mask = make_causal_mask(causal_mask, torch.bfloat16, inputs_embeds.device)
        if attention_mask is not None:
            attention_mask = make_attention_mask(
                attention_mask, torch.bfloat16, inputs_embeds.device
            )
...
class LlamaForCausalLM(LlamaPreTrainedModel, CausalLMSymbolicTrace, LlamaBasedModelConverter):
