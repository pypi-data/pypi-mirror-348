...
@memoize
def getY(sampleX, targetFunctionInstance, verbose=True, runDir=None, resumeSamples=False):
    ...
    if localParalellization:
        return getYParallel(sampleX, targetFunctionInstance, verbose, resumeSamples=resumeSamples)
...
def getYParallel(sampleX, targetFunctionInstance, verbose=True, resumeSamples=False):
    ...
    processInputs = _getParallelRunInputSamples(
        sampleX, targetFunctionCopies, numberOfProcesses, verbose, resumeSamples
    )
    ...
    for processNumber in range(numberOfProcesses):
        ...
        if resumeSamples:
            ...
            targetFunctionCopy.runDir = (
                os.path.abspath(os.path.join(runDir, parallelDirName + getTimeString())) + "_" + str(processNumber)
            )
...
def _getParallelRunInputSamples(sampleX, targetFunctionCopies, numberOfProcesses, verbose, resumeSamples=False):
...
def taskCreatorAsync(taskQueue, resultQueue, processInputs, runDirBase, numberOfWorkerProcesses, numberOfRunningJobs):
    ...
    try:
        ...
        for sampleX, targetFunctionInstance, sampleIndex, verbose, _ in processInputs:
...
def calledParallelExecution(args):
    ...
    sampleX, targetFunctionInstance, sampleSetIndex, verbose, resumeSamples = args
