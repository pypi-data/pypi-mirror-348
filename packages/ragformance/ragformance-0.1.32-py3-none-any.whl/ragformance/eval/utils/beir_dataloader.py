...
def load_custom(
        ...
        queries = {k: v for k, v in queries.items() if "relevant_document_ids" in v and len(v["relevant_document_ids"]) > 0}
        ...
        if filter_corpus:
            filtered_ids = set([ a["corpus_id"] for q in queries_as_array for a in q["relevant_document_ids"]])
