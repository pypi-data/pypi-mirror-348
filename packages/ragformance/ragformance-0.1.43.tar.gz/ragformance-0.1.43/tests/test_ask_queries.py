...
class TestAskQueriesCorpus(unittest.TestCase):
    ...
    def test_openwebui_rag_ask_queries(self):
        ...
        with open("tests/test_config_ollama.json", "r") as jsonfile:
            config = json.load(jsonfile)

        openwebui_rag = OpenwebuiRag()
        nb_docs_processed, config_updated = openwebui_rag.upload_corpus(corpus=corpus,config=config)
        ...
        answers: List[AnswerModel] = openwebui_rag.ask_queries(queries, config=config_updated)
        assert len(answers) == 1 and answers[0].retrieved_documents_ids == ["c_1"] and "Paris" in answers[0].model_answer
