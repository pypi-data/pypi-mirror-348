...
class OpenwebuiRag(RagInterface):
    def upload_corpus(self, corpus: List[DocModel], config: Dict = {}):
        ...
        try:
            ...
            logging.info(
                f"Récupération du fichier de configuration :'."
            )
            if "rag" in config and isinstance(config["rag"], list):
                for rag_config in config["rag"]:
                    if isinstance(rag_config, dict) and rag_config.get("name") == "rag_open-webui":
                        logging.info(
                            f"Configuration de rag_open-webui extraite avec succée'."
                        )
                        config = rag_config
                    else : 
                        logging.error(
                            f"Échec de la récupération de la configuration au niveau de rag_open-webui."
                        )
                        return []       
            else :
                logging.error(
                    f"Échec de la récupération de la configuration au niveau de rag."
                )
                return [] 
            ...
            coll_info = client.create_collection(
                ...
                description=f"Collection pour benchmark",
            )
