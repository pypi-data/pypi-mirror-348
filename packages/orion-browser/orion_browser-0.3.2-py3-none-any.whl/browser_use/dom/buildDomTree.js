(
    ...
    const { doHighlightElements, focusHighlightIndex, viewportExpansion, debugMode, elementsConfig } = args;
    ...
    // 辅助函数：检测是否存在事件委托
function checkEventDelegation(element) {
    // 1. 检查父元素是否有常见的事件代理选择器
    let parent = element.parentElement;
    while (parent) {
        // 检查常见的事件代理框架模式
        if (parent.hasAttribute('data-event-delegate') || 
            parent.hasAttribute('[click]') ||
            parent.hasAttribute('v-on:click') ||
            parent.hasAttribute('@click') ||
            parent.hasAttribute('(click)') || // Angular
            parent.hasAttribute('*ngFor')) {
            return true;
        }
        
        // 检查父元素是否有可能的事件代理ID或class
        if (parent.id && 
            (parent.id.includes('container') || parent.id.includes('wrapper'))) {
            if (parent.onclick || 
                getEventListeners(parent)?.click?.length > 0) {
                return true;
            }
        }
        
        // 检查常见的事件代理库属性
        if (parent.hasAttribute('data-reactid') || 
            parent.hasAttribute('data-vue') ||
            parent.hasAttribute('data-bind')) {  // Knockout
            return true;
        }
    ...
    function isInteractiveElement(element) {
        ...
        if (steps.length > 0 && element.nodeType === Node.ELEMENT_NODE) {
            ...
            if (step) {
                const desc = step?.domOperationInfo?.interaction?.click?.effect || step?.domOperationInfo?.interaction?.scroll?.effect || step?.domOperationInfo?.description || step?.description
                return {
                    type: step.event,
        ...
        // if(checkEventDelegation(element)) {
        //     return true;
        // }
        ...
        if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            if (nodeData.isVisible) {
                ...
                if (nodeData.isTopElement) {
                    ...
                    // console.log('nodeData', interactive)
