...
class ExtractCommon(Pass):
        ...
        with ctx_frame(ctx, task) as frame:
            ...
            if len(binders) > 1 and composites:
                ...
                if extractables:
                    ...
                    plan = self._create_extraction_plan(binders, composites, extractables, frame)
    ...
    def _create_extraction_plan(self, binders: OrderedSet[ir.Task], composites: OrderedSet[ir.Task], extractables: list[ir.Task], frame: Frame):
        """
        ...
        for binder in binders:
            if binder not in common_body:
                ...
                deps = self._compute_local_dependencies(frame, binder, common_body, exposed_vars)
                if deps:
