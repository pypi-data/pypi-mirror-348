...
class RelExecutor:
    ...
    def execute_model(self, model: Model, result_cols: Optional[List[str]] = None) -> DataFrame:
        self._prepare_graph_index(model)
    ...
    def _prepare_graph_index(self, model: Model):
        ...
        if not self._use_graph_index():
            return

        table_sources = [key for key, value in model._bindable_tables.items() if isinstance(value, SnowflakeTable)]
        app_name = resources.get_app_name()
        program_span_id = debugging.get_program_span_id()

        assert self.engine is not None
        with debugging.span("poll_use_index", sources=table_sources, model=model, engine=self.engine):
            resources.poll_use_index(app_name, table_sources, self.database, self.engine, program_span_id)
