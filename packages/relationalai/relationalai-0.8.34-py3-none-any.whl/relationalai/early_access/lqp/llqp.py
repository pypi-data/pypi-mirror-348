...
option_to_key = {
    ...
    PrettyOptions.SHOW_VAR_TYPES: "show_var_types",
    PrettyOptions.PRINT_NAMES: "print_names"
}

option_to_default = {
    PrettyOptions.STYLED: True,
    PrettyOptions.SHOW_VAR_TYPES: False,
    PrettyOptions.PRINT_NAMES: True
}

# Used for precise testing
ugly_config = {
    str(PrettyOptions.STYLED): False,
    str(PrettyOptions.SHOW_VAR_TYPES): True,
...
pretty_config = {
    ...
    str(PrettyOptions.SHOW_VAR_TYPES): False,
...
def to_llqp(node: Union[ir.LqpNode, ir.PrimitiveType], indent_level: int, options: Dict = {}) -> str:
    ...
    if isinstance(node, ir.Def):
        ...
        llqp += to_llqp(node.body, indent_level + 2, options) + "\n"
        llqp += ind + conf.SIND() + conf.RPAREN() + conf.RPAREN()

    elif isinstance(node, ir.Abstraction):
        llqp += ind + conf.LPAREN() + conf.LBRACKET()
        options["_in_abstr_vars"] = True
        llqp += list_to_llqp(node.vars, 0, " ", options)
        options["_in_abstr_vars"] = False
        llqp += conf.RBRACKET() + "\n"
        ...
        options["_in_abstr_vars"] = True
        llqp += list_to_llqp(node.body.vars, 0, " ", options)
        options["_in_abstr_vars"] = False
        llqp += conf.RBRACKET() + "\n"
        ...
        if has_option(options, PrettyOptions.SHOW_VAR_TYPES) or options.get("_in_abstr_vars", False):
            llqp += conf.type_anno("::" + type_to_llqp(node.type))
