...
from .models import GeometryRequest, GeometryResponse
...
class EngineClient:
    ...
    _api_url: str = "https://engine.kerfed.com/api/v2"
    ...
    def geometry_start(
        ...
        create = self.authorized.post(
            f"{self._api_url}/geometry", json=serialize(request)
        )
        create.raise_for_status()

        return create.json()["task_id"]

    def geometry_result(self, task_id: str, timeout: float = 30.0) -> GeometryResponse:
        """
        ...
        for _ in range(max_iter):
            try:
                response = self.authorized.get(
                    url=f"{self._api_url}/geometry/{task_id}",
            ...
            if response.status_code == 200:
                ...
                result = deserialize(response.json(), GeometryResponse)
                # log the outcome field before returning
                logger.debug(result.outcome)
                return result
    ...
    def upload_url(self, file_name: str) -> dict:
        ...
        response = self.authorized.post(
            f"{self._api_url}/upload", json={"file_name": file_name}
        )
        response.raise_for_status()
...
def deserialize(item: dict, model):
