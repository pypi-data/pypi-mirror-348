...
class Progression:
    ...
    def n(self, value: int):
        ...
        """
        if (
            not NumberValidator.is_positive_number(value) or
    ...
    def __init__(
        ...
        if (
            not NumberValidator.is_number(start) or
            not NumberValidator.is_number(end)
        ):
            raise Exception('The provided "start" and "end" parameter must be numbers.')

        if (
            not NumberValidator.is_positive_number(n) or
            n < 2
        ):
