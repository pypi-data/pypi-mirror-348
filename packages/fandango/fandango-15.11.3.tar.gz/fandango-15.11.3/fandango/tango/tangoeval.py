...
class TangoEval(object):
    ...
    # Using regexps as loaded from fandango.tango.defaults, patched for dots
    what = "quality|time|value|exception|delta|all|"\
        "hist|ALARM|WARNING|VALID|INVALID|OK"
    alnumdot = re_alnum  # as defined in fandango.tango.defaults
    alnum = "[a-zA-Z0-9\-_]+"  # A most restrictive version without ._-
    wdot = r'(?:(?:{}|(?:\.(?!(?:{}))))+)'.format(alnum,what)
    no_alnum = no_alnum
    no_quotes = no_quotes
    ...
    rewhat = "(?:(?:[\.])(?P<what>"+what+"))?"
    reattr = "(?:(?:[/])(?P<attribute>(?:" + wdot + "))" + rewhat + ")?"

    retango = '(?:(?:tango://)?' + redev + reattr + ')' # +'(?!/)'

    # Excludes attr_names between quotes, accepts value type methods
    regexp = no_quotes + retango + no_quotes
        ...
        if attributes:
            ...
            if self.use_events:
                ...
                proxy = lambda a: CachedAttributeProxy(
                    ...
                    listeners=[self],
                    log_level='DEBUG' if self._trace else None,
                )
            else:
                #Create a lazy reader
    ...
    def trace(self, msg):
        if self._trace:
            print("%s TangoEval: %s" % (fn.time2str(),str(msg)))
    ...
    def eventReceived(self, src, type_, value):
        ...
        try:
            ...
            if self.event_hook:
                if type_ in ("change", "archive",
                             "quality", "user_event", "periodic"):
    ...
    def eval(self, formula=None, previous=None, _locals=None, _raise=FAILED_VALUE):
        ...
        for target, device, attribute, what in targets:
            ...
            try:
                # Reading or Overriding attribute value, 
                # if overriden value will not be kept for future iterations
                # r will be the default value in case of exception
                r = (
                    _raise
                    if not any(
