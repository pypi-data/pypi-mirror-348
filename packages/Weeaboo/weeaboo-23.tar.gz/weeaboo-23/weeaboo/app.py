...
class SiteMeter:
    ...
    def before_request(self):
        self.actor.send(f"{datetime.now()} {g.hitinfo}").andforget(log)

def _before():
    path = request.full_path
    if path.find('?') == len(path) - 1:
        path = path[:-1]
    g.hitinfo = hitinfo = f"{request.method} {path} {request.headers.get('Referer')} {request.headers.get('X-Forwarded-For')} {request.headers.get('User-Agent')}"
    log.debug(hitinfo)
