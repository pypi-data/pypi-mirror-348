...
def get_request_file(file_path, file_type=None):
    ...
    # Determine the MIME type using get_media_header
    media_types = None
    if file_type:
        from your_module import get_media_types
        media_types = get_media_types(file_type)
        from your_module import confirm_type
        if not confirm_type(file_path=file_path, media_types=media_types):
            raise ValueError(f"File {file_path} is not of the allowed type(s): {file_type}")
