...
def create_sample_timeseries_data():
    ...
    # 時間列を追加
    sensor_columns["time"] = Column(ch=None, name="time", unit="s", values=time_data)
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            ...
            sensor_columns[col_name] = Column(
                ch=None, name=col_name, unit="℃", values=values  # 温度を想定
            )
...
def main():
    ...
    print(
        f"- 時間範囲: {timeseries_data['time'].values[0]:.1f}〜{timeseries_data['time'].values[-1]:.1f} {timeseries_data['time'].unit}"
    )
    ...
    selected_data.ops.plot_coordinate_timeseries(
        value_column="sensor_1_1", time_column="time", animate=False, ax=ax1
    ).end()
    plt.tight_layout()
    ...
    step_data.ops.plot_spatial_heatmap(
        value_column="sensor_1_1", resolution=(30, 30), show_points=False, ax=ax3
    ).end()
    ...
    print(
        "# アニメーションは実行時間が長くなる可能性があるためコメントアウトしています"
    )
