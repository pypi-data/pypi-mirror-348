...
def linear_interpolate(x_points, y_points, x_new):
    """純粋な Python で実装した線形内挿関数（範囲外は線形外挿を行います）

    指定された点に基づいて線形内挿または外挿を行います。
    範囲外の値に対しては端点の傾きを用いた線形外挿を行います。

    Args:
        x_points: 元の x 座標値のリスト
        y_points: 元の y 座標値のリスト
        x_new: 内挿する新しい x 座標値のリスト

    Returns:
        list: 内挿または外挿された y 座標値のリスト
    """
    y_new = []
...
def linspace(start, stop, num):
    ...
    指定された開始値と終了値の間に、指定した数の等間隔の値を生成します。
    NumPy の linspace に似た機能を提供します。
...
def find_nearest_index(x_points, x_value):
    ...
    指定された値に最も近い値を持つ要素のインデックスを返します。
    距離が等しい場合は小さい方のインデックスを選択します。
